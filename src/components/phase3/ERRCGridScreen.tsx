...
function ERRCColumn({ title, subtitle, color, items, onAdd, onRemove, onEdit }: ERRCColumnProps) {
                    ...
                    >
                        <div className="flex items-start gap-2">
                            <div className="flex-1">
                                <span className={cn("text-xs px-2 py-0.5 rounded-full", colors.pill)}>
                                    {item.factor}
                                </span>
                                {item.reason && (
...
export function ERRCGridScreen({ errc, onChange, onContinue }: ERRCGridScreenProps) {
    const isValid = errc.eliminate.length >= 1 && errc.reduce.length >= 1 &&
        errc.raise.length >= 1 && errc.create.length >= 1;
        ...
        <div className="space-y-8">
            ...
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground max-w-lg mx-auto">
                    Real strategy means choosing what NOT to do. You must have at least one item in each column.
                </p>
            </div>
            ...
            {!isValid && (
                <div className="max-w-3xl mx-auto bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-400">
                        Each column needs at least one item to continue. Real Blue Ocean strategy requires trade-offs.
                    </p>
                </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
                <ERRCColumn
                    ...
                    onRemove={(i) => updateColumn('eliminate', errc.eliminate.filter((_, idx) => idx !== i))}
                    onEdit={(i, item) => updateColumn('eliminate', errc.eliminate.map((el, idx) => idx === i ? item : el))}
                />
                <ERRCColumn
                    title="Reduce"
                    ...
                    onRemove={(i) => updateColumn('reduce', errc.reduce.filter((_, idx) => idx !== i))}
                    onEdit={(i, item) => updateColumn('reduce', errc.reduce.map((el, idx) => idx === i ? item : el))}
                />
                <ERRCColumn
                    title="Raise"
                    ...
                    onRemove={(i) => updateColumn('raise', errc.raise.filter((_, idx) => idx !== i))}
                    onEdit={(i, item) => updateColumn('raise', errc.raise.map((el, idx) => idx === i ? item : el))}
                />
                <ERRCColumn
                    title="Create"
                    ...
                    onRemove={(i) => updateColumn('create', errc.create.filter((_, idx) => idx !== i))}
                    onEdit={(i, item) => updateColumn('create', errc.create.map((el, idx) => idx === i ? item : el))}
                />
            </div>
