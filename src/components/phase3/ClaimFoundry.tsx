...
import { ArrowRight, Check, X, AlertTriangle, Shield, Star } from 'lucide-react';
...
function ClaimCard({
    ...
    const allGatesPassed = claim.isSpecific && claim.isUnique && claim.movesBuyers;
...
export function ClaimFoundry({ claims, primaryClaimId, onChange, onContinue }: ClaimFoundryProps) {
    const handleSetPrimary = (claimId: string) => {
        onChange(claims, claimId);
    };

    const handleToggleGate = (claimId: string, gate: 'isSpecific' | 'isUnique' | 'movesBuyers') => {
        onChange(
    ...
    const hasPrimary = claims.some(c => c.id === primaryClaimId && !c.riskFlags.includes('blocked'));
        ...
        <div className="space-y-8">
            ...
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground max-w-lg mx-auto">
                    Choose your primary UVP. Every claim must pass the Reeves USP gates: specific, unique, moves buyers.
                </p>
            </div>
            ...
            <div className="flex justify-center pt-4">
                ...
                >
                    {hasPrimary ? 'Continue' : 'Select a Primary Claim'} <ArrowRight className="h-5 w-5 ml-2" />
                </Button>
            </div>
        </div>
    );
