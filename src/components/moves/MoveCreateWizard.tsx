...
// Mock Cohorts Data (Simulating Mod Pooling / Cohorts Module)
const COHORTS_POOL = [
    ...
    { id: "COH-05", name: "Agency Owners", description: "Service business owners >$500k" }, // Expanded pool
];
    ...
    const handleNext = async () => {
        if (step === 'objective' && category) {
            ...
            if (currentQ < CLARIFICATION_QUESTIONS.length - 1) {
                ...
                // Generate Brief & Execution Plan IMMEDIATELY
                setIsLoading(true);
                await new Promise(r => setTimeout(r, 1200));

                const icpName = COHORTS_POOL.find(c => c.id === selectedCohort)?.name || "Target Audience";
                const b = generateMockBrief(category!, contextDesc, icpName);
                const exec = generateMockExecution(b);
...
function generateMockBrief(category: MoveCategory, context: string, icp: string): MoveBriefData {
    return {
        name: `${MOVE_CATEGORIES[category].name} Sprint`,
        category,
        goal: `Execute a high-impact ${category} campaign to achieve results.`,
        tone: "Professional & Authoritative",
        duration: 7,
        icp: icp, // Use selected ICP
        strategy: "Focus on high-leverage activities to maximize impact.",
