...
export function PageTopGoals({
  ...
  const handleSuggestedClick = useCallback((goal: string) => {
    if (items.includes(goal)) return;
    const newItems = [...items, goal];
    onChange(newItems.join(", "));
  }, [items, onChange]);
  ...
  return (
    <OnboardingLayout
      ...
      stepTitle="What outcomes do your customers want?"
      stepDescription="The goals that drive them to buy. What does success look like?"
      onBack={onBack}
      onNext={onNext}
      canGoNext={isValid}
      showBack={!!onBack}
    >
      <div ref={wrapperRef} className="ob-animate-in">
        ...
        <div className="ob-content-item ob-interactive flex gap-2">
          <div className="flex-1 flex items-center gap-3 px-4 py-3 rounded-[var(--radius-md)] border border-[var(--border-2)] bg-[var(--bg-surface)] focus-within:border-[var(--rf-charcoal)] focus-within:ring-2 focus-within:ring-[var(--rf-charcoal)]/10 transition-all">
            ...
            <input
              ...
              placeholder="Add a customer goal..."
        ...
        {items.length > 0 && (
          <div className="ob-content-item ob-interactive mt-4 flex flex-wrap gap-2">
            {items.map((item, i) => (
              <div
                key={i}
                className="goal-item group flex items-center gap-2 px-3 py-2 rounded-full bg-[var(--rf-charcoal)] text-[var(--rf-ivory)]"
              >
        ...
        {items.length < 5 && (
          <div className="ob-content-item ob-interactive mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTED.filter(s => !items.includes(s)).map(goal => (
                <button
                  key={goal}
                  onClick={() => handleSuggestedClick(goal)}
        ...
        <p className="mt-4 text-[11px] text-[var(--ink-3)]">
          {items.length} goal{items.length !== 1 ? "s" : ""} added
          {items.length >= 2 && <span className="text-[var(--status-success)] ml-2">✓ Good range</span>}
        </p>
      </div>
    </OnboardingLayout>
  );
