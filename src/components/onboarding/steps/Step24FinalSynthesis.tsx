...
import { ArrowRight, CheckCheck, Rocket, LayoutDashboard, Loader2 } from "lucide-react";
...
export default function Step23FinalSynthesis() {
    ...
    const handleEnterApp = async () => {
        ...
        try {
            // Call Backend to Finalize & Cleanup
            if (session?.sessionId) {
                const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
                await fetch(`${apiUrl}/api/v1/onboarding/${session.sessionId}/finalize`, {
                    method: 'POST'
                });
            }
    ...
    return (
        <div ref={containerRef} className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto space-y-8 pb-12">
            ...
            <div className="action-btn pt-8">
                <BlueprintButton onClick={handleEnterApp} disabled={isFinalizing} size="lg" className="px-16 py-8 text-lg shadow-2xl hover:scale-105 transition-transform">
                    {isFinalizing ? <Loader2 size={24} className="mr-3 animate-spin" /> : <LayoutDashboard size={24} className="mr-3" />}
                    {isFinalizing ? "Finalizing Raptor..." : "Enter Command Center"}
                </BlueprintButton>
