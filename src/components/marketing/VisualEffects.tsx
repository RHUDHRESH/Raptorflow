...
export function AnimatedGradient({
    className = '',
    colors = ['hsl(var(--background))', 'hsl(var(--muted))', 'hsl(var(--background))'],
    speed = 5,
...
export function ParticleField({
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            const fgColor = style.getPropertyValue('--foreground').trim() || '45 45 45';
...
export function Wave({
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            for (let x = 0; x <= width; x += 10) {
                const y = height / 2 + Math.sin(x * frequency + frame * 0.02) * amplitude;
                points.push(`${x},${y}`);
            }

            path.setAttribute('d', `M0,${height} L${points.join(' L')} L${width},${height} Z`);
            frame++;
    ...
    return (
        <svg viewBox="0 0 1000 100" preserveAspectRatio="none" className={className}>
...
export function Blob({ className = '', color = 'currentColor', size = 400 }: BlobProps) {
    const pathRef = useRef<SVGPathElement>(null);
