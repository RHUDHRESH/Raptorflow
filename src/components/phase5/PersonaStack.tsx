...
import { ArrowRight, Check, AlertCircle, HelpCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
function ConfidenceBadge({ confidence }: { confidence: DataConfidence }) {
    const config: Record<DataConfidence, { bg: string; text: string; label: string }> = {
        proven: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400', label: 'Proven' },
        inferred: { bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-700 dark:text-amber-400', label: 'Inferred' },
        assumed: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', label: 'Assumed' }
    };
...
function PersonaCard({
    ...
    const toggleConfidence = (
        ...
        items[index] = { ...items[index], confidence: nextConfidence[items[index].confidence] };
            ...
            >
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                        <span className="text-primary font-bold">{persona.role.charAt(0)}</span>
                    </div>
            ...
            {expanded && (
                <div className="p-4 pt-0 space-y-4">
                    ...
                    {persona.kpis.length > 0 && (
                        <div>
                            <h4 className="text-sm font-medium text-muted-foreground mb-2">KPIs</h4>
                            <div className="flex flex-wrap gap-2">
                                {persona.kpis.map((kpi, i) => (
                                    <span key={i} className="text-xs bg-muted px-2 py-1 rounded">{kpi}</span>
                                ))}
                            </div>
                        </div>
                    )}
                    ...
                    {/* Objections */}
                    <div>
                        <h4 className="text-sm font-medium text-muted-foreground mb-2">Objections</h4>
                        <div className="space-y-2">
                    ...
                    {persona.proofNeeds.length > 0 && (
                        <div>
                            <h4 className="text-sm font-medium text-muted-foreground mb-2">Proof They Need</h4>
                            <div className="flex flex-wrap gap-2">
                                {persona.proofNeeds.map((proof, i) => (
                                    <span key={i} className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-2 py-1 rounded">
                                        {proof}
                                    </span>
                                ))}
                            </div>
                    ...
                    {persona.language.length > 0 && (
                        <div>
                            <h4 className="text-sm font-medium text-muted-foreground mb-2">Language They Use</h4>
                            <div className="flex flex-wrap gap-2">
                                {persona.language.map((word, i) => (
                                    <span key={i} className="text-xs bg-muted px-2 py-1 rounded italic">"{word}"</span>
                                ))}
                            </div>
                        </div>
                    )}
...
export function PersonaStack({ personas, icpName, onChange, onContinue }: PersonaStackProps) {
    ...
    const provenCount = personas.reduce((acc, p) => {
        return acc + p.goals.filter(g => g.confidence === 'proven').length +
        ...
        <div className="space-y-8">
            ...
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground max-w-lg mx-auto">
                    Personas are roles, not bios. Every claim is tagged as proven, inferred, or assumed.
                </p>
            </div>
            ...
            <div className="max-w-md mx-auto bg-card border rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={cn(
                        "text-sm font-bold",
                        provenCount === totalCount ? "text-green-600" :
                            provenCount > totalCount / 2 ? "text-amber-600" : "text-red-600"
                    )}>
