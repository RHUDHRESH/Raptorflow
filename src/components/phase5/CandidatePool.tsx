...
function FitScoreBreakdown({ score }: { score: ICPFitScore }) {
    const bars = [
        ...
        { label: 'Switchability', value: score.switchability, color: 'bg-green-500' },
        { label: 'Reachability', value: score.reachability, color: 'bg-purple-500' },
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={cn(
                            "text-lg font-bold",
                            candidate.fitScore.total >= 80 ? "text-green-600" :
                                candidate.fitScore.total >= 60 ? "text-amber-600" : "text-red-600"
                        )}>
                            {candidate.fitScore.total}%
                        </span>
                    </div>
                    <p className="text-sm text-muted-foreground mb-2">
                        Displacing: <span className="font-medium">{candidate.displacedAlternative}</span>
                    </p>
                    <p className="text-sm text-muted-foreground">
                        Value hook: <span className="font-medium">{candidate.dominantValue}</span>
                    </p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                    <Button
                        ...
                        onClick={onToggle}
                    >
                        {candidate.kept ? <Check className="h-4 w-4 mr-1" /> : <X className="h-4 w-4 mr-1" />}
...
export function CandidatePool({ candidates, onChange, onContinue }: CandidatePoolProps) {
    const keptCount = candidates.filter(c => c.kept).length;

    const handleToggle = (candidateId: string) => {
        onChange(candidates.map(c =>
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex justify-center gap-8 text-center">
                ...
                <div>
                    <div className="text-3xl font-bold text-muted-foreground">{candidates.length - keptCount}</div>
            ...
            <div className="flex justify-center pt-4">
                ...
                >
                    {keptCount < 1 ? 'Keep at least 1 ICP' :
                        keptCount > 5 ? 'Max 5 ICPs' :
