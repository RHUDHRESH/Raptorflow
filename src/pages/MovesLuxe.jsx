...
const MOCK_COHORTS = [
  { id: 'c1', name: 'Enterprise CTOs', tags: ['ROI-first', 'Time-scarce', 'Predictability'], avatar: '🎯' },
  { id: 'c2', name: 'Startup Founders', tags: ['Speed', 'Novelty', 'High Tolerance'], avatar: '🚀' },
  { id: 'c3', name: 'Marketing Directors', tags: ['Habit', 'Trust', 'Word-of-mouth'], avatar: '📊' },
  { id: 'c4', name: 'Creators', tags: ['Authenticity', 'Engagement', 'Collabs'], avatar: '🎨' }
]
...
const JOURNEY_STAGES = [
  { id: 'unaware', label: 'Unaware', color: 'bg-neutral-300' },
  { id: 'problem_aware', label: 'Problem Aware', color: 'bg-amber-400' },
  { id: 'solution_aware', label: 'Solution Aware', color: 'bg-blue-400' },
  { id: 'product_aware', label: 'Product Aware', color: 'bg-purple-400' },
  { id: 'most_aware', label: 'Most Aware', color: 'bg-green-400' },
];
...
export default function MovesLuxe() {
  ...
  // hydrate intent from query params (e.g., coming from Cohorts/Matrix/Muse/Campaigns)
  useEffect(() => {
  ...
  const activeCampaign = MOCK_CAMPAIGNS.find(c => c.id === activeCampaignId)
  ...
  const handleGenerateRecommendations = () => {
    ...
    setTimeout(() => {
      ...
      setLoadingRecommendations(false)
      setGuidedStep(5) // Moved to step 5 because step 3 is now journey
    }, 800)
  }
  ...
  const MoveCard = ({ move, index }) => {
    ...
    return (
      ...
      >
        <Link
          to={`/moves/${move.id}`}
          className="block runway-card p-6 hover:shadow-xl transition-all group h-full flex flex-col"
        >
          ...
          <div className="mb-4 flex-1">
            ...
            {fromStage && toStage && (
              <div className="flex items-center gap-2 mb-4 text-xs">
                <div className={cn("px-2 py-1 rounded", fromStage.color, "bg-opacity-20 text-neutral-900")}>
                  {fromStage.label}
                </div>
                <ArrowRight className="w-3 h-3 text-neutral-400" />
                <div className={cn("px-2 py-1 rounded", toStage.color, "bg-opacity-20 text-neutral-900")}>
                  {toStage.label}
                </div>
              </div>
            )}
  ...
  const JourneyStageSelector = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-3">
        <p className="micro-label">From Stage</p>
          ...
          {JOURNEY_STAGES.map(stage => (
            <button
              ...
              className={cn(
                ...
                journeyStageFrom === stage.id ? "border-neutral-900 bg-neutral-50" : "border-neutral-200 hover:border-neutral-400"
              )}
            >
      ...
      <div className="space-y-3">
        <p className="micro-label">To Stage</p>
        <div className="space-y-2">
          {JOURNEY_STAGES.map(stage => (
            <button
              key={`to-${stage.id}`}
              ...
              className={cn(
                "w-full text-left px-4 py-3 rounded-lg border transition-all flex items-center justify-between",
                journeyStageTo === stage.id ? "border-neutral-900 bg-neutral-50" : "border-neutral-200 hover:border-neutral-400"
              )}
            >
              <span className="text-sm font-medium text-neutral-900">{stage.label}</span>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {/* Step tracker */}
          <div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.2em] text-neutral-500 overflow-x-auto">
        ...
        <div className="p-6 border-t border-neutral-200 flex justify-between items-center bg-neutral-50">
          <div className="text-sm text-neutral-600">
            {guidedStep < 5 && 'Universal language only: keep goals/cohorts/timeframe consistent across industries.'}
          </div>
          <div className="flex gap-2">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.2em] text-neutral-500 overflow-x-auto">
            ...
            <span>→</span>
            <span className={cn(manualStep >= 3 ? 'text-neutral-900' : '')}>3 Journey</span>
            <span>→</span>
            <span className={cn(manualStep >= 4 ? 'text-neutral-900' : '')}>4 Tempo</span>
          </div>
      ...
      >
        <div>
          ...
          <p className="font-sans text-[10px] font-bold uppercase tracking-[0.3em] text-neutral-400">
            {activeCampaign ? `Campaign: ${activeCampaign.name}` : 'All Campaigns'}
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        {activeCampaignId && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-neutral-100 text-neutral-600 rounded-lg hover:bg-neutral-200 text-sm"
          >
            <X className="w-4 h-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {filteredMoves.length === 0 && (
          <div className="col-span-full border border-dashed border-neutral-200 p-10 rounded-xl text-center text-neutral-500">
            No moves found. {activeCampaignId ? 'Try clearing the campaign filter or create a new move.' : 'Start with Guided Recommendations or create one manually.'}
          </div>
        )}
      </div>
