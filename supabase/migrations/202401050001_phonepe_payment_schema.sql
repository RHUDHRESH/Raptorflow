...
-- Policy for webhook logs (system access only)
CREATE POLICY "Service role can access webhook logs" ON payment_webhook_logs
    FOR ALL USING (auth.role() = 'service_role');

-- Policy for payment events log (system access only)
CREATE POLICY "Service role can access payment events" ON payment_events_log
    FOR ALL USING (auth.role() = 'service_role');
...
-- ==========================================
-- FUNCTIONS FOR PAYMENT OPERATIONS
-- ==========================================

-- Function to get user payment statistics
CREATE OR REPLACE FUNCTION get_user_payment_stats(user_email TEXT)
RETURNS TABLE(
    ...
    SELECT
        ...
        COALESCE(
            (SELECT SUM(refund_amount)
             ...
             WHERE pt.customer_email = user_email AND pr.status = 'COMPLETED'),
            0
        ) as total_refunded
    FROM payment_transactions
