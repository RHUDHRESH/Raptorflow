-- ============================================================================
-- AUTH SYSTEM FIX MIGRATION (NON-DESTRUCTIVE)
-- Date: 2025-02-10
-- Purpose: Safe migration with data preservation and users-model alignment
-- ============================================================================

BEGIN;

-- SAFETY CHECK: Abort if tables have data and no backup exists
DO $$
DECLARE
    profiles_count INT;
...
BEGIN
    ...
    IF profiles_count > 0 THEN
        ...
        IF profiles_count > 0 THEN
            RAISE NOTICE 'WARNING: profiles table has % rows. Data will be preserved.', profiles_count;
        END IF;
    END IF;
END $$;
...
-- 3. PROFILES TABLE: Use CREATE IF NOT EXISTS instead of DROP/CREATE
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') THEN
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS ucid TEXT UNIQUE DEFAULT public.generate_ucid();
        ...
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS subscription_plan TEXT DEFAULT 'free' CHECK (subscription_plan IN ('free', 'ascent', 'glide', 'soar'));
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS subscription_status TEXT DEFAULT 'none' CHECK (subscription_status IN ('none', 'active', 'past_due', 'cancelled', 'expired'));
        ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS workspace_preferences JSONB DEFAULT '{}'::jsonb;
    END IF;
END $$;

-- 4. WORKSPACES TABLE: Non-destructive
...
-- 5. WORKSPACE MEMBERS TABLE: Non-destructive
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'workspace_members') THEN
        ALTER TABLE public.workspace_members ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;
    END IF;
END $$;

-- 6. USER SESSIONS TABLE: Non-destructive
...
BEGIN
    -- Create profile (with conflict handling for existing users)
    ...
    -- Create default workspace only if none exists
...
CREATE OR REPLACE FUNCTION auth.check_is_service_role()
RETURNS BOOLEAN AS $$
BEGIN
    -- Check if current user is authenticated service role
