...
class SystemTester:
    ...
    def test_backend_health(self) -> bool:
        ...
        try:
            ...
            self.log_test(
                "Backend Health Check", passed, f"Status: {response.status_code}"
            )
            return passed
    ...
    def test_frontend_build(self) -> bool:
        ...
        try:
            ...
            if not frontend_dir.exists():
                self.log_test(
                    "Frontend Directory", False, "Frontend directory not found"
                )
                return False
    ...
    def test_database_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.backend_url}/api/v1/health/detailed", timeout=5
            )
            if response.status_code == 200:
                data = response.json()
                db_status = data.get("database", "unknown")
                passed = db_status == "connected" or db_status == "ok"
                self.log_test("Database Connection", passed, f"Status: {db_status}")
                return passed
            else:
                self.log_test(
                    "Database Connection", False, "Health endpoint not responding"
                )
                return False
        except Exception as e:
    ...
    def test_redis_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("Redis Connection", passed, f"Status: {redis_status}")
                return passed
            else:
                self.log_test(
                    "Redis Connection", False, "Health endpoint not responding"
                )
                return False
        except Exception as e:
        ...
        try:
            ...
            self.log_test(
                ...
                (
                    "No type errors"
                    if passed
                    else f"Errors found: {len(result.stderr.splitlines())}"
                ),
            )
            return passed
        except Exception as e:
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        print("=" * 50)
        print(
            f"📊 Test Results: {passed_count}/{total_count} passed ({success_rate:.1f}%)"
        )
