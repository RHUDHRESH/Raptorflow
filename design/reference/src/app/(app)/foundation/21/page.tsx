...
export default function FoundationStep21() {
  ...
  const startBuild = async () => {
    ...
    const apiCall = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/foundation/complete`, {
          method: "POST",
          headers: {
  ...
  const Slider = ({ labelL, labelR, val, setVal }: any) => (
    <div className="flex items-center gap-4 w-full">
      <span className="w-24 text-right text-[10px] font-bold uppercase tracking-widest text-zinc-500">{labelL}</span>
      <input 
        type="range" min="0" max="100" value={val} 
        onChange={(e) => setVal(parseInt(e.target.value))}
        className="flex-1 h-1 bg-zinc-800 rounded-full appearance-none accent-[#f59e0b] cursor-pointer"
      />
      <span className="w-24 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-500">{labelR}</span>
    </div>
  );
  ...
  const summary = useMemo(() => {
    ...
    const comps = sectionData.competitors?.competitors?.map((c: any) => c.url.replace(/^https?:\/\//, "")).join(", ") || "the market";
    const voice = sectionData.brand_personality?.voiceTraits?.join(", ") || "professional";

    return (
      <p className="text-lg text-white font-light leading-relaxed">
        &quot;Here is what I understand about <span className="text-[#f59e0b] font-medium">{bizName}</span>. 
        You serve <span className="text-white font-medium">{icp.name}</span> — {icp.pain}. 
        The problem you solve is {problem}. 
        Your positioning: {positioning}. 
        Your goal for the next 90 days is <span className="text-white font-medium">{goal}</span>. 
        The competitors I&apos;ll be watching are {comps}. 
        And {bizName} should always sound <span className="text-white font-medium">{voice}</span>.&quot;
      </p>
    );
  }, [sectionData]);
  ...
  if (part === 4) {
    return (
      <div className="fixed inset-0 bg-[#0f0f0f] z-50 flex flex-col items-center justify-center p-6 transition-all duration-1000">
        {!isComplete && !error && (
          <div className="w-full max-w-lg space-y-12 flex flex-col items-center animate-in fade-in duration-1000">
            <h2 className="text-3xl font-bold text-white tracking-widest uppercase">RaptorFlow</h2>
            ...
            <div className="w-full bg-zinc-900 h-[2px] fixed bottom-0 left-0 right-0">
              <div 
        ...
        {isComplete && (
          <div className="w-full max-w-lg space-y-8 flex flex-col items-center animate-in fade-in zoom-in-95 duration-[1200ms]">
            ...
            <div className="text-center space-y-2">
              <h2 className="text-4xl font-bold text-white">Your office is ready.</h2>
              <p className="text-xl text-zinc-400 font-light italic mt-4">
  ...
  return (
    <div className="flex flex-col items-center px-6 pt-20 pb-24 min-h-screen bg-[#1a1a1a]">
      <div className="w-full max-w-[640px] space-y-12">
        ...
        {/* PART 1: NAME */}
        {part >= 1 && (
          <div className={cn("space-y-8 animate-in fade-in slide-in-from-top-4 duration-700", part > 1 && "opacity-20 pointer-events-none grayscale scale-[0.98]")}>
            <div className="space-y-2 text-center">
              <h1 className="text-3xl font-bold text-white">Your Campaign Strategist is about to come to life.</h1>
              <p className="text-zinc-400 text-base">What would you like to call them?</p>
            </div>
            <div className="flex flex-col items-center gap-8">
              <input
                autoFocus
                className="text-center text-3xl font-light text-white bg-transparent border-b-2 border-[#f59e0b] outline-none pb-2 px-4 w-full max-w-sm placeholder:text-zinc-800"
        ...
        {/* PART 2: PERSONALITY */}
        {part >= 2 && (
          <div className={cn("space-y-8 animate-in fade-in slide-in-from-top-4 duration-700", part > 2 && "opacity-20 pointer-events-none grayscale scale-[0.98]")}>
            <div className="space-y-1 text-center">
              <h2 className="text-2xl font-bold text-white">Who is {strategistName}?</h2>
              <p className="text-sm text-zinc-500 uppercase tracking-widest font-bold">Shape how they communicate with you</p>
            </div>

            <div className="space-y-2 bg-[#1e1e1e] p-8 rounded-2xl border border-zinc-800">
              <Slider labelL="Direct" labelR="Diplomatic" val={personality.direct} setVal={(v: any) => setPersonality({...personality, direct: v})} />
              <Slider labelL="Challenger" labelR="Conservative" val={personality.challenger} setVal={(v: any) => setPersonality({...personality, challenger: v})} />
              <Slider labelL="Data-Driven" labelR="Intuitive" val={personality.dataDriven} setVal={(v: any) => setPersonality({...personality, dataDriven: v})} />
              
              <div className="mt-8 flex items-center gap-3 bg-black/40 border border-zinc-800 rounded-xl p-3">
                <Sparkles className="w-4 h-4 text-amber-500 shrink-0" />
                <p className="text-xs text-zinc-400">
                  Based on your answers, we suggest: <span className="text-white font-bold">[Direct, Challenger, Data-driven]</span> — adjust freely.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-1000">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">{strategistName} speaking:</span>
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#f59e0b] flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#f59e0b] animate-pulse" /> Final Strategist Brief
                </span>
              </div>
              
              <div className="bg-[#1e1e1e] border border-zinc-700/50 rounded-2xl p-8 relative shadow-2xl overflow-hidden group">
                {/* Visual Accent */}
                ...
                {/* Edit Links */}
                <div className="mt-8 pt-8 border-t border-zinc-800 flex flex-wrap gap-x-6 gap-y-2">
                  {[
            ...
            <div className="flex flex-col items-center gap-6 pt-4">
              <button
                onClick={startBuild}
                className="w-full bg-[#f59e0b] hover:bg-white text-black font-extrabold text-lg rounded-full py-5 transition-all shadow-[0_10px_30px_rgba(245,158,11,0.2)]"
              >
                Looks good — let&apos;s go
              </button>
              <p className="text-xs text-zinc-500 uppercase tracking-tighter">By launching, you authorise your AI team to begin monitoring and strategy work.</p>
            </div>
          </div>
        )}
