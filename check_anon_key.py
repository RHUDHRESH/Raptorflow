...
try:
    ...
    print(
        f'Current anon key: {anon_key[:20]}...{anon_key[-10:] if anon_key else "NONE"}'
    )
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                if "exp" in payload and payload["exp"] < time.time():
                    ...
                    print(
                        f'Expired at: {time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(payload["exp"]))}'
                    )
                    print(
                        f'Current time: {time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())}'
                    )
                else:
                    print("✅ Anon key should be valid")
                    if "exp" in payload:
                        print(
                            f'Expires at: {time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(payload["exp"]))}'
                        )
