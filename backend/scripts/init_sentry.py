...
from backend.core.sentry_alerting import get_alerting_manager
from backend.core.sentry_dashboards import get_dashboard_manager
...
def main():
    ...
    print(f"Sentry DSN: {config.dsn[:30]}..." if len(config.dsn) > 30 else f"Sentry DSN: {config.dsn}")
    print()
