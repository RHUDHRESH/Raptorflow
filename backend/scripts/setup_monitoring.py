...
def setup_alerts(project_id: str):
    ...
    print("Step 1: gcloud beta monitoring channels create --display-name=\"Matrix Alerts\" --type=email --channel-labels=email_address=admin@raptorflow.io")
    
    # 2. Create alert policy for Kill-Switch
    print("Step 2: Creating Kill-Switch Alert Policy")
    print("gcloud alpha monitoring policies create --display-name=\"Global Kill-Switch Triggered\" --condition=\"metric.type='logging.googleapis.com/user/kill_switch_total' AND resource.type='cloud_run_revision'\" --notification-channels=\"[CHANNEL_ID]\"")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Monitoring Setup for RaptorFlow")
