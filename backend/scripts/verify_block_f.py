...
async def verify_block_f():
    ...
    res = await fm.execute_with_fallback(failing_task, fallback_value={"status": "fallback"})
    ...
    tripped = await cb.check_and_trip("verify_ws", drift_score=0.2, reason="Drift Spike")
