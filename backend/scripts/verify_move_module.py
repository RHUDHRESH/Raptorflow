...
from backend.agents.moves import MoveGenerator, MoveRefiner, ResourceManager, ProgressTracker, MovePersistence
...
async def verify():
    ...
    res = await manager({"current_moves": [{"title": "test", "required_skills": ["Search"]}]})
    assert any("Resources verified" in m for m in res["messages"])
    print("✓ Resource Manager: Tool availability check passed.")

    # 4. Verify Progress Tracking
    tracker = ProgressTracker()
    res = await tracker({"current_moves": [{"status": "executed"}, {"status": "pending"}]})
    assert "50.0%" in res["messages"][0]
    print("✓ Progress Tracker: Progress calculation correct.")

    # 5. Verify Move Persistence (Mock DB)
    with patch("backend.agents.moves.save_move", new_callable=AsyncMock) as mock_save, \
         patch("backend.agents.moves.log_agent_decision", new_callable=AsyncMock) as mock_log:
