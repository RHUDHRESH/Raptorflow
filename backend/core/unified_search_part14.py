...
from backend.core.unified_search_part1 import SearchQuery, SearchMode
...
class RateLimiter:
    ...
    async def check_api_key_limit(self, api_key_hash: str, limit_per_hour: int) -> bool:
        ...
        # Clean old requests
        self.api_key_limits[api_key_hash] = [
            req_time for req_time in self.api_key_limits[api_key_hash]
            if req_time > hour_ago
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_requests': sum(len(requests) for requests in self.user_limits.values()),
            'total_api_requests': sum(len(requests) for requests in self.api_key_limits.values())
        }
...
class AuthenticationManager:
    ...
    def update_user_role(self, user_id: str, new_role: UserRole) -> bool:
        ...
        if not RolePermissionMapper.can_upgrade_role(user.role, new_role):
            logger.warning(f"Role upgrade not allowed: {user.role.value} -> {new_role.value}")
            return False
...
class SecurityManager:
    ...
    async def check_search_permission(
        ...
        if user and not user.has_permission(required_permission):
            return False, f"Insufficient permissions. Required: {required_permission.value}"
        ...
        if api_key and required_permission not in api_key.permissions:
            return False, f"API key lacks permission. Required: {required_permission.value}"
        
        # Check rate limits
        if user:
            if not await self.rate_limiter.check_user_limit(user.id, user.rate_limit_per_hour):
                return False, "User rate limit exceeded"
        
        if api_key:
            if not await self.rate_limiter.check_api_key_limit(api_key.key_hash, api_key.rate_limit_per_hour):
