...
class CompressionMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            if supported_encodings and content_encoding in ["gzip", "deflate"]:
                ...
                scope = await self._handle_compressed_request(scope, receive, content_encoding)
    ...
    async def _handle_compressed_response(
        ...
        if (len(response_body) >= self.config.min_size and
            content_type in self.config.types and
            supported_encodings):
            ...
            if compression_type == CompressionType.GZIP:
                compressed_body = gzip.compress(response_body, compresslevel=self.config.level)
    ...
    def _choose_compression(self, supported_encodings: List[CompressionType]) -> CompressionType:
        ...
        return {
            **self.stats,
            "avg_compression_time_ms": (
                self.stats["compression_time_ms"] / max(1, self.stats["responses_compressed"])
            ),
