...
class MetricsCollector:
    ...
    async def increment_counter(self, name: str, value: float = 1.0, labels: Optional[Dict[str, str]] = None, category: MonitoringCategory = MonitoringCategory.SYSTEM):
    ...
    async def set_gauge(self, name: str, value: float, labels: Optional[Dict[str, str]] = None, category: MonitoringCategory = MonitoringCategory.SYSTEM):
        """Set gauge metric."""
        async with self._lock:
            key = self._make_key(name, labels)
    ...
    async def record_histogram(self, name: str, value: float, labels: Optional[Dict[str, str]] = None, category: MonitoringCategory = MonitoringCategory.SYSTEM):
    ...
    async def record_timer(self, name: str, duration_ms: float, labels: Optional[Dict[str, str]] = None, category: MonitoringCategory = MonitoringCategory.PERFORMANCE):
        """Record timer metric."""
        async with self._lock:
            key = self._make_key(name, labels)
    ...
    async def get_metric_stats(self, name: str, labels: Optional[Dict[str, str]] = None, minutes: int = 5) -> Dict[str, float]:
...
def create_default_monitoring_setup():
    ...
    default_channels = [
        ...
        NotificationChannel(
            ...
            type="webhook",
            config={
                'url': 'https://hooks.raptorflow.com/alerts'
            }
        )
    ]
...
metrics_collector, alert_manager, observability_dashboard = create_default_monitoring_setup()
