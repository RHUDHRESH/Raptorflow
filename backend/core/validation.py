...
class RequestValidator:
    ...
    def validate_agent_request(
        self, request_data: Dict[str, Any], client_ip: Optional[str] = None
    ...
    def _validate_business_rules(
        self, request: EnhancedAgentRequest, client_ip: Optional[str] = None
    ):
        ...
        if self._is_suspicious_request(request):
            logger.warning(
                f"Suspicious request from user {request.user_id}: {request.request[:100]}"
            )
            ...
            if client_ip:
                ...
                if self.request_count[ip_key] > 50:
                    ...
                    raise HTTPException(
                        status_code=403, detail="Too many suspicious requests"
                    )
...
def validate_agent_request(
    request_data: Dict[str, Any], client_ip: Optional[str] = None
) -> EnhancedAgentRequest:
