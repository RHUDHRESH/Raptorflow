...
class AdvancedRateLimiter:
    ...
    def __init__(self, config: AdvancedRateLimitConfig = None):
        ...
        logger.info(
            f"Advanced rate limiter initialized with strategy: {self.config.strategy.value}"
        )
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            await self._update_client_info(
                client_id, user_id, user_tier, ip_address, user_agent, api_key
            )
            ...
            if client_info and client_info.is_blocked:
                if (
                    client_info.block_expires
                    and datetime.now() > client_info.block_expires
                ):
                    ...
                    return RateLimitResult(
                        ...
                        retry_after=(
                            int(
                                (
                                    client_info.block_expires - datetime.now()
                                ).total_seconds()
                            )
                            if client_info.block_expires
                            else None
                        ),
                    )
            ...
            if self.config.strategy == RateLimitStrategy.FIXED_WINDOW:
                result = await self._check_fixed_window_limit(
                    client_id, endpoint, limits, state
                )
                ...
                result = await self._check_leaky_bucket_limit(
                    client_id, endpoint, limits, state
                )
            else:  # ADAPTIVE
                result = await self._check_adaptive_limit(
                    client_id, endpoint, limits, state
                )
            ...
            state.current_minute[endpoint] = state.current_minute.get(endpoint, 0) + (
                1 if result.allowed else 0
            )
            state.current_hour[endpoint] = state.current_hour.get(endpoint, 0) + (
                1 if result.allowed else 0
            )
            state.current_day[endpoint] = state.current_day.get(endpoint, 0) + (
                1 if result.allowed else 0
            )
    ...
    async def _calculate_effective_limits(
        self, client_info: Optional[ClientInfo], endpoint: str
    ) -> Dict[str, int]:
    ...
    async def _check_fixed_window_limit(
        ...
        if minute_count >= limits["minute"]:
            return RateLimitResult(
                ...
                limit=limits["minute"],
                tier=(
                    self.clients[client_id].user_tier
    ...
    async def _check_adaptive_limit(
        ...
        if client_info:
            ...
            if client_info.trust_score > 0.8:
                ...
                if result.allowed:
                    result.allowed = state.current_minute.get(endpoint, 0) < int(
                        limits["minute"] * 0.7
                    )
                    if not result.allowed:
    ...
    async def _detect_abuse(
        ...
        for ts in state.request_timestamps:
            if ts > datetime.now() - timedelta(minutes=5):
                unique_endpoints.add(
                    endpoint
                )  # Simplified - would track actual endpoints
        ...
        if (
            current_hour in range(2, 6) and recent_requests > 5
        ...
        if state.abuse_score > self.config.abuse_threshold:
            await self._trigger_abuse_alert(
                client_id, endpoint, state.abuse_score, state
            )

            # Consider blocking if abuse is severe
            if state.abuse_score > 0.9 and state.violation_count > 5:
                await self._block_client(
                    client_id, "High abuse score detected", duration_hours=1
                )

    async def _trigger_abuse_alert(
        self, client_id: str, endpoint: str, abuse_score: float, state: RateLimitState
    ) -> None:
        """Trigger abuse detection alert."""
        alert_id = hashlib.md5(
            f"{client_id}{endpoint}{datetime.now().isoformat()}".encode()
        ).hexdigest()
        ...
        alert = AbuseAlert(
            ...
            details={
                ...
                "last_violation": (
                    state.last_violation.isoformat() if state.last_violation else None
                ),
                "endpoint": endpoint,
        ...
        logger.warning(
            f"Abuse alert triggered for {client_id}: {severity.value} - {abuse_score:.2f}"
        )

    async def _block_client(
        self, client_id: str, reason: str, duration_hours: int = 1
    ) -> None:
        ...
        if client_info:
            ...
            logger.warning(
                f"Client {client_id} blocked for {duration_hours} hours: {reason}"
            )
    ...
    def get_client_stats(self, client_id: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "block_expires": (
                client_info.block_expires.isoformat()
                if client_info.block_expires
                else None
            ),
            ...
            stats.update(
                {
                    ...
                    "violation_count": state.violation_count,
                    "last_violation": (
                        state.last_violation.isoformat()
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        current_time = datetime.now()
        uptime = current_time - (
            self.clients[list(self.clients.keys())[0]].created_at
            if self.clients
            else current_time
        )
        ...
        return {
            ...
            "started_at": (
                self.clients[list(self.clients.keys())[0]].created_at.isoformat()
                if self.clients
...
def get_advanced_rate_limiter(
    config: AdvancedRateLimitConfig = None,
) -> AdvancedRateLimiter:
