...
class TimeoutMetrics:
    ...
    async def record_timeout(
        ...
        if agent_name not in self.timeout_stats["by_agent"]:
            self.timeout_stats["by_agent"][agent_name] = {
                "count": 0,
                "recoveries": 0,
                "failures": 0,
            }
    ...
    async def record_recovery(
        self, operation_type: str, agent_name: str, strategy: str, success: bool
    ):
        ...
        if total_timeouts > 0:
            self.timeout_stats["recovery_success_rate"] = (
                total_recoveries / total_timeouts
            )

        await self._check_alert_conditions()

    async def record_failure(
        self, operation_type: str, agent_name: str, error_type: str
    ):
...
class CacheMetrics:
    ...
    async def record_cache_request(
        self, agent_name: str, operation_type: str, hit: bool, response_time: float
    ):
        """Record a cache request event."""
        self.cache_stats["total_requests"] += 1
        self.cache_stats["total_response_time"] += response_time
        self.cache_stats["avg_response_time"] = (
            self.cache_stats["total_response_time"] / self.cache_stats["total_requests"]
        )
    ...
    async def record_cache_set(
        ...
        if compressed:
            ...
            self.cache_stats["compression_ratio"] = (
                current_ratio + 0.1
            ) / 2  # Assume 10% compression
    ...
    async def record_cache_error(
        self, agent_name: str, operation_type: str, error_type: str
    ):
        ...
        logger.error(
            f"Cache error for agent '{agent_name}' ({operation_type}): {error_type}"
        )
    ...
    async def update_memory_usage(self, memory_usage_mb: float):
        ...
        if memory_usage_mb > 400:  # Assume 400MB limit
            await self._send_performance_alert(
                "high_memory_usage", f"High memory usage: {memory_usage_mb:.1f}MB"
            )
    ...
    async def _check_performance_alerts(self):
        ...
        # Calculate hit rate
        hit_rate = (
            self.cache_stats["cache_hits"] / total_requests if total_requests > 0 else 0
        )
        ...
        if hit_rate < self.performance_alerts["low_hit_rate"]:
            await self._send_performance_alert(
                "low_hit_rate", f"Low cache hit rate: {hit_rate:.2%}"
            )

        # Check error rate alert
        error_rate = (
            self.cache_stats["cache_errors"] / total_requests
            if total_requests > 0
            else 0
        )
        if error_rate > self.performance_alerts["high_error_rate"]:
            await self._send_performance_alert(
                "high_error_rate", f"High cache error rate: {error_rate:.2%}"
            )
...
class MetricsCollector:
    ...
    def __init__(self, max_history_size: int = 10000):
        ...
        logger.info(
            f"Metrics collector initialized with max history size: {max_history_size}"
        )
    ...
    def record_execution(
        ...
        try:
            ...
            logger.info(
                f"Recorded execution for agent {agent_id}: {success} in {execution_time_ms}ms"
            )
    ...
    def record_llm_usage(
        ...
        try:
            ...
            logger.info(
                f"Recorded LLM usage for agent {agent_id}: {cost_usd} USD, {input_tokens + output_tokens} tokens"
            )
    ...
    def record_tool_usage(
        ...
        try:
            ...
            logger.info(
                f"Recorded tool usage for agent {agent_id}: {tool_name} in {execution_time_ms}ms"
            )
    ...
    def record_error(
        ...
        try:
            ...
            logger.warning(
                f"Recorded error for agent {agent_id}: {error_type} - {error_message}"
            )
    ...
    def get_performance_summary(self, time_window_minutes: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            if not recent_metrics:
                return {
                    "message": f"No metrics found in the last {time_window_minutes} minutes"
                }
            ...
            return {
                ...
                "timestamp_range": {
                    "start": min(
                        metric.timestamp for metric in recent_metrics
                    ).isoformat(),
                    "end": max(
                        metric.timestamp for metric in recent_metrics
                    ).isoformat(),
    ...
    async def _persist_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.debug(
                    f"Would persist {metrics_count} metrics for {agent_count} agents"
                )
...
class RequestMetricsCollector:
    ...
    def end_request(
        ...
        try:
            ...
            logger.debug(
                f"Recorded metric for request {request_id}: {execution_time:.3f}s"
            )
            return metric
    ...
    def _check_performance_issues(self, metric: RequestMetric):
        ...
        if (
            ...
            and metric.llm_tokens_used > self.performance_thresholds["high_token_usage"]
        ):
    ...
    def _cleanup_old_metrics(self):
        ...
        if (now - self.last_cleanup).total_seconds() > self.cleanup_interval:
            ...
            logger.info(
                f"Performed metrics cleanup. Total metrics: {len(self.metrics)}"
            )
    ...
    def get_metrics_summary(
        ...
        successful_requests = sum(
            1 for m in filtered_metrics if m.status == RequestStatus.SUCCESS
        )
