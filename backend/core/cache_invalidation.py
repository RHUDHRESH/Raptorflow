...
class DependencyRule:
    ...
    def get_dependent_keys(self, source_key: str) -> List[str]:
        ...
        try:
            dependent_key = re.sub(self.source_key_pattern, self.dependent_key_pattern, source_key)
...
class DependencyManager:
    ...
    def add_dependency_rule(self, rule: DependencyRule):
        ...
        logger.debug(f"Added dependency rule: {rule.source_key_pattern} -> {rule.dependent_key_pattern}")
    ...
    def remove_dependency_rule(self, rule: DependencyRule):
        ...
        if rule in self.dependency_rules:
            ...
            logger.debug(f"Removed dependency rule: {rule.source_key_pattern} -> {rule.dependent_key_pattern}")
...
class CacheInvalidationManager:
    ...
    def __init__(
        ...
        max_concurrent_invalidations: int = 20
    ):
    ...
    async def initialize(self):
        ...
        self.event_invalidator.subscribe(
            InvalidationType.DEPENDENCY.value,
    ...
    async def invalidate_user_scope(
        self,
        user_id: str,
        reason: str = "User scope invalidation"
    ) -> str:
    ...
    async def invalidate_workspace_scope(
        self,
        workspace_id: str,
        reason: str = "Workspace scope invalidation"
    ) -> str:
    ...
    async def _execute_invalidation(self, event: InvalidationEvent):
        ...
        try:
            ...
            logger.info(f"Invalidated: {event.id} - {event.target} in {processing_time:.3f}s")
    ...
    async def _handle_dependency_invalidation(self, event: InvalidationEvent):
        ...
        invalidation_order = self.dependency_manager.get_invalidation_order(event.target)
