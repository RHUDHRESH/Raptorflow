...
class DatabaseScalingEngine:
                ...
                pre_scaling_actions.append(
                    {
                        ...
                        "predicted_utilization": next_hour_prediction[
                            "predicted_utilization"
                        ],
                        "recommended_pool_size": next_hour_prediction[
    ...
    async def _make_scaling_decision(self, load_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if utilization >= self.critical_threshold:
            ...
            decision["new_pool_size"] = min(
                total_connections + self.scale_step * 2, self.max_pool_size
            )
            decision["confidence"] = 0.95
            decision["recommendations"].append(
                "Immediate scaling required - critical load"
            )
        ...
        # Scale up
            ...
            if sustained_high:
                ...
                decision["new_pool_size"] = min(
                    total_connections + self.scale_step, self.max_pool_size
                )
                decision["confidence"] = 0.8
                decision["recommendations"].append(
                    "Scale up due to sustained high load"
                )
            else:
        ...
        # Scale down (only if safe)
            ...
            and total_connections > self.min_pool_size
        ):
            ...
            if sustained_low:
                ...
                decision["new_pool_size"] = max(
                    total_connections - self.scale_step, self.min_pool_size
                )
                decision["confidence"] = 0.7
                decision["recommendations"].append(
                    "Scale down due to sustained low load"
                )
            else:
        ...
        decision["recommendations"].extend(
            await self._get_contextual_recommendations(load_data)
        )
    ...
    def _analyze_growth_trends(self) -> Dict[str, Any]:
        ...
        older_loads = (
            self.load_history[-48:-24]
            if len(self.load_history) >= 48
            else self.load_history[:24]
        )
        ...
        return {
            ...
            "trend": (
                "increasing"
                if growth_rate > 0.05
                else "stable" if abs(growth_rate) < 0.05 else "decreasing"
            ),
        }
                ...
                peak_periods.append(
                    {
                        ...
                        "severity": (
                            "high"
                            if pattern["avg_utilization"] > self.critical_threshold
                            else "medium"
                        ),
                    }
                )
    ...
    async def _update_scaling_parameters(self, patterns: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if baseline > 0.5:
                ...
                logger.info(
                    f"Adjusted scaling thresholds based on high baseline load: {baseline:.1%}"
                )
    ...
    async def _predict_next_hour_load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(same_day_data) >= 3:
                predicted_utilization = statistics.mean(
                    [l["utilization"] for l in same_day_data]
                )
                confidence = 0.7
            else:
                predicted_utilization = statistics.mean(
                    [l["utilization"] for l in hour_data]
                )
                confidence = 0.5
            ...
            recommended_size = int(
                self.min_pool_size
                + (predicted_utilization * (self.max_pool_size - self.min_pool_size))
            )
            ...
            return {
                ...
                "recommended_pool_size": max(
                    self.min_pool_size, min(self.max_pool_size, recommended_size)
                ),
        ...
        try:
            ...
            for hour_ahead in range(24):
                ...
                if len(hour_data) >= 3:
                    ...
                    predictions.append(
                        {"hour": future_hour, "predicted_utilization": 0.5}
                    )

            return {
                "hourly_predictions": predictions,
                "peak_predicted": max(p["predicted_utilization"] for p in predictions),
                "avg_predicted": statistics.mean(
                    [p["predicted_utilization"] for p in predictions]
                ),
            }
    ...
    async def _get_contextual_recommendations(
        ...
        try:
            ...
            if 8 <= hour <= 18:  # Business hours
                if utilization > 0.7:
                    recommendations.append(
                        "High load during business hours - consider proactive scaling"
                    )
            ...
            if load_data["day_of_week"] >= 5:  # Weekend
                if utilization > 0.5:
                    recommendations.append(
                        "Unexpected weekend load - investigate usage patterns"
                    )
            ...
            if len(self.load_history) > 50:
                ...
                if trend > 0.05:
                    recommendations.append(
                        "Upward trend detected - monitor for continued growth"
                    )
    ...
    def _add_to_history(
        self, history: List[Dict[str, Any]], data: Dict[str, Any]
    ) -> None:
    ...
    async def get_scaling_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "last_scaling": (
                    self.scaling_history[-1] if self.scaling_history else None
                ),
            },
            "patterns": {
                "hourly_patterns": self._analyze_hourly_patterns(),
