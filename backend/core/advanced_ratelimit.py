...
class AdvancedRateLimiter:
    ...
    async def _check_rule(self, rule: RateLimitRule, key: str) -> Dict[str, Any]:
        ...
        if state.current_requests >= rule.burst_limit:
            ...
            state.blocked_until = state.window_start + timedelta(seconds=rule.window_seconds)
            return {
                "allowed": False,
                "remaining": 0,
                "reset_time": state.blocked_until.isoformat(),
        ...
        if state.current_requests >= rule.requests:
            return {
                ...
                "retry_after": int((state.window_start + timedelta(seconds=rule.window_seconds) - now).total_seconds())
            }
    ...
    async def cleanup_expired_states(self):
        ...
        async with self._lock:
            ...
            for key, state in self.states.items():
                ...
                if state.last_request and now - state.last_request > timedelta(hours=24):
    ...
    async def get_global_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            for key, state in self.states.items():
                if state.last_request and datetime.utcnow() - state.last_request < timedelta(hours=1):
                    if "user:" in key:
