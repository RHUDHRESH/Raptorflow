...
class RequestDeduplicator:
    ...
    def __init__(
        ...
        self.content_hash_index: Dict[str, Set[str]] = defaultdict(set)  # content_hash -> request_ids
        self.semantic_hash_index: Dict[str, Set[str]] = defaultdict(set)  # semantic_hash -> request_ids
    ...
    async def add_request(self, request: InferenceRequest) -> Tuple[bool, Optional[InferenceRequest]]:
        ...
        async with self._lock:
            ...
            if exact_duplicate:
                ...
                logger.info(f"Found exact duplicate for request {request.id}: {exact_duplicate.id}")
            ...
            if semantic_duplicate:
                ...
                logger.info(f"Found semantic duplicate for request {request.id}: {semantic_duplicate.id}")
            ...
            total_duplicates = self.stats["exact_duplicates"] + self.stats["semantic_duplicates"]
            self.stats["deduplication_rate"] = total_duplicates / self.stats["total_requests"]
            
            return True, None
    
    async def _find_exact_duplicate(self, request: InferenceRequest) -> Optional[InferenceRequest]:
    ...
    async def _find_semantic_duplicate(self, request: InferenceRequest) -> Optional[InferenceRequest]:
        ...
        for req_id in similar_ids:
            if req_id in self.pending_requests:
                ...
                if not similar_request.is_expired():
                    ...
                    similarity = await self._calculate_similarity(request.prompt, similar_request.prompt)
    ...
    async def mark_completed(self, request_id: str, response: Any, error: Optional[str] = None):
        ...
        async with self._lock:
            if request_id in self.processing_requests:
                ...
                request.error = error
                request.status = RequestStatus.COMPLETED if error is None else RequestStatus.FAILED
    ...
    async def get_request(self, request_id: str) -> Optional[InferenceRequest]:
        ...
        async with self._lock:
            return (self.pending_requests.get(request_id) or 
                   self.processing_requests.get(request_id) or 
                   self.completed_requests.get(request_id))
    
    async def get_pending_requests(self, model_name: Optional[str] = None) -> List[InferenceRequest]:
...
class BatchProcessor:
    ...
    async def add_request(self, request: InferenceRequest) -> Tuple[bool, Optional[str]]:
    ...
    async def _create_batches(self):
        ...
        async with self._lock:
            for model_name, queue in self.model_queues.items():
                ...
                if len(requests_to_batch) >= 2:  # Only batch if we have multiple requests
    ...
    async def _complete_batch(self, batch: BatchRequest, success: bool, error: Optional[str] = None):
        ...
        async with self._lock:
            ...
            if total_batches > 0:
                self.stats["average_batch_size"] = self.stats["total_requests_processed"] / total_batches
            
            # Calculate batch efficiency (requests per batch)
            if self.stats["successful_batches"] > 0:
                self.stats["batch_efficiency"] = (
                    self.stats["total_requests_processed"] / self.stats["successful_batches"]
                )
    ...
    async def get_batch(self, batch_id: str) -> Optional[BatchRequest]:
        """Get batch by ID."""
        async with self._lock:
            return (self.pending_batches.get(batch_id) or 
                   self.processing_batches.get(batch_id) or 
                   self.completed_batches.get(batch_id))
            ...
            return {
                "batch_processor": {
                    **self.stats,
                    ...
                    "model_queues": {model: len(queue) for model, queue in self.model_queues.items()},
                },
