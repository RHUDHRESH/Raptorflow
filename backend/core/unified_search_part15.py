...
from core.unified_search_part1 import ContentType, SearchMode, SearchProvider
from core.unified_search_part8 import metrics_collector, performance_tracker
...
class UsageAnalyzer:
    ...
    async def analyze_query_patterns(
        ...
        recent_operations = [
            op
            for op in performance_tracker.completed_operations
    ...
    async def analyze_user_behavior(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_session_length": (
                ...
                / len(set(op.operation_id.split("_")[0] for op in recent_operations))
                if recent_operations
                else 0
            ),
        }

    async def analyze_content_preferences(
        self, time_range_hours: int = 24
    ) -> Dict[str, Any]:
...
class PerformanceAnalyzer:
    ...
    async def analyze_response_times(
        ...
        return {
            ...
            "slow_operations": [
                op.to_dict()
                for op in recent_operations
                if op.duration_ms > percentiles["p95"]
            ],
        }
    ...
    def _analyze_error_trends(self, operations: List) -> Dict[str, Any]:
        ...
        return {
            ...
            "peak_error_hour": (
                max(errors_by_hour.items(), key=lambda x: x[1])[0]
                if errors_by_hour
                else None
            ),
        }
...
class QualityAnalyzer:
    ...
    async def analyze_result_quality(
        self, time_range_hours: int = 24
    ) -> Dict[str, Any]:
        ...
        avg_results_count = statistics.mean(
            [op.results_count for op in recent_operations]
        )
        cache_hit_rate = sum(1 for op in recent_operations if op.cache_hit) / len(
            recent_operations
        )
        ...
        for provider in SearchProvider:
            ...
            if provider_ops:
                success_rate = sum(1 for op in provider_ops if op.success) / len(
                    provider_ops
                )
                avg_response_time = statistics.mean(
                    [op.duration_ms for op in provider_ops]
                )
                provider_quality[provider.value] = {
                    "success_rate": success_rate,
                    "avg_response_time": avg_response_time,
                    "quality_score": success_rate
                    * (1000 / max(avg_response_time, 100)),  # Proxy quality
                }
    ...
    def _analyze_quality_trends(self, operations: List) -> Dict[str, Any]:
        ...
        for op in operations:
            if op.success:
                hour = op.start_time.hour
                # Proxy quality score
                quality_score = min(1.0, op.results_count / 10) * (
                    1000 / max(op.duration_ms, 100)
                )
        ...
        return {
            ...
            "quality_variance": (
                statistics.variance(list(hourly_quality.values()))
                if len(hourly_quality) > 1
                else 0
            ),
        }
    ...
    async def generate_comprehensive_insights(
        ...
        try:
            ...
            insights["usage_insights"] = (
                await self.usage_analyzer.analyze_query_patterns(time_range_hours)
            )
            insights["usage_insights"].update(
                await self.usage_analyzer.analyze_user_behavior(time_range_hours)
            )
            insights["usage_insights"].update(
                await self.usage_analyzer.analyze_content_preferences(time_range_hours)
            )

            # Performance insights
            insights["performance_insights"] = (
                await self.performance_analyzer.analyze_response_times(time_range_hours)
            )
            insights["performance_insights"].update(
                await self.performance_analyzer.analyze_success_rates(time_range_hours)
            )

            # Quality insights
            insights["quality_insights"] = (
                await self.quality_analyzer.analyze_result_quality(time_range_hours)
            )
            ...
            insights["trends"] = {
                "search_volume": (
                    await self.trend_analyzer.analyze_trends(
                        "search_volume", time_range_hours
                    )
                ).to_dict(),
                "response_time": (
                    await self.trend_analyzer.analyze_trends(
                        "response_time", time_range_hours
                    )
                ).to_dict(),
                "success_rate": (
                    await self.trend_analyzer.analyze_trends(
                        "success_rate", time_range_hours
                    )
                ).to_dict(),
            }
            ...
            insights["anomalies"] = [
                anomaly.to_dict()
                for anomaly in await self.anomaly_detector.detect_anomalies(
                    time_range_hours
                )
            ]
    ...
    def _generate_recommendations(self, insights: Dict[str, Any]) -> List[str]:
        ...
        if avg_response_time > 3000:  # 3 seconds
            recommendations.append(
                "Consider optimizing search algorithms to reduce response times"
            )
        ...
        if query_diversity < 0.5:
            recommendations.append(
                "Encourage more diverse query patterns to improve system utilization"
            )
        ...
        if cache_hit_rate < 0.3:
            recommendations.append(
                "Implement cache warming strategies to improve performance"
            )
