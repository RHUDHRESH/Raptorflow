...
class FederationRegistry:
    ...
    async def update_member_metrics(
        self, member_id: str, response_time_ms: float, success: bool
    ):
        ...
        async with self._lock:
            ...
            # Update success rate
            member.success_rate = (
                member.total_requests - member.failed_requests
            ...
            if member.response_time_ms == 0:
                ...
                member.response_time_ms = (member.response_time_ms * 0.8) + (
    ...
    def get_registry_stats(self) -> Dict[str, Any]:
        ...
        active_members = len(
            [m for m in self.members.values() if m.status == FederationStatus.ACTIVE]
        )
        available_members = len([m for m in self.members.values() if m.is_available])

        capability_counts = {
            cap: len(members) for cap, members in self.capabilities_index.items()
        }
        ...
        return {
            ...
            "protocols": {
                protocol.value: len(
                    [m for m in self.members.values() if m.protocol == protocol]
                )
...
class FederationQueryRouter:
    ...
    async def _route_broadcast(
        ...
        if target_capabilities:
            ...
            for capability in target_capabilities:
                capable_members.extend(
                    await self.registry.get_members_by_capability(capability)
                )
    ...
    async def _route_adaptive(
        ...
        for member in members:
            ...
            time_score = max(
                0, 1 - (member.response_time_ms / 5000)
            )  # Normalize to 0-1
...
class FederationExecutor:
    ...
    async def execute_federated_search(
        ...
        try:
            ...
            merged_results = await self._merge_results(
                successful_responses, fed_query.merge_strategy
            )

            # Update member metrics
            for response in successful_responses:
                await self.registry.update_member_metrics(
                    response.member_id, response.response_time_ms, True
                )

            # Record failed requests
            for member in target_members:
                if not any(
                    r.member_id == member.member_id for r in successful_responses
                ):
                    await self.registry.update_member_metrics(
                        member.member_id, timeout_seconds * 1000, False
                    )
    ...
    async def _execute_member_query(
        self, member: FederationMember, fed_query: FederationQuery
        ...
        try:
            ...
            if member.protocol == FederationProtocol.REST_API:
                results, response_time = await self._execute_rest_query(
                    member, fed_query
                )
            elif member.protocol == FederationProtocol.GRPC:
                results, response_time = await self._execute_grpc_query(
                    member, fed_query
                )
            else:
    ...
    async def _execute_rest_query(
        ...
        payload = {
            ...
            "content_types": [
                ct.value for ct in fed_query.original_query.content_types
            ],
        }
        ...
        if member.authentication:
            ...
            if auth_type == "api_key":
                headers["Authorization"] = (
                    f"Bearer {member.authentication.get('api_key')}"
                )
    ...
    async def _merge_results(
        self, responses: List[FederationResponse], merge_strategy: str
    ) -> List[SearchResult]:
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        if total_queries > 0:
            ...
            return {
                ...
                "avg_members_per_query": sum(
                    len(q["selected_members"]) for q in self.query_history
                )
                / total_queries,
            }
        else:
    ...
    async def search_federated(
        ...
        try:
            ...
            for member_id in metadata.get("target_members", []):
                self.metrics.member_utilization[member_id] = (
                    self.metrics.member_utilization.get(member_id, 0) + 1
                )
            ...
            self.metrics.error_types["execution_error"] = (
                self.metrics.error_types.get("execution_error", 0) + 1
            )
        ...
        try:
            if member.protocol == FederationProtocol.REST_API:
                ...
                async with self.executor.http_session.get(
                    ...
                    if response.status == 200:
                        await self.registry.update_member_status(
                            member.member_id, FederationStatus.ACTIVE
                        )
                    else:
                        await self.registry.update_member_status(
                            member.member_id, FederationStatus.ERROR
                        )
