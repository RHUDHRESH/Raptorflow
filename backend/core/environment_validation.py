...
def validate_optional_environment() -> Dict[str, Any]:
    ...
    config = {
        ...
        "enable_rate_limiting": os.getenv("ENABLE_RATE_LIMITING", "false").lower()
        == "true",
        "enable_usage_tracking": os.getenv("ENABLE_USAGE_TRACKING", "true").lower()
        == "true",
        "enable_background_jobs": os.getenv("ENABLE_BACKGROUND_JOBS", "false").lower()
        == "true",
    }
    ...
    if config["max_file_size_mb"] <= 0 or config["max_file_size_mb"] > 1000:
        logger.warning(
            f"Invalid MAX_FILE_SIZE_MB: {config['max_file_size_mb']}, using default 50"
        )
