...
import httpx
from httpx import AsyncClient, Response, TimeoutException, ConnectError, NetworkError

from core.enhanced_exceptions import (
    ExternalServiceError,
    NetworkError as RaptorNetworkError,
        ...
        try:
            ...
            if limit > 0:
                return cls(
                    limit=limit,
                    remaining=remaining,
                    reset_time=datetime.fromtimestamp(reset_time) if reset_time > 0 else datetime.utcnow() + timedelta(seconds=retry_after),
...
class APIIntegrationClient:
    ...
    async def request(
        ...
        **kwargs
        ...
        if self.client is None:
            raise ExternalServiceError("API client not initialized", service=self.config.base_url)
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                logger.warning(f"Request timeout (attempt {attempt + 1}/{self.config.max_retries}): {method} {endpoint}")
                
                if attempt < self.config.max_retries - 1:
                    await asyncio.sleep(self.config.retry_delay * (self.config.retry_backoff_factor ** attempt))
                    continue
                break
                ...
                logger.warning(f"Connection error (attempt {attempt + 1}/{self.config.max_retries}): {method} {endpoint}")
                ...
                last_error = e
                self.metrics.connection_errors += 1
                logger.warning(f"Network error (attempt {attempt + 1}/{self.config.max_retries}): {method} {endpoint}")
                ...
                if response.status_code == 429:
                    ...
                    if attempt < self.config.max_retries - 1:
                        logger.warning(f"Rate limited (attempt {attempt + 1}/{self.config.max_retries}): {method} {endpoint}")
                ...
                if attempt < self.config.max_retries - 1:
                    logger.warning(f"Server error (attempt {attempt + 1}/{self.config.max_retries}): {method} {endpoint} - {response.status_code}")
                    await asyncio.sleep(self.config.retry_delay * (self.config.retry_backoff_factor ** attempt))
                    continue
                break
        ...
        if isinstance(last_error, TimeoutException):
            ...
            if last_error.response.status_code == 429:
                retry_after = int(last_error.response.headers.get("Retry-After", self.config.retry_delay))
                raise handle_rate_limit_error(
                    f"API rate limit exceeded: {last_error.response.status_code}",
                    limit=self.metrics.rate_limit_info.limit if self.metrics.rate_limit_info else self.config.rate_limit_requests,
                    window=self.config.rate_limit_window,
                    retry_after=retry_after
                )
            else:
    ...
    def _get_circuit_breaker_retry_after(self) -> int:
        ...
        time_since_open = datetime.utcnow() - self._circuit_breaker_time
        retry_after = self.config.circuit_breaker_timeout - int(time_since_open.total_seconds())
    ...
    def get_client_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if self.metrics.rate_limit_info else None,
            "config": {
                "base_url": self.config.base_url,
