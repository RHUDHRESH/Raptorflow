...
class MemoryCache:
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        async with self._lock:
            ...
            self.stats.avg_access_time_ms = (
                (self.stats.avg_access_time_ms * (self.stats.total_requests - 1) + access_time_ms) /
    ...
    async def _evict_entry(self) -> bool:
        ...
        if self.eviction_policy == EvictionPolicy.LRU:
            ...
            key_to_evict = min(self.frequency_order.keys(), key=lambda k: self.frequency_order[k])
            ...
            # Evict largest entry
            key_to_evict = max(self.cache.keys(), key=lambda k: self.cache[k].size_bytes)
...
class HybridCache:
    ...
    async def set(
        ...
        if CacheLevel.DISTRIBUTED in cache_levels and self.distributed_cache:
            success &= await self.distributed_cache.set(key, value, ttl_seconds, metadata)
    ...
    def get_combined_stats(self) -> Dict[str, Any]:
        ...
        distributed_stats = self.distributed_cache.get_stats() if self.distributed_cache else None
        ...
        return {
            ...
            'combined': {
                ...
                'total_size_mb': memory_stats.memory_usage_mb + disk_stats.disk_usage_mb,
...
class CacheManager:
    ...
    def __init__(self):
        self.memory_cache = MemoryCache(max_size_mb=256, eviction_policy=EvictionPolicy.LRU)
        self.disk_cache = DiskCache(max_size_mb=1024, compression_enabled=True)
        self.distributed_cache = DistributedCache()
        self.hybrid_cache = HybridCache(self.memory_cache, self.disk_cache, self.distributed_cache)
        
        # Specialized caches for different data types
        self.search_cache = MemoryCache(max_size_mb=128, eviction_policy=EvictionPolicy.LFU)
        self.result_cache = MemoryCache(max_size_mb=256, eviction_policy=EvictionPolicy.TTL_EXPIRED)
        self.metadata_cache = MemoryCache(max_size_mb=64, eviction_policy=EvictionPolicy.LRU)
    ...
    def _generate_cache_key(self, query: SearchQuery, provider: Optional[SearchProvider] = None) -> str:
    ...
    async def get_cached_query_metadata(self, query: SearchQuery) -> Optional[Dict[str, Any]]:
