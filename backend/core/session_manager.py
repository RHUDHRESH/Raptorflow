...
class SessionManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Session manager initialized: TTL={session_ttl_minutes}min, max_sessions={max_sessions_per_user}"
        )
    ...
    def create_session(
        ...
        try:
            ...
            logger.info(
                f"Created session {session_id} for user {user_id} in workspace {workspace_id}"
            )
    ...
    def get_workspace_sessions(self, workspace_id: str) -> List[str]:
        ...
        try:
            ...
            for session_id, session in self.sessions.items():
                if (
                    session.session_data.workspace_id == workspace_id
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            expired_sessions = len(
                [s for s in self.sessions.values() if s.status == SessionStatus.EXPIRED]
            )
