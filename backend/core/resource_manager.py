...
from core.enhanced_exceptions import SystemError, handle_system_error
...
class ResourceManager:
    ...
    def __init__(
        self, cleanup_interval: float = 60.0, max_resource_age: float = 3600.0
    ):
    ...
    def register_resource(
        ...
        with self._lock:
            ...
            self._resource_refs[resource_id] = weakref.ref(
                resource, lambda ref: self._resource_deallocated(resource_id)
            )
    ...
    async def cleanup_all_resources(self) -> Dict[str, Any]:
        ...
        """
        cleanup_results = {"success": 0, "failed": 0, "already_cleaned": 0}
    ...
    async def _periodic_cleanup(self):
        ...
        with self._lock:
            for resource_id, resource_info in self._resources.items():
                ...
                last_access_age = (
                    current_time - resource_info.last_accessed
                ).total_seconds()
                if (
                    last_access_age > self.max_resource_age / 2
                    and resource_info.ref_count == 0
                ):
                    resources_to_cleanup.append(resource_id)
        ...
        if resources_to_cleanup:
            logger.info(
                f"Periodic cleanup: cleaned {len(resources_to_cleanup)} old resources"
            )
    ...
    async def _check_memory_usage(self):
        ...
        try:
            ...
            self._metrics.memory_metrics.peak_usage = max(
                self._metrics.memory_metrics.peak_usage, current_memory
            )
    ...
    async def force_garbage_collection(self):
        ...
        logger.debug(
            f"Garbage collection completed: {collected} objects collected in {gc_time:.3f}s"
        )
...
def auto_cleanup(
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            # Register resource
            cleanup_func = getattr(
                resource, "close", getattr(resource, "cleanup", None)
            )
            resource_id = manager.register_resource(
...
if __name__ == "__main__":
    ...
    async def test_resource_manager():
        ...
        resource_id = manager.register_resource(
            resource, ResourceType.CUSTOM, cleanup_func=resource.close
        )
