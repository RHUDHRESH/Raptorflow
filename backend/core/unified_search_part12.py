...
from backend.core.unified_search_part1 import SearchMode, ContentType
from backend.core.unified_search_part7 import SimpleSearchRequest, SimpleResearchRequest
from backend.core.unified_search_part10 import unified_search_engine, get_search_interface
...
class SearchRequest(BaseModel):
    ...
    mode: str = Field(default="standard", description="Search mode: lightning, standard, deep, exhaustive")
    max_results: int = Field(default=10, ge=1, le=100, description="Maximum number of results")
    content_types: List[str] = Field(default=["web"], description="Content types to search")
    ...
    def validate_content_types(cls, v):
        ...
        for ct in v:
            if ct not in valid_types:
                raise ValueError(f"Invalid content type: {ct}. Must be one of: {valid_types}")
        return v
...
class ResearchRequest(BaseModel):
    ...
    depth: str = Field(default="moderate", description="Research depth: surface, moderate, deep, exhaustive")
    max_sources: int = Field(default=20, ge=1, le=100, description="Maximum number of sources")
    time_limit_minutes: int = Field(default=30, ge=5, le=120, description="Time limit in minutes")
    content_types: List[str] = Field(default=["web"], description="Content types to research")
    verification_required: bool = Field(default=True, description="Require content verification")
    synthesis_format: str = Field(default="comprehensive", description="Synthesis format")
...
class CrawlRequest(BaseModel):
    ...
    max_concurrent: int = Field(default=5, ge=1, le=20, description="Maximum concurrent crawls")
    timeout_seconds: int = Field(default=30, ge=5, le=120, description="Timeout per URL")
...
class UnifiedSearchAPI:
    ...
    def setup_routes(self):
        ...
        async def news_search(
            query: str = Query(..., description="Search query"),
            max_results: int = Query(default=10, ge=1, le=30, description="Maximum results"),
        ...
        async def academic_search(
            query: str = Query(..., description="Search query"),
            max_results: int = Query(default=15, ge=1, le=40, description="Maximum results")
        ):
        ...
        async def research(request: ResearchRequest, background_tasks: BackgroundTasks):
            ...
            try:
                ...
                logger.info(f"Research completed [{request_id}]: {len(report.key_findings)} findings")
        ...
        async def get_metrics():
            """Get system metrics."""
            try:
                from backend.core.unified_search_part8 import metrics_collector
        ...
        async def get_analytics(
            ...
            try:
                from backend.core.unified_search_part8 import analytics_engine
        ...
        async def get_config():
            ...
            try:
                from backend.core.unified_search_part9 import config_manager
        ...
        async def reload_config():
            ...
            try:
                ...
                return APIResponse(
                    success=success,
                    data={"message": "Configuration reloaded successfully" if success else "Configuration reload failed"}
                )
            except Exception as e:
    ...
    async def _record_search_metrics(self, request_id: str, query: str, result_count: int):
    ...
    async def _record_research_metrics(self, request_id: str, topic: str, finding_count: int):
        """Record research metrics in background."""
        try:
            from backend.core.unified_search_part8 import metrics_collector
            metrics_collector.increment_counter('api_research_requests_total')
            metrics_collector.record_histogram('api_research_findings_count', finding_count)
        except Exception as e:
