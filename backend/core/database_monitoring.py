...
class DatabaseMonitor:
    ...
    async def _check_connection_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if utilization > 0.9:
                status = "critical"
                alerts.append(f"Connection pool utilization critical: {utilization:.1%}")
    ...
    async def _check_query_performance(self) -> Dict[str, Any]:
        ...
        try:
            # Get slow queries
            slow_queries_result = await self.supabase.table("slow_queries").select("*").limit(10).execute()
            ...
            if summary.get("avg_query_time", 0) > 2.0:
                status = "warning"
                alerts.append(f"High average query time: {summary['avg_query_time']:.2f}s")
    ...
    async def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get slow queries
            slow_queries_result = await self.supabase.table("slow_queries").select("*").limit(20).execute()
            slow_queries = slow_queries_result.data or []
            
            # Get frequent queries
            frequent_queries_result = await self.supabase.table("frequent_queries").select("*").limit(20).execute()
            ...
            return {
                ...
                "recommendations": self._generate_recommendations(patterns, slow_queries)
            }
    ...
    def _generate_recommendations(self, patterns: List[Dict], slow_queries: List[Dict]) -> List[str]:
        ...
        if high_priority_patterns:
            ...
            recommendations.extend([f"- {p.get('recommendation')}" for p in high_priority_patterns[:3]])
        
        # Analyze slow queries
        if slow_queries:
            recommendations.append(f"Found {len(slow_queries)} slow queries:")
            recommendations.extend([f"- Optimize: {q.get('query', '')[:50]}..." for q in slow_queries[:3]])
