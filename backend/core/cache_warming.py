...
class AccessPatternAnalyzer:
    ...
    def detect_pattern(self, key: str) -> AccessPattern:
        ...
        accesses_per_minute = len(key_accesses) / (
            (datetime.now() - timestamps[0]).total_seconds() / 60
        )
        if (
            accesses_per_minute >= self.steady_threshold
            and std_interval < avg_interval * 0.5
        ):
        ...
        if len(key_parts) > 1 and key_parts[-1].isdigit():
            ...
            sequential_keys = [
                k for k in self.pattern_cache.keys() if k.startswith(base_key)
            ]
...
class MLPredictor:
    ...
    def add_training_data(self, records: List[AccessRecord]):
        ...
        for record in records:
            ...
            self.training_data.append(
                {"features": features, "label": label, "timestamp": record.timestamp}
            )
    ...
    def predict_access_probability(
        self, key: str, entity_type: str, context: Dict[str, Any], future_time: datetime
    ) -> float:
    ...
    def _extract_features(self, record: AccessRecord) -> List[float]:
        ...
        recent_accesses = [
            r for r in self.training_data if r["timestamp"] > recent_time
        ]
    ...
    def _extract_features_for_prediction(
        ...
        historical_accesses = [
            d for d in self.training_data if key_pattern in str(d.get("features", []))
        ]
...
class CacheWarmer:
    ...
    async def predict_warming_candidates(
        self, time_horizon: timedelta = timedelta(hours=1)
        ...
        for key in unique_keys:
            try:
                ...
                if probability > 0.3:  # Threshold for warming
                    ...
                    # Estimate benefit
                    benefit = self._estimate_warming_benefit(key, probability, pattern)
    ...
    async def warm_cache(self, predictions: List[WarmingPrediction]):
        ...
        try:
            ...
            self.stats["average_warming_time"] = (current_avg + warming_time) / 2

            logger.info(
                f"Warmed {successful}/{len(predictions)} cache entries in {warming_time:.2f}s"
            )
            ...
            if entity_type in self.data_generators:
                ...
                if data:
                    ...
                    success = await cache.set(
                        ...
                        priority=(
                            "HIGH" if prediction.warming_priority > 7 else "NORMAL"
                        ),
                    )
    ...
    def _calculate_warming_priority(
        self, key: str, probability: float, pattern: AccessPattern
    ) -> int:
    ...
    def _estimate_warming_benefit(
        ...
        historical_accesses = [
            r for r in self.pattern_analyzer.access_history if r.key == key
        ]
        ...
        while True:
            try:
                ...
                self.ml_predictor.training_data = [
                    d
                    for d in self.ml_predictor.training_data
