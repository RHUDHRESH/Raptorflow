...
class DependencyManager:
    ...
    def require_dependency(
        self, name: str, error_message: Optional[str] = None
    ) -> bool:
        """
...
class MLDependencies:
    ...
    def is_ml_available(self) -> bool:
        ...
        return self.dep_manager.is_available("torch") and self.dep_manager.is_available(
            "sklearn"
        )

    def is_advanced_ml_available(self) -> bool:
        """Check if advanced ML features are available."""
        return self.is_ml_available() and self.dep_manager.is_available("transformers")

    def is_optimization_available(self) -> bool:
        """Check if hyperparameter optimization is available."""
        return (
            self.dep_manager.is_available("optuna")
            or self.dep_manager.is_available("ray")
            or self.dep_manager.is_available("hyperopt")
        )
...
class DataDependencies:
    ...
    def _check_data_deps(self):
        ...
        data_deps = ["pandas", "numpy", "pyarrow", "polars"]
...
def require_core_dependencies():
    ...
    core_deps = ["fastapi", "uvicorn", "pydantic", "asyncpg", "redis", "httpx"]
