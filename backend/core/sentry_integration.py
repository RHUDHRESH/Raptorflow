...
class SentryIntegrationManager:
    ...
    def _load_config_from_env(self) -> SentryConfig:
        ...
        return SentryConfig(
            ...
            attach_stacktrace=os.getenv("SENTRY_ATTACH_STACKTRACE", "true").lower()
            == "true",
            send_default_pii=os.getenv("SENTRY_SEND_DEFAULT_PII", "false").lower()
            == "true",
            send_request_payloads=os.getenv(
                "SENTRY_SEND_REQUEST_PAYLOADS", "true"
            ).lower()
            == "true",
            send_response_payloads=os.getenv(
                "SENTRY_SEND_RESPONSE_PAYLOADS", "true"
            ).lower()
            == "true",
    ...
    def _initialize_sdk(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                # Remove None values
                sentry_config = {
                    k: v for k, v in sentry_config.items() if v is not None
                }
                ...
                self._logger.info(
                    f"Sentry SDK initialized successfully in {self.config.environment.value}"
                )
    ...
    def _before_breadcrumb_handler(
            ...
            if "headers" in request and isinstance(request["headers"], dict):
                for key in list(request["headers"].keys()):
                    if any(
                        ...
                        for pattern in ["authorization", "cookie", "token"]
                    ):
    ...
    def _set_initial_tags(self) -> None:
        ...
        configure_scope(
            lambda scope: set_tag("environment", self.config.environment.value)
        )
