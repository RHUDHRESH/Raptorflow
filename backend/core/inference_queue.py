...
class WorkerInfo:
    ...
    # Configuration
    supported_priorities: List[QueuePriority] = field(default_factory=lambda: list(QueuePriority))
    ...
    def get_load_percentage(self) -> float:
        ...
        return (self.current_load / self.max_concurrent) * 100 if self.max_concurrent > 0 else 0
...
class InferenceQueue:
    ...
    async def enqueue(self, request: InferenceRequest) -> bool:
        ...
        async with self._lock:
            ...
            if total_queued >= self.max_size:
                ...
                logger.warning(f"Queue {self.queue_id} is full, dropping request {request.id}")
                return False
            ...
            logger.debug(f"Enqueued request {request.id} with priority {priority.value}")
            return True
        ...
        async with self._lock:
            ...
            if self.queue_type == QueueType.PRIORITY:
                ...
                for priority, queue in self.queues.items():
                    if queue and (oldest_time is None or queue[0].created_at < oldest_time):
                        oldest_request = queue[0]
                        oldest_time = queue[0].created_at
                
                if oldest_request:
                    request = self.queues[self._get_priority_from_request(oldest_request)].popleft()
                ...
                if available_requests:
                    ...
                    if total_weight > 0:
                        ...
                        selected_idx = np.random.choice(len(available_requests), p=probabilities)
    ...
    def _update_metrics(self):
        ...
        completed_requests = [
            req for _, req in self.request_history
            if req.status in [RequestStatus.COMPLETED, RequestStatus.FAILED] and
            req.started_at and req.completed_at
        ]
        ...
        if completed_requests:
            ...
            self.metrics.avg_processing_time = np.mean(processing_times) if processing_times else 0
    ...
    async def get_metrics(self) -> QueueMetrics:
        ...
        async with self._lock:
            ...
            self.metrics.pending_requests = sum(len(queue) for queue in self.queues.values())
...
class InferenceQueueManager:
    ...
    async def get_available_workers(self, queue_id: Optional[str] = None) -> List[WorkerInfo]:
        ...
        async with self._lock:
            ...
            for worker_id, worker in self.workers.items():
                if worker.is_available():
                    if queue_id is None or self.worker_assignments.get(worker_id) == queue_id:
    ...
    async def _auto_scale_workers(self):
        ...
        while self._running:
            try:
                ...
                # Scale up if needed
                if (system_load > self.scale_up_threshold or total_pending > 10) and \
                   len(self.workers) < self.max_workers:
                    await self._scale_up()
                
                # Scale down if needed
                elif system_load < self.scale_down_threshold and total_pending == 0 and \
                     len(self.workers) > 1:
    ...
    async def _scale_down(self):
        ...
        if idle_workers and len(self.workers) > 1:
            ...
            worker_to_remove = max(idle_workers, key=lambda w: (datetime.utcnow() - w.last_activity).total_seconds())
        ...
        while self._running:
            try:
                async with self._lock:
                    ...
                    if self.queues:
                        wait_times = [
                            queue.metrics.avg_wait_time for queue in self.queues.values()
                        ]
                        self.stats["avg_queue_wait_time"] = np.mean(wait_times) if wait_times else 0
                    
                    # Calculate system throughput
                    total_processed = sum(
                        queue.metrics.completed_requests for queue in self.queues.values()
                    )
...
class LoadBalancer:
    ...
    def select_worker(self, workers: List[WorkerInfo]) -> Optional[WorkerInfo]:
        ...
        if self.strategy == "round_robin":
            ...
            weights = [w.health_score * (1 - w.get_load_percentage() / 100) for w in workers]
            total_weight = sum(weights)
            if total_weight > 0:
                probabilities = [w / total_weight for w in weights]
