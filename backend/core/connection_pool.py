    ...
    async def _perform_health_checks(self) -> None:
        ...
        for conn_info in self._pool:
            ...
            try:
                ...
                is_healthy = await self._check_connection_health(conn_info["connection"])
                conn_info["health_status"] = "healthy" if is_healthy else "unhealthy"
                
                if not is_healthy:
                    logger.warning(f"Connection failed health check, removing from pool")
                    await self._close_connection(conn_info["connection"])
...
class DatabaseConnectionManager:
    ...
    async def release_connection(self, connection: Any, pool_name: str = "supabase") -> None:
...
async def get_connection_pool_health() -> Dict[str, Any]:
    ...
    for pool_name, pool_stats in stats.items():
        health_status["pools"][pool_name] = {
            "status": "healthy" if pool_stats["available_connections"] > 0 else "unhealthy",
            "total": pool_stats["pool_size"],
            "active": pool_stats["active_connections"],
            "available": pool_stats["available_connections"],
            "utilization": pool_stats["active_connections"] / pool_stats["pool_size"] if pool_stats["pool_size"] > 0 else 0
        }
