...
class MonitoringDashboard:
    ...
    async def get_dashboard_data(
        self, dashboard_type: DashboardType = DashboardType.OVERVIEW
    ) -> Dict[str, Any]:
            ...
            "recent_errors": [
                {
                    ...
                    "timestamp": (
                        error.context.timestamp.isoformat() if error.context else None
                    ),
                }
    ...
    async def _get_widget_data(
        self, widget: DashboardWidget, monitor, error_handler
    ) -> Dict[str, Any]:
    ...
    async def trigger_alert(self, rule_id: str, metric_value: float):
        ...
        logger.warning(
            f"Alert triggered: {rule.name} - Value: {metric_value}, Threshold: {rule.threshold}"
        )

    async def _send_alert_notifications(
        self, alert_data: Dict[str, Any], channels: List[AlertChannel]
    ):
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(
            f"Dashboard has {len(config['widgets'])} widgets and {len(config['alert_rules'])} alert rules"
        )
