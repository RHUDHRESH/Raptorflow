...
class MetricsCollector:
    ...
    def __init__(self, max_values: int = 100000, aggregation_interval: int = 60):
        ...
        # Metric storage
        self.metric_values: Dict[str, deque] = defaultdict(lambda: deque(maxlen=max_values))
    ...
    def _initialize_default_metrics(self):
        ...
        default_metrics = [
            ...
            MetricDefinition(
                ...
                aggregation_methods=[AggregationMethod.AVERAGE, AggregationMethod.P95, AggregationMethod.P99],
            ),
            MetricDefinition(
                name="request_rate",
                category=MetricCategory.PERFORMANCE,
                ...
                for method in metric_def.aggregation_methods:
                    ...
                    if aggregated_value is not None:
                        ...
                        if len(self.metric_aggregations[metric_name]) > 1000:
                            self.metric_aggregations[metric_name] = self.metric_aggregations[metric_name][-1000:]
            
            self.collection_stats["aggregation_time_ms"] = (time.time() - start_time) * 1000
            
        except Exception as e:
            logger.error(f"Aggregation computation error: {e}")
    
    def _compute_aggregation(self, values: List[float], method: AggregationMethod) -> Optional[float]:
    ...
    async def _evaluate_alerts(self):
        ...
        try:
            ...
            self.collection_stats["alert_evaluation_time_ms"] = (time.time() - start_time) * 1000
    ...
    async def _cleanup_old_data(self):
        ...
        for metric_name in list(self.metric_aggregations.keys()):
            ...
            self.metric_aggregations[metric_name] = [
                ...
                if agg.time_window_end >= cutoff_time
            ]
    ...
    def get_metric_aggregations(
        ...
        try:
            ...
            if aggregation_method:
                aggregations = [agg for agg in aggregations if agg.aggregation_method == aggregation_method]
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "total_aggregations": sum(len(agg) for agg in self.metric_aggregations.values()),
