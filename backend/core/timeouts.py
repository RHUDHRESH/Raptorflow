...
class TimeoutError(Exception):
    ...
    def __init__(self, message: str, timeout_seconds: int, elapsed_seconds: float, operation_type: OperationType = None, recovery_suggestions: List[RecoveryStrategy] = None):
...
class TimeoutManager:
        ...
        try:
            ...
            try:
                ...
                raise TimeoutError(
                    ...
                    elapsed,
                    operation_type,
                    recovery_strategies or self._get_default_recovery_strategies(operation_type)
                )
    ...
    def _get_default_recovery_strategies(self, operation_type: OperationType) -> List[RecoveryStrategy]:
            ...
            try:
                if strategy == RecoveryStrategy.RETRY_WITH_BACKOFF:
                    ...
                    for attempt in range(self.config.max_retries):
                        ...
                        logger.info(f"Recovery attempt {attempt + 1}/{self.config.max_retries} for {task_id}")
                    ...
                    try:
                        ...
                        if cached:
                            return {"recovered": True, "strategy": "cached_response", "data": cached}
                    except ImportError:
                        pass
    ...
    async def execute_with_enhanced_timeout(
        ...
        try:
            ...
            return self._set_error(
                state, 
                f"Agent execution timed out after {e.elapsed_seconds:.1f}s"
            )
