...
class PerformanceMonitor:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        critical_alerts = [
            a for a in active_alerts if a.rule.severity == AlertSeverity.CRITICAL
        ]
        error_alerts = [
            a for a in active_alerts if a.rule.severity == AlertSeverity.ERROR
        ]
        warning_alerts = [
            a for a in active_alerts if a.rule.severity == AlertSeverity.WARNING
        ]
    ...
    async def cleanup_old_data(self, older_than_hours: int = 24):
        ...
        async with self._lock:
            ...
            self.alert_history = [
                ...
                or (a.resolved_at and a.resolved_at > cutoff_time)
            ]
...
class PerformanceMonitoringMiddleware:
    ...
    async def _monitor_request(self, scope, receive, send):
        ...
        await self.monitor.record_metric(
            "request_rate",
            1.0,  # 1 request
            tags={"method": scope["method"], "path": scope["path"]},
            unit="req/s",
        )
...
def monitor_performance(
    metric_name: str, tags: Optional[Dict[str, str]] = None, unit: str = ""
):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Record error rate
                await record_metric(
                    "error_rate", 1.0, {"function": func.__name__}, "count"
                )
