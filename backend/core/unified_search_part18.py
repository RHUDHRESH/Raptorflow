...
class StreamingSearchExecutor:
    ...
    async def create_session(self, user_id: Optional[str] = None) -> str:
        ...
        session = RealtimeSearchSession(session_id=session_id, user_id=user_id)
    ...
    async def _process_search_request(
        self, search_request: Dict[str, Any], worker_name: str
    ):
    ...
    async def _execute_search_streaming(
        self, query: SearchQuery, providers: List[SearchProvider]
    ...
    def _generate_mock_results(
        self, query: SearchQuery, provider: SearchProvider, count: int
    ) -> List[SearchResult]:
        ...
        for i in range(count):
            result = SearchResult(
                ...
                content=f"This is mock content from {provider.value} for the query: {query.text}. "
                * 10,
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        completed_searches = sum(
            s.completed_searches for s in self.active_sessions.values()
        )
        ...
        return {
            ...
            "success_rate": (
                completed_searches / total_searches if total_searches > 0 else 0
            ),
...
class RealtimeSearchAPI:
    ...
    async def _handle_websocket_message(
        self, websocket: WebSocket, session_id: str, message: str
    ):
    ...
    async def _handle_search_request(
        ...
        try:
            ...
            search_query = SearchQuery(
                ...
                mode=(
                    SearchMode(mode)
                    if mode in [m.value for m in SearchMode]
            ...
            request_id = await self.executor.execute_streaming_search(
                session_id, search_query, provider_enums
            )

            await websocket.send_text(
                json.dumps({"type": "search_started", "request_id": request_id})
            )
...
class LiveDataManager:
    ...
    def __init__(self):
        ...
        self.subscribers: Dict[str, Set[str]] = defaultdict(
            set
        )  # stream_id -> set of session_ids
