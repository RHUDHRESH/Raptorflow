...
from core.enhanced_exceptions import SystemError
from core.enhanced_exceptions import TimeoutError as RaptorTimeoutError
from core.enhanced_exceptions import handle_system_error, handle_timeout_error
...
class AsyncTaskManager:
    ...
    async def initialize(self):
        ...
        logger.info(
            f"Async task manager initialized (max_concurrent={self.config.max_concurrent_tasks})"
        )
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    if attempt < retries:
                        ...
                        logger.warning(
                            f"Task {name} failed (attempt {attempt + 1}/{retries + 1}), retrying in {delay}s: {e}"
                        )
                        await asyncio.sleep(delay)
                    else:
                        logger.error(
                            f"Task {name} failed after {retries + 1} attempts: {e}"
                        )
                        raise
...
async def run_with_timeout(
    coro: Coroutine, timeout: float, operation_name: str = "operation"
) -> Any:
...
async def gather_with_concurrency(
    coros: List[Coroutine], max_concurrent: int = 10, timeout: Optional[float] = None
) -> List[Any]:
    ...
    try:
        return await asyncio.wait_for(
            asyncio.gather(*bounded_coros, return_exceptions=True), timeout=timeout
        )
    except asyncio.TimeoutError:
        raise handle_timeout_error(
    ...
    async def test_task_manager():
        ...
        for i in range(5):
            task_id = await manager.submit_task(
                sample_task(f"task_{i}", 0.1), f"sample_task_{i}"
            )
