...
class CrawlerConfig:
    ...
    blocked_mime_types: Set[str] = field(
        default_factory=lambda: {
            "application/pdf",
            "application/zip",
...
class ResearchConfig:
    ...
    synthesis_formats: List[str] = field(
        default_factory=lambda: ["summary", "detailed", "comprehensive"]
    )
    ...
    def validate(self) -> List[str]:
        ...
        for threshold_name, threshold_value in self.alert_thresholds.items():
            if threshold_value < 0 or threshold_value > 100:
                errors.append(
                    f"monitoring alert_threshold {threshold_name} must be between 0 and 100"
                )
...
class ConfigManager:
    ...
    async def _load_raw_config(
        self, file_path: str, format: ConfigFormat
    ) -> Dict[str, Any]:
    ...
    def _parse_config(self, raw_config: Dict[str, Any]) -> UnifiedSearchConfig:
        ...
        if "global" in raw_config:
            ...
            config.max_total_results = global_config.get(
                "max_total_results", config.max_total_results
            )
            config.default_search_mode = global_config.get(
                "default_search_mode", config.default_search_mode
            )
            config.default_language = global_config.get(
                "default_language", config.default_language
            )
            config.default_region = global_config.get(
                "default_region", config.default_region
            )
            config.safe_search_default = global_config.get(
        ...
        if "security" in raw_config:
            ...
            config.api_key_required = security_config.get(
                "api_key_required", config.api_key_required
            )
            config.rate_limit_global = security_config.get(
                "rate_limit_global", config.rate_limit_global
            )
            config.cors_enabled = security_config.get(
                "cors_enabled", config.cors_enabled
            )
            config.cors_origins = security_config.get(
        ...
        if "performance" in raw_config:
            ...
            config.connection_pool_size = performance_config.get(
                "connection_pool_size", config.connection_pool_size
            )
            config.request_timeout_seconds = performance_config.get(
                "request_timeout_seconds", config.request_timeout_seconds
            )
            config.max_concurrent_searches = performance_config.get(
    ...
    def get_provider_config(
        self, provider: Union[str, SearchProvider]
    ) -> Optional[ProviderConfig]:
        """Get configuration for specific provider."""
        provider_name = (
            provider.value if isinstance(provider, SearchProvider) else provider
        )
        return self.config.providers.get(provider_name)

    def update_provider_config(
        self, provider: Union[str, SearchProvider], config: ProviderConfig
    ):
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "environment": self.config.environment,
            "enabled_providers": [
                name for name, config in self.config.providers.items() if config.enabled
            ],
            "features": {
