...
class SearchResult:
    ...
    def __post_init__(self):
        ...
        if self.url:
            ...
            self.tld = parsed.hostname.split('.')[-1] if parsed.hostname and '.' in parsed.hostname else ""
...
class BaseSearchProvider(ABC):
    ...
    def update_metrics(self, success: bool, response_time_ms: float):
        ...
        if self.consecutive_failures >= 5:
            ...
            logger.warning(f"Provider {self.name} disabled after {self.consecutive_failures} failures")
