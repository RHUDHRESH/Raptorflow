...
from backend.core.unified_search_part1 import (
    SearchResult, SearchProvider, ContentType, RateLimiter
)
...
class ContentExtractor:
    ...
    async def extract_content(self, url: str, session: aiohttp.ClientSession) -> Optional[ExtractedContent]:
    ...
    async def _extract_with_readability(self, url: str, session: aiohttp.ClientSession) -> Optional[ExtractedContent]:
        ...
        try:
            async with session.get(url, timeout=self.policy.timeout) as response:
                ...
                return ExtractedContent(
                    ...
                    quality_score=self._calculate_quality_score(cleaned_content, metadata),
                    timestamp=time.time()
                )
    ...
    async def _extract_with_beautifulsoup(self, url: str, session: aiohttp.ClientSession) -> Optional[ExtractedContent]:
    ...
    async def _extract_with_jina_reader(self, url: str, session: aiohttp.ClientSession) -> Optional[ExtractedContent]:
        ...
        try:
            ...
            async with session.get(jina_url, headers=headers, timeout=self.policy.timeout) as response:
                if response.status != 200:
                    return None
    ...
    async def _extract_with_firecrawl(self, url: str, session: aiohttp.ClientSession) -> Optional[ExtractedContent]:
    ...
    def _extract_links(self, content: str, base_url: str) -> List[str]:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if full_url.startswith('http') and not any(skip in full_url for skip in ['#', 'javascript:', 'mailto:']):
...
class AdvancedCrawler:
    ...
    async def crawl_urls(self, urls: List[str]) -> List[ExtractedContent]:
        ...
        valid_urls = []
        for url in urls:
            if self._is_valid_url(url) and url not in self.crawled_urls and url not in self.failed_urls:
    ...
    def _is_valid_url(self, url: str) -> bool:
        ...
        try:
            ...
            # Domain filtering
            if self.policy.allowed_domains and parsed.netloc not in self.policy.allowed_domains:
                return False
            
            if self.policy.blocked_domains and parsed.netloc in self.policy.blocked_domains:
                return False
