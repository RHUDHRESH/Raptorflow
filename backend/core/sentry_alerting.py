...
class SentryAlertingManager:
    ...
    def _process_evaluation_result(
        ...
        with self._lock:
            ...
            if len(self._evaluation_results[rule.rule_id]) > 100:
                self._evaluation_results[rule.rule_id] = self._evaluation_results[
                    rule.rule_id
                ][-100:]
    ...
    def _check_rate_limit(self, channel: NotificationChannel) -> bool:
        ...
        if channel.value in self._notification_counters:
            self._notification_counters[channel.value] = [
                timestamp
                for timestamp in self._notification_counters[channel.value]
        ...
        # Check rate limit
        return (
            len(self._notification_counters[channel.value]) < config.rate_limit_per_hour
        )
    ...
    def _send_email_notification(
        ...
        try:
            ...
            # Send email
            with smtplib.SMTP(
                config.config["smtp_server"], config.config["smtp_port"]
            ) as server:
            ...
            self._notification_counters[config.channel_type.value].append(
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_alerts = len(
                [a for a in self._alert_history if a.fired_at > day_ago]
            )
