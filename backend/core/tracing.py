...
class TracingManager:
    ...
    async def start_span(
        ...
        async with self._lock:
            ...
            logger.debug(
                f"Started span {span_id} for trace {trace_id}: {operation_name}"
            )
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            span = await self.tracing_manager.start_span(
                operation_name=operation_name, trace_id=correlation_id
            )
...
class CorrelationContext:
    ...
    async def __aenter__(self):
        ...
        self.original_correlation_id = getattr(
            asyncio.current_task(), "_correlation_id", None
        )
...
async def log_trace_event(
    trace_id: str, event_name: str, level: str = "info", **kwargs
):
    ...
    if active_spans:
        await tracing.add_span_log(
            active_spans[-1].span_id, level, event_name, **kwargs
        )
