...
class MetricsCollector:
    ...
    def set_gauge(
        ...
        metric = Metric(
            name=name, metric_type=MetricType.GAUGE, value=value, labels=labels or {}
        )
        self.metrics.append(metric)
    ...
    def get_metrics_summary(self, since_minutes: int = 60) -> Dict[str, Any]:
        ...
        for key, value in self.counters.items():
            if key in self.labels:
                summary["counters"][key] = {"value": value, "labels": self.labels[key]}

        # Process gauges
        for key, value in self.gauges.items():
            if key in self.labels:
                summary["gauges"][key] = {"value": value, "labels": self.labels[key]}
...
class PerformanceTracker:
    ...
    def end_operation(
        ...
        self.metrics_collector.record_timer(
            "operation_duration_ms", metrics.duration_ms, labels
        )
        self.metrics_collector.increment_counter("operations_total", 1.0, labels)

        if success:
            self.metrics_collector.increment_counter(
                "operations_successful", 1.0, labels
            )
            self.metrics_collector.record_histogram(
                "results_count", results_count, labels
            )
        else:
        ...
        self.metrics_collector.record_histogram(
            "response_size_bytes", response_size_bytes, labels
        )
    ...
    def get_operation_stats(self, since_minutes: int = 60) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_results_count": (
                statistics.mean(result_counts) if result_counts else 0.0
            ),
            "cache_hit_rate": sum(1 for op in recent_operations if op.cache_hit)
...
class AnalyticsEngine:
    ...
    def _analyze_providers(self, time_range_hours: int) -> Dict[str, Any]:
        ...
        for provider in SearchProvider:
            ...
            operations = self.metrics_collector.counters.get(
                f"operations_total[{provider_key},success=True]", 0
            )
            failures = self.metrics_collector.counters.get(
                f"operations_total[{provider_key},success=False]", 0
            )
    ...
    def _analyze_queries(self, time_range_hours: int) -> Dict[str, Any]:
        ...
        recent_operations = [
            op
            for op in self.performance_tracker.completed_operations
        ...
        query_stats["avg_query_length"] = (
            statistics.mean(query_lengths) if query_lengths else 0
        )
        ...
        query_stats["most_common_modes"] = dict(
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if success_rate < 0.9:
            recommendations.append(
                "Success rate is below 90%. Consider investigating error patterns and improving fault tolerance."
            )
        ...
        if avg_duration > 5000:  # 5 seconds
            recommendations.append(
                "Average response time is high. Consider optimizing search algorithms or adding caching."
            )
        ...
        for provider, stats in provider_analysis.items():
            if stats["success_rate"] < 0.8:
                recommendations.append(
                    f"Provider {provider} has low success rate ({stats['success_rate']:.1%}). Consider switching to alternative providers or improving error handling."
                )
        ...
        if cache_hit_rate < 0.3:
            recommendations.append(
                "Cache hit rate is low. Consider implementing more aggressive caching strategies."
            )
