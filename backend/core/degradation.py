...
class CircuitBreaker:
    ...
    async def on_failure(self):
        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            ...
            self.half_open_calls = 0
            logger.warning(f"Circuit breaker OPEN for service {self.service_name} (half-open failure)")
        elif self.state == CircuitBreakerState.CLOSED:
            if self.failure_count >= self.config.failure_threshold:
                self.state = CircuitBreakerState.OPEN
                logger.warning(f"Circuit breaker OPEN for service {self.service_name} (threshold reached)")
...
class GracefulDegradationManager:
        ...
        **kwargs
        ...
        if use_retry and retry_policy:
            return await self._execute_with_retry(
                service_name, func, retry_policy,
                use_circuit_breaker, use_fallback,
                *args, **kwargs
            )
        ...
        **kwargs
        ...
        for attempt in range(1, retry_policy.max_attempts + 1):
            try:
                ...
                logger.warning(f"Attempt {attempt} failed for {service_name}, retrying in {delay:.2f}s: {e}")
    ...
    def register_default_fallbacks(self):
        ...
        async def external_api_fallback(endpoint: str, *args, **kwargs):
            logger.warning(f"External API {endpoint} unavailable, returning default response")
...
def resilient_service(
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if use_retry and retry_policy and service_name not in manager.retry_policies:
