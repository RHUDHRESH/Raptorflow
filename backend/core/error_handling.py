...
class ErrorHandler:
    ...
    async def handle_error(
        ...
        error_details = ErrorDetails(
            ...
            metadata={"type": type(error).__name__, "module": type(error).__module__},
        )
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(
            ...
            for keyword in ["database", "db", "sql", "connection"]
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["external", "api", "service", "third"]
        ):
            return ErrorCategory.EXTERNAL_SERVICE

        # System errors
        if any(
            keyword in error_type.lower() for keyword in ["system", "os", "file", "io"]
        ):
    ...
    def _determine_severity(
        ...
        error_message = str(error).lower()
        if any(
            keyword in error_message for keyword in ["critical", "fatal", "emergency"]
        ):
            return ErrorSeverity.CRITICAL
    ...
    async def get_errors_by_category(
        ...
        async with self._lock:
            filtered_errors = [
                error for error in self.error_log if error.category == category
            ]
...
class ErrorHandlingMiddleware:
    ...
    async def _send_error_response(
        self, error_details: ErrorDetails, scope, receive, send
    ):
...
def create_error_context(
    request: Request, user_id: Optional[str] = None, tenant_id: Optional[str] = None
) -> ErrorContext:
