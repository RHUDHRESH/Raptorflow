...
class CacheEntry:
    ...
    def access(self) -> Dict[str, Any]:
        ...
        time_diff = (self.last_accessed - self.timestamp).total_seconds() / 3600  # hours
        if time_diff > 0:
            self.access_frequency = (self.access_frequency + (self.hit_count / time_diff)) / 2
...
class AgentCache:
    ...
    def _update_memory_usage(self, delta_bytes: int):
        ...
        if self.config.enable_metrics:
            ...
            if len(self._metrics["memory_usage_history"]) > 100:
                self._metrics["memory_usage_history"] = self._metrics["memory_usage_history"][-100:]
    ...
    async def get(
        ...
        try:
            cache_key = self._generate_cache_key(agent_name, request, context, user_id, workspace_id)
    ...
    async def _evict_for_memory(self, required_bytes: int):
        ...
        logger.info(f"Evicted {evicted_count} cache entries, freed {freed_bytes / 1024:.1f} KB")
    ...
    async def _warmup_agent(self, agent_name: str):
        ...
        try:
            ...
            logger.info(f"Warmed up cache for agent {agent_name} with {len(common_requests)} requests")
    ...
    def _get_common_requests_for_agent(self, agent_name: str) -> List[Dict[str, Any]]:
        ...
        common_requests = {
            "icp_architect": [
                {"request": "Generate ICP profile for tech startup", "context": {"industry": "technology"}},
                {"request": "Create customer persona", "context": {"business_type": "B2B"}},
                {"request": "Analyze market segment", "context": {"region": "US"}}
            ],
            "content_generator": [
                {"request": "Generate blog post", "context": {"topic": "AI", "length": "medium"}},
                {"request": "Create social media content", "context": {"platform": "twitter"}},
                {"request": "Write email newsletter", "context": {"audience": "developers"}}
            ],
            "data_analyzer": [
                {"request": "Analyze sales data", "context": {"period": "monthly"}},
                {"request": "Generate insights report", "context": {"type": "executive"}},
                {"request": "Predict trends", "context": {"horizon": "quarterly"}}
            ]
        }
        
        return common_requests.get(agent_name, [
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for agent_name, stats in self._metrics["hit_rate_by_agent"].items():
            ...
            agent_hit_rates[agent_name] = agent_hits / agent_total if agent_total > 0 else 0
