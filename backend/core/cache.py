...
class RequestCache:
    ...
    async def get(self, request: Request, endpoint_key: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get cached response"""
        try:
            ...
            if cached_data:
                ...
                if datetime.utcnow() - cached_at < timedelta(seconds=cache_entry["ttl"]):
    ...
    async def set(
        ...
        try:
            ...
            # Store in Redis
            success = await self.redis_client.set(cache_key, json.dumps(cache_entry), ex=ttl)
