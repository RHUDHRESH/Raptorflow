...
class MigrationManager:
    ...
    def _load_migrations(self) -> List[Migration]:
        ...
        for file_path in sorted(self.migrations_dir.glob("*.sql")):
            if file_path.name.startswith(('202', '001', '002')):  # Valid migration files
    ...
    async def _get_executed_migrations(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            result = await self.supabase.table("schema_migrations").select("*").execute()
    ...
    async def _verify_migration(self, migration: Migration) -> bool:
        ...
        try:
            ...
            result = await self.supabase.table("schema_migrations").select("*").eq("version", migration.version).execute()
            if not result.data:
                return False
        ...
        for migration in all_migrations:
            if migration.version not in executed:
                ...
                if not await self._verify_migration(migration):
                    logger.warning(f"Migration verification failed, re-queuing: {migration}")
                    pending.append(migration)
    ...
    async def migrate_up(self, target_version: Optional[str] = None) -> Dict[str, Any]:
        ...
        if results["status"] == "running":
            ...
            results["message"] = f"Successfully executed {len(results['completed'])} migrations"
        else:
    ...
    async def get_migration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_migration": max(executed.values(), key=lambda x: x["executed_at"]) if executed else None,
    ...
    async def create_rollback_migration(self, version: str, description: str) -> str:
        ...
        rollback_filename = f"{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_rollback_{version}.sql"
        ...
        for migration in migrations:
            if migration.version not in executed:
                ...
                if "FOREIGN KEY" in migration.content:
                    issues.append(f"Migration {migration.filename} may have unmet dependencies")
