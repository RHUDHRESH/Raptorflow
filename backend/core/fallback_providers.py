...
class ProviderHealth:
    ...
    def get_health_score(self) -> float:
        ...
        performance_score = (
            (1.0 - min(1.0, self.avg_response_time / self.max_response_time)) * 0.5 +
    ...
    def _update_status(self):
        ...
        elif self.success_rate >= self.min_success_rate and self.avg_response_time <= self.max_response_time:
            self.status = ProviderStatus.HEALTHY
        elif self.success_rate >= 0.8 and self.avg_response_time <= self.max_response_time * 1.5:
...
class FallbackProviderManager:
        ...
        **kwargs
        ...
        for i, provider_id in enumerate(selected_providers):
            ...
            try:
                ...
                async with self._lock:
                    ...
                    if failover_start:
                        ...
                        self.stats["avg_failover_time"] = (
                            (self.stats["avg_failover_time"] * (self.stats["failover_events"] - 1) + failover_time) /
                            self.stats["failover_events"]
                        )
                
                logger.info(f"Successfully executed with provider {provider_id} in {response_time:.2f}s")
                ...
                if provider.circuit_breaker_open and provider.consecutive_failures == self.config.circuit_breaker_threshold:
                    async with self._lock:
        ...
        raise Exception(f"All providers failed for model {model}. Last error: {last_error}")
    
    async def _select_providers(self, model: str, provider_chain: List[str]) -> List[str]:
        """Select providers based on strategy."""
        available_providers = [
            pid for pid in provider_chain
            if self.providers[pid].is_available()
        ]
        ...
        if self.config.strategy == FailoverStrategy.PRIORITY_BASED:
            ...
            if total_weight > 0:
                ...
                import random
                selected_idx = random.choices(range(len(available_providers)), weights=probabilities)[0]
                
                # Move selected provider to front, keep order for others
                selected = [available_providers[selected_idx]]
                remaining = available_providers[:selected_idx] + available_providers[selected_idx + 1:]
    ...
    def _get_adaptive_score(self, provider_id: str) -> float:
        ...
        response_time_score = max(0, 1.0 - provider.avg_response_time / self.config.max_response_time)
        
        return (health_score * 0.4 + priority_score * 0.2 + 
                recent_success_rate * 0.2 + response_time_score * 0.2)
