...
class TransactionConsistencyManager:
    ...
    async def transaction_context(
        ...
        if not transaction_id:
            transaction_id = f"TX{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:8]}"
        ...
        try:
            ...
            await conn.execute(f"BEGIN TRANSACTION ISOLATION LEVEL {isolation_level.value}")
    ...
    def _are_locks_compatible(self, existing_type: LockType, requested_type: LockType) -> bool:
        ...
        if existing_type == LockType.EXCLUSIVE:
            ...
            return requested_type == LockType.SHARED or requested_type == LockType.INTENT_SHARED
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": "Transaction consistency manager is operational" if overall_healthy else "Transaction consistency manager has issues",
                "features": {
...
class TransactionWrapper:
    ...
    def __init__(self, connection: asyncpg.Connection, context: TransactionContext, manager: TransactionConsistencyManager):
    ...
    async def acquire_resource_lock(self, resource_key: str, lock_type: LockType = LockType.EXCLUSIVE):
        ...
        if lock_info:
            self.context.locks_acquired.append(f"{self.manager.lock_prefix}{resource_key}")
            
            # Add rollback action to release lock
            self.context.rollback_actions.append(
                lambda: self.manager.release_lock(resource_key, self.context.transaction_id)
            )
...
def get_transaction_manager(redis_client: redis.Redis, db_pool: asyncpg.Pool) -> TransactionConsistencyManager:
