...
class ConstraintChecker:
    ...
    def audit_plan(plan: Dict[str, Any], limits: ResourceLimits) -> ConstraintAudit:
        ...
        if months * 4 > limits.time_weeks:
            violations.append(f"Plan duration ({months * 4} weeks) exceeds limit ({limits.time_weeks} weeks).")
        ...
        return ConstraintAudit(
            ...
            optimization_suggestions=["Consolidate tools", "Reduce monthly theme complexity"] if not is_feasible else []
        )
