...
class RedisProductionConfig:
    ...
    def __init__(self):
        ...
        self.backup_enabled = os.getenv("REDIS_BACKUP_ENABLED", "true").lower() == "true"
        ...
        # Monitoring settings
        self.metrics_enabled = os.getenv("REDIS_METRICS_ENABLED", "true").lower() == "true"
        self.slow_log_enabled = os.getenv("REDIS_SLOW_LOG_ENABLED", "true").lower() == "true"
        self.slow_log_threshold = int(os.getenv("REDIS_SLOW_LOG_THRESHOLD", "10000"))  # 10ms
...
class RedisProductionManager:
    ...
    async def optimize_redis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.slow_log_enabled:
                await self.client.config_set("slowlog-log-slower-than", self.config.slow_log_threshold)
    ...
    async def get_redis_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "last_save_time_human": datetime.fromtimestamp(info.get("rdb_last_save_time", 0)).isoformat() if info.get("rdb_last_save_time") else None,
            }
            ...
            stats["manager_uptime"] = (datetime.utcnow() - self._start_time).total_seconds()
    ...
    async def cleanup_expired_keys(self) -> int:
        ...
        try:
            ...
            slow_operations = len([entry for entry in slowlog if entry[0] > self.config.slow_log_threshold])
...
async def redis_increment_counter(key: str, amount: int = 1, ttl: Optional[int] = None) -> int:
