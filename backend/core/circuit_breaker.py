...
class CircuitBreaker:
    ...
    async def _execute_fallback(self, *args, **kwargs) -> Any:
        ...
        if not self.config.fallback_enabled or not self._fallback_functions:
            raise CircuitBreakerError(
                "Circuit is open and no fallback available", circuit_state=self.state
            )
    ...
    async def _record_success(self):
        ...
        self._request_history.append({"timestamp": datetime.now(), "success": True})
    ...
    async def _update_redis_state(self):
        ...
        try:
            ...
            self.redis.setex(
                self.state_key, 3600, json.dumps(state_data)  # 1 hour expiration
            )
        except Exception as e:
    ...
    async def _update_redis_metrics(self):
        ...
        try:
            ...
            self.redis.setex(
                self.metrics_key, 3600, json.dumps(metrics_data)  # 1 hour expiration
            )
        except Exception as e:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "average_response_time": round(
                        self.metrics.average_response_time, 2
                    ),
                    "circuit_opens": self.metrics.circuit_opens,
                    "fallback_calls": self.metrics.fallback_calls,
                },
...
class CircuitBreakerManager:
    ...
    async def health_check_all(self) -> Dict[str, Any]:
        ...
        healthy_circuits = sum(
            1 for check in health_checks.values() if check["status"] == "healthy"
        )
        degraded_circuits = sum(
            1 for check in health_checks.values() if check["status"] == "degraded"
        )
        unhealthy_circuits = sum(
            1 for check in health_checks.values() if check["status"] == "unhealthy"
        )
...
def circuit_breaker_protected(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            circuit = manager.get_circuit_breaker(
                name,
