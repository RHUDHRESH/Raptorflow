...
class UnifiedSearchEngine:
        ...
        try:
            ...
            if self.config.monitoring.enabled:
                await system_monitor.start_monitoring(
                    self.config.monitoring.monitoring_interval_seconds
                )
            ...
            logger.info(
                f"Unified Search Engine initialized successfully. Version: {self.config.version}"
            )
            return True
    ...
    async def _load_configuration(self, config_path: Optional[str] = None):
        ...
        logger.info(
            f"Configuration loaded: {self.config.system_name} v{self.config.version}"
        )
    ...
    async def _initialize_interface(self):
        ...
        self.interface = UnifiedSearchInterface(
            {
                "brave_api_key": self.config.providers.get(
                    "brave", ProviderConfig()
                ).api_key,
                "serper_api_key": self.config.providers.get(
    ...
    async def search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        metrics = performance_tracker.start_operation(
            operation_id=operation_id, operation_type="search", query=query.text
        )
        ...
        try:
            ...
            metrics_collector.record_histogram(
                "search_results_count", len(consolidated_results)
            )
            ...
            logger.info(
                f"Search completed: {len(consolidated_results)} results for query: {query.text}"
            )
            ...
            # End performance tracking
            performance_tracker.end_operation(
                operation_id=operation_id, success=False, error_message=str(e)
            )
    ...
    async def research(self, plan: ResearchPlan) -> Any:
        ...
        try:
            ...
            metrics_collector.record_histogram(
                "research_findings_count", len(report.key_findings)
            )
            ...
            logger.info(
                f"Research completed: {len(report.key_findings)} findings for topic: {plan.topic}"
            )
            return report
    ...
    async def crawl_urls(self, urls: List[str]) -> List[Any]:
        ...
        try:
            ...
            metrics_collector.record_histogram(
                "crawl_content_count", len(extracted_contents)
            )

            logger.info(
                f"Crawl completed: {len(extracted_contents)} contents from {len(urls)} URLs"
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        # Check providers
        if self.providers:
            provider_health = await self.health_checker.check_all_providers_health(
    ...
    async def _notify_search_callbacks(
    ...
    async def _on_config_changed(self, new_config: UnifiedSearchConfig):
        ...
        if self.config.monitoring.enabled != new_config.monitoring.enabled:
            if new_config.monitoring.enabled:
                await system_monitor.start_monitoring(
                    new_config.monitoring.monitoring_interval_seconds
                )
            else:
                await system_monitor.stop_monitoring()
...
async def search(
    query: Union[str, SearchQuery], mode: str = "standard", max_results: int = 10
) -> List[SearchResult]:
    ...
    if isinstance(query, str):
        search_query = SearchQuery(
            ...
            mode=(
                ...
                if mode.lower() in [m.value for m in SearchMode]
...
async def research(topic: str, question: str, depth: str = "moderate") -> Any:
    ...
    from core.unified_search_part6 import ResearchDepth, ResearchPlan

    plan = ResearchPlan(
        topic=topic,
        research_question=question,
        depth=(
            ResearchDepth(depth.lower())
            if depth.lower() in [d.value for d in ResearchDepth]
            else ResearchDepth.MODERATE
        ),
