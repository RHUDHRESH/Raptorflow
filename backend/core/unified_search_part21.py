...
class SearchNode:
    ...
    def load_ratio(self) -> float:
        ...
        return (
            self.current_searches / self.max_concurrent_searches
    ...
    def failure_rate(self) -> float:
        """Get failure rate."""
        return (
            self.failed_searches / self.total_searches
...
class NodeRegistry:
    ...
    async def update_node_status(self, node_id: str, status: NodeStatus) -> bool:
        ...
        async with self._lock:
            ...
            if old_status != status:
                logger.info(
                    f"Node {node_id} status changed: {old_status.value} -> {status.value}"
                )
    ...
    async def update_node_metrics(self, node_id: str, metrics: Dict[str, Any]) -> bool:
        ...
        async with self._lock:
            ...
            node.current_searches = metrics.get(
                "current_searches", node.current_searches
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        capability_counts = {
            cap: len(nodes) for cap, nodes in self.capabilities_index.items()
        }
        ...
        return {
            ...
            "avg_load_ratio": (
                sum(node.load_ratio for node in self.get_healthy_nodes())
                / healthy_nodes
                if healthy_nodes > 0
                else 0
            ),
...
class LoadBalancer:
    ...
    def _round_robin_selection(
        self, nodes: List[SearchNode], count: int
    ) -> List[SearchNode]:
    ...
    def _hash_based_selection(
        self, nodes: List[SearchNode], request: DistributedSearchRequest, count: int
    ) -> List[SearchNode]:
...
class DistributedSearchCoordinator:
    ...
    async def execute_distributed_search(
        ...
        logger.info(
            f"Executing distributed search {request_id} on {len(target_nodes)} nodes"
        )
            ...
            for result in results:
                if isinstance(result, DistributedSearchResult):
                    ...
                    if result.success:
                        ...
                        logger.warning(
                            f"Node {result.node_id} failed: {result.error_message}"
                        )
                elif isinstance(result, Exception):
                    logger.error(f"Search execution error: {result}")

            logger.info(
                f"Distributed search {request_id} completed: {successful_nodes}/{len(target_nodes)} nodes successful"
            )
    ...
    async def _execute_node_search(
        self, node: SearchNode, request: DistributedSearchRequest
            ...
            async with self.http_session.post(url, json=payload) as response:
                if response.status == 200:
                    ...
                    for result_data in response_data.get("results", []):
                        result = SearchResult(
                            ...
                            provider=SearchProvider(
                                result_data.get("provider", "native")
                            ),
                    ...
                    node.avg_response_time_ms = (
                        node.avg_response_time_ms * (node.total_searches - 1)
                        + execution_time
                    ) / node.total_searches
...
class DistributedSearchManager:
    ...
    def __init__(self):
        ...
        self.coordinator = DistributedSearchCoordinator(
            self.node_registry, self.load_balancer
        )
