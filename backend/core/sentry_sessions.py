...
class SentrySessionManager:
    ...
    def _set_sentry_user_context(self, session: SessionInfo) -> None:
        ...
        try:
            ...
            # Set session tags
            configure_scope(lambda scope: scope.set_tag("session_id", session.session_id))
            configure_scope(lambda scope: scope.set_tag("session_type", session.session_type.value))
            
            if session.user_id:
                configure_scope(lambda scope: scope.set_tag("user_id", session.user_id))
            
            # Set session context
            configure_scope(lambda scope: scope.set_context("session", {
                "session_id": session.session_id,
    ...
    def update_session(self, 
        ...
        with self._lock:
            ...
            # Add activity event
            self._add_session_event(session_id, "session_updated", {
                "endpoint": endpoint,
                "metadata": metadata,
                "tags": tags,
            })
    ...
    def track_session_error(self, 
        ...
        self._add_session_event(session_id, "error_occurred", {
            "error_id": error_id,
            "error_type": error_type,
            "severity": severity,
            "user_impacted": user_impacted,
    ...
    def track_session_request(self, 
                             session_id: Optional[str] = None,
                             endpoint: Optional[str] = None,
                             method: str = "GET",
                             status_code: int = 200,
        ...
        with self._lock:
            if session_id in self._session_metrics:
                ...
                if metrics.total_requests == 1:
                    ...
                    total_time = metrics.avg_response_time_ms * (metrics.total_requests - 1) + response_time_ms
        ...
        self._add_session_event(session_id, "request_completed", {
            "endpoint": endpoint,
            "method": method,
    ...
    def track_conversion_event(self, 
        ...
        self._add_session_event(session_id, "conversion_event", {
    ...
    def update_session_status(self, session_id: str, status: SessionStatus) -> bool:
        ...
        with self._lock:
            ...
            self._add_session_event(session_id, "status_changed", {
    ...
    def _terminate_session(self, session_id: str, reason: str) -> bool:
        ...
        with self._lock:
            ...
            self._add_session_event(session_id, "session_terminated", {
                "reason": reason,
                "duration_minutes": (
                    datetime.now(timezone.utc) - session.created_at
                ).total_seconds() / 60,
    ...
    def get_session_events(self, session_id: str, limit: Optional[int] = None) -> List[SessionEvent]:
        """Get session events."""
        with self._lock:
    ...
    def get_user_sessions(self, user_id: str) -> List[SessionInfo]:
        ...
        with self._lock:
            ...
            return [self._sessions.get(sid) for sid in session_ids if sid in self._sessions]
    ...
    def get_session_analytics(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            active_sessions = len([s for s in recent_sessions if s.status == SessionStatus.ACTIVE])
            expired_sessions = len([s for s in recent_sessions if s.status == SessionStatus.EXPIRED])
            terminated_sessions = len([s for s in recent_sessions if s.status == SessionStatus.TERMINATED])
            error_sessions = len([s for s in recent_sessions if s.status == SessionStatus.ERROR])
            ...
            sessions_with_errors = len([
                sid for sid, errors in self._session_errors.items()
                if errors and sid in self._sessions and self._sessions[sid].created_at > cutoff_time
            ])
            ...
            if performance_metrics:
                avg_response_time = sum(m.avg_response_time_ms for m in performance_metrics) / len(performance_metrics)
                avg_error_rate = sum(m.error_rate for m in performance_metrics) / len(performance_metrics)
                total_conversions = sum(m.conversion_events for m in performance_metrics)
            else:
                avg_response_time = 0
                avg_error_rate = 0
                total_conversions = 0
            ...
            return {
                ...
                "error_rate_per_session": sessions_with_errors / total_sessions if total_sessions > 0 else 0,
...
@contextmanager
def session_context(user_id: Optional[str] = None,
                   session_type: SessionType = SessionType.WEB,
                   **kwargs):
