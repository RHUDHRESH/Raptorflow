...
class FileHandleCleanupStrategy(ResourceCleanupStrategy):
    ...
    async def cleanup(self, resource_info: ResourceInfo) -> bool:
        ...
        try:
            ...
            logger.error(f"File handle cleanup failed for {resource_info.resource_id}: {e}")
            return False
...
class AsyncTaskCleanupStrategy(ResourceCleanupStrategy):
    ...
    async def cleanup(self, resource_info: ResourceInfo) -> bool:
        ...
        try:
            ...
            logger.error(f"Async task cleanup failed for {resource_info.resource_id}: {e}")
            return False
...
class ResourceManager:
    ...
    def __init__(self, leak_check_interval: int = 60, cleanup_interval: int = 300):
        ...
        logger.info(f"Resource manager initialized with leak check interval: {leak_check_interval}s")
    ...
    def register_resource(
        ...
        try:
            ...
            if not self._check_quota(resource_type, workspace_id):
                ...
                logger.warning(f"Quota violation for {resource_type.value} in workspace {workspace_id}")
                return False
    ...
    def unregister_resource(self, resource_id: str) -> bool:
        ...
        try:
            ...
            if resource_info.workspace_id:
                self.resources_by_workspace[resource_info.workspace_id].discard(resource_id)
        ...
        try:
            ...
            if resource_info.cleanup_callback:
                try:
                    ...
                    logger.error(f"Custom cleanup callback failed for {resource_id}: {e}")
    ...
    async def cleanup_resources_by_type(self, resource_type: ResourceType) -> int:
        ...
        logger.info(f"Cleaned up {cleanup_count}/{len(resource_ids)} resources of type {resource_type.value}")
    ...
    async def cleanup_resources_by_owner(self, owner: str) -> int:
        ...
        logger.info(f"Cleaned up {cleanup_count}/{len(resource_ids)} resources for owner {owner}")
        return cleanup_count
    ...
    def _check_quota(self, resource_type: ResourceType, workspace_id: Optional[str]) -> bool:
        ...
        for resource_id, resource_info in self.resources.items():
            ...
            if age_seconds > threshold and idle_seconds > threshold / 2:
                ...
                leak = ResourceLeak(
                    ...
                    suggested_action=self._get_suggested_action(resource_info.resource_type, severity),
    ...
    def _get_suggested_action(self, resource_type: ResourceType, severity: str) -> str:
        ...
        base_action = actions.get(resource_type, "Clean up resource using appropriate strategy")
    ...
    async def _automatic_cleanup(self):
        ...
        leaked_resources = [
            rid for rid, rinfo in self.resources.items()
        ...
        if cleanup_count > 0:
            logger.info(f"Automatic cleanup completed: {cleanup_count} resources cleaned")
        ...
        try:
            ...
            if self.metrics["total_resources_created"] > 0:
                self.metrics["cleanup_success_rate"] = (
                    self.metrics["total_resources_cleaned"] / self.metrics["total_resources_created"] * 100
                )
    ...
    def get_leaked_resources(self, severity: Optional[str] = None) -> List[Dict[str, Any]]:
