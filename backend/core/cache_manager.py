...
from .cache_error_handler import (
    get_error_handler,
    CacheErrorHandler
)
...
class UnifiedCacheManager:
    ...
    async def restore_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            await self._error_handler.handle_error(
                e, 'restore_backup', 'cache_backup'
            )
            return False
    
    # Analytics and monitoring
    async def get_analytics_dashboard(self, time_window: TimeWindow = TimeWindow.HOURLY) -> Dict[str, Any]:
    ...
    async def get_optimization_recommendations(self, key_pattern: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get optimization recommendations."""
        if not self._initialized:
            await self.initialize()
        
        recommendations = await self._optimizer.get_optimization_recommendations(key_pattern)
...
async def get_cache_analytics(time_window: TimeWindow = TimeWindow.HOURLY) -> Dict[str, Any]:
