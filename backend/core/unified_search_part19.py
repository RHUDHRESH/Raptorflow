...
from core.unified_search_part1 import ContentType, SearchResult
    ...
    def __init__(self):
        self.language_patterns = {
            Language.ENGLISH: {
                ...
                "patterns": [
                    r"\b(the|and|is|in|to|of|a|that|it|with|for|as|be|are|was|were|been|being)\b"
                ],
            ...
            Language.SPANISH: {
                ...
                "patterns": [
                    r"\b(el|la|de|que|y|a|en|un|es|se|no|te|lo|le|da|del|los|las)\b"
                ],
                "confidence_threshold": 0.3,
            },
            Language.FRENCH: {
                ...
                "patterns": [
                    r"\b(le|de|et|à|un|il|être|en|avoir|que|pour|dans|ce|son|sur|avec)\b"
                ],
                "confidence_threshold": 0.3,
            },
            Language.GERMAN: {
                ...
                "patterns": [
                    r"\b(der|die|und|in|den|von|zu|das|mit|sich|des|auf|für|ist|im|dem)\b"
                ],
                "confidence_threshold": 0.3,
            },
        }
...
class TextAnalyzer:
    ...
    async def analyze_text(
        self, text: str, content_type: ContentType = ContentType.TEXT
    ) -> TextAnalysis:
    ...
    def _analyze_sentiment(self, text: str) -> Tuple[float, float]:
        ...
        positive_count = sum(
            1 for word in words if word in self.sentiment_words["positive"]
        )
        negative_count = sum(
            1 for word in words if word in self.sentiment_words["negative"]
        )
...
class ContentProcessor:
    ...
    def _calculate_trust_score(
        self, url: Optional[str], content_type: ContentType
    ) -> float:
...
class ContentIndexer:
    ...
    def get_content(
        self, content_id: str
    ) -> Optional[Tuple[ExtractedContent, ContentMetadata, TextAnalysis]]:
        """Get indexed content."""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_keywords_per_content": (
                sum(len(analysis.keywords) for analysis in self.analysis_index.values())
                / len(self.analysis_index)
