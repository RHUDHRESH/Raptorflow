...
from core.unified_search_part1 import SearchQuery, SearchResult, ContentType
...
class ImageProcessor:
    ...
    async def process_image(self, image_data: Union[str, bytes], media_type: MediaType) -> MediaContent:
        ...
        return MediaContent(
            ...
            base64_data=base64.b64encode(image_bytes).decode() if isinstance(image_data, str) else base64.b64encode(image_bytes).decode(),
    ...
    async def _extract_image_features(self, image_bytes: bytes, media_type: MediaType) -> Dict[str, Any]:
...
class AudioProcessor:
    ...
    async def process_audio(self, audio_data: Union[str, bytes], media_type: MediaType) -> MediaContent:
    ...
    def _validate_audio(self, audio_bytes: bytes, media_type: MediaType):
        ...
        if size_mb > self.max_file_size_mb:
            raise ValueError(f"Audio too large: {size_mb:.1f}MB > {self.max_file_size_mb}MB")
        
        # Check audio format
        if media_type not in self.supported_formats:
            raise ValueError(f"Unsupported audio format: {media_type}")
    
    async def _extract_audio_features(self, audio_bytes: bytes, media_type: MediaType) -> Dict[str, Any]:
...
class VideoProcessor:
    ...
    async def process_video(self, video_data: Union[str, bytes], media_type: MediaType) -> MediaContent:
    ...
    def _validate_video(self, video_bytes: bytes, media_type: MediaType):
        ...
        if size_mb > self.max_file_size_mb:
            raise ValueError(f"Video too large: {size_mb:.1f}MB > {self.max_file_size_mb}MB")
        
        # Check video format
        if media_type not in self.supported_formats:
            raise ValueError(f"Unsupported video format: {media_type}")
    
    async def _extract_video_features(self, video_bytes: bytes, media_type: MediaType) -> Dict[str, Any]:
    ...
    def _generate_video_description(self, features: Dict[str, Any], key_frames: List[Dict[str, Any]]) -> str:
    ...
    def _extract_video_tags(self, features: Dict[str, Any], key_frames: List[Dict[str, Any]]) -> List[str]:
...
class MultiModalSearchEngine:
    ...
    async def search(self, query: MultiModalQuery) -> List[MultiModalResult]:
        ...
        # Text-based search
        if query.text_query:
            text_results = await self._search_text(query.text_query, query.modality_weights, query.filters)
            results.extend(text_results)
        
        # Image-based search
        if query.image_query:
            image_results = await self._search_by_image(query.image_query, query.modality_weights, query.filters)
            results.extend(image_results)
        
        # Audio-based search
        if query.audio_query:
            audio_results = await self._search_by_audio(query.audio_query, query.modality_weights, query.filters)
            results.extend(audio_results)
        
        # Video-based search
        if query.video_query:
            video_results = await self._search_by_video(query.video_query, query.modality_weights, query.filters)
    ...
    async def _search_text(self, text_query: str, modality_weights: Dict[ModalityType, float], filters: Dict[str, Any]) -> List[MultiModalResult]:
        ...
        for content_id, content in self.content_index.items():
            ...
            if similarity > 0.3:  # Threshold
                ...
                relevance_score = similarity * modality_weights.get(content.modality, 1.0)
    ...
    async def _search_by_image(self, query_image: MediaContent, modality_weights: Dict[ModalityType, float], filters: Dict[str, Any]) -> List[MultiModalResult]:
        ...
        for content_id in image_content_ids:
            ...
            if similarity > 0.3:
                ...
                relevance_score = similarity * modality_weights.get(ModalityType.IMAGE, 1.0)
    ...
    async def _search_by_audio(self, query_audio: MediaContent, modality_weights: Dict[ModalityType, float], filters: Dict[str, Any]) -> List[MultiModalResult]:
        ...
        for content_id in audio_content_ids:
            ...
            if similarity > 0.3:
                ...
                relevance_score = similarity * modality_weights.get(ModalityType.AUDIO, 1.0)
    ...
    async def _search_by_video(self, query_video: MediaContent, modality_weights: Dict[ModalityType, float], filters: Dict[str, Any]) -> List[MultiModalResult]:
        ...
        for content_id in video_content_ids:
            ...
            if similarity > 0.3:
                ...
                relevance_score = similarity * modality_weights.get(ModalityType.VIDEO, 1.0)
    ...
    def _calculate_image_similarity(self, query_image: MediaContent, content: MediaContent) -> float:
        ...
        if query_dims and content_dims:
            ...
            content_ratio = content_dims[0] / content_dims[1] if content_dims[1] > 0 else 0
    ...
    def _calculate_audio_similarity(self, query_audio: MediaContent, content: MediaContent) -> float:
        ...
        if query_duration > 0 and content_duration > 0:
            duration_similarity = 1.0 - abs(query_duration - content_duration) / max(query_duration, content_duration)
            ...
            if query_tempo > 0 and content_tempo > 0:
                tempo_similarity = 1.0 - abs(query_tempo - content_tempo) / max(query_tempo, content_tempo)
                return (duration_similarity + tempo_similarity) / 2
        
        return 0.0
    
    def _calculate_video_similarity(self, query_video: MediaContent, content: MediaContent) -> float:
    ...
    def _rank_and_filter_results(self, results: List[MultiModalResult], query: MultiModalQuery) -> List[MultiModalResult]:
