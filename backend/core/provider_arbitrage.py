...
class PerformanceMonitor:
    ...
    async def get_metrics(self, provider: str, model: str) -> Optional[ProviderMetrics]:
        ...
        try:
            from core.fallback_providers import get_fallback_manager
            ...
            if ph:
                return ProviderMetrics(
                    ...
                    throughput=(
                        1.0 / ph.avg_response_time if ph.avg_response_time > 0 else 10
                    ),
    ...
    def get_trend_analysis(
        self, provider: str, model: str, hours: int = 24
    ) -> Dict[str, float]:
        ...
        try:
            ...
            latency_trend = self._calculate_trend(
                [m.latency_p95 for m in recent_history]
            )
            success_rate_trend = self._calculate_trend(
                [m.success_rate for m in recent_history]
            )
            throughput_trend = self._calculate_trend(
                [m.throughput for m in recent_history]
            )
...
class ProviderRegistry:
    ...
    def __init__(self):
        ...
        logger.info(
            f"ProviderRegistry initialized with {len(self.providers)} providers"
        )
    ...
    def get_available_providers(self) -> List[ProviderInfo]:
        ...
        return [
            ...
            if p.current_status == ProviderStatus.AVAILABLE
        ]
    ...
    def get_providers_by_capability(self, capability: str) -> List[ProviderInfo]:
        ...
        return [
            ...
            and p.current_status == ProviderStatus.AVAILABLE
        ]
...
class ProviderArbitrage:
    ...
    def __init__(self, preferred_providers: List[str], cost_sensitivity: float = 0.8):
        ...
        self.cost_sensitivity = (
            cost_sensitivity  # 0.0 = performance focused, 1.0 = cost focused
        )
        ...
        logger.info(
            f"ProviderArbitrage initialized: preferred={preferred_providers}, cost_sensitivity={cost_sensitivity}"
        )
    ...
    async def select_provider(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            fallback_provider = (
                self.preferred_providers[0] if self.preferred_providers else "openai"
            )
            return {
                "provider_selection": {
    ...
    def _analyze_requirements(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            requirements = {
                "text_length": len(
                    str(request_data.get("prompt", request_data.get("content", "")))
                ),
                "requires_vision": any(
                    keyword in str(request_data).lower()
                    for keyword in ["image", "vision", "picture", "photo"]
                ),
                "requires_function_calling": any(
                    keyword in str(request_data).lower()
                    for keyword in ["function", "tool", "call"]
                ),
    ...
    async def _get_candidate_providers(
        ...
        try:
            ...
            preferred_available = [
                p for p in available_providers if p.name in self.preferred_providers
            ]
            other_available = [
                p for p in available_providers if p.name not in self.preferred_providers
            ]
            ...
            for provider in preferred_available + other_available:
                for model in provider.supported_models:
                    ...
                    if (
                        requirements.get("requires_vision")
                        and "vision" not in provider.capabilities
                    ):
                        continue

                    if (
                        requirements.get("requires_function_calling")
                        and "function-calling" not in provider.capabilities
                    ):
                        continue

                    if (
                        requirements.get("requires_long_context")
                        and "long-context" not in provider.capabilities
                    ):
                        continue

                    # Get pricing and metrics
                    pricing = await self.pricing_monitor.get_pricing(
                        provider.name, model
                    )
        ...
        try:
            ...
            for candidate in candidates:
                score = await self._calculate_candidate_score(
                    candidate, requirements, min_cost
                )
    ...
    async def _calculate_candidate_score(
        self, candidate: Dict[str, Any], requirements: Dict[str, Any], min_cost: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            pricing_age = (
                datetime.utcnow() - pricing.effective_date
            ).total_seconds() / 3600  # hours
            metrics_age = (
                datetime.utcnow() - metrics.last_updated
            ...
            estimated_tokens = (
                requirements.get("text_length", 1000) // 4
            )  # Rough estimate
            ...
            max_cost = max(
                c["pricing"].average_cost_per_1k for c in [candidate]
            )  # Would be all candidates
            cost_savings = (max_cost - pricing.average_cost_per_1k) * (
            ...
            if cost_score > 0.8:
                reasoning_parts.append(
                    f"Excellent cost efficiency (${pricing.average_cost_per_1k:.4f}/1K tokens)"
                )
            if latency_score > 0.8:
                reasoning_parts.append(f"Low latency ({metrics.latency_p95:.2f}s)")
            if reliability_score > 0.95:
                reasoning_parts.append(
                    f"High reliability ({metrics.reliability_score:.2f})"
                )

            reasoning = (
                ". ".join(reasoning_parts)
                if reasoning_parts
                else "Balanced performance and cost"
            )
    ...
    def get_arbitrage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_cost_savings_per_routing": self.cost_savings
            / max(total_routings, 1),
            "most_used_provider": (
                max(self.routing_stats.items(), key=lambda x: x[1])[0]
            ...
            for (
                ...
                for model in provider_info.supported_models:
                    pricing = asyncio.create_task(
                        self.pricing_monitor.get_pricing(provider_name, model)
                    )
                ...
                for model in provider_info.supported_models:
                    metrics = asyncio.create_task(
                        self.performance_monitor.get_metrics(provider_name, model)
                    )
                    # Note: In real implementation, this would be awaited properly
