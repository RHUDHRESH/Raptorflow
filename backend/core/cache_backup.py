...
class BackupStorage:
    ...
    def __init__(self, storage_config: Dict[str, Any]):
        ...
        self.backup_directory = storage_config.get(
    ...
    async def store_backup(
        self, backup_data: Dict[str, Any], metadata: BackupMetadata
    ) -> str:
        """Store backup data and metadata."""
        try:
            # Generate file paths
            backup_filename = f"{metadata.backup_id}.{metadata.format.value}"
            if self.compression_enabled and metadata.format in [
                BackupFormat.JSON,
                BackupFormat.PICKLE,
            ]:
                backup_filename += ".gz"
            ...
            metadata_path = os.path.join(
                self.metadata_dir, f"{metadata.backup_id}.json"
            )
    ...
    async def load_backup(
        self, backup_id: str
    ) -> Tuple[Dict[str, Any], BackupMetadata]:
                ...
                if filename.endswith(".json"):
                    ...
                    if AIOFILES_AVAILABLE:
                        async with aiofiles.open(
                            os.path.join(self.metadata_dir, filename), "r"
                        ) as f:
                            metadata_dict = json.loads(await f.read())
                    else:
...
class CacheBackupManager:
    ...
    async def restore_backup(
        self, backup_id: str, target_cache: Optional[Any] = None
    ) -> str:
        ...
        try:
            ...
            if backup_type == BackupType.FULL:
                ...
                cache_data = {
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "since_last_full": (
                        self.last_full_backup.isoformat()
    ...
    async def _restore_cache_data(
        self, backup_data: Dict[str, Any], target_cache: Optional[Any]
    ):
    ...
    async def _restore_incremental_backup(
        self, backup_data: Dict[str, Any], target_cache
    ):
    ...
    async def _restore_differential_backup(
        self, backup_data: Dict[str, Any], target_cache
    ):
    ...
    def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            active_backups = sum(
                1
                for op in self.backup_operations.values()
...
async def restore_cache_backup(
    backup_id: str, target_cache: Optional[Any] = None
) -> str:
