...
class RateLimitMiddleware:
    ...
    def __init__(
        ...
        self.app = app
        self.client_identifier_func = client_identifier_func or self._default_client_identifier
    ...
    async def rate_limit_middleware(self, request: Request, call_next: Callable):
        ...
        try:
            ...
            response.headers["X-RateLimit-Remaining"] = str(
                max(0, self.rate_limiter.config.requests_per_minute - client_stats.get("requests_this_minute", 0))
            )
...
def create_rate_limiter_middleware(
    ...
    return RateLimitMiddleware(
        app=app,
        client_identifier_func=client_identifier_func,
        config=config
    )
