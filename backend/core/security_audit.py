...
class SecurityAudit:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.events_by_compliance: Dict[ComplianceStandard, List[str]] = defaultdict(
            list
        )
    ...
    def _requires_immediate_reporting(
        ...
        for standard in compliance_standards:
            if standard in self.compliance_requirements:
                if (
                    event_type
                    in self.compliance_requirements[standard]["immediate_reporting"]
                ):
                    return True
    ...
    def _generate_digital_signature(self, event: AuditEvent) -> str:
        ...
        data_to_sign = (
            f"{event.event_id}:{event.timestamp.isoformat()}:{event.checksum}".encode()
        )
    ...
    async def _trigger_compliance_alert(
        self, standard: ComplianceStandard, event: AuditEvent
    ):
    ...
    def search_audit_events(
        ...
        if compliance_standard:
            events = [
                e for e in events if compliance_standard in e.compliance_standards
            ]
    ...
    def _analyze_events_for_compliance(
        ...
        return {
            ...
            "period_start": (
                min(events, key=lambda x: x.timestamp).timestamp.isoformat()
                if events
                else None
            ),
            "period_end": (
                max(events, key=lambda x: x.timestamp).timestamp.isoformat()
                if events
                else None
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if not violations:
            recommendations.append(
                "No compliance violations detected - maintain current practices"
            )
    ...
    def export_audit_data(
        ...
        events = self.search_audit_events(
            start_time=start_time, end_time=end_time, limit=10000, **(filters or {})
        )
    ...
    def _export_xml(self, events: List[AuditEvent]) -> str:
        ...
        for event in events:
            ...
            xml_lines.append(
                f"      <timestamp>{event.timestamp.isoformat()}</timestamp>"
            )
    ...
    def _remove_from_indexes(self, event: AuditEvent):
        ...
        if event.user_id and event.user_id in self.events_by_user:
            self.events_by_user[event.user_id] = [
                eid
                for eid in self.events_by_user[event.user_id]
        ...
        if event.event_type in self.events_by_type:
            self.events_by_type[event.event_type] = [
                eid
                for eid in self.events_by_type[event.event_type]
                if eid != event.event_id
            ]
        ...
        if event.risk_level in self.events_by_risk:
            self.events_by_risk[event.risk_level] = [
                eid
                for eid in self.events_by_risk[event.risk_level]
                if eid != event.event_id
            ]
        ...
        for standard in event.compliance_standards:
            if standard in self.events_by_compliance:
                self.events_by_compliance[standard] = [
                    eid
                    for eid in self.events_by_compliance[standard]
                    if eid != event.event_id
                ]
