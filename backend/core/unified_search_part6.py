...
from core.unified_search_part3 import AdvancedCrawler, ExtractedContent
...
class ContentVerifier:
    ...
    async def verify_content(
        self, content: str, sources: List[SearchResult]
    ) -> Dict[str, Any]:
        ...
        if verification_result["verification_details"]:
            avg_factuality = sum(
                v["factuality"] for v in verification_result["verification_details"]
            ) / len(verification_result["verification_details"])
            avg_consensus = sum(
                v["consensus"] for v in verification_result["verification_details"]
            ) / len(verification_result["verification_details"])
        ...
        verification_result["source_diversity"] = (
            len(domains) / len(sources) if sources else 0
        )
    ...
    async def _verify_claim(
        self, claim: str, sources: List[SearchResult]
    ) -> Dict[str, Any]:
        ...
        for source in sources:
            ...
            if overlap > len(claim_words) * 0.5:  # 50% word overlap
                ...
                if any(
                    word in content
                    for word in ["confirm", "support", "agree", "verify"]
                ):
                    verification["supporting_sources"] += 1
                elif any(
                    word in content
                    for word in ["dispute", "contradict", "refute", "disagree"]
                ):
        ...
        total_sources = (
            ...
            + verification["neutral_sources"]
        )

        if total_sources > 0:
            verification["consensus"] = (
                verification["supporting_sources"] / total_sources
            )
            verification["factuality"] = (
                verification["consensus"] * 0.8
                + (verification["neutral_sources"] / total_sources) * 0.2
            )
...
class ResearchSynthesizer:
    ...
    def _generate_executive_summary(
        ...
        top_findings = sorted(
            findings, key=lambda f: f.confidence * f.relevance, reverse=True
        )[:5]
        ...
        insights_text = (
            "\n\nKey insights include: " + "; ".join(key_insights)
            if key_insights
            else ""
        )
    ...
    def _generate_detailed_analysis(
        ...
        limitations = "\n".join(
            [f"- {limit}" for limit in self._identify_limitations(findings, sources)]
        )
    ...
    def _generate_analysis_section(
        self, findings: List[ResearchFinding], sources: List[SearchResult]
    ) -> str:
        ...
        analysis_points.append(
            f"Average confidence across findings: {avg_confidence:.2f}"
        )

        # Source diversity
        domains = set(s.domain for s in sources)
        analysis_points.append(
            f"Source diversity: {len(domains)} unique domains from {len(sources)} sources"
        )

        # Top themes
        high_confidence = [f for f in findings if f.confidence > 0.8]
        analysis_points.append(
            f"High confidence findings: {len(high_confidence)} out of {len(findings)}"
        )
    ...
    def _calculate_quality_metrics(
        ...
        if findings:
            metrics["avg_confidence"] = sum(f.confidence for f in findings) / len(
                findings
            )
            metrics["avg_relevance"] = sum(f.relevance for f in findings) / len(
                findings
            )
            metrics["avg_factuality"] = sum(f.factuality for f in findings) / len(
                findings
            )

        if sources:
            metrics["avg_source_authority"] = sum(
                s.domain_authority for s in sources
            ) / len(sources)
            metrics["source_diversity"] = len(set(s.domain for s in sources)) / len(
                sources
            )
    ...
    def _generate_recommendations(
        ...
        if len(findings) < 5:
            recommendations.append(
                "Consider expanding search to include more sources for comprehensive coverage"
            )

        if findings and sum(f.confidence for f in findings) / len(findings) < 0.7:
            recommendations.append(
                "Additional verification recommended due to moderate confidence levels"
            )

        high_authority_sources = sum(1 for s in sources if s.domain_authority > 0.8)
        if high_authority_sources < len(sources) * 0.3:
            recommendations.append(
                "Include more high-authority sources to strengthen research credibility"
            )
    ...
    def _identify_limitations(
        ...
        if findings and sum(f.confidence for f in findings) / len(findings) < 0.6:
            limitations.append(
                "Moderate confidence levels indicate need for further verification"
            )

        content_types = set(s.content_type for s in sources)
        if len(content_types) < 3:
            limitations.append(
                f"Limited content type diversity: {', '.join([ct.value for ct in content_types])}"
            )
    ...
    def _calculate_completeness_score(
        ...
        verified_findings = sum(
            1 for f in findings if f.verification_status == "verified"
        )
...
class DeepResearchAgent:
    ...
    async def conduct_research(self, plan: ResearchPlan) -> ResearchReport:
        ...
        try:
            ...
            refined_report.research_duration_minutes = int(
                (end_time - start_time).total_seconds() / 60
            )

            logger.info(
                f"Research completed in {refined_report.research_duration_minutes} minutes"
            )
    ...
    async def _extraction_phase(
        self, plan: ResearchPlan, sources: List[SearchResult]
    ) -> List[ExtractedContent]:
        """Extract and analyze content from sources."""
        logger.info("Starting extraction phase")

        if not self.crawler:
            from core.unified_search_part3 import CrawlPolicy
    ...
    async def _verification_phase(
        self, plan: ResearchPlan, contents: List[ExtractedContent]
        ...
        for content in contents:
            ...
            finding = ResearchFinding(
                ...
                verification_status=(
                    "verified" if plan.verification_required else "unverified"
                ),
            )
            ...
            if plan.verification_required:
                ...
                finding.verification_status = (
                    "verified" if finding.factuality > 0.6 else "unverified"
                )
    ...
    async def _refinement_phase(
        self, plan: ResearchPlan, report: ResearchReport
    ) -> ResearchReport:
