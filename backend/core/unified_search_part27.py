...
class BackendService:
    ...
    def is_available(self) -> bool:
        ...
        return (
            ...
            self.failure_rate < self.circuit_breaker_threshold
        )
...
class LoadBalancer:
    ...
    def add_service(self, service: BackendService):
        ...
        logger.info(f"Added backend service: {service.name} ({service.host}:{service.port})")
    ...
    async def select_service(
        ...
        async with self._lock:
            ...
            if self.algorithm == LoadBalancingAlgorithm.ROUND_ROBIN:
                ...
                service = self._hash_based_selection(healthy_services, request_path, client_ip)
    ...
    def _weighted_round_robin_selection(self, services: List[BackendService]) -> BackendService:
    ...
    def _least_connections_selection(self, services: List[BackendService]) -> BackendService:
        """Select service with least connections."""
        return min(services, key=lambda s: s.current_connections)
    
    def _least_response_time_selection(self, services: List[BackendService]) -> BackendService:
    ...
    def update_service_metrics(self, service_id: str, response_time_ms: float, success: bool):
        ...
        if service.total_requests == 1:
            ...
            service.avg_response_time = (
                (service.avg_response_time * (service.total_requests - 1) + response_time_ms) /
...
class RateLimiter:
    ...
    async def _check_fixed_window(
        ...
        # Reset if new window
        if 'window' not in self.fixed_windows[window_key] or self.fixed_windows[window_key]['window'] != current_window:
    ...
    async def _check_sliding_window(
        ...
        # Remove old entries
        while self.sliding_windows[window_key] and self.sliding_windows[window_key][0] < window_start:
        ...
        if current_count >= rule.requests_per_window:
            return False, {
                ...
                'count': current_count,
                'limit': rule.requests_per_window,
                'retry_after': int((self.sliding_windows[window_key][0] + timedelta(seconds=rule.window_size_seconds) - current_time).total_seconds())
            }
    ...
    async def _check_token_bucket(
        ...
        if tokens >= 1:
            ...
            return False, {
                ...
                'tokens': tokens,
                'max_tokens': rule.requests_per_window,
                'retry_after': int((1 - tokens) * (rule.window_size_seconds / rule.requests_per_window))
            }
    ...
    async def _check_leaky_bucket(
        ...
        if len(bucket) >= rule.requests_per_window:
            ...
            retry_after = int((oldest_request + timedelta(seconds=1 / leak_rate) - current_time).total_seconds())
        ...
        while True:
            try:
                ...
                for key in list(self.sliding_windows.keys()):
                    while (self.sliding_windows[key] and 
                           self.sliding_windows[key][0] < cutoff_time):
                ...
                for key in list(self.leaky_buckets.keys()):
                    while (self.leaky_buckets[key] and 
                           self.leaky_buckets[key][0] < cutoff_time):
    ...
    async def forward_request(
        ...
        try:
            ...
            async with self.http_session.request(
                ...
                return response.status_code, response_headers, response_body, response_time
    ...
    def get_gateway_stats(self) -> Dict[str, Any]:
        ...
        if recent_metrics:
            ...
            stats = {
                ...
                'requests_per_minute': len(recent_metrics) / 10  # Assuming recent metrics cover last 10 minutes
            }
        else:
            stats = {
                'total_requests': 0,
        ...
        stats['rate_limiter'] = {
            'active_rules': len([r for r in self.rate_limiter.rules.values() if r.enabled]),
