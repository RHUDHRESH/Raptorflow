...
class Migration:
    ...
    def __init__(self, version: str, name: str, sql: str, rollback_sql: Optional[str] = None):
...
class MigrationRunner:
    ...
    async def ensure_migrations_table(self) -> None:
        ...
        try:
            ...
            result = self.client.table(self.migrations_table).select("version").limit(1).execute()
    ...
    async def get_applied_migrations(self) -> List[str]:
        ...
        try:
            result = self.client.table(self.migrations_table).select("version").order("applied_at").execute()
            return [migration["version"] for migration in result.data]
        except Exception as e:
            logger.error(f"Failed to get applied migrations: {e}")
            return []
    
    async def load_migrations(self, migrations_dir: str = "supabase/migrations") -> List[Migration]:
    ...
    async def rollback_migration(self, version: str) -> bool:
        ...
        try:
            ...
            result = self.client.table(self.migrations_table).select("*").eq("version", version).execute()
            ...
            self.client.table(self.migrations_table).delete().eq("version", version).execute()
