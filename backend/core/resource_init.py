...
class ResourceSystemManager:
    ...
    async def _log_system_status(self):
        ...
        try:
            ...
            if self.resource_manager:
                ...
                logger.info(
                    f"Resource Manager Status: {resource_summary['total_resources']} resources tracked"
                )
            ...
            if self.metrics_collector:
                ...
                logger.info(
                    f"Metrics Collector Status: {metrics_summary['total_metrics']} metrics defined, {metrics_summary['total_values']} values recorded"
                )
            ...
            if self.resource_analyzer:
                ...
                logger.info(
                    f"Resource Analyzer Status: {analysis_summary['total_patterns_detected']} patterns detected, {analysis_summary['total_recommendations']} recommendations generated"
                )
            ...
            if self.quota_manager:
                ...
                logger.info(
                    f"Quota Manager Status: {quota_metrics['total_checks']} checks performed, {quota_metrics['total_violations']} violations"
                )
            ...
            if self.cleanup_scheduler:
                ...
                logger.info(
                    f"Cleanup Scheduler Status: {scheduler_metrics['total_tasks']} tasks configured, {scheduler_metrics['success_rate']:.1f}% success rate"
                )
    ...
    async def health_check(self) -> dict:
        ...
        health_status = {"overall": "healthy", "timestamp": None, "systems": {}}
        ...
        try:
            ...
            if self.quota_manager:
                ...
                recent_violations = [
                    v
                    for v in self.quota_manager.quota_violations
                    if (datetime.now() - v.violation_time).total_seconds()
                    < 3600  # Last hour
                ]
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Raptorflow Resource and Metrics System"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--daemon", action="store_true", help="Run as daemon (continuous operation)"
    )
