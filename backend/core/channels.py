        ...
        if any("founder" in str(p).lower() for p in personas):
            allocations.append(
                ChannelWeight(
                    channel="linkedin",
                    weight=0.5,
                    reasoning="High founder concentration.",
                )
            )
            allocations.append(
                ChannelWeight(
                    channel="email", weight=0.3, reasoning="Direct executive outreach."
                )
            )
            allocations.append(
                ChannelWeight(
                    channel="twitter",
                    weight=0.2,
                    reasoning="Real-time industry signals.",
                )
            )
        else:
            allocations.append(
                ChannelWeight(
                    channel="search", weight=0.4, reasoning="Intent-based discovery."
                )
            )
            allocations.append(
                ChannelWeight(
                    channel="social", weight=0.6, reasoning="Broad awareness."
                )
            )
