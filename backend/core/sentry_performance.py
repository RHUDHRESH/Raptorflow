...
class PerformanceThresholds:
    ...
    response_time_ms: Dict[PerformanceThreshold, float] = field(default_factory=lambda: {
    ...
    error_rate_percent: Dict[PerformanceThreshold, float] = field(default_factory=lambda: {
    ...
    memory_usage_mb: Dict[PerformanceThreshold, float] = field(default_factory=lambda: {
        PerformanceThreshold.EXCELLENT: 100,
...
class SentryPerformanceMonitor:
    ...
    def _send_api_metrics_to_sentry(self, metrics: APIEndpointMetrics) -> None:
        ...
        try:
            ...
            configure_scope(lambda scope: scope.set_tag("http.status_code", str(metrics.status_code)))
            configure_scope(lambda scope: scope.set_tag("endpoint", metrics.endpoint))
            
            # Set measurements
            configure_scope(lambda scope: set_measurement(
                "response_time",
            ...
            # Set context
            configure_scope(lambda scope: scope.set_context("api_metrics", {
    ...
    def _check_api_performance(self, metrics: APIEndpointMetrics) -> None:
        ...
        threshold = self._evaluate_performance_threshold(
            metrics.response_time_ms,
            self._thresholds.response_time_ms
        )
    ...
    def _send_db_metrics_to_sentry(self, metrics: DatabaseQueryMetrics) -> None:
        ...
        try:
            # Set tags
            configure_scope(lambda scope: scope.set_tag("db.query_type", metrics.query_type))
            if metrics.table:
                configure_scope(lambda scope: scope.set_tag("db.table", metrics.table))
            configure_scope(lambda scope: scope.set_tag("db.success", str(metrics.success)))
            ...
            # Set context
            configure_scope(lambda scope: scope.set_context("db_metrics", {
    ...
    def _evaluate_performance_threshold(self, value: float, thresholds: Dict[PerformanceThreshold, float]) -> PerformanceThreshold:
        ...
        with self._lock:
            ...
            if recent_api_metrics:
                ...
                api_summary = {
                    ...
                    "error_rate": len([m for m in recent_api_metrics if m.status_code >= 400]) / len(recent_api_metrics),
                    "status_code_distribution": self._get_status_code_distribution(recent_api_metrics),
            ...
            if recent_db_metrics:
                query_times = [m.execution_time_ms for m in recent_db_metrics if m.success]
                db_summary = {
                    ...
                    "p95_query_time_ms": self._percentile(query_times, 95) if query_times else 0,
                    "error_rate": len([m for m in recent_db_metrics if not m.success]) / len(recent_db_metrics),
                    "query_type_distribution": self._get_query_type_distribution(recent_db_metrics),
                }
            ...
            return {
                ...
                "performance_health": self._evaluate_overall_health(api_summary, db_summary),
            }
    ...
    def _get_status_code_distribution(self, metrics: List[APIEndpointMetrics]) -> Dict[str, int]:
    ...
    def _get_top_endpoints(self, metrics: List[APIEndpointMetrics], limit: int = 10) -> List[Dict[str, Any]]:
    ...
    def _get_query_type_distribution(self, metrics: List[DatabaseQueryMetrics]) -> Dict[str, int]:
    ...
    def _evaluate_overall_health(self, api_summary: Dict[str, Any], db_summary: Dict[str, Any]) -> str:
...
def track_performance(operation_name: Optional[str] = None, operation_type: str = "function"):
    """
