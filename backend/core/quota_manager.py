...
class QuotaManager:
    ...
    async def check_quota(
        ...
        try:
            ...
            applicable_quotas = self._find_applicable_quotas(resource_type, user_id, workspace_id)
            ...
            for quota in applicable_quotas:
                ...
                result = self._evaluate_quota_check(quota, usage, new_usage, usage_amount)
    ...
    async def _monitor_usage_trends(self):
        ...
        try:
            for quota_id, usage_history in self.usage_history.items():
                ...
                high_usage_ratio = sum(1 for u in recent_usage if u > quota.limit * 0.8) / len(recent_usage)
                
                if high_usage_ratio > 0.8:  # 80% of periods have high usage
                    logger.info(f"High usage trend detected for quota {quota_id}: {high_usage_ratio:.1%}")
    ...
    async def _adjust_dynamic_quota(self, quota_id: str, recent_usage: List[Union[int, float]]):
        ...
        try:
            ...
            if new_limit != quota.limit:
                ...
                logger.info(f"Adjusted dynamic quota {quota_id}: {quota.limit} (was {quota.limit * 0.8:.0f})")
    ...
    def get_quota_status(
        ...
        status["violations"] = [
            v.to_dict() for v in self.quota_violations
