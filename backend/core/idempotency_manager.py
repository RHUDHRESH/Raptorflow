...
class IdempotencyManager:
    ...
    async def check_idempotency(
        ...
        try:
            ...
            record = IdempotencyRecord(
                ...
                completed_at=datetime.fromisoformat(record_dict["completed_at"]) if record_dict.get("completed_at") else None,
    ...
    async def create_idempotency_record(
        ...
        try:
            ...
            if request_size > self.max_request_size_kb * 1024:
                raise ValueError(f"Request too large: {request_size} bytes (max: {self.max_request_size_kb * 1024})")
        ...
        try:
            ...
            if response_data:
                ...
                if response_size > self.max_response_size_kb * 1024:
                    logger.warning(f"Response too large, truncating: {response_size} bytes")
                    # Truncate response data
                response_data = self._truncate_large_data(response_data, self.max_response_size_kb * 1024)
    ...
    async def get_response(self, operation_type: str, idempotency_key: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def idempotent(
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                # Check idempotency
                existing_record = await self.check_idempotency(idempotency_key, request_data)
                ...
                if existing_record:
                    if existing_record.status == IdempotencyStatus.COMPLETED:
                        ...
                        return await self._wait_for_completion(idempotency_key, timeout_seconds=30)
                    elif existing_record.status == IdempotencyStatus.FAILED:
                        # Return cached error
                        raise Exception(f"Operation failed: {existing_record.error_data}")
                    else:
    ...
    async def _wait_for_completion(self, idempotency_key: str, timeout_seconds: int = 30) -> Any:
    ...
    def _truncate_large_data(self, data: Dict[str, Any], max_size: int) -> Dict[str, Any]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": "Idempotency manager is operational" if overall_healthy else "Idempotency manager has issues",
                "features": {
