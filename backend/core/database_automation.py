...
class DatabaseAutomation:
    ...
    async def _perform_hourly_optimization(self) -> Dict[str, Any]:
        ...
        results = {"timestamp": datetime.utcnow(), "tasks": {}}
        ...
        try:
            ...
            analysis = {
                ...
                "optimization_opportunities": len(
                    perf_report.get("query_patterns", [])
                ),
                "performance_score": self._calculate_performance_score(perf_report),
            }

            return {"status": "success", "analysis": analysis, "report": perf_report}
    ...
    async def _generate_maintenance_recommendations(
        ...
        try:
            # Analyze performance
            perf_analysis = (
                results.get("tasks", {}).get("performance", {}).get("analysis", {})
            )
            ...
            if perf_analysis.get("optimization_opportunities", 0) > 3:
                recommendations.append(
                    "Multiple optimization opportunities found - review query patterns"
                )

            performance_score = perf_analysis.get("performance_score", 100)
            if performance_score < 80:
                recommendations.append(
                    f"Performance score ({performance_score}) below threshold - investigate bottlenecks"
                )
            ...
            if cleanup_result.get("records_cleaned", 0) > 1000:
                recommendations.append(
                    "Large amount of old data cleaned - consider more frequent cleanup"
                )
            ...
            if index_result.get("indexes_optimized", 0) > 0:
                recommendations.append(
                    "Indexes were optimized - monitor for performance improvements"
                )
    ...
    async def _check_connection_pool_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if utilization > 0.9:
                ...
                actions.append(
                    "Connection pool utilization critical - consider increasing max connections"
                )
    ...
    async def _apply_auto_optimizations(
        ...
        try:
            ...
            pool_health = optimization_results.get("tasks", {}).get(
                "connection_pool", {}
            )
            if pool_health.get("status") == "critical":
                logger.warning(
                    "Connection pool critical - manual intervention required"
                )
            ...
            if query_health.get("long_running_queries", 0) > 0:
                logger.info(
                    f"Found {query_health['long_running_queries']} long-running queries"
                )
        ...
        try:
            ...
            for alert in critical_alerts:
                ...
                if "connection pool" in alert.get("message", "").lower():
                    logger.critical(
                        "Connection pool critical - immediate attention required"
                    )
