...
class PaymentFraudDetector:
    ...
    def _load_default_rules(self):
        ...
        default_rules = [
            ...
            FraudRule(
                ...
                conditions={
                    "require_device_history": True,
                    "require_ip_history": True
                },
                weight=0.5
            ),
    ...
    async def assess_payment_risk(
        ...
        try:
            ...
            assessment.recommended_action = self._get_recommended_action(assessment.risk_level)
    ...
    async def _evaluate_rule(
        ...
        if rule.method == DetectionMethod.RULE_BASED:
            return await self._evaluate_rule_based(rule, user_profile, amount, customer_info, device_info, location_info)
        elif rule.method == DetectionMethod.BEHAVIORAL_ANALYSIS:
            return await self._evaluate_behavioral_rule(rule, user_profile, amount, customer_info)
            ...
            return await self._evaluate_geolocation_rule(rule, user_profile, location_info)
    ...
    async def _evaluate_rule_based(
        ...
        if rule.id == "high_velocity_1min":
            ...
            recent_transactions = await self._get_recent_transactions(user_profile.user_id, minutes=1)
            
            if len(recent_transactions) >= rule.conditions["max_transactions_per_minute"]:
                return FraudSignal(
                    rule_id=rule.id,
            ...
            recent_transactions = await self._get_recent_transactions(user_profile.user_id, hours=1)
            
            if len(recent_transactions) >= 5:  # Minimum transactions to evaluate
                failed_count = sum(1 for tx in recent_transactions if tx.get("status") == "FAILED")
    ...
    async def _evaluate_behavioral_rule(
        ...
        if rule.id == "unusual_amount":
            ...
            if user_profile.transaction_count >= rule.conditions["min_transaction_count"]:
                ...
                if device_new and ip_new and len(user_profile.device_fingerprints) > 0:
                    return FraudSignal(
                        ...
                        details={
                            ...
                            "new_ip": ip_new,
                            "device_fingerprint": device_fingerprint[:8] + "...",  # Partial for security
        ...
        if rule.id == "geolocation_anomaly" and location_info:
            ...
            if current_location and user_profile.risk_factors.get("last_location"):
                ...
                distance = self._calculate_distance(
                    current_location,
                    user_profile.risk_factors["last_location"]
                )
    ...
    def _calculate_risk_score(self, signals: List[FraudSignal]) -> float:
        ...
        weighted_score = sum(signal.risk_score * signal.confidence for signal in signals)
    ...
    async def _get_user_profile(self, user_id: str) -> UserProfile:
        ...
        if profile_data:
            ...
            profile = UserProfile(
                ...
                last_transaction=datetime.fromisoformat(profile_dict["last_transaction"]) if profile_dict.get("last_transaction") else None,
            ...
            # Create new profile
            profile = UserProfile(user_id=user_id, risk_score=0.0, risk_level=FraudRiskLevel.LOW)
    ...
    async def _store_user_profile(self, profile: UserProfile):
        ...
        try:
            ...
            self.redis.setex(profile_key, self._profile_cache_ttl, json.dumps(profile_data))
            
        except Exception as e:
            logger.error(f"Error storing user profile: {e}")
    
    async def _update_user_profile(self, profile: UserProfile, assessment: FraudAssessment):
    ...
    async def _store_assessment(self, assessment: FraudAssessment):
        ...
        try:
            ...
            self.redis.setex(assessment_key, 86400, json.dumps(assessment_data))  # 24 hours
            
        except Exception as e:
            logger.error(f"Error storing assessment: {e}")
    
    async def _get_recent_transactions(self, user_id: str, minutes: int = 1, hours: int = 0) -> List[Dict[str, Any]]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": "Fraud detection system is operational" if overall_healthy else "Fraud detection system has issues",
                "features": {
