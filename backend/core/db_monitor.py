...
class DatabasePoolMonitor:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats.pool_utilization_percent > 80:
                ...
                health_status["issues"].append(f"High pool utilization: {stats.pool_utilization_percent:.1f}%")
                health_status["recommendations"].append("Consider increasing max_pool_size")
            
            # Check failed connections
            if stats.failed_connections > 10:
                health_status["healthy"] = False
                health_status["issues"].append(f"High failure rate: {stats.failed_connections} failed connections")
            ...
            if stats.avg_connection_time_ms > 1000:  # 1 second
                health_status["issues"].append(f"Slow connection times: {stats.avg_connection_time_ms:.1f}ms avg")
                health_status["recommendations"].append("Optimize database network or increase timeouts")
            
            # Check if pool is at capacity
            if stats.active_connections >= getattr(self.pool, 'max_size', 50) * 0.9:
                health_status["issues"].append("Pool near maximum capacity")
                health_status["recommendations"].append("Monitor for potential connection exhaustion")
            
            # Add recent errors if any
            if stats.connection_errors:
                health_status["recent_errors"] = stats.connection_errors[-5:]  # Last 5 errors
    ...
    async def optimize_pool_settings(self) -> Dict[str, Any]:
        ...
        if stats.pool_utilization_percent > 80:
            recommendations["recommended_settings"]["max_size"] = getattr(self.pool, 'max_size', 50) + 10
            recommendations["reasoning"].append("High utilization suggests increasing max_size")
        ...
        if stats.avg_connection_time_ms > 500:
            ...
            recommendations["reasoning"].append("Slow connection times suggest more warm connections")
        
        if stats.failed_connections > 5:
            recommendations["recommended_settings"]["max_lifetime"] = 1800  # 30 minutes
            recommendations["reasoning"].append("High failure rate may indicate stale connections")
        ...
        while self.optimization_enabled:
            try:
                ...
                if not health["healthy"]:
                    logger.warning(f"Database pool health issues detected: {health['issues']}")
                
                # Optimize settings every 5 minutes
                if (self.last_optimization is None or 
                    datetime.utcnow() - self.last_optimization > timedelta(minutes=5)):
    ...
    async def _auto_optimize(self):
        ...
        try:
            ...
            if recommendations["recommended_settings"]:
                logger.info(f"Pool optimization recommendations: {recommendations['reasoning']}")
                ...
                for setting, value in recommendations["recommended_settings"].items():
                    ...
                    if value != current_value:
                        logger.info(f"Recommended {setting}: {current_value} -> {value}")
