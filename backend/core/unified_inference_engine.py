...
class SimulatedInferenceEngine(BaseInferenceEngine):
    ...
    def _generate_contextual_response(
        ...
        if "?" in prompt:
            response += "I've analyzed your question and prepared the following insights:\n\n"
    ...
    def _generate_default_response(
        ...
        response += "In a production environment, this would be processed by a real LLM.\n"
    ...
    async def generate(
        ...
        cache_key = hashlib.sha256(json.dumps(cache_content, sort_keys=True).encode()).hexdigest()
        ...
        if cache_key in self._response_cache:
            ...
            return UnifiedResponse(
                ...
                tokens_used=self.token_counter.count_tokens(cached_response, request.model),
                cost_usd=0.0,
                cached=True,
            )

        # Generate response based on prompt content
        keyword_response = self._generate_keyword_response(request.prompt, request.style)
    ...
    def set_mode(self, mode: InferenceMode) -> None:
        ...
        if mode == InferenceMode.REAL and self._real_engine is None:
            try:
                ...
                logger.error(
                    "Failed to initialize real engine", error=str(e)
                )
                self._mode = InferenceMode.SIMULATE
