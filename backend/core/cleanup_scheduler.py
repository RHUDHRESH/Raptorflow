    ...
    async def execute(self, task: CleanupTask) -> CleanupResult:
        ...
        try:
            ...
            if resource_type_str:
                ...
                cleaned_count = await resource_manager.cleanup_resources_by_type(resource_type)
            else:
...
class CleanupScheduler:
    ...
    def __init__(self, max_concurrent_tasks: int = 5):
        ...
        logger.info(f"Cleanup scheduler initialized with max concurrent tasks: {max_concurrent_tasks}")
    ...
    def add_task(self, task: CleanupTask) -> bool:
        ...
        try:
            ...
            if task.trigger_type == CleanupTriggerType.SCHEDULED and task.schedule_expression:
    ...
    def _schedule_task(self, task: CleanupTask):
        ...
        try:
            ...
            if len(parts) != 5:
                logger.warning(f"Invalid schedule expression for task {task.task_id}: {task.schedule_expression}")
                return
            ...
            def job():
                if task.enabled and self._running:
                    asyncio.create_task(self.task_queue.put((task, CleanupTriggerType.SCHEDULED)))
            ...
            if minute == "*" and hour == "*":
                ...
                logger.warning(f"Complex scheduling not fully supported for task {task.task_id}")
            
            logger.debug(f"Scheduled task {task.task_id} with expression: {task.schedule_expression}")
        ...
        try:
            ...
            for attempt in range(task.retry_count + 1):
                try:
                    ...
                    if result.success:
                        ...
                        logger.warning(f"Task {task.task_id} failed on attempt {attempt + 1}: {result.error}")
                        
                        if attempt < task.retry_count:
                            await asyncio.sleep(task.retry_delay_seconds)
                
                except Exception as e:
                    logger.error(f"Task {task.task_id} exception on attempt {attempt + 1}: {e}")
            ...
            if result:
                task.average_duration_seconds = (
                    (task.average_duration_seconds * (task.run_count - 1) + result.execution_time_seconds)
                    / task.run_count
                )
            ...
            if result:
                ...
                self.scheduler_metrics["total_resources_cleaned"] += result.resources_cleaned
                ...
                self.scheduler_metrics["average_execution_time"] = (
                    (current_avg * (total_runs - 1) + result.execution_time_seconds) / total_runs
                )
            
            logger.info(f"Completed task {task.task_id}: {result.message if result else 'No result'}")
    ...
    async def _check_system_thresholds(self):
        ...
        try:
            ...
            # Check memory usage
            memory_values = metrics_collector.get_metric_values("system_memory_percent", limit=1)
            if memory_values:
                ...
                if memory_percent > 85:
                    ...
                    if memory_task and memory_task.enabled:
                        await self.task_queue.put((memory_task, CleanupTriggerType.THRESHOLD_BASED))
            
            # Check resource count
            resource_values = metrics_collector.get_metric_values("resource_count", limit=1)
            if resource_values:
                ...
                if resource_count > 1000:  # Threshold for too many resources
                    ...
                    if resource_task and resource_task.enabled:
                        await self.task_queue.put((resource_task, CleanupTriggerType.THRESHOLD_BASED))
        ...
        return {
            **self.scheduler_metrics,
            ...
            "success_rate": (
                ...
                self.scheduler_metrics["total_tasks_run"] * 100
                if self.scheduler_metrics["total_tasks_run"] > 0 else 0
            ),
        }
