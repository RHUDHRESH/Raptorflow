...
class PivotEngine:
    ...
    @staticmethod
    def evaluate_pivot(quality_score: float, outcomes: List[Dict[str, Any]]) -> List[PivotRecommendation]:
        """Surgically evaluates if a pivot is required."""
        recommendations = []
        
        # SOTA Heuristic: If quality is low, pivot
        if quality_score < 0.6:
            recommendations.append(PivotRecommendation(
        ...
        logger.info(f"Pivot evaluation complete. Recommendations: {len(recommendations)}")
