...
class DocumentationConfig:
    ...
    description: str = "Production-grade agentic platform for growth marketing automation"
    version: str = "1.0.0"
    ...
    def __post_init__(self):
        if not self.servers:
            self.servers = [
                ...
                {"url": "https://api.raptorflow.com", "description": "Production server"}
            ]
    ...
    async def generate_postman_collection(self) -> Dict[str, Any]:
        ...
        collection = {
            ...
            "variable": [
                {
                    "key": "base_url",
                    "value": self.config.servers[0]["url"] if self.config.servers else "http://localhost:8000"
                },
                {
                    "key": "access_token",
                    "value": ""
        ...
        for path, methods in paths.items():
            for method, endpoint_spec in methods.items():
                if method.lower() in ["get", "post", "put", "delete", "patch"]:
                    item = {
                        "name": endpoint_spec.get("summary", f"{method.upper()} {path}"),
                        "request": {
                            "method": method.upper(),
                            "header": [
                                {
                    ...
                    if "responses" in endpoint_spec:
                        ...
                        for status_code, response in endpoint_spec["responses"].items():
                            item["response"].append({
                                "name": response.get("description", f"Status {status_code}"),
...
"""
    ...
    async def export_documentation(self, doc_type: DocumentationType) -> Union[Dict[str, Any], str]:
...
async def generate_api_docs(app: FastAPI, doc_type: DocumentationType = DocumentationType.OPENAPI):
