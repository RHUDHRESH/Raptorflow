...
class DataAggregator:
    ...
    async def aggregate_data(
        ...
        cache_key = (
            f"{source}_{time_range.value}_{aggregation.value}_{interval_minutes}"
        )
        ...
        async with self._lock:
            ...
            filtered_data = [
                dp for dp in self.data_sources[source] if dp.timestamp >= cutoff_time
            ]
    ...
    def _aggregate_by_interval(
        ...
        for interval_start, group_points in sorted(groups.items()):
            ...
            if aggregation == AggregationType.SUM:
                ...
                aggregated_value = statistics.median(
                    values
...
class DashboardRenderer:
    ...
    async def _render_line_chart(self, widget: DashboardWidget) -> Dict[str, Any]:
            ...
            "datasets": [
                {
                    ...
                    "backgroundColor": widget.parameters.get(
        ...
        return {
            ...
            "options": {
                "responsive": True,
                "scales": {
                    "y": {"beginAtZero": widget.parameters.get("begin_at_zero", True)}
                },
            },
        }
    ...
    async def _render_table(self, widget: DashboardWidget) -> Dict[str, Any]:
        ...
        latest_data = sorted(
            aggregated_data, key=lambda dp: dp.timestamp, reverse=True
        )[:limit]
    ...
    async def _render_gauge(self, widget: DashboardWidget) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_value": max_value,
            "unit": widget.parameters.get("unit", ""),
            "thresholds": widget.parameters.get(
    ...
    async def _render_top_n_list(self, widget: DashboardWidget) -> Dict[str, Any]:
        ...
        return {
            ...
            "title": widget.title,
            "items": [
                {"name": name, "value": value, "rank": idx + 1}
...
class AnalyticsDashboard:
    ...
    async def generate_report(
        self, report_name: str, time_range: TimeRange, metrics: List[str]
    ) -> AnalyticsReport:
    ...
    async def _generate_insights(
        self, metrics: Dict[str, List[AnalyticsDataPoint]], time_range: TimeRange
    ) -> List[str]:
        ...
        if "search_requests" in metrics:
            ...
            if len(volume_data) >= 2:
                ...
                if recent_avg > earlier_avg * 1.2:
                    insights.append(
                        "Search volume has increased significantly in recent period"
                    )
                elif recent_avg < earlier_avg * 0.8:
                    insights.append(
                        "Search volume has decreased significantly in recent period"
                    )
        ...
        if "search_duration" in metrics:
            ...
            if duration_data:
                ...
                if avg_duration > 2000:
                    insights.append(
                        "Average response time is above 2 seconds - consider optimization"
                    )
    ...
    async def _generate_recommendations(
        self, metrics: Dict[str, List[AnalyticsDataPoint]], insights: List[str]
    ) -> List[str]:
        ...
        for insight in insights:
            if "increased significantly" in insight:
                ...
                recommendations.append(
                    "Optimize search queries and consider caching strategies"
                )
                ...
                recommendations.append(
                    "Current performance is optimal - maintain monitoring"
                )
