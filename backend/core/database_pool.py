...
class DatabaseConnectionPool:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Database pool initialized successfully (min={self.config.min_size}, max={self.config.max_size})")
            return True
    ...
    async def get_session(self) -> AsyncSession:
        ...
        if self.engine is None:
            raise DatabaseError("Database pool not initialized", operation="get_session")
    ...
    async def execute_query(
        ...
        if self.engine is None:
            raise DatabaseError("Database pool not initialized", operation="execute_query")
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                last_error = e
                logger.warning(f"Query timeout (attempt {attempt + 1}/{self.config.max_retries}): {query[:100]}...")
    ...
    async def _perform_health_check(self):
        ...
        try:
            ...
            await self._handle_pool_error("health_check_timeout", TimeoutError("Health check timeout"))
        except Exception as e:
