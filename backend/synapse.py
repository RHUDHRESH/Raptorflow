...
class Synapse:
    ...
    async def load_state(self, flow_id: str) -> Dict:
        ...
        try:
            result = self.client.table("campaign_arcs").select("current_pulse").eq("id", flow_id).single().execute()
            return result.data.get("current_pulse", {}) if result.data else {}
        except Exception as e:
            logger.error(f"❌ Synapse: Failed to load state: {e}")
            return {}

    async def log_thought(self, entity_id: str, entity_type: str, agent_name: str, thought: str, workspace_id: str, tokens: int = 0):
    ...
    async def run_node(self, node_name: str, context: Dict, flow_id: str = "temp"):
        ...
        if not workspace_id:
            logger.warning("⚠️ Synapse: No workspace_id in context. Persistence may fail.")
    ...
    async def run_sequence(self, node_list: List[str], context: Dict, flow_id: str = "temp"):
    ...
    async def run_move(self, move_name: str, context: Dict, flow_id: str = "temp"):
        ...
        move_sequences = {
            ...
            "competitive_analysis": ["research_node", "strategy_node", "content_creator"],
            "product_launch": ["research_node", "strategy_node", "content_creator", "seo_skill"]
        }
