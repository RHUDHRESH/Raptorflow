...
class SwarmOrchestrator:
    ...
    def _build_swarm_tree(self) -> Dict[str, Tuple[str, ...]]:
        return {
            "controller": tuple(self.team_members),
            "strategists": ("researchers", "creatives", "quality", "competitor_intelligence"),
            "researchers": ("strategists", "quality", "competitor_intelligence"),
            "creatives": ("quality", "competitor_intelligence"),
            "quality": ("strategists", "creatives"),
            "competitor_intelligence": ("strategists", "researchers"),
        }
    ...
    def _ensure_metadata(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state.setdefault("competitor_insights", [])
        state.setdefault("competitor_analyses", [])
        state.setdefault("active_competitor_watchlist", [])
        state.setdefault("competitive_landscape_summary", None)
        state.setdefault("market_positioning_analysis", None)
        state.setdefault("competitive_advantages", [])
        state.setdefault("competitive_threats", [])
        queue_controller = state.get("queue_controller")
        capability_profile = (
            state.get("capability_profile")
            or getattr(queue_controller, "capability_profile", None)
    ...
    async def route_intent(self, query: str) -> SwarmIntent:
        ...
        system_msg = SystemMessage(
            content=(
                "You are the Intent Router for RaptorFlow.\n"
                "Classify user prompts into asset families: email, social, meme, strategy, or competitor_analysis.\n"
                "Extract @mentions which represent campaigns, cohorts, or competitors.\n"
                "Assign confidence based on how much detail is provided.\n"
                "If the prompt is vague (e.g. 'write an email'), confidence must be < 0.7.\n"
                "For competitor-related requests, route to competitor_intelligence specialist."
            )
        )
