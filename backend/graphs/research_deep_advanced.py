...
from models.research_schemas import ResearchDeepState, FactClaim, WebDocument
from agents.shared.research_specialists import LibrarianAgent, FactCheckerAgent, SynthesisAgent
...
async def discovery_node(state: ResearchDeepState):
    ...
    search = SearchProvider(api_key=os.getenv("SERPER_API_KEY", ""))
    all_urls = []
    for q in state.queries:
        links = await search.search(q, num_results=3)
        all_urls.extend(links)
    
    return {"status": "scraping", "research_logs": state.research_logs + [f"Discovered {len(all_urls)} potential sources."]}
