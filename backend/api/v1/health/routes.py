...
async def readiness_check():
    ...
    try:
        ...
        if sentinel and sentinel._master:
            await asyncio.wait_for(sentinel._master.ping(), timeout=5.0)
...
def _configured_integrations() -> Dict[str, bool]:
    ...
    return {
        ...
        "redis_sentinel": bool(settings.REDIS_SENTINEL_ENABLED),
...
async def database_health():
    ...
    if pool_stats.get("status") in {"error", "unavailable"}:
        raise HTTPException(
            status_code=503,
            detail=f"Database unhealthy: {pool_stats.get('error', 'unknown')}",
        )
