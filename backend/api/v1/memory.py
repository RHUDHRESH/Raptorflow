...
from memory.cache import create_cached_coordinator
...
class ConsolidationRequest(BaseModel):
    ...
    agent_ids: Optional[List[str]] = Field(
        None, description="Specific agents to consolidate"
    )
...
def get_coordinator(workspace_id: str) -> SwarmMemoryCoordinator:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to initialize memory coordinator"
        )


@router.get("/workspace/{workspace_id}/stats", response_model=MemoryStatsResponse)
async def get_memory_stats(
    workspace_id: str, coordinator: SwarmMemoryCoordinator = Depends(get_coordinator)
):
    """
...
@router.post(
    "/workspace/{workspace_id}/consolidate", response_model=ConsolidationResponse
)
async def trigger_consolidation(
    ...
    try:
        result = await coordinator.consolidate_swarm_memories(
            force=request.force, agent_ids=request.agent_ids
        )
...
@router.post(
    "/workspace/{workspace_id}/agent/context", response_model=AgentContextResponse
)
async def get_agent_context(
    ...
    try:
        ...
        return AgentContextResponse(
            ...
            relevant_swarm_memory=serialize_fragments(
                context.get("relevant_swarm_memory", [])
            ),
    ...
    try:
        ...
        if health_status["issues"]:
            health_status["status"] = (
                "degraded" if len(health_status["issues"]) <= 2 else "unhealthy"
            )
