...
async def initiate_payment_enhanced(
    ...
    try:
        ...
        security_context = SecurityContext(
            ...
            device_fingerprint=request.customer_info.device_fingerprint if request.customer_info else None
        )
        ...
        transaction_data = {
            ...
            'user_id': request.customer_info.id if request.customer_info else 'anonymous',
        ...
        fraud_assessment = await fraud_detector.assess_transaction_fraud(
            transaction_data, user_profile, request.customer_info.device_fingerprint if request.customer_info else None
        )
            ...
            await payment_monitor.record_transaction_event(
                TransactionEvent(
                    ...
                    metadata={
                        ...
                        'fraud_signals': [asdict(signal) for signal in fraud_assessment.signals]
                    }
                )
            )
...
@router.post("/webhook")
async def handle_webhook_enhanced(
    request: Request,
    background_tasks: BackgroundTasks
):
    """
    ...
    try:
        ...
        await payment_monitor.record_transaction_event(
            TransactionEvent(
                ...
                status=TransactionStatus.COMPLETED if webhook_data.get('code') == 'PAYMENT_SUCCESS' else TransactionStatus.FAILED,
    ...
    try:
        ...
        return {
            ...
            'components': {
                ...
                'payment_monitoring': monitoring_metrics.__dict__ if monitoring_metrics else {},
...
def _calculate_overall_security_score(
    ...
    try:
        ...
        if security_metrics.get('total_events', 0) > 0:
            blocked_ratio = security_metrics.get('blocked_requests', 0) / security_metrics['total_events']
