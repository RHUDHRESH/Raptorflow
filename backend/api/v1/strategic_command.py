...
from backend.services.macm import get_context_assembler
...
async def create_blueprint(request: BlueprintRequest, auth: AuthContext = Depends(get_auth_context)):
    """
    ...
    result = await brain.run_node("strategy_node", {
        "workspace_id": auth.workspace_id,
        "goal": request.goal,
...
async def get_hierarchical_agenda(auth: AuthContext = Depends(get_auth_context)):
    ...
    tasks = []
    if move_ids:
        tasks_res = client.table("scheduled_tasks") \
            .select("*") \
...
async def update_task_status(
    ...
    # 1. Update the task
    result = client.table("scheduled_tasks").update({
        "status": update.status,
        "updated_at": datetime.now(UTC).isoformat()
    }).eq("id", task_id).eq("workspace_id", auth.workspace_id).execute()
    ...
    if update.status in ["failed", "pushed_back"]:
        task = result.data[0]
        if task.get("move_id"):
            await brain.run_node("arc_recal_node", {
                "workspace_id": auth.workspace_id,
...
async def get_expert_reasoning(entity_id: str, auth: AuthContext = Depends(get_auth_context)):
    """
    Returns the full expert reasoning trace for a move or campaign.
    """
    client = get_supabase_client()
    result = client.table("agent_thought_logs") \
        .select("*") \
...
async def conclude_move(move_id: str, auth: AuthContext = Depends(get_auth_context)):
    ...
    move_res = client.table("moves").select("*").eq("id", move_id).eq("workspace_id", auth.workspace_id).single().execute()
    ...
    result = await brain.run_node("post_mortem_node", {
        "workspace_id": auth.workspace_id,
        "move": move
    })
    ...
    if success_score >= 0.8 and decision == "extend":
        ...
        from backend.services.strategic_arc import StrategicArcService
