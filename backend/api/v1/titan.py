...
class TitanResearchRequest(BaseModel):
    query: str = Field(
        ..., description="The primary research objective or company name."
    )
    mode: str = Field(
        default="LITE", description="Research mode: LITE, RESEARCH, or DEEP."
    )
    focus_areas: Optional[List[str]] = Field(
        default_factory=list, description="Optional focus areas (e.g. pricing, ICP)."
    )
    max_results: Optional[int] = Field(
        None, description="Limit on the number of results."
    )
    use_stealth: bool = Field(
        default=True, description="Whether to use stealth escalation ladder."
    )
    ...
    try:
        result = await orchestrator.execute(
            ...
            or (
                ...
                else 50 if request.mode == "DEEP" else 10
            ),
...
async def get_research_status(request_id: str, user: User = Depends(get_current_user)):
    ...
    return {
        ...
        "status": "completed",
        "message": "Result available in Evidence Vault",
    }
