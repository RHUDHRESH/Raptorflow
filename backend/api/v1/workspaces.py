...
Authentication is intentionally not required for local reconstruction mode.
...
_REQUIRED_STEP_IDS = [step["id"] for step in CANONICAL_ONBOARDING_STEPS if step["required"]]
...
def _answered_steps_count(answers: Dict[str, Any]) -> int:
    return sum(1 for step in CANONICAL_ONBOARDING_STEPS if _has_value(answers.get(step["id"])))
...
def _build_business_context(workspace_row: Dict[str, Any], answers: Dict[str, Any]) -> Dict[str, Any]:
    company_name = answers.get("company_name") or workspace_row.get("name") or "Workspace"
    ...
    competitor_objects = [{"name": name, "type": "direct"} for name in competitor_names[:8]]
    ...
    business_context = {
        ...
        "intelligence": {
            ...
            "positioning": {
                ...
                "differentiators": [differentiator] + proof_points[:3] if differentiator else proof_points[:4],
            ...
            "messaging": {
                ...
                "guardrails": constraints + [f"Avoid phrase: {item}" for item in banned[:3]],
            },
...
def _build_onboarding_status(
    ...
    raw_answers = onboarding.get("answers") if isinstance(onboarding.get("answers"), dict) else {}
    ...
    bcm_from_settings = bool(settings.get("bcm_ready")) or bool((settings.get("bcm") or {}).get("ready"))
    ...
    return OnboardingStatusResponse(
        ...
        schema_version=str(onboarding.get("schema_version") or ONBOARDING_SCHEMA_VERSION),
        completed=completed,
        bcm_ready=bcm_ready,
...
def _update_workspace_settings(workspace_id: str, settings: Dict[str, Any]) -> Dict[str, Any]:
    supabase = get_supabase_client()
    result = (
        supabase.table("workspaces").update({"settings": settings}).eq("id", workspace_id).execute()
    )
    if not result.data:
...
def _build_default_settings(existing: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    base = existing.copy() if isinstance(existing, dict) else {}
    onboarding = base.get("onboarding") if isinstance(base.get("onboarding"), dict) else {}
...
def _upsert_foundation_from_business_context(workspace_id: str, business_context: Dict[str, Any]) -> None:
    try:
        supabase = get_supabase_client()
        ...
        logger.warning("Failed to upsert foundation during onboarding completion: %s", exc)


async def _seed_workspace_foundation(workspace_id: str, template_type: TemplateType) -> None:
    ...
    try:
        ...
        if result.data:
            logger.info("Seeded foundation for workspace %s from %s template", workspace_id, template_type)
        else:
...
@router.get("/{workspace_id}/onboarding/status", response_model=OnboardingStatusResponse)
...
@router.post("/{workspace_id}/onboarding/complete", response_model=OnboardingCompleteResponse)
