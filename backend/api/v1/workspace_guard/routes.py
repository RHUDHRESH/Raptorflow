...
def _is_bcm_ready_from_settings(settings: Dict[str, Any]) -> bool:
    return bool(settings.get("bcm_ready")) or bool(
...
def _mark_bcm_ready_in_workspace(workspace_row: Dict[str, Any]) -> None:
    ...
    supabase = get_supabase_client()
    supabase.table("workspaces").update({"settings": settings}).eq(
        "id", workspace_row["id"]
    ).execute()


def enforce_bcm_ready(workspace_id: str) -> None:
    import backend.api.v1.workspace_guard as wg_module

    cfg = wg_module.get_settings()
    if not cfg.ENFORCE_BCM_READY_GATE:
        return

    workspace_row = wg_module.get_workspace_row(workspace_id)
    settings = _workspace_settings(workspace_row)
    if _is_bcm_ready_from_settings(settings):
        return

    latest_bcm = wg_module.bcm_service.get_latest(workspace_id)
    if latest_bcm:
