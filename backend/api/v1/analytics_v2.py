...
async def get_moves_performance(auth: AuthContext = Depends(get_auth_context)):
    ...
    try:
        ...
        # Aggregate stats
        res = await supabase.table("moves") \
        ...
        stats = {
            ...
            "avg_progress": sum([m["progress"] for m in moves]) / len(moves) if moves else 0,
            "by_category": {}
        }
...
async def get_muse_performance(auth: AuthContext = Depends(get_auth_context)):
    ...
    try:
        ...
        res = await supabase.table("muse_assets") \
        ...
        stats = {
            ...
            "avg_quality": sum([a.get("quality_score", 0) or 0 for a in assets]) / len(assets) if assets else 0,
            "by_type": {}
        }
