...
async def check_external_services_health() -> List[HealthCheck]:
    ...
    try:
        start_time = datetime.now()
        client = get_resilient_client()
        response = await client.get("serper", "https://google.serper.dev/search", params={"q": "test"})
        response_time = (datetime.now() - start_time).total_seconds() * 1000
    ...
    try:
        ...
        return HealthCheck(
            ...
            details={
                ...
                "embedding_dimension": len(embedding[0]) if len(embedding) > 0 else None,
            },
            response_time_ms=response_time
        )
...
async def check_monitoring_health() -> HealthCheck:
    ...
    try:
        ...
        status = HealthStatus.HEALTHY if sentry_status["status"] == "healthy" else HealthStatus.DEGRADED
