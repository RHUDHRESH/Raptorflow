...
async def get_rate_limit_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve rate limiting status"
        )
...
async def reset_client_rate_limit(client_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to reset client rate limit")


@router.post("/block/{client_id}")
async def block_client_temporarily(client_id: str, duration_minutes: int = 60):
...
async def test_rate_limit(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Rate limit test failed")
