...
def _service() -> AssetService:
    ...
    return AssetService(
        ...
        storage_manager=StorageManager(supabase_url=supabase_url, service_key=service_key),
...
async def delete_asset(
    ...
    service = _service()
    try:
        deleted = await service.delete_asset(workspace_id=workspace_id, asset_id=asset_id)
        if not deleted:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
    except Exception as exc:
        if _is_assets_table_missing(exc):
