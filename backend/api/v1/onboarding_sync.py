...
async def execute_step(
    ...
    try:
        ...
        # Execute step
        result = await workflow.execute_step(workspace_id, request.step_id, request.data)
...
async def get_next_step(
    ...
    try:
        ...
        next_step = await workflow._determine_next_step(workspace_id, state.current_step or "")
...
async def sync_check(
    ...
    try:
            ...
            if step_state.status == StepStatus.COMPLETED:
                ...
                for dep_step in dependencies:
                    ...
                    if not dep_state or dep_state.status != StepStatus.COMPLETED:
                        issues.append(f"Step {step_id} completed but dependency {dep_step} not completed")
        ...
        return {
            ...
            "is_locked": state.lock_expires_at and state.lock_expires_at > datetime.now().timestamp()
        }
