...
from backend.core.validation import get_validator, ValidationError
from backend.core.security import get_security_validator, SecurityLevel
from backend.core.rate_limiter import get_rate_limiter, RateLimitResult
from backend.core.metrics import start_request_tracking, end_request_tracking, RequestStatus
...
class RateLimiter:
    ...
    def _check_local(
        ...
        cache["requests"] = [
            req_time for req_time in cache["requests"]
...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def _end_metrics_tracking(
        ...
        try:
            ...
            end_request_tracking(
                ...
                status=status,
                error_code=str(response.status_code) if status != RequestStatus.SUCCESS else None,
                client_ip=self._get_client_ip(request),
                user_agent=request.headers.get("user-agent"),
            )
        except Exception as e:
    ...
    def _create_security_response(self, error_message: str, request_id: str) -> JSONResponse:
    ...
    def _create_validation_response(self, error_message: str, request_id: str) -> JSONResponse:
