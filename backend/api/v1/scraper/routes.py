...
def _lookup_redis_client():
    ...
    This allows tests to patch scraper_api.get_redis_client and have it
    take effect in the routes module.
    """
    import sys

    scraper_module = sys.modules.get("backend.api.v1.scraper")
    if scraper_module and hasattr(scraper_module, "get_redis_client"):
        patched = scraper_module.get_redis_client  # Don't call yet
...
class UnifiedScraper:
    ...
    def _build_cache_key(
        self, url: str, strategy: ScrapingStrategy, legal_basis: str
    ) -> str:
        payload = {
            "url": url.strip(),
    ...
    async def scrape(
        ...
        try:
            ...
            logger.info(
                "Starting scrape", url=url, user_id=user_id, strategy=strategy.value
            )
            ...
            if strategy in [
                ScrapingStrategy.TURBO,
                ScrapingStrategy.OPTIMIZED,
                ScrapingStrategy.PARALLEL,
                ScrapingStrategy.ASYNC,
            ]:
    ...
    async def _scrape_ultra_fast(
        self, url: str, user_id: str, strategy: ScrapingStrategy
    ) -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                screenshot = (
                    ...
                    in [ScrapingStrategy.TURBO, ScrapingStrategy.OPTIMIZED]
                    else None
                )
                ...
                soup = BeautifulSoup(
                    content, "html.parser", parse_only=self.content_parser
                )
    ...
    async def _scrape_conservative(
    ...
    async def _scrape_aggressive(
        self, url: str, user_id: str, legal_basis: str
    ) -> Dict[str, Any]:
        """Aggressive scraping - fast, parallel requests"""
        # Parallel HTTP requests
        tasks = [self._fetch_with_session(url), self._fetch_metadata(url)]
        ...
        return {**content_result, **metadata_result, "parallel": True}

    async def _scrape_balanced(
        self, url: str, user_id: str, legal_basis: str
    ) -> Dict[str, Any]:
    ...
    def get_analytics(self, days: int = 7) -> Dict[str, Any]:
        ...
        recent_metrics = [
            ...
            if (
                datetime.now(timezone.utc) - datetime.fromisoformat(m["timestamp"])
            ).days
            <= days
        ]
...
async def scrape_endpoint(request: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    strategy_str = (
        ...
        or scraper_profile.get("default_strategy")
        or "optimized"
    )
