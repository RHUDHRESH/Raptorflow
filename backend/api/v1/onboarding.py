...
async def classify_evidence(session_id: str, evidence_data: Dict[str, Any]):
    ...
    try:
        if not evidence_classifier:
            raise HTTPException(status_code=503, detail="Evidence classifier not available")
...
async def extract_facts(session_id: str, evidence_list: List[Dict[str, Any]]):
    ...
    try:
        if not extraction_orchestrator:
            raise HTTPException(status_code=503, detail="Extraction orchestrator not available")
        
        result = await extraction_orchestrator.extract_facts_from_evidence(evidence_list)
...
async def detect_contradictions(session_id: str, facts: List[Dict[str, Any]]):
    ...
    try:
        if not contradiction_detector:
            raise HTTPException(status_code=503, detail="Contradiction detector not available")
...
async def reddit_research(session_id: str, company_info: Dict[str, Any]):
    ...
    try:
        if not reddit_researcher:
            raise HTTPException(status_code=503, detail="Reddit researcher not available")
...
async def generate_perceptual_map(session_id: str, company_info: Dict[str, Any], competitors: List[Dict[str, Any]]):
    """Generate AI-powered perceptual map"""
    try:
        if not perceptual_map_generator:
            raise HTTPException(status_code=503, detail="Perceptual map generator not available")
        
        result = await perceptual_map_generator.generate_perceptual_map(company_info, competitors)
...
async def generate_neuroscience_copy(session_id: str, product_info: Dict[str, Any]):
    ...
    try:
        if not neuroscience_copywriter:
            raise HTTPException(status_code=503, detail="Neuroscience copywriter not available")
        
        result = await neuroscience_copywriter.generate_copywriting_campaign(product_info)
...
async def generate_channel_strategy(session_id: str, company_info: Dict[str, Any], competitors: List[Dict[str, Any]] = None):
    """Generate AI-powered channel strategy"""
    try:
        if not channel_recommender:
            raise HTTPException(status_code=503, detail="Channel recommender not available")
...
async def analyze_category_paths(session_id: str, company_info: Dict[str, Any], competitors: List[Dict[str, Any]] = None):
    """Generate Safe/Clever/Bold category path recommendations"""
    try:
        if not category_advisor:
            raise HTTPException(status_code=503, detail="Category advisor not available")
        
        result = await category_advisor.analyze_category_paths(company_info, competitors)
        
        # Store category paths in database
        await onboarding_repo.store_category_paths(session_id, {
...
async def calculate_market_size(session_id: str, company_info: Dict[str, Any]):
    ...
    try:
        if not market_size_calculator:
            raise HTTPException(status_code=503, detail="Market size calculator not available")
        ...
        await onboarding_repo.store_market_size(session_id, {
...
async def analyze_competitors(session_id: str, company_info: Dict[str, Any], discovered_competitors: List[Dict[str, Any]] = None):
    """Perform comprehensive competitor analysis"""
    try:
        if not competitor_analyzer:
            raise HTTPException(status_code=503, detail="Competitor analyzer not available")
        
        result = await competitor_analyzer.analyze_competitors(company_info, discovered_competitors)
        
        # Store competitor analysis
        await onboarding_repo.store_competitor_analysis(session_id, {
        ...
        return {
            ...
            "competitor_analysis": {
                ...
                "competitors": [{"name": c.name, "type": c.competitor_type.value, "threat": c.threat_level.value} for c in result.competitors],
                "advantages": [{"description": a.description, "category": a.category} for a in result.competitive_advantages],
...
async def analyze_focus_sacrifice(session_id: str, company_info: Dict[str, Any], icp_data: Dict[str, Any] = None, capabilities: List[Dict[str, Any]] = None, positioning: Dict[str, Any] = None):
    """Analyze strategic focus and sacrifice tradeoffs"""
    try:
        if not focus_sacrifice_engine:
            raise HTTPException(status_code=503, detail="Focus/sacrifice engine not available")
        
        result = await focus_sacrifice_engine.analyze_focus_sacrifice(company_info, icp_data, capabilities, positioning)
        
        # Store focus/sacrifice analysis
        await onboarding_repo.store_focus_sacrifice(session_id, {
        ...
        return {
            "success": True,
            "focus_sacrifice": {
                "focus_items": [{"description": f.description, "category": f.category.value, "impact": f.impact_score} for f in result.focus_items],
                "sacrifice_items": [{"description": s.description, "impact": s.impact.value, "alternative_message": s.alternative_message} for s in result.sacrifice_items],
...
async def generate_truth_sheet(session_id: str, evidence_list: List[Dict[str, Any]], existing_entries: List[Dict[str, Any]] = None):
    """Generate truth sheet from evidence"""
    try:
        if not truth_sheet_generator:
            raise HTTPException(status_code=503, detail="Truth sheet generator not available")
        
        result = await truth_sheet_generator.generate_truth_sheet(evidence_list, existing_entries)
        
        # Store truth sheet
        await onboarding_repo.store_truth_sheet(session_id, {
        ...
        return {
            "success": True,
            "truth_sheet": {
                "entries": [{"id": e.id, "category": e.category.value, "field_name": e.field_name, "value": e.value, "confidence": e.confidence.value, "verified": e.verified} for e in result.entries],
...
async def validate_proof_points(session_id: str, claims: List[str], evidence: List[Dict[str, Any]] = None):
    """Validate claims with proof points"""
    try:
        if not proof_point_validator:
            raise HTTPException(status_code=503, detail="Proof point validator not available")
        
        result = await proof_point_validator.validate_claims(claims, evidence)
        
        # Store validation results
        await onboarding_repo.store_proof_points(session_id, {
        ...
        return {
            "success": True,
            "validation": {
                "validations": [{"claim_id": v.claim_id, "claim_text": v.claim_text, "status": v.status.value, "strength": v.strength.value, "confidence_score": v.confidence_score, "recommendations": v.recommendations} for v in result.validations],
                "overall_score": result.overall_score,
                "strong_claims": result.strong_claims,
                "weak_claims": result.weak_claims,
...
async def generate_messaging_rules(session_id: str, company_info: Dict[str, Any], positioning: Dict[str, Any] = None):
    """Generate messaging rules and guardrails"""
    try:
        if not messaging_rules_engine:
            raise HTTPException(status_code=503, detail="Messaging rules engine not available")
        
        result = await messaging_rules_engine.generate_messaging_rules(company_info, positioning)
        
        await onboarding_repo.store_messaging_rules(session_id, {
            "rules": [{"id": r.id, "category": r.category.value, "name": r.name, "severity": r.severity.value} for r in result.rules],
        ...
        return {
            "success": True,
            "messaging_rules": {
                "rules": [{"id": r.id, "category": r.category.value, "name": r.name, "description": r.description, "severity": r.severity.value, "status": r.status.value} for r in result.rules],
                "rule_count": result.rule_count,
                "categories_covered": result.categories_covered,
                "recommendations": result.recommendations,
                "summary": result.summary
...
async def generate_soundbites(session_id: str, company_info: Dict[str, Any], positioning: Dict[str, Any] = None, icp_data: Dict[str, Any] = None):
    """Generate soundbites library"""
    try:
        if not soundbites_generator:
            raise HTTPException(status_code=503, detail="Soundbites generator not available")
        
        result = await soundbites_generator.generate_soundbites(company_info, positioning, icp_data)
        
        await onboarding_repo.store_soundbites(session_id, {
            "soundbites": [{"id": s.id, "type": s.type.value, "content": s.content, "score": s.score} for s in result.soundbites],
        ...
        return {
            "success": True,
            "soundbites": {
                "library": [{"id": s.id, "type": s.type.value, "content": s.content, "audience": s.audience.value, "tone": s.tone.value, "score": s.score, "use_cases": s.use_cases} for s in result.soundbites],
...
async def generate_icp_deep(session_id: str, company_info: Dict[str, Any], positioning: Dict[str, Any] = None, count: int = 3):
    """Generate comprehensive ICP profiles"""
    try:
        if not icp_deep_generator:
            raise HTTPException(status_code=503, detail="ICP deep generator not available")
        
        result = await icp_deep_generator.generate_icp_profiles(company_info, positioning, count)
        
        await onboarding_repo.store_icp_deep(session_id, {
            "profiles": [icp_deep_generator.profile_to_dict(p) for p in result.profiles],
...
async def generate_positioning(session_id: str, company_info: Dict[str, Any], positioning: Dict[str, Any] = None, icp_data: Dict[str, Any] = None):
    """Generate positioning statements"""
    try:
        if not positioning_generator:
            raise HTTPException(status_code=503, detail="Positioning generator not available")
        
        result = await positioning_generator.generate_positioning(company_info, positioning, icp_data)
        
        await onboarding_repo.store_positioning(session_id, {
            "statements": [{"id": s.id, "type": s.type.value, "framework": s.framework.value, "statement": s.statement, "score": s.score} for s in result.statements],
        ...
        return {
            ...
            "positioning": {
                ...
                "only_we_claims": result.only_we_claims,
                "matrix": {"axes": result.matrix.axes, "your_position": result.matrix.your_position, "white_space": result.matrix.white_space} if result.matrix else None,
                "recommendations": result.recommendations,
                "summary": result.summary
            }
        }
        ...
        if step_id == 2:
            ...
            if vertex_ai_client:
                try:
                        ...
                        try:
                            ...
                            for i, fact in enumerate(ai_facts[:5]):  # Limit to 5 facts
                                facts.append({
                                    ...
                                    "confidence": min(fact.get("confidence", 85), 99),
                                    "sources": [{"type": "ai", "name": "Vertex AI Analysis"}],
                                    "status": "pending",
                                    "code": f"F-{i+1:03d}"
                                })
                            ...
                            facts = [
                                {"id": "f1", "category": "Company", "label": "AI Analysis", "value": ai_response[:200], "confidence": 75, "sources": [{"type": "ai", "name": "Vertex AI"}], "status": "pending", "code": "F-001"}
                            ]
                    else:
                        # Fallback if AI fails
                        facts = [
                            {"id": "f1", "category": "Company", "label": "Company Name", "value": foundation_data.get('company_name', 'Unknown'), "confidence": 90, "sources": [{"type": "ai", "name": "Vertex AI"}], "status": "pending", "code": "F-001"}
                        ]
                    ...
                    facts = [
                        {"id": "f1", "category": "Company", "label": "Company Name", "value": foundation_data.get('company_name', 'Unknown'), "confidence": 90, "sources": [{"type": "user_input", "name": "Foundation Data"}], "status": "pending", "code": "F-001"}
                    ]
            else:
...
async def upload_file(
    ...
    try:
        ...
        logger.info(f"Enhanced file upload completed: {file.filename} ({file_size} bytes)")
...
async def process_url(session_id: str, request: URLProcessRequest):
    ...
    try:
        ...
        return {
            ...
            "search_confidence": scrape_result.get("search_confidence", 0.0),
            "db_id": saved_item.get("id") if saved_item else None,
            "error": scrape_result.get("error") if scrape_result["status"] == "error" else None,
        }
