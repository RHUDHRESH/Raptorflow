...
async def process_ocr(file_path: str, file_content: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "success",
            "extracted_text": (
                f"[OCR Placeholder] File processed: {file_path} "
                f"({file_size} bytes, type: {file_type})\n[DB Persisted]"
            ),
            "page_count": 1,
            "confidence": 0.5,
        }
...
async def upload_file(
    ...
    try:
        ...
        public_url = storage_service.upload_file(
            file.file, gcs_path, content_type=file.content_type
        )
        ...
        if not public_url:
            logger.warning(
                f"GCS upload failed for {file.filename}, falling back to non-persistent processing"
            )
