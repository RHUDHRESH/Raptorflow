...
async def get_current_user_id(
    ...
    try:
        if credentials and credentials.credentials:
            ...
            return credentials.credentials.split(":")[0] if ":" in credentials.credentials else credentials.credentials
...
async def create_session(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid session type: {session_type}"
            )
...
async def get_workspace_sessions(workspace_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve workspace sessions"
        )
...
async def get_user_analytics(
    ...
    try:
        ...
        if not user_pattern:
            raise HTTPException(
                status_code=404,
                detail="No user analytics data available"
            )
...
async def get_performance_analytics(
    ...
    try:
        analytics_manager = await get_analytics_manager()
        performance_data = await analytics_manager.performance_analytics.analyze_performance(
...
async def get_resource_trends(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve resource trends"
        )
