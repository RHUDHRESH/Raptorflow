...
from memory.swarm_coordinator import _coordinator_registry
                ...
                for workspace_id, coordinator in _coordinator_registry.items():
                    if coordinator.active_agents:  # Only consolidate active workspaces
                        try:
                            ...
                            logger.debug(
                                f"Consolidated memory for workspace {workspace_id}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Consolidation failed for workspace {workspace_id}: {e}"
                            )

                # Log summary
                successful = sum(
                    1 for r in consolidation_results if "error" not in r["result"]
                )
                logger.info(
                    f"Periodic consolidation completed: {successful}/{len(consolidation_results)} workspaces"
                )

                # Wait before next consolidation
                await asyncio.sleep(
                    self.config["consolidation_interval"].total_seconds()
                )
                    ...
                    try:
                        ...
                        for agent_id, usage in coordinator.agent_memory_usage.items():
                            ...
                            if last_activity:
                                ...
                                if datetime.now() - last_activity_dt > timedelta(
                                    hours=24
                                ):
                        ...
                        logger.error(
                            f"Cleanup failed for workspace {workspace_id}: {e}"
                        )
            ...
            try:
                ...
                for workspace_id, coordinator in _coordinator_registry.items():
                    try:
                        ...
                        cache_hit_rate = stats.get("cache_performance", {}).get(
                            "hit_rate", 0
                        )
                        if cache_hit_rate < 0.7:
                            workspace_health["issues"].append("Low cache hit rate")
                            health_summary["issues_found"].append(
                                f"{workspace_id}: Low cache hit rate"
                            )
                        ...
                        if stats.get("last_consolidation"):
                            ...
                            if datetime.now() - last_consolidation > timedelta(
                                minutes=10
                            ):
                                workspace_health["issues"].append("Consolidation lag")
                                health_summary["issues_found"].append(
                                    f"{workspace_id}: Consolidation lag"
                                )
                        ...
                        if total_fragments > 5000:
                            ...
                            health_summary["issues_found"].append(
                                f"{workspace_id}: High memory usage"
                            )
                        ...
                        logger.debug(
                            f"Health check for {workspace_id}: {workspace_health['status']}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Health check failed for workspace {workspace_id}: {e}"
                        )
                        health_summary["unhealthy_workspaces"] += 1
                        health_summary["issues_found"].append(
                            f"{workspace_id}: Health check failed"
                        )
                ...
                await asyncio.sleep(
                    self.config["health_check_interval"].total_seconds()
                )
            ...
            try:
                ...
                for workspace_id, coordinator in _coordinator_registry.items():
                    try:
                        ...
                        if cache_size > 1000:
                            logger.info(
                                f"Large cache detected for workspace {workspace_id}: {cache_size} entries"
                            )
                        ...
                        logger.error(
                            f"Cache cleanup failed for workspace {workspace_id}: {e}"
                        )
                        cleanup_stats["errors"] += 1
                ...
                await asyncio.sleep(
                    self.config["cache_cleanup_interval"].total_seconds()
                )
...
async def trigger_consolidation_for_workspace(
    workspace_id: str, force: bool = False
) -> Dict[str, Any]:
...
async def cleanup_workspace_memory(workspace_id: str) -> Dict[str, Any]:
    ...
    cleanup_results = {"agents_cleaned": 0, "errors": []}
