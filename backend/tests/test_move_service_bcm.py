...
async def test_complete_move_records_event(MockRecorder, MockRepo):
    ...
    mock_repo.get_by_id = AsyncMock(return_value={"id": "move-1", "status": "active", "title": "Test Move"})
    mock_repo.complete_move = AsyncMock(return_value={"id": "move-1", "status": "completed"})
