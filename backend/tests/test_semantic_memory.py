...
async def test_semantic_memory_search():
    ...
    with patch("backend.memory.semantic.vector_search", new_callable=AsyncMock) as mock_search, \
         patch("backend.inference.InferenceProvider.get_embeddings", return_value=mock_embedder):
...
async def test_semantic_memory_remember():
    with patch("backend.memory.semantic.save_memory", new_callable=AsyncMock) as mock_save:
        memory = SemanticMemory()
