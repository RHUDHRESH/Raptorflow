...
async def test_procedural_memory_store_retrieve():
    ...
    """
    mock_results = [("id1", "To find competitor pricing, use Firecrawl on their pricing page.", {"intent": "competitor_pricing"}, 0.9)]
    ...
    with patch("backend.memory.procedural.vector_search", new_callable=AsyncMock) as mock_vector_search, \
        ...
        results = await memory.get_procedure(tenant_id="test-tenant", intent="competitor research")
