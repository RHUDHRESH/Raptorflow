...
import pytest
from mocks import MockContext, create_test_fragment, create_test_state, run_with_mocks
...
class TestMemorySystemDeep:
    ...
    class TestMemoryFragmentValidation:
        ...
        def test_fragment_serialization(self):
            ...
            original = MemoryFragment(
                agent_id="test_agent", content="Test content", importance_score=0.8
            )
    ...
    class TestConsolidatorEdgeCases:
        ...
        async def test_consolidation_without_embedder(self, consolidator):
            ...
            result = await consolidator.consolidate_agent_memories(
        ...
        async def test_fragment_deduplication(self, consolidator):
            ...
            consolidator._gather_memory_fragments = lambda agent_ids: [
                fragment1,
                fragment2,
                fragment3,
            ]
        ...
        async def test_memory_recording_validation(self, coordinator):
            ...
            with pytest.raises(ValueError):
                await coordinator.record_agent_memory(
                    agent_id="test_agent", content="test", importance=1.5
                )

            # Test with non-existent agent
            with pytest.raises(ValueError):
                await coordinator.record_agent_memory(
                    agent_id="non_existent", content="test", importance=0.5
                )
    ...
    class TestCacheEdgeCases:
        ...
        async def test_cache_concurrent_access(self, cache):
            ...
            async def worker(worker_id):
                for i in range(10):
                    await cache.set(
                        f"worker_{worker_id}_key_{i}", f"worker_{worker_id}_value_{i}"
                    )
        ...
        async def test_error_recovery_scenarios(self, workspace_id):
            ...
            with MockContext():
                ...
                success = await coordinator.record_agent_memory(
                    "test_agent", "Test memory", importance=0.5
                )
                assert success is True
        ...
        async def test_performance_under_load(self, workspace_id):
            ...
            with MockContext():
                ...
                for i in range(num_agents):
                    await coordinator.initialize_agent_memory(
                        f"agent_{i}", "researcher"
                    )
                ...
                async def record_memory(agent_id):
                    for j in range(10):
                        await coordinator.record_agent_memory(
                            agent_id, f"Memory {j} for {agent_id}", importance=0.5
                        )
        ...
        async def test_state_hydration_with_workspace(self):
            ...
            with MockContext():
                ...
                state = create_test_state(
                    workspace_id="test_workspace", last_agent="test_agent"
                )
