...
async def test_refine_context_loop(MockRecorder, MockAgent, MockProjector):
    ...
    mock_db.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute = mock_execute
    
    # Mock Projector
    mock_projector = MockProjector.return_value
    mock_projector.get_latest_state = AsyncMock(return_value=Mock(model_dump_json=lambda: "{}"))
