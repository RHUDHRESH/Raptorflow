...
from backend.agents.supervisor import create_team_supervisor, RouterOutput, HierarchicalSupervisor
...
from backend.agents.planner import create_task_decomposer, Plan
from backend.models.fortress import FortressTask

from backend.agents.classifier import create_intent_classifier, Intent, create_ambiguity_resolver
...
async def test_ambiguity_resolution_logic():
    ...
    mock_llm = AsyncMock()
    mock_llm.ainvoke.return_value = MagicMock(content="Question 1? Question 2? Question 3?")
...
async def test_intent_classification_logic():
    ...
    mock_chain.ainvoke.return_value = Intent(classification="campaign", confidence=0.99, reasoning="SOTA check")
    
    with patch("backend.agents.classifier.IntentClassifier.__init__", return_value=None):
...
async def test_task_decomposition_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = Plan(tasks=[
...
async def test_hierarchical_supervisor_delegation():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = RouterOutput(next_node="research", instructions="Find competitors")
    
    # We mock the initialization of HierarchicalSupervisor to inject our mock_chain
    with patch("backend.agents.supervisor.HierarchicalSupervisor.__init__", return_value=None) as mock_init:
...
async def test_hitl_interrupt_logic():
    ...
    workflow.add_node("approval_node", lambda x: {"results": x.get("results", []) + ["Approved"]})
    ...
    checkpointer = MemorySaver()
    app = workflow.compile(checkpointer=checkpointer, interrupt_before=["approval_node"])
...
async def test_dynamic_replanning_signal():
    ...
    workflow.add_conditional_edges("planner", router, {"step_a": "step_a", "step_b": "step_b", END: END})
    ...
    state = await app.ainvoke({"signals": ["need_research"], "next_steps": [], "results": []})
