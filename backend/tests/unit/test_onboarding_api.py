...
with patch('backend.llm.LLMCache'), \
     patch('backend.startup.initialize_app'), \
     patch('backend.redis_services_activation.activate_redis_services'):
...
async def test_process_onboarding_step_api():
    ...
    payload = {
        ...
        "data": {
            ...
            "step_name": "Initial Greeting"
        }
    }
    
    mock_llm_response = LLMResponse(
        content="Hello Test Corp! Ready for Initial Greeting.",
        model="gemini-1.5-pro"
    )
    
    with patch('backend.llm.llm_manager.generate', return_value=mock_llm_response), \
         patch('backend.llm.LLMCache'):
        
        response = client.post(
            f"/api/v1/onboarding-universal/{session_id}/steps/{step_id}",
            json=payload
        )
