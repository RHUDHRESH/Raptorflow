...
async def test_derive_trinity_success():
    ...
    with patch(
        "backend.core.supabase.get_supabase_client", return_value=MagicMock()
    ), patch("backend.db.icps.ICPRepository.__init__", return_value=None), patch(
        "backend.db.foundations.FoundationRepository.__init__", return_value=None
    ), patch(
        "backend.services.business_context_graph.get_business_context_graph",
        ...
        icp_service.foundation_repository.get_by_workspace = AsyncMock(
            return_value={"company_name": "Test Co", "industry": "Software"}
        )
        ...
        mock_state = {"context_data": BusinessContextState(ricps=[mock_ricp])}
