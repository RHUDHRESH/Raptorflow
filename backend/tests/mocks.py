...
class MockSwarmLearning:
    ...
    async def retrieve_swarm_knowledge(
        ...
        return {
            "L1": [{"content": f"Mock L1 knowledge {i}", "metadata": {}} for i in range(3)],
            "L2": [{"content": f"Mock L2 knowledge {i}", "metadata": {}} for i in range(2)],
            "L3": [{"content": f"Mock L3 knowledge {i}", "metadata": {}} for i in range(1)]
        }
...
def setup_mocks():
    ...
    sys.modules['backend.inference'] = type('MockModule', (), {
        'InferenceProvider': MockInferenceProvider
    })()
    
    sys.modules['backend.memory.manager'] = type('MockModule', (), {
        'MemoryManager': MockMemoryManager
    })()
    
    sys.modules['backend.memory.swarm_learning'] = type('MockModule', (), {
        'SwarmLearning': MockSwarmLearning
    })()
    
    sys.modules['backend.models.capabilities'] = type('MockModule', (), {
        'CapabilityProfile': MockCapabilityProfile
    })()
    
    sys.modules['backend.core.config'] = type('MockModule', (), {
        'get_settings': lambda: MockSettings()
    })()
