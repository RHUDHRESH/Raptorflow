...
class RegressionTestFramework:
        ...
        for path in common_paths:
            ...
            for method in common_methods:
                ...
                test = await self._run_single_api_test(
                    ...
                    method.upper(),
                    path,
                    self.baseline_results.get(f"{method.lower()}_{path.replace('/', '_')}"),
                    self.current_results.get(f"{method.lower()}_{path.replace('/', '_')}")
                )
    ...
    async def _run_single_api_test(
        ...
        try:
                    ...
                    for change_type, changes in data_diff.items():
                        for change in changes:
                            if isinstance(change, dict):
                                ...
                                differences.append(ChangeDetection(
                                    change_type=change_type,
                                    path=path,
                                    old_value=change.get('old_value'),
                                    new_value=change.get('new_value'),
                                    impact=self._assess_change_impact(change_type, path),
            ...
            passed = len(differences) == 0 and (
                performance_impact is None or 
                abs(performance_impact) <= self.config.performance_threshold
            )
    ...
    async def run_performance_regression_tests(self) -> List[RegressionTest]:
        ...
        for method, endpoint in performance_endpoints:
            ...
            for _ in range(5):
                ...
                try:
                    url = f"{self.config.base_url}{endpoint}"
                    data = {"email": "test@example.com", "password": "test"} if method == "POST" else None
            ...
            if valid_times:
                ...
                baseline_avg = self.baseline_results.get(f"perf_{method}_{endpoint}", {}).get('avg_time', avg_time)
                
                performance_change = ((avg_time - baseline_avg) / baseline_avg) * 100 if baseline_avg > 0 else 0
        ...
        for path in set(baseline_paths.keys()) | set(current_paths.keys()):
            ...
            for method in set(baseline_methods.keys()) | set(current_methods.keys()):
                ...
                if baseline_details and current_details:
                    ...
                    for status_code in set(baseline_responses.keys()) | set(current_responses.keys()):
                        baseline_schema = baseline_responses.get(status_code, {}).get('content', {}).get('application/json', {}).get('schema', {})
                        current_schema = current_responses.get(status_code, {}).get('content', {}).get('application/json', {}).get('schema', {})
                        
                        if baseline_schema and current_schema:
                            schema_diff = DeepDiff(baseline_schema, current_schema, ignore_order=True)
                            if schema_diff:
                                for change_type, changes in schema_diff.items():
                                    for change in changes:
                                        differences.append(ChangeDetection(
                                            ...
                                            old_value=change.get('old_value') if isinstance(change, dict) else change,
                                            new_value=change.get('new_value') if isinstance(change, dict) else change,
                                            impact=self._assess_change_impact(change_type, f"{path}.{method}.{status_code}"),
    ...
    async def run_all_regression_tests(self) -> RegressionReport:
        ...
        performance_regressions = sum(1 for test in all_tests if test.performance_impact and abs(test.performance_impact) > self.config.performance_threshold)
        breaking_changes = sum(1 for test in all_tests if any(diff.breaking for diff in test.differences))
        ...
        report = RegressionReport(
            ...
            summary={
                ...
                "breaking_spec_changes": sum(1 for change in spec_changes if change.breaking),
                "success_rate": (passed_tests / total_tests * 100) if total_tests > 0 else 0,
                "high_impact_changes": sum(1 for test in all_tests for diff in test.differences if diff.impact == ChangeImpact.HIGH),
                "critical_impact_changes": sum(1 for test in all_tests for diff in test.differences if diff.impact == ChangeImpact.CRITICAL)
            }
        )
    ...
    def _generate_recommendations(self, report: RegressionReport) -> List[str]:
        ...
        if report.failed_tests > 0:
            recommendations.append(f"Address {report.failed_tests} failing regression tests before deployment")
        
        if report.performance_regressions > 0:
            recommendations.append(f"Investigate {report.performance_regressions} performance regressions")
        
        if report.breaking_changes > 0:
            recommendations.append(f"Review {report.breaking_changes} breaking changes and update documentation")
        
        if report.summary.get("critical_impact_changes", 0) > 0:
            recommendations.append("Critical changes detected - consider version bump and migration guide")
        
        success_rate = (report.passed_tests / report.total_tests * 100) if report.total_tests > 0 else 0
        if success_rate < 95:
            recommendations.append(f"Success rate ({success_rate:.1f}%) below threshold - review changes")
    ...
    def save_report(self, report: RegressionReport) -> None:
        ...
        current_results_file = Path(self.config.current_results_dir) / f"baseline_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--baseline-spec", default="docs/openapi_baseline.yaml", help="Baseline OpenAPI spec")
    parser.add_argument("--current-spec", default="docs/openapi_comprehensive.yaml", help="Current OpenAPI spec")
    parser.add_argument("--baseline-results", default="test_results/baseline", help="Baseline results directory")
    parser.add_argument("--current-results", default="test_results/current", help="Current results directory")
    ...
    parser.add_argument("--performance-threshold", type=float, default=10.0, help="Performance regression threshold (%)")
