...
class ProductionTestSuite:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        logger.info(f"Test suite completed: {passed}/{len(self.results)} passed, {failed} failed, {critical} critical")
    ...
    async def _run_security_tests(self) -> None:
        ...
        try:
            ...
            for var in sensitive_vars:
                value = os.getenv(var)
                if value and not value.startswith("${") and not value.startswith("your-"):
                    hardcoded_found.append(var)
            
            if hardcoded_found:
                test.set_failed(f"Found hardcoded environment variables: {hardcoded_found}")
            else:
        ...
        try:
            ...
            if not manager.client:
                ...
                if pool_utilization > 0.8:
                    ...
                    test.set_passed(f"Connection pool utilization acceptable: {pool_utilization:.2%}")
    ...
    async def _run_resilience_tests(self) -> None:
        ...
        try:
            ...
            if status["status"] == "healthy":
                ...
                test.set_failed(f"Error tracking issues: {status.get('error', 'Unknown')}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if health_data["status"] == "healthy":
                    ...
                    test.set_failed(f"Health issues detected: {health_data.get('status')}")
    ...
    async def _run_monitoring_tests(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                required_metrics = ["overall_status", "total_response_time_ms", "check_count"]
        ...
        try:
            ...
            if not manager.client:
                ...
                if health["status"] == "healthy":
                    ...
                    test.set_critical(f"Database connection issues: {health.get('message')}")
    ...
    async def _run_infrastructure_tests(self) -> None:
        ...
        try:
            ...
            if not manager.client:
                ...
                if health["status"] == "healthy":
                    ...
                    test.set_critical(f"Redis connection issues: {health.get('message')}")
        ...
        try:
            ...
            if missing_vars:
                test.set_critical(f"Missing required environment variables: {missing_vars}")
            else:
...
async def main():
    ...
    if command == "run":
        ...
        for category, cat_summary in results["categories"].items():
            status = "PASS" if cat_summary["critical"] == 0 and cat_summary["failed"] == 0 else "FAIL"
            print(f"{category.upper()}: {status} ({cat_summary['passed']}/{cat_summary['total']})")
