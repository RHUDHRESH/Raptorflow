...
from backend.agents.strategists import (
    ...
    TaglineAnalysis,
    UVPAnalysis,
    create_anti_persona_profiler,
    create_brand_voice_aligner,
    create_budget_allocator,
    create_campaign_designer,
    create_category_architect,
    create_channel_selector,
    create_conflict_resolver,
    create_founder_profiler,
    create_funnel_designer,
    create_icp_demographer,
    create_icp_psychographer,
    create_kpi_definer,
    create_move_sequencer,
    create_pain_point_mapper,
    create_strategy_refresh_hook,
    create_strategy_replanner,
    create_tagline_generator,
    create_uvp_architect,
)
...
async def test_founder_profiler_logic():
    ...
    with patch(
        ...
        assert result["context_brief"]["founder_profile"]["personality_traits"] == [
            "Bold"
        ]
...
async def test_conflict_resolution_logic():
    ...
    mock_chain.ainvoke.return_value = ConflictAnalysis(
        conflicts=[
            StrategicConflict(
                description="Theme mismatch",
                severity=0.5,
                resolution_suggestion="Fix theme",
            )
        ],
    ...
    with patch(
        "backend.agents.strategists.StrategicConflictResolver.__init__",
        return_value=None,
    ):
        ...
        state = {
            "context_brief": {"campaign_arc": {}},
            "research_bundle": {"historical_context": []},
        }
...
async def test_funnel_design_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = FunnelAnalysis(
        stages=[
            FunnelStage(
                stage_name="TOFU", content_type="Blog", conversion_goal="Awareness"
            )
        ]
    )
...
async def test_kpi_definition_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = KPIAnalysis(
        kpis=[Metric(name="Conversion", target_value="5%", measurement_method="GA4")]
    )
...
async def test_channel_selection_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = ChannelSelection(
        selected_channels=[
            SelectedChannel(
                channel_name="LinkedIn", priority="primary", relevance_score=0.9
            )
        ]
    )

    with patch(
        "backend.agents.strategists.ChannelSelector.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["channels"]["selected_channels"][0]["channel_name"]
            == "LinkedIn"
        )
...
async def test_budget_allocation_logic():
    ...
    mock_chain.ainvoke.return_value = BudgetAnalysis(
        ...
        distribution=[
            ChannelBudget(
                channel="LinkedIn", allocation_percentage=0.6, reasoning="SOTA B2B"
            )
        ],
    )

    with patch(
        "backend.agents.strategists.BudgetAllocator.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["budget_plan"]["distribution"][0]["channel"]
            == "LinkedIn"
        )
...
async def test_move_sequencing_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = MoveSequence(
        moves=[
            ExecutionMove(
                week_number=1,
                title="Warmup",
                action_items=["Post 1"],
                desired_outcome="Awareness",
            )
        ]
    )
...
async def test_campaign_arc_logic():
    ...
    with patch(
        "backend.agents.strategists.CampaignArcDesigner.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["campaign_arc"]["campaign_title"] == "SOTA Launch"
        )
...
async def test_tagline_generator_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = TaglineAnalysis(
        taglines=[Tagline(text="Marketing. Finally under control.", vibe="Expensive")]
    )

    with patch(
        "backend.agents.strategists.TaglineGenerator.__init__", return_value=None
    ):
        ...
        assert (
            ...
            in result["context_brief"]["taglines"]["taglines"][0]["text"]
        )
...
async def test_category_architect_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = CategoryAnalysis(
        proposals=[
            CategoryProposal(
                category_name="MOS", rationale="SOTA OS", market_potential=0.9
            )
        ]
    )

    with patch(
        "backend.agents.strategists.CategoryArchitect.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["categories"]["proposals"][0]["category_name"]
            == "MOS"
        )
...
async def test_anti_persona_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = AntiPersonaAnalysis(
        anti_personas=[
            AntiPersona(
                persona_name="Hobbyist",
                why_to_avoid="Low budget",
                common_signals=["No LLC"],
            )
        ]
    )

    with patch(
        "backend.agents.strategists.AntiPersonaProfiler.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["anti_personas"]["anti_personas"][0]["persona_name"]
            == "Hobbyist"
        )
...
async def test_brand_voice_alignment_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = BrandVoiceAnalysis(
        alignments=[
            AlignmentResult(
                uvp_title="SOTA", is_aligned=True, score=0.95, feedback="Perfect"
            )
        ]
    )

    with patch(
        "backend.agents.strategists.BrandVoiceAligner.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["brand_alignment"]["alignments"][0]["score"] == 0.95
        )
...
async def test_uvp_architect_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = UVPAnalysis(
        winning_positions=[
    ...
    with patch("backend.agents.strategists.UVPArchitect.__init__", return_value=None):
        ...
        assert (
            result["context_brief"]["uvps"]["winning_positions"][0]["title"]
            == "Fast Move"
        )
...
async def test_pain_point_mapper_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = PainPointAnalysis(
        burning_problems=[
    ...
    with patch(
        "backend.agents.strategists.PainPointMapper.__init__", return_value=None
    ):
        ...
        assert (
            result["context_brief"]["pain_points"]["burning_problems"][0]["severity"]
            == 0.9
        )
...
async def test_icp_psychographic_logic():
    ...
    with patch(
        "backend.agents.strategists.ICPPsychographicProfiler.__init__",
        return_value=None,
    ):
        ...
        assert (
            "Speed" in result["context_brief"]["icp_psychographics"]["core_motivations"]
        )
...
async def test_icp_demographic_logic():
    ...
    with patch(
        "backend.agents.strategists.ICPDemographicProfiler.__init__", return_value=None
    ):
        ...
        assert (
            "India" in result["context_brief"]["icp_demographics"]["geographic_focus"]
        )
