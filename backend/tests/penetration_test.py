...
class PaymentPenetrationTester:
    ...
    def add_vulnerability(self, vuln: VulnerabilityReport):
        ...
        logger.warning(
            f"Vulnerability found: {vuln.vulnerability_type} - {vuln.description}"
        )
    ...
    async def test_idempotency_bypass(self) -> List[VulnerabilityReport]:
        ...
        try:
            ...
            response1 = self.session.post(
                f"{self.base_url}/api/payments/initiate", json=payment_data, timeout=10
            )
    ...
    async def test_webhook_signature_bypass(self) -> List[VulnerabilityReport]:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/webhooks/phonepe",
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Penetration testing completed. Found {total_vulns} vulnerabilities."
        )
        return report

    def generate_report(
        self, report: Dict[str, Any], output_file: str = "penetration_test_report.json"
    ):
        ...
        print("=" * 50)
        print(
            f"Total Vulnerabilities: {report['test_summary']['total_vulnerabilities']}"
        )
...
class AutomatedSecurityScanner:
    ...
    async def scan_and_alert(self, alert_threshold: int = 5):
        ...
        if report["test_summary"]["total_vulnerabilities"] > alert_threshold:
            logger.critical(
                f"SECURITY ALERT: {report['test_summary']['total_vulnerabilities']} vulnerabilities found!"
            )
