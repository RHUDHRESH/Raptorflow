...
class TestMetricsCollector:
    ...
    async def test_define_metric(self, metrics_collector):
        ...
        assert metrics_collector.metric_definitions["test_metric"].category == MetricCategory.PERFORMANCE
    ...
    async def test_metric_aggregation(self, metrics_collector):
        ...
        avg_agg = next((a for a in aggregations if a.aggregation_method == AggregationMethod.AVERAGE), None)
    ...
    async def test_get_metric_values_with_filters(self, metrics_collector):
        ...
        with patch('datetime.datetime') as mock_datetime:
            ...
            metrics_collector.record_metric("filtered_metric", 100, tags={"env": "test"})
...
class TestAlertSystem:
    ...
    async def test_alert_rule_conditions(self):
        ...
        for condition in conditions:
            ...
            assert collector._check_condition(75.0, condition, 50.0) == (condition in ["gt", "gte"])
            assert collector._check_condition(25.0, condition, 50.0) == (condition in ["lt", "lte"])
            assert collector._check_condition(50.0, condition, 50.0) == (condition == "eq")
...
class TestMetricsPerformance:
    ...
    async def test_high_volume_metrics(self):
        ...
        try:
            ...
            total_values = sum(len(values) for values in collector.metric_values.values())
