...
async def test_episodic_memory_vector_search():
    ...
    """
    mock_results = [("id1", "User asked about budget last time.", {"type": "episodic"}, 0.85)]
    mock_embedder = AsyncMock()
    mock_embedder.aembed_query.return_value = [0.1] * 768

    with patch("backend.memory.episodic.get_memory", new_callable=AsyncMock) as mock_get_memory, \
        ...
        results = await memory.search_long_term(tenant_id="test-tenant", query="budget history")
