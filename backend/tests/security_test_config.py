...
class PenetrationTestConfig:
    ...
    def __post_init__(self):
        ...
        if self.exclude_endpoints is None:
            self.exclude_endpoints = ["/health", "/metrics", "/docs"]
...
PRODUCTION_CONFIG = SecurityTestConfig(
    load_test=LoadTestConfig(standard_users=20, stress_users=100, spike_users=200),
    penetration_test=DEFAULT_PENETRATION_CONFIG,
    base_url="https://api.raptorflow.com",
)

STAGING_CONFIG = SecurityTestConfig(
    load_test=LoadTestConfig(standard_users=10, stress_users=50, spike_users=100),
    penetration_test=DEFAULT_PENETRATION_CONFIG,
    base_url="https://staging-api.raptorflow.com",
)

DEVELOPMENT_CONFIG = SecurityTestConfig(
    load_test=LoadTestConfig(standard_users=5, stress_users=20, spike_users=50),
    penetration_test=DEFAULT_PENETRATION_CONFIG,
...
PERFORMANCE_BENCHMARKS = {
    "payment_initiation": {"max_response_time_ms": 1500, "min_throughput_rps": 20.0},
    "payment_status": {"max_response_time_ms": 500, "min_throughput_rps": 50.0},
    "refund_processing": {"max_response_time_ms": 2000, "min_throughput_rps": 10.0},
    "webhook_processing": {"max_response_time_ms": 1000, "min_throughput_rps": 100.0},
}
