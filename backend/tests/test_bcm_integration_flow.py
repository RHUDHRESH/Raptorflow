...
async def test_onboarding_to_ledger_integration(MockRecordEvent, MockSupabase, MockRepo):
    """Test that completing onboarding records a STRATEGIC_SHIFT in the ledger"""
    mock_db = Mock()
    MockSupabase.return_value = mock_db
    
    mock_repo = MockRepo.return_value
    mock_repo.get_by_workspace = AsyncMock(return_value={"id": "session-1", "step_data": {"step_1": {"name": "Test"}}})
...
@patch("backend.services.bcm_integration.bcm_evolution.recorder.record_event")
async def test_icp_derivation_to_ledger_integration(MockRecordEvent, MockGraph, MockSupabase, MockFoundationRepo, MockICPRepo):
    ...
    mock_foundation.get_by_workspace = AsyncMock(return_value={"company_name": "Test Corp"})
    ...
    mock_graph.enhance_icp_node = AsyncMock(return_value={"context_data": Mock(ricps=[mock_ricp])})
