...
from backend.agents.exceptions import ValidationError
...
async def test_loop_detective():
    ...
    with patch('backend.agents.base.get_tool_registry') as mock_get_tool_registry, \
         patch('backend.agents.base.get_skills_registry') as mock_get_skills_registry:
...
async def test_universal_retry():
    ...
    with patch('backend.agents.base.get_tool_registry'), \
         patch('backend.agents.base.get_skills_registry'):
...
async def test_smart_recovery():
    ...
    with patch('backend.agents.base.get_tool_registry') as mock_get_tool_registry, \
        ...
        mock_tool = AsyncMock()
        # Tool FAILS
        mock_tool.arun.return_value = MagicMock(success=False, error="Simulated Database Connection Error")
        mock_registry.get.return_value = mock_tool
        mock_get_tool_registry.return_value = mock_registry
