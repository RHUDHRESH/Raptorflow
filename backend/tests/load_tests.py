...
class LoadTestRunner:
    ...
    async def run_load_tests(self) -> List[LoadTestResult]:
        ...
        logger.info(f"Starting load tests: {self.config.concurrent_users} users, {self.config.requests_per_second} RPS")
        
        for endpoint in self.config.endpoints:
            result = await self._test_endpoint(endpoint)
            self.results.append(result)
            
            logger.info(f"Endpoint {endpoint}: {result.successful_requests}/{result.total_requests} successful, "
    ...
    async def _test_endpoint(self, endpoint: str) -> LoadTestResult:
        ...
        async def make_request():
            ...
            async with semaphore:
                async with AsyncClient(base_url=self.base_url, timeout=self.config.timeout) as client:
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        overall_avg_response_time = sum(r.average_response_time_ms for r in self.results) / len(self.results)
        overall_p95_response_time = sum(r.p95_response_time_ms for r in self.results) / len(self.results)
        ...
        performance_grade = self._calculate_performance_grade(overall_avg_response_time, overall_p95_response_time, overall_error_rate)
    ...
    def _calculate_performance_grade(self, avg_response_time: float, p95_response_time: float, error_rate: float) -> str:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for result in self.results:
            if result.error_rate > 0.05:
                recommendations.append(f"High error rate ({result.error_rate:.1%}) on {result.endpoint} - check error handling")
            
            if result.average_response_time_ms > 500:
                recommendations.append(f"Slow response time ({result.average_response_time_ms:.1f}ms) on {result.endpoint} - consider optimization")
            
            if result.p95_response_time_ms > 1000:
                recommendations.append(f"High P95 response time ({result.p95_response_time_ms:.1f}ms) on {result.endpoint} - investigate outliers")
        
        if not recommendations:
            recommendations.append("All endpoints performing well within acceptable thresholds")
...
async def test_load_performance(load_test_config):
    ...
    for result in results:
        assert result.successful_requests > 0, f"No successful requests for {result.endpoint}"
        assert result.error_rate < 0.1, f"High error rate ({result.error_rate:.1%}) for {result.endpoint}"
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"Average Response Time: {report['overall_performance']['average_response_time_ms']:.1f}ms")
        print(f"P95 Response Time: {report['overall_performance']['p95_response_time_ms']:.1f}ms")
        print(f"Requests Per Second: {report['overall_performance']['requests_per_second']:.1f}")
