...
async def test_matrix_service_get_aggregated_overview():
    ...
    with patch("backend.services.matrix_service.SystemSanityCheck") as mock_sanity, \
        ...
        # Mock Sanity Check
        mock_sanity.return_value.run_suite = AsyncMock(return_value={"status": "healthy"})
        # Mock Latency
        mock_latency.return_value.record_and_check = AsyncMock(return_value=False)
        # Mock Cost
        mock_cost.return_value.get_burn_report = AsyncMock(return_value={"daily_burn": 10.0})
