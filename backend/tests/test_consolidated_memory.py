...
class TestSwarmMemorySystem:
    ...
    @pytest.fixture
    async def cached_coordinator(self, coordinator):
        """Cached coordinator fixture."""
        return create_cached_coordinator(coordinator, cache_size_mb=10)
    ...
    async def test_agent_memory_recording(self, coordinator, sample_fragments):
        ...
        # Initialize agent
        success = await coordinator.initialize_agent_memory("researcher_1", "researcher")
    ...
    async def test_memory_consolidation(self, consolidator, sample_fragments):
        ...
        assert result.last_consolidation > consolidator.consolidated_memory.last_consolidation - timedelta(seconds=1)
    ...
    async def test_cached_coordinator(self, cached_coordinator):
        ...
        # Initialize agent
        await cached_coordinator.coordinator.initialize_agent_memory("test_agent", "researcher")
    ...
    async def test_performance_benchmarks(self, coordinator):
        ...
        logger.info(f"Performance: Record={record_time:.2f}s, Search={search_time:.2f}s")
