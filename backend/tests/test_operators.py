...
from backend.agents.operators import (
    create_notion_operator, create_slack_operator, 
    create_cdn_uploader, create_social_publisher,
    create_post_execution_auditor, AuditResult
)
...
async def test_post_execution_audit_logic():
    ...
    with patch("backend.agents.operators.PostExecutionAuditor.__init__", return_value=None):
...
async def test_cdn_upload_logic():
    ...
    result = await operator(state)
    assert "https://cdn.raptorflow.ai/test-bucket/" in result["current_brief"]["permanent_image_url"]
