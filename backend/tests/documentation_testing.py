...
class DocumentationTester:
    ...
    async def _validate_links(self, content: str, result: DocumentationTestResult) -> None:
    ...
    async def _check_single_link(
        ...
        result: DocumentationTestResult
    ) -> None:
        """Check a single link."""
        async with semaphore:
            ...
            link_result.is_internal = url.startswith(self.config.base_url) or not url.startswith(('http://', 'https://'))
            ...
            try:
                ...
                if not link_result.is_valid:
                    ...
                    severity = SeverityLevel.HIGH if link_result.status_code >= 500 else SeverityLevel.MEDIUM
    ...
    def _check_code_examples(self, content: str, result: DocumentationTestResult) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if line.strip().startswith('```'):
                ...
                if language:
                    ...
                    valid_languages = ['python', 'javascript', 'bash', 'curl', 'json', 'yaml']
    ...
    def _check_content_accuracy(self, content: str, result: DocumentationTestResult) -> None:
            ...
            for method in methods:
                if f"{method} /" in line:
                    ...
                    if endpoint_match:
                        ...
                        if not self._method_endpoint_exists_in_spec(method.lower(), endpoint):
                            result.issues.append(DocumentationIssue(
                                type=IssueType.OUTDATED_CONTENT,
                                severity=SeverityLevel.HIGH,
    ...
    async def test_all_documentation(self) -> List[DocumentationTestResult]:
        ...
        logger.info(f"Documentation testing completed: {len(valid_results)} files tested")
    ...
    def generate_test_report(self, results: List[DocumentationTestResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "link_success_rate": (valid_links / total_links * 100) if total_links > 0 else 0,
    ...
    def _generate_recommendations(
        ...
        if issues_by_type.get('broken_link', 0) > 0:
            recommendations.append(f"Fix {issues_by_type['broken_link']} broken links to improve user experience")
        
        if issues_by_type.get('outdated_content', 0) > 0:
            recommendations.append(f"Update {issues_by_type['outdated_content']} outdated content references")
        
        if issues_by_type.get('incorrect_code', 0) > 0:
            recommendations.append(f"Review and fix {issues_by_type['incorrect_code']} code examples")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="doc_test_results", help="Output directory")
    parser.add_argument("--base-url", default="https://api.raptorflow.com", help="Base API URL")
    parser.add_argument("--no-external-links", action="store_true", help="Skip external link checking")
    parser.add_argument("--no-code-checks", action="store_true", help="Skip code example validation")
    parser.add_argument("--no-accuracy-checks", action="store_true", help="Skip content accuracy checking")
