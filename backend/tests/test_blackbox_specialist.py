...
def test_blackbox_specialist_base():
    ...
    with patch("backend.inference.InferenceProvider.get_model") as mock_get_model:
        ...
        specialist = ConcreteSpecialist(
            agent_id="test-specialist", model_tier="mundane"
        )
    ...
    try:
        ...
        with patch("backend.inference.InferenceProvider.get_model") as mock_get_model:
            ...
            mock_llm.invoke.return_value = MagicMock(
                content="Drift detected: Tone is too playful."
            )

            agent = StrategicDriftAgent()
            result = agent.run(
                "move-123", {"trace": "playful tone", "brand_kit": "serious"}
            )
...
def test_competitor_intelligence_agent():
    ...
    try:
        ...
        with patch("backend.inference.InferenceProvider.get_model") as mock_get_model:
            ...
            mock_get_model.return_value = mock_llm
            mock_llm.invoke.return_value = MagicMock(
                content="Competitor X lowered prices."
            )

            agent = CompetitorIntelligenceAgent()
            result = agent.run(
                "move-123", {"telemetry_data": [{"action": "scrape", "res": "ok"}]}
            )
