...
class TestTimeouts:
    ...
    @pytest.mark.asyncio
    async def test_timeout_manager_basic_functionality(
        self, timeout_manager, agent_cache
    ):
    ...
    async def test_timeout_manager_per_operation_timeouts(self, timeout_manager):
        ...
        llm_timeout = timeout_manager.config.get_timeout_for_operation(
            OperationType.LLM_INFERENCE
        )
        tool_timeout = timeout_manager.config.get_timeout_for_operation(
            OperationType.TOOL_EXECUTION
        )
        db_timeout = timeout_manager.config.get_timeout_for_operation(
    ...
    async def test_timeout_manager_recovery_strategies(self, timeout_manager):
        ...
        strategies = timeout_manager._get_default_recovery_strategies(
    ...
    async def test_agent_cache_ttl_expiration(self, agent_cache):
        ...
        # Set with short TTL
        success = await agent_cache.set(
            "test_agent", test_request, test_response, ttl=5
        )
        assert success
    ...
    async def test_agent_cache_compression(self, agent_cache):
        ...
        success = await agent_cache.set(
            "test_agent", "compression_test", large_response
        )
        assert success
    ...
    async def test_agent_cache_intelligent_invalidation(self, agent_cache):
        ...
        # Set cache entry
        success = await agent_cache.set(
            "test_agent", "invalidation_test", test_response
        )
        assert success
    ...
    async def test_timeout_monitoring(self, timeout_manager, metrics_collector):
        ...
        await timeout_manager.record_failure(
            "agent_execution", "test_agent", "TestError"
        )
    ...
    async def test_cache_performance_monitoring(self, agent_cache, metrics_collector):
        ...
        await agent_cache.record_cache_request(
            "test_agent", "test_operation", True, 0.1
        )
        await agent_cache.record_cache_request(
            "test_agent", "test_operation", False, 0.2
        )
        await agent_cache.record_cache_request(
            "test_agent", "test_operation", True, 0.05
        )
    ...
    async def test_integration_scenarios(
        self, timeout_manager, agent_cache, mock_agent
    ):
    ...
    async def test_error_handling(self, timeout_manager, agent_cache, mock_agent):
        ...
        with patch.object(mock_agent, "handle_execution_error") as mock_handle_error:
            ...
            mock_handle_error.assert_called_once_with(
                "test_agent", error=pytest.any(TimeoutError), context=pytest.any(dict)
            )
    ...
    async def test_performance_requirements(self, timeout_manager, agent_cache):
        ...
        logger.info(
            f"✓ Performance requirements test passed - Duration: {duration:.3f}s, Hit Rate: {hit_rate:.2f}"
        )
    ...
    async def run_all_tests(self):
        ...
        await self.test_timeout_manager_basic_functionality(
            timeout_manager, agent_cache
        )
        await self.test_timeout_manager_per_operation_timeouts(
            timeout_manager, agent_cache
        )
        await self.test_timeout_manager_recovery_strategies(
            timeout_manager, agent_cache
        )
