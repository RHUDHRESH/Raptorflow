...
from backend.agents.quality import (
    create_brand_guardian, BrandAlignment, 
    create_hype_filter, HypeLintResult,
    create_stats_scorer, ContentStats,
    create_editor_node, EditorialCritique,
    create_refiner_node,
    create_red_team_agent, RedTeamAnalysis, StrategicRisk,
    create_legal_guard, ComplianceResult,
    create_quality_gate,
    create_feedback_integrator,
    create_memory_governor, MemoryDecision
)
...
async def test_memory_governor_logic():
    ...
    mock_chain.ainvoke.return_value = MemoryDecision(
        should_remember=True, importance_score=0.9, extracted_fact="New tone rule", category="tone"
    )
...
async def test_legal_guard_logic():
    ...
    mock_chain.ainvoke.return_value = ComplianceResult(
        is_compliant=False,
        issues=["Unsubstantiated claim"],
        risk_level="High"
    )
    
    with patch("backend.agents.quality.LegalComplianceGuard.__init__", return_value=None):
...
async def test_red_team_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = RedTeamAnalysis(risks=[
...
async def test_brand_guardian_tone_logic():
    ...
    mock_chain.ainvoke.return_value = BrandAlignment(
        is_aligned=True,
        score=0.9,
        tone_feedback="Perfectly surgical.",
        violations=[]
    )
