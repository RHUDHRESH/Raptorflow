...
async def run_test():
    ...
    with patch('backend.services.storage.storage_service') as mock_storage:
        ...
        mock_storage.upload_file.return_value = "gs://test-bucket/muse/workspace-123/content-456.md"
        
        # Import the function to test
        from backend.api.v1.muse import save_content_to_database
        ...
        for arg in params:
            if isinstance(arg, list) and len(arg) > 0 and "gcs_uri" in arg[0]:
                if arg[0]["gcs_uri"] == "gs://test-bucket/muse/workspace-123/content-456.md":
