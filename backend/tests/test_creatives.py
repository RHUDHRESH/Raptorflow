...
from backend.agents.creatives import (
    create_linkedin_architect, LinkedInPost, 
    create_twitter_architect, TwitterThread, 
    create_ad_architect, AdAnalysis, AdCopy,
    create_content_transformer, BlogPost,
    create_tone_adjuster,
    create_visual_prompter, ImagePrompt,
    create_diagram_architect, SVGDiagram,
    create_multi_variant_generator,
    create_formatting_filter, CreativeLintResult
)
...
async def test_content_transformer_logic():
    ...
    with patch("backend.agents.creatives.ContentTransformer.__init__", return_value=None):
...
async def test_ad_architect_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = AdAnalysis(ads=[
        AdCopy(platform="FB", headline="Stop Guessing.", body_text="Use Muse.", call_to_action="Get Started")
    ])
...
async def test_linkedin_architect_logic():
    ...
    with patch("backend.agents.creatives.LinkedInArchitect.__init__", return_value=None):
