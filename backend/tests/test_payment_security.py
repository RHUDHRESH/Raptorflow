...
class TestPhonePeSecurity(TestPaymentSecurityFramework):
    ...
    async def test_webhook_signature_valid(self, phonepe_security):
        ...
        string_to_sign = (
            f"{encoded_body}/v3/validate{phonepe_security.phonepe_salt_key}"
        )
    ...
    async def test_rate_limiting(self, phonepe_security):
        ...
        rate_limited_count = sum(
            1 for r in results if not r.valid and "Rate limit exceeded" in r.errors
        )
...
class TestPaymentMonitoring(TestPaymentSecurityFramework):
    ...
    async def test_alert_generation(self, payment_monitor):
        ...
        slow_transaction_alerts = [
            alert for alert in alerts if "slow transaction" in alert.title.lower()
        ]
...
class TestIntegrationScenarios(TestPaymentSecurityFramework):
    ...
    @pytest.mark.asyncio
    async def test_security_breach_simulation(
        self, phonepe_security, fraud_detector, payment_monitor, session_manager
    ):
        ...
        blocked_attempts = sum(
            1 for attempt in breach_attempts if attempt["fraud_assessment"].should_block
        )
