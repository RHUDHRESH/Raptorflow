...
async def test_compressor_summarization_trigger():
    ...
    with patch("backend.memory.compressor.summarize_recursively", new_callable=AsyncMock) as mock_summarize, \
         patch("backend.memory.compressor.InferenceProvider") as mock_inference:
