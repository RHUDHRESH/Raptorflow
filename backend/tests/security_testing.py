...
class SecurityTestSuite:
    ...
    async def _setup_session(self) -> None:
        ...
        headers = {
            'User-Agent': 'RaptorFlow-Security-Test/1.0'
        }
    ...
    def _check_security_headers(self, headers: Dict[str, str]) -> List[SecurityVulnerability]:
    ...
    async def test_sql_injection(self) -> SecurityTestResult:
        ...
        test_endpoints = [
            ...
            ("/users/me", "GET"),
            ("/workspaces", "GET"),
            ("/api/v1/ai/inference", "POST")
        ]
        
        for endpoint, method in test_endpoints:
            for payload in self.test_payloads['sql_injection']:
                test_data = {"email": payload, "password": "test"} if method == "POST" else {"search": payload}
                
                status, response_text, headers = await self._make_request(method, endpoint, test_data)
    ...
    async def test_xss(self) -> SecurityTestResult:
        ...
        for endpoint, method in test_endpoints:
            for payload in self.test_payloads['xss']:
                test_data = {"name": payload, "description": payload} if method == "POST" else {"query": payload}
    ...
    async def test_authentication_bypass(self) -> SecurityTestResult:
        ...
        for endpoint in protected_endpoints:
            status, response_text, headers = await self._make_request("GET", endpoint, headers=invalid_headers)
    ...
    async def test_rate_limiting(self) -> SecurityTestResult:
        ...
        for i in range(request_count):
            status, response_text, headers = await self._make_request("GET", test_endpoint)
    ...
    async def test_vertexai_security(self) -> SecurityTestResult:
        ...
        for payload in prompt_injection_payloads:
            ...
            status, response_text, headers = await self._make_request("POST", "/api/v1/ai/inference", test_data)
            
            if status == 200:
                # Check if system prompt was revealed
                if "system prompt" in response_text.lower() or "previous instructions" in response_text.lower():
                    vulnerabilities.append(SecurityVulnerability(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="security_results", help="Output directory")
