...
async def test_fortress_e2e_stress():
    ...
    with patch("backend.db.psycopg.AsyncConnection.connect", new_callable=AsyncMock), \
        ...
        massive_prompt = "Plan a full market entry for a new surgical AI tool in the US market."
        ...
        with patch("backend.graphs.spine_v3.logger.info", logger_info):
            ...
            assert len(app.nodes) >= 8 # discover, 3x research, 2x strategy, 2x creative, qa
