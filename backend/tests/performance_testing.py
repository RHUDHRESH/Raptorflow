...
class PerformanceTestSuite:
    ...
    async def _make_request(
        self, scenario: TestScenario, user_id: str
    ) -> PerformanceResult:
    ...
    async def _run_load_test(self, scenario: TestScenario) -> PerformanceReport:
        ...
        for i in range(scenario.concurrent_users):
            ...
            task = self._user_simulation(
                scenario, user_id, scenario.duration, scenario.requests_per_second
            )
        ...
        report = PerformanceReport(
            ...
            error_rate=(sum(1 for r in all_results if not r.success) / len(all_results))
            * 100,
        ...
        logger.info(
            f"Load test completed: {scenario.name} - {report.successful_requests}/{report.total_requests} successful"
        )
        return report
    ...
    def _check_thresholds(
        self, report: PerformanceReport, thresholds: List[PerformanceThreshold]
    ) -> Dict[str, bool]:
        ...
        for threshold in thresholds:
            if threshold.metric == PerformanceMetric.RESPONSE_TIME:
                ...
                if threshold.max_value and avg_time > threshold.max_value:
                    ...
                    and abs(avg_time - threshold.target_value)
                ...
                avg_memory = (
                    statistics.mean(report.memory_usage) if report.memory_usage else 0
                )
        ...
        scenarios = [
            ...
            TestScenario(
                ...
                thresholds=[
                    ...
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=1.0
                    ),
                ],
            ),
            TestScenario(
                ...
                thresholds=[
                    ...
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=2.0
                    ),
                ],
            ),
            TestScenario(
                ...
                thresholds=[
                    ...
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=1.5
                    ),
                ],
            ),
            ...
            TestScenario(
                ...
                thresholds=[
                    ...
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=5.0
                    ),
                ],
            ),
            ...
            TestScenario(
                ...
                thresholds=[
                    PerformanceThreshold(
                        metric=PerformanceMetric.RESPONSE_TIME, max_value=3.0
                    ),
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=10.0
                    ),
                ],
            ),
            ...
            TestScenario(
                ...
                thresholds=[
                    PerformanceThreshold(
                        metric=PerformanceMetric.RESPONSE_TIME, max_value=2.0
                    ),
                    PerformanceThreshold(
                        metric=PerformanceMetric.ERROR_RATE, max_value=5.0
                    ),
    ...
    def generate_performance_charts(self, reports: List[PerformanceReport]) -> None:
        ...
        for i, report in enumerate(reports):
            plt.subplot(2, 2, 1)
            plt.hist(
                report.response_times, bins=50, alpha=0.7, label=report.scenario_name
            )
        ...
        plt.tight_layout()
        chart_file = (
            Path(self.config.output_dir) / f"performance_charts_{timestamp}.png"
        )
            ...
            "summary": {
                ...
                "overall_success_rate": (
                    (
                        ...
                        / sum(r.total_requests for r in reports)
                        * 100
                    )
                    if reports
                    else 0
            ...
            "scenarios": [
                {
                    ...
                    "cpu_usage": {
                        "avg": (
                            statistics.mean(report.cpu_usage) if report.cpu_usage else 0
                        ),
                    ...
                    "memory_usage": {
                        "avg": (
                            statistics.mean(report.memory_usage)
                            if report.memory_usage
                            else 0
                        ),
    ...
    def save_reports(self, reports: List[PerformanceReport]) -> None:
        ...
        json_file = (
            Path(self.config.output_dir) / f"performance_report_{timestamp}.json"
        )
    ...
    async def run_performance_tests(
        self, scenarios: Optional[List[TestScenario]] = None
    ...
    def _check_alerts(self, report: PerformanceReport) -> None:
        ...
        # Response time alert
        if report.avg_response_time > self.config.alert_thresholds.get(
            "response_time", 2.0
        ):
            alerts.append(
                {
        ...
        for alert in alerts:
            logger.warning(
                f"Performance Alert [{alert['severity'].upper()}]: {alert['message']} (Scenario: {alert['scenario']})"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-raw-data", action="store_true", help="Disable raw data saving"
    )
    ...
    async def main():
        async with PerformanceTestSuite(config) as suite:
            ...
            print(
                f"Success Rate: {sum(r.successful_requests for r in reports) / sum(r.total_requests for r in reports) * 100:.1f}%"
            )
            print(
                f"Average Response Time: {sum(r.avg_response_time for r in reports) / len(reports):.3f}s"
            )
