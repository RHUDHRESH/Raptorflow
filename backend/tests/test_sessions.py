...
class TestRedisSessionManager:
    ...
    async def test_create_session(self, session_manager):
        ...
        session_id = await session_manager.create_session(
            user_id=user_id, workspace_id=workspace_id, session_type=session_type
        )
...
class TestPerformanceOptimizer:
    ...
    def test_end_tracking(self, performance_optimizer):
        ...
        metrics = asyncio.run(
            performance_optimizer.end_tracking(
...
class TestResourcePool:
    ...
    def memory_pool(self):
        ...
        return MemoryBufferPool(pool_name="test-memory", buffer_size=1024, max_size=5)
    ...
    async def test_session_with_performance_tracking(self):
        ...
        with patch("core.sessions.redis.from_url") as mock_redis:
            ...
            session_manager = RedisSessionManager(
                redis_url="redis://localhost:6379", secret_key="test-secret"
            )
            ...
            # Create session
            session_id = await session_manager.create_session(
                user_id="test-user", workspace_id="test-workspace"
            )
...
class TestSessionAnalytics:
    ...
    async def test_performance_insights(self):
        ...
        high_response_insights = [
            insight for insight in insights if insight.title == "High Response Times"
        ]
...
class TestSessionPerformance:
            ...
            for i in range(100):
                task = asyncio.create_task(
                    session_manager.create_session(
                        user_id=f"user-{i}", workspace_id="test-workspace"
                    )
                )
                tasks.append(task)
    ...
    async def test_memory_usage_under_load(self):
        ...
        for i in range(1000):
            await metrics_collector.record_metric(
                MetricType.PERFORMANCE, "test_metric", i, "count"
            )
        ...
        assert (
            len(metrics_collector.metrics_buffer) <= 10000
