...
from agents.researchers import (
    ...
    create_competitor_mapper,
    create_competitor_tracker,
    create_evidence_bundler,
    create_gap_finder,
    create_linkedin_profiler,
    create_positioning_mapper,
    create_reddit_analyst,
    create_research_qa_guard,
    create_research_summarizer,
    create_source_validator,
    create_swot_analyzer,
...
async def test_competitor_tracker_logic():
    ...
    with patch(
        "backend.agents.researchers.save_entity", new_callable=AsyncMock
    ) as mock_save:
...
async def test_research_qa_logic():
    ...
    with patch(
        "backend.agents.researchers.ResearchQAGuard.__init__", return_value=None
    ):
...
async def test_positioning_mapper_logic():
    ...
    with patch(
        "backend.agents.researchers.PositioningMapper.__init__", return_value=None
    ):
        ...
        assert (
            result["research_bundle"]["positioning_map"]["x_axis"]
            == "Price (Low -> High)"
        )
...
async def test_visual_trend_analyzer_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = VisualAnalysis(
        styles=[
            VisualStyle(
                aesthetic="Minimalist",
                color_palette=["#000", "#FFF"],
                signal="Clean UI trend",
            )
        ]
    )

    with patch(
        "backend.agents.researchers.VisualTrendAnalyzer.__init__", return_value=None
    ):
        ...
        assert (
            result["research_bundle"]["visual_trends"]["styles"][0]["aesthetic"]
            == "Minimalist"
        )
...
async def test_brand_history_context_logic():
    ...
    with patch(
        "backend.agents.researchers.get_memory", new_callable=AsyncMock
    ) as mock_mem:
        mock_mem.return_value = [
            ("id", "Past campaign: Launch 2024", {"type": "episodic"}, 0.9)
        ]
...
async def test_research_summarizer_logic():
    ...
    mock_llm = MagicMock()
    with patch(
        "backend.agents.researchers.summarize_recursively", new_callable=AsyncMock
    ) as mock_sum:
...
async def test_gap_finder_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = GapAnalysis(
        gaps=[
            MarketGap(
                description="Low focus on privacy",
                severity=0.8,
                opportunity="SOTA Privacy First Agent",
            )
        ]
    )
...
async def test_trend_extractor_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = TrendAnalysis(
        trends=[
            MarketTrend(
                name="AI Native", strength=0.9, signal="High interest in local LLMs"
            )
        ]
    )
...
async def test_source_validator_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = ValidatorOutput(
        validations=[
            SourceValidation(
                url="https://trusted.com",
                credibility_score=0.9,
                reasoning="SOTA Domain",
                is_trusted=True,
            )
        ]
    )
    ...
    with patch(
        "backend.agents.researchers.SourceValidator.__init__", return_value=None
    ):
        ...
        assert (
            result["research_bundle"]["validations"]["validations"][0][
                "credibility_score"
            ]
            == 0.9
        )
...
async def test_evidence_bundler_logic():
    ...
    with patch(
        "backend.agents.researchers.EvidenceBundler.__init__", return_value=None
    ):
...
async def test_competitor_mapper_logic():
    ...
    with patch(
        "backend.agents.researchers.CompetitorFeatureMapper.__init__", return_value=None
    ):
...
async def test_linkedin_profiler_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = LinkedInAnalysis(
        profiles=[
            LinkedInProfile(
                name="John Doe", key_themes=["AI", "SaaS"], authority_score=0.9
            )
        ]
    )

    with patch(
        "backend.agents.researchers.LinkedInProfiler.__init__", return_value=None
    ):
        ...
        assert (
            result["research_bundle"]["linkedin_insights"]["profiles"][0]["name"]
            == "John Doe"
        )
...
async def test_reddit_analyst_logic():
    ...
    mock_llm = MagicMock()
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = RedditAnalysis(
        insights=[
            RedditInsight(
                pain_point="Cost", sentiment="angry", evidence_quote="Too expensive!"
            )
        ]
    )

    with patch(
        "backend.agents.researchers.RedditSentimentAnalyst.__init__", return_value=None
    ):
        ...
        assert (
            result["research_bundle"]["social_insights"]["insights"][0]["pain_point"]
            == "Cost"
        )
...
async def test_perplexity_search_logic():
    """Verify that the tool executes native search correctly."""
    mock_results = [
        {"title": "Trend X", "snippet": "X is rising", "url": "https://trend.com"}
    ]

    with patch(
        "backend.core.search_native.NativeSearch.query", new_callable=AsyncMock
    ) as mock_query:
        ...
        assert "Trend X" in result["data"]
        assert "https://trend.com" in result["data"]
...
async def test_tavily_search_logic():
    """Verify that the tool executes native search correctly."""
    mock_results = [
        {"title": "SOTA AI", "snippet": "Evidence", "url": "https://test.com"}
    ]
