...
from backend.core.health_monitor import (
    HealthMonitorAdvanced, HealthStatus, HealthMetric, HealthAlert,
    HealthPrediction, PredictiveAnalytics, SystemHealthReport
)
from backend.core.health_analytics import (
    HealthAnalytics, AlertRule, AlertType, AlertSeverity, NotificationChannel,
    NotificationEngine, NotificationConfig
)
...
class TestHealthMonitorAdvanced:
    ...
    def sample_metrics(self):
        ...
        return [
            ("cpu_usage", 75.5, "%", {"threshold_warning": 70, "threshold_critical": 90}),
            ("memory_usage", 60.2, "%", {"threshold_warning": 80, "threshold_critical": 95}),
            ("response_time", 150.0, "ms", {"threshold_warning": 500, "threshold_critical": 1000}),
            ("error_rate", 2.5, "%", {"threshold_warning": 5, "threshold_critical": 10})
        ]
    ...
    async def test_alert_generation(self, monitor):
        ...
        active_alerts = [alert for alert in monitor.alerts.values() if not alert.resolved]
...
class TestPredictiveAnalytics:
    ...
    def test_anomaly_detection(self, analytics):
        ...
        # Test anomalous data
        analytics.anomaly_detectors["test_metric"].predict.return_value = [-1]  # Anomaly
        is_anomaly = analytics.detect_anomaly("test_metric", [1.0, 2.0, 3.0])
    ...
    def test_confidence_calculation(self, analytics):
        ...
        for i in range(10):
            analytics.feature_history["test_metric"].append({
                'value': 50.5 + i,  # Close to predictions
...
class TestNotificationEngine:
    ...
    def sample_alert(self):
        ...
        from backend.core.health_analytics import HealthAlert, AlertType, AlertSeverity
    ...
    async def test_notification_configuration(self, notification_engine):
        ...
        assert notification_engine.configs[NotificationChannel.EMAIL].recipients == ["test@example.com"]
    ...
    async def test_rate_limiting(self, notification_engine):
        ...
        result1 = await notification_engine.send_notification(alert, [NotificationChannel.EMAIL])
        result2 = await notification_engine.send_notification(alert, [NotificationChannel.EMAIL])
        result3 = await notification_engine.send_notification(alert, [NotificationChannel.EMAIL])
...
class TestIntegrationScenarios:
    ...
    async def test_massive_alert_scenario(self):
        ...
        for i in range(100):
            await analytics.process_metric(f"metric_{i % 10}", 95.0)  # All exceed thresholds
            await asyncio.sleep(0.001)  # Small delay
...
if __name__ == "__main__":
    ...
    pytest.main([
        ...
        "-v",
        "--tb=short",
        "-k", "test_metric_recording or test_alert_generation"
    ])
