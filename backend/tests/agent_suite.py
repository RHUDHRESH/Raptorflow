...
class AgentTestSuite:
    ...
    def _load_test_suites(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.test_suites)} test suites and {len(self.test_results)} test results")
    ...
    def create_test_suite(self, suite_id: str, name: str, description: str,
                       test_cases: List[TestCase], parallel_execution: bool = True,
    ...
    async def run_test_suite(self, suite_id: str, test_filter: Optional[str] = None) -> Dict[str, Any]:
        """Run a test suite."""
        try:
            ...
            logger.info(f"Test suite {suite_id} completed: {summary['total_passed']}/{summary['total_tests']} passed")
    ...
    async def _run_unit_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                error_message="" if actual_result else f"Agent {test_case.agent_name} instantiation failed"
            )
    ...
    async def _run_performance_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            throughput = config.test_requests / sum(response_times) if sum(response_times) > 0 else 0
            
            # Check performance criteria
            passed = (
                avg_response_time <= config.max_response_time and
    ...
    async def _run_load_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            # Load test execution
            logger.info(f"Starting load test: {config.concurrent_users} concurrent users, {config.test_duration}s duration")
    ...
    async def _run_end_to_end_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            for step in workflow_steps:
                ...
                step_success = step_result is not None and "error" not in str(step_result).lower()
            ...
            return TestResult(
                ...
                expected_result=test_case.expected_result,
                error_message="" if overall_success else "Workflow failed at step: " + str(len(results)),
                metrics={
    ...
    def get_test_results(self, suite_id: Optional[str] = None, test_type: Optional[TestType] = None,
    ...
    def generate_test_report(self, suite_id: str, format: str = "json") -> str:
        ...
        try:
            ...
            results = [r for r in self.test_results if r.test_id in [tc.test_id for tc in suite.test_cases]]
...
async def run_test_suite(suite_id: str, test_filter: Optional[str] = None) -> Dict[str, Any]:
    """Run a test suite."""
    suite = get_agent_test_suite()
    return await suite.run_test_suite(suite_id, test_filter)


def get_test_results(suite_id: Optional[str] = None, test_type: Optional[TestType] = None,
                 limit: int = 100) -> Dict[str, Any]:
    """Get test results."""
    suite = get_agent_test_suite()
