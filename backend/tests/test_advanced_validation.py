...
from backend.core.advanced_validation import (
    AdvancedValidator, ValidationMode, ThreatLevel, ValidationResult,
    ThreatIntelligence, ThreatIndicator, ThreatCategory, ThreatSeverity
)
from backend.core.validation_performance import (
    ValidationOptimizer, PerformanceLevel, AdaptiveCache
)
from backend.core.threat_intelligence import (
    ThreatEvent, AttackPattern, ThreatSource
)
from backend.core.health_analytics import (
    HealthAnalytics, AlertRule, AlertType, AlertSeverity
)
...
class TestAdvancedValidator:
    ...
    async def test_validate_malicious_request(self, validator, malicious_request):
        ...
        assert any("injection" in threat.get("attack_type", "").lower() 
    ...
    async def test_validation_modes(self, sample_request):
        ...
        assert strict_validator._get_mode_threshold() < permissive_validator._get_mode_threshold()
    ...
    async def test_risk_score_calculation(self, validator):
        ...
        risk_score = validator._calculate_risk_score(threats, [1.0, 2.0, 3.0], False, {})
    ...
    def test_metrics_tracking(self, validator):
        ...
        validator._update_metrics(ValidationResult(
            is_valid=True, threat_level=ThreatLevel.LOW, confidence=0.8,
            threats_detected=[], risk_score=0.2, processing_time=0.1,
...
class TestThreatIntelligence:
    ...
    async def test_behavioral_analysis(self, threat_intel):
        ...
        assert any(threat.category == ThreatCategory.DENIAL_OF_SERVICE for threat in threats)
...
class TestValidationPerformance:
    ...
    def test_performance_levels(self):
        ...
        ultra_fast_threshold = ultra_fast.validation_thresholds[PerformanceLevel.ULTRA_FAST]
        ...
        assert ultra_fast_threshold["cache_hit_rate"] > paranoid_threshold["cache_hit_rate"]
        
        # Paranoid should allow more validation time
        assert paranoid_threshold["max_validation_time"] > ultra_fast_threshold["max_validation_time"]
...
class TestIntegrationScenarios:
    ...
    async def test_performance_under_load(self):
        ...
        tasks = [
            optimizer.validate_with_cache(req, mock_validation)
            for req in requests
        ]
...
class TestPerformanceBenchmarks:
    ...
    async def benchmark_cache_performance(self):
        ...
        # Test data
        requests = [
            {"request": f"Test request {i}", "data": "x" * 100}
            for i in range(1000)
        ]
...
if __name__ == "__main__":
    ...
    pytest.main([
        ...
        "-v",
        "--tb=short",
        "-k", "test_validate_normal_request or test_validate_malicious_request"
    ])
