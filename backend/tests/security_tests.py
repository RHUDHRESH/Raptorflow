...
class SecurityTestRunner:
    ...
    async def _test_sql_injection(self):
        ...
        async with AsyncClient(base_url=self.config.base_url, timeout=self.config.timeout) as client:
            for endpoint in endpoints:
                for payload in sql_payloads:
                    try:
                        ...
                        sql_errors = ["sql", "mysql", "postgresql", "sqlite", "ora-", "syntax error"]
                        
                        if any(error in response_text for error in sql_errors) and response.status_code != 404:
                        ...
                        if any(error in response.text.lower() for error in sql_errors) and response.status_code != 404:
                            self.vulnerabilities.append(SecurityVulnerability(
                                vulnerability_type="SQL Injection",
                                severity="HIGH",
                                endpoint=endpoint,
        ...
        async with AsyncClient(base_url=self.config.base_url, timeout=self.config.timeout) as client:
            for endpoint in protected_endpoints:
                try:
                    ...
                    response = await client.get(endpoint, headers={"Authorization": "Bearer invalid_token"})
                    ...
                    response = await client.get(endpoint, headers={"Authorization": "Bearer admin_token"})
        ...
        async with AsyncClient(base_url=self.config.base_url, timeout=self.config.timeout) as client:
            for endpoint in endpoints:
                for test_case in test_cases:
                    try:
                        response = await client.post(endpoint, json={test_case["field"]: test_case["value"]})
    ...
    async def _test_authorization(self):
        ...
        async with AsyncClient(base_url=self.config.base_url, timeout=self.config.timeout) as client:
            for endpoint in admin_endpoints:
                try:
                    response = await client.get(endpoint, headers={"Authorization": user_token})
    ...
    def _generate_security_recommendations(self) -> List[str]:
        ...
        if "Cross-Site Scripting (XSS)" in vuln_types:
            recommendations.append("Implement Content Security Policy (CSP) headers and output encoding to prevent XSS")
        
        if "Authentication Bypass" in vuln_types:
            recommendations.append("Strengthen authentication mechanisms and implement proper token validation")
        
        if "Authorization Bypass" in vuln_types:
            recommendations.append("Implement role-based access control (RBAC) and regular authorization audits")
        ...
        if not recommendations:
            recommendations.append("Continue following security best practices and regular security audits")
...
async def test_security_vulnerabilities(security_test_config):
    ...
    assert len(critical_high_vulns) == 0, f"Critical/High vulnerabilities found: {[v.vulnerability_type for v in critical_high_vulns]}"
