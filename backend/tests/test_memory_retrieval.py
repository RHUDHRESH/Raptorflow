...
async def test_retrieve_relevant_memories_flow():
    ...
    with patch("backend.inference.InferenceProvider.get_embeddings") as mock_get_emb:
        ...
        # Mock DB
        with patch("backend.memory.cognitive.engine.get_db_connection", new_callable=MagicMock) as mock_get_conn:
            mock_conn = AsyncMock()
            ...
            # Mock result
            mock_cursor.fetchall.return_value = [("Always use serif fonts.",), ("Tone should be calm.",)]
            
            tenant_id = "00000000-0000-0000-0000-000000000000"
            memories = await engine.retrieve_relevant_memories(tenant_id, "What are font rules?")
