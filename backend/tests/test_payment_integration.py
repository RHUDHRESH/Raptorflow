...
class TestPaymentSDKGateway:
    ...
    async def test_transaction_consistency(self, setup_sdk_gateway):
        ...
        async with transaction_manager.transaction_context(
            isolation_level="READ_COMMITTED", metadata={"test": True}
        ) as tx:
            ...
            tx.execute(
                "SELECT 1 FROM payments WHERE merchant_order_id = %s",
                (merchant_order_id,),
            )
            records = tx.fetchall()
    ...
    async def test_encryption_decryption_cycle(self, setup_sdk_gateway):
        ...
        # Encrypt data
        encrypted_data, metadata = await gateway.encrypt_data(
            test_data, DataClassification.SENSITIVE_FINANCIAL
        )
...
class TestPaymentIntegration:
    ...
    async def test_complete_payment_flow(
        self, test_redis_client, test_payment_repository
    ):
        ...
        webhook_handler = get_secure_webhook_handler(
            test_redis_client, test_payment_repository
        )
        ...
        # 2. Check status
        status_result = await phonepe_gateway.check_payment_status(
        ...
        if payment_result.status == "COMPLETED":
            ...
            webhook_result = await webhook_handler.handle_webhook(
                payload=webhook_data, signature="sha256:hash"
            )
