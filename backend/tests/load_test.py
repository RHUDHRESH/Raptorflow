...
class PaymentLoadTest(HttpUser):
    ...
    def initiate_payment(self):
        ...
        payment_data = {
            ...
            "metadata": {"source": "load_test", "user_agent": "Locust Load Tester"},
        }

        with self.client.post(
            "/api/payments/initiate", json=payment_data, catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def check_payment_status(self):
        ...
        with self.client.get(
            f"/api/payments/{transaction_id}/status", catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def process_refund(self):
        ...
        with self.client.post(
            "/api/payments/refund", json=refund_data, catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
...
class LoadTestRunner:
    ...
    def run_load_test(
        self, num_users: int = 10, spawn_rate: int = 2, duration: int = 300
    ):
        """
        ...
        """
        logger.info(
            f"Starting load test: {num_users} users, {spawn_rate}/s spawn rate, {duration}s duration"
        )
    ...
    def run_stress_test(
        self, num_users: int = 50, spawn_rate: int = 10, duration: int = 600
    ):
        """
        ...
        """
        logger.info(
            f"Starting stress test: {num_users} users, {spawn_rate}/s spawn rate, {duration}s duration"
        )
    ...
    def run_spike_test(
        self, num_users: int = 100, spawn_rate: int = 50, duration: int = 120
    ):
        """
        ...
        """
        logger.info(
            f"Starting spike test: {num_users} users, {spawn_rate}/s spawn rate, {duration}s duration"
        )
