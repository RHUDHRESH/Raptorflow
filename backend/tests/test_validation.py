...
class TestSecurityValidator:
    ...
    def test_validate_safe_request(self, security_validator):
        ...
        is_valid, error = asyncio.run(security_validator.validate_request(safe_request))
    ...
    def test_validate_xss_attempt(self, security_validator):
        ...
        is_valid, error = asyncio.run(security_validator.validate_request(xss_request))
    ...
    def test_validate_sql_injection_attempt(self, security_validator):
        ...
        is_valid, error = asyncio.run(security_validator.validate_request(sql_request))
    ...
    def test_validate_privilege_escalation_attempt(self, security_validator):
        ...
        is_valid, error = asyncio.run(security_validator.validate_request(priv_request))
    ...
    def test_blocked_ip_enforcement(self, security_validator):
        ...
        is_valid, error = asyncio.run(
            security_validator.validate_request(request_data, client_ip="192.168.1.100")
        )
...
class TestRequestMetrics:
    ...
    def test_performance_issue_detection(self, metrics_collector):
        ...
        assert (
            ...
            > metrics_collector.performance_thresholds["very_slow_request"]
        )
...
class TestAgentDispatcher:
    ...
    async def test_fallback_agent_selection(self, dispatcher):
        ...
        dispatcher._update_agent_health(
            primary_agent, False, error="Primary agent failed"
        )
