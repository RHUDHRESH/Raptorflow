...
class RedisLoadTester:
    ...
    async def test_session_management_load(self):
        ...
        for i in range(num_sessions):
            ...
            # Create session
            session = await self.session_service.create_session(f"user{i}", f"workspace{i}")
            
            # Retrieve session
            retrieved = await self.session_service.get_session(session.id)
            
            # Update session
            await self.session_service.update_session(session.id, {"last_activity": datetime.now()})
    ...
    async def test_distributed_locks_load(self):
        ...
        # Flatten results
        all_lock_times = [time for worker_times in worker_results for time in worker_times]
        ...
        async def mixed_worker(worker_id: int):
            ...
            for i in range(50):
                ...
                if operation == 0:
                    ...
                    # Session operation
                    session = await self.session_service.create_session(f"user{worker_id}", f"workspace{worker_id}")
                    ...
                    await self.semantic_cache.cache_response(query, {"answer": f"Response {i}"})
                    await self.semantic_cache.get_cached_response(query)
                
                elif operation == 4:
                    # Lock operation
                    lock = await self.lock_manager.create_lock(f"mixed_lock:{worker_id}:{i}")
    ...
    def generate_report(self):
        ...
        if 'basic_operations' in self.results:
            print(f"  Basic Ops: {self.results['basic_operations']['ops_per_second']:.0f} ops/sec")
        if 'session_management' in self.results:
            print(f"  Sessions: {self.results['session_management']['sessions_per_second']:.0f} sessions/sec")
        if 'rate_limiting' in self.results:
            print(f"  Rate Limits: {self.results['rate_limiting']['checks_per_second']:.0f} checks/sec")
        if 'semantic_cache' in self.results:
            print(f"  Cache: {self.results['semantic_cache']['cache_ops_per_second']:.0f} ops/sec")
        if 'distributed_locks' in self.results:
            print(f"  Locks: {self.results['distributed_locks']['locks_per_second']:.0f} locks/sec")
        if 'mixed_workload' in self.results:
            print(f"  Mixed: {self.results['mixed_workload']['ops_per_second']:.0f} ops/sec")
        
        print(f"\n✅ Load testing completed at {datetime.now().isoformat()}")
        
        # Save report to file
        report_file = f"redis_load_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
