...
async def test_semantic_rag_logic():
    ...
    with patch("backend.agents.context_assembler.vector_search", new_callable=AsyncMock) as mock_search:
...
async def test_recursive_summarization_logic():
    ...
    assert mock_llm.ainvoke.call_count > 1 # Should be called multiple times for long text
...
async def test_memory_retrieval_semantic():
    ...
    with patch("backend.db.get_db_connection", return_value=mock_cm):
        results = await get_memory(workspace_id="ws_1", query_embedding=[0.1]*768, memory_type="semantic")
...
async def test_memory_retrieval_episodic():
    ...
    with patch("backend.db.get_db_connection", return_value=mock_cm):
        results = await get_memory(workspace_id="ws_1", query_embedding=[0.1]*768, memory_type="episodic")
