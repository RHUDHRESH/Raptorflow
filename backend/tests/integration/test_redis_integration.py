...
class TestRedisIntegration:
    ...
    async def test_distributed_locks(self):
        ...
        async with await lock_manager.create_lock(
            "context_test", timeout=5
        ) as ctx_lock:
    ...
    async def test_cache_performance(self):
        ...
        queries = [f"Test query {i} with some additional context" for i in range(100)]

        responses = [{"answer": f"Response {i}", "confidence": 0.9} for i in range(100)]
...
class TestRedisPerformance:
    ...
    async def test_redis_under_load(self):
        ...
        ), f"Redis too slow: {operations_per_second} ops/sec"
        ...
        ), f"Only {successful_writes}/1000 writes successful"
        ...
        print(
            f"Redis handled 1000 operations in {total_time:.2f}s "
