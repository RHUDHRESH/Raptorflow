...
def test_apply_pivot_endpoint_success():
    ...
    mock_service.apply_pivot = AsyncMock(return_value={"status": "success", "campaign_id": "test-id"})
    
    app.dependency_overrides[get_campaign_service] = lambda: mock_service
    
    try:
        response = client.post("/v1/campaigns/test-id/pivot", json={"title": "Test Pivot", "description": "Do something else"})
...
def test_apply_pivot_endpoint_not_found():
    ...
    try:
        response = client.post("/v1/campaigns/non-existent-id/pivot", json={"title": "Test"})
