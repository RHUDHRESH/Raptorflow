...
async def test_rag_retrieval_node_execution():
    ...
    with patch("backend.agents.rag_retrieval.MemoryManager", return_value=mock_memory_manager), \
         patch("backend.agents.rag_retrieval.InferenceProvider") as mock_inference:
        
        mock_model = MagicMock()
        mock_expansion_chain = AsyncMock()
        mock_expansion_chain.ainvoke.return_value = MagicMock(expanded_queries=["q1", "q2"])
