...
class ContractTestFramework:
    ...
    def _load_specifications(self) -> None:
        ...
        try:
            ...
            if (
                self.config.previous_spec_path
                and Path(self.config.previous_spec_path).exists()
            ):
                with open(self.config.previous_spec_path, "r") as f:
                    self.previous_spec = yaml.safe_load(f)
                logger.info(
                    f"Loaded previous OpenAPI spec: {self.config.previous_spec_path}"
                )
            else:
                logger.info(
                    "No previous specification found, skipping compatibility tests"
                )
                ...
                for status_code, response_details in details["responses"].items():
                    if status_code.startswith("2") and "content" in response_details:
                        if "application/json" in response_details["content"]:
                            schema = response_details["content"][
                                "application/json"
                            ].get("schema")
                            if schema:
                                # Make request to validate response
                                status, response_data, headers = (
                                    await self._make_request(method.upper(), path)
                                )

                                if status == int(status_code):
                                    schema_violations = self._validate_response_schema(
            ...
            for method in common_methods:
                ...
                for status_code in current_responses:
                    if status_code in previous_responses:
                        ...
                        previous_schema = (
                            previous_responses[status_code]
                            .get("content", {})
                            .get("application/json", {})
                            .get("schema")
                        )
    ...
    async def test_version_comparison(self) -> ContractTestResult:
        ...
        if (
            "info" not in self.current_spec
            or "version" not in self.current_spec["info"]
        ):
            violations.append(
                ContractViolation(
            ...
            "violations": [
                {
                    ...
                    "change_type": (
                        violation.change_type.value if violation.change_type else None
                    ),
                }
                for violation in all_violations
            ],
    ...
    def save_report(self, results: List[ContractTestResult]) -> None:
        ...
        if self.config.generate_diff_report and self.previous_spec:
            ...
            diff_data = DeepDiff(
                self.previous_spec, self.current_spec, ignore_order=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", default="contract_results", help="Output directory"
    )
    parser.add_argument(
        "--no-diff", action="store_true", help="Disable diff report generation"
    )
