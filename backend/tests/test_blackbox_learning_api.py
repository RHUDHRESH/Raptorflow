...
from backend.api.v1.blackbox_learning import get_blackbox_service
...
def test_validate_insight(mock_service):
    ...
    response = client.post(f"/v1/blackbox/learning/validate/{learning_id}", json={"status": "approved"})
    assert response.status_code == 200
...
async def test_trigger_learning_cycle(mock_service):
    mock_service.trigger_learning_cycle = AsyncMock(return_value={"status": "cycle_complete"})
