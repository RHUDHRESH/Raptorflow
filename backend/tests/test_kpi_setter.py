...
def mock_strategic_agents():
    with (
        ...
        patch(
            "backend.graphs.moves_campaigns_orchestrator.KPIDefiner",
            new_callable=MagicMock,
        ) as mock_kpi,
        patch(
        ...
        # Ensure returns are PLAIN DICTS for LangGraph serialization
        kpi_instance = mock_kpi.return_value
        kpi_instance.side_effect = AsyncMock(
            return_value={"kpi_targets": {"leads": 100}}
        )

        auditor_instance = mock_auditor.return_value
        auditor_instance.side_effect = AsyncMock(
