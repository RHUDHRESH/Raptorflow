...
async def test_search_memories():
    ...
    assert all(
        isinstance(result, MemoryChunk) for result in results
...
async def test_get_by_id():
    ...
    assert (
        retrieved_chunk.memory_type == MemoryType.CONVERSATION
...
async def test_delete_memory():
    ...
    chunk_id = await vector_memory.store(
        workspace_id, MemoryType.CAMPAIGN, "Test content"
    )
...
async def test_workspace_isolation():
    ...
    for result in results1:
        assert (
            result.workspace_id == workspace1
        ), f"Result should belong to workspace1, got {result.workspace_id}"

    for result in results2:
        assert (
            result.workspace_id == workspace2
        ), f"Result should belong to workspace2, got {result.workspace_id}"
