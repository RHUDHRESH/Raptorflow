...
class SimpleMemoryController:
            ...
            try:
                ...
                async def init_pool():
                    ...
                    if redis_pool._initialized:
                        ...
                        logger.info(
                            "Redis memory controller initialized with connection pooling"
                        )
                        return True
                    else:
                        return False
    ...
    async def store_memory(
        self, key: str, value: Any, ttl: int = 3600, memory_type: str = "working"
    ) -> bool:
        ...
        try:
            ...
            if self.redis_client:
                ...
                logger.debug(
                    f"Stored {memory_type} memory key in fallback: {prefixed_key}"
                )
            ...
            if len(self.metrics["operation_times"]) > 1000:  # Keep last 1000 operations
                self.metrics["operation_times"] = self.metrics["operation_times"][
                    -1000:
                ]
            ...
            operation_time = time.time() - start_time
            logger.error(
                f"Failed to store {memory_type} memory key '{key}' after {operation_time:.3f}s: {e}"
            )
            return False

    async def retrieve_memory(
        self, key: str, memory_type: str = "working"
    ) -> Optional[Any]:
        ...
        try:
            ...
            if self.redis_client:
                ...
                if result:
                    self.metrics["fallback_hits"] += 1
                    logger.debug(
                        f"Retrieved {memory_type} memory key from fallback: {prefixed_key}"
                    )
                    return result.get("value")
                self.metrics["fallback_hits"] += 1
    ...
    async def store_vector(
        ...
        success = await self.store_memory(
            vector_id, vector_data, ttl=86400, memory_type="vector"  # 24 hours
        )
        ...
        try:
            ...
            if self.redis_client:
                ...
                if keys:
                    ...
                    logger.info(
                        f"Cleared {len(keys)} {memory_type} memory entries from Redis"
                    )
            else:
                # Clear from fallback
                keys_to_remove = [
                    k for k in self.memory_store.keys() if k.startswith(prefix)
                ]
                for key in keys_to_remove:
                    del self.memory_store[key]
                logger.info(
                    f"Cleared {len(keys_to_remove)} {memory_type} memory entries from fallback"
                )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.metrics["operation_times"]:
                avg_time = sum(self.metrics["operation_times"]) / len(
    ...
    async def _perform_intelligent_cleanup(self):
        ...
        try:
            ...
            logger.info(
                f"Intelligent cleanup completed: {cleanup_level}, "
        ...
        try:
            ...
            if self.redis_client:
                ...
                if old_keys:
                    ...
                    memory_freed = (
                        items_cleaned * 0.2
                    )  # Estimate 0.2MB per working memory item
            else:
    ...
    async def store_bcm(
        self, workspace_id: str, manifest: Dict[str, Any], tier: str = "tier0"
    ) -> bool:
        ...
        try:
            ...
            return await self.store_memory(
                key=key, value=manifest, ttl=ttl, memory_type="bcm"
            )
        except Exception as e:
            logger.error(f"Failed to store BCM: {e}")
            return False

    async def retrieve_bcm(
        self, workspace_id: str, tier: str = "tier0"
    ) -> Optional[Dict[str, Any]]:
        """Retrieve BCM manifest by workspace and tier."""
        try:
            key = f"{workspace_id}:{tier}"
            return await self.retrieve_memory(key=key, memory_type="bcm")
        except Exception as e:
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if total > 0:
                base_stats["redis_hit_rate"] = (
                    self.metrics["redis_hits"] / total
                ) * 100
                base_stats["fallback_hit_rate"] = (
                    self.metrics["fallback_hits"] / total
                ) * 100
            ...
            if cleanup_history:
                ...
                avg_duration = sum(c.duration_ms for c in cleanup_history) / len(
                    cleanup_history
                )
