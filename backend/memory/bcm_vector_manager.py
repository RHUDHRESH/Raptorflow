...
class BCMVectorManager:
    ...
    async def generate_snippet_bundle(
        ...
        for section in sections:
            ...
            content = await self.vector_memory.get_section_content(workspace_id, section)
            if not content:
                continue
                
            # Get or create embedding
            embedding = await self._get_or_create_embedding(workspace_id, section, content)
            bundle[section] = {
                "text": content,
                "embedding": embedding,
    ...
    def _hash_section(self, section_data: Any) -> str:
        ...
        return hashlib.sha256(json.dumps(section_data, sort_keys=True).encode()).hexdigest()
