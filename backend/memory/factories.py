...
def create_cached_coordinator(
    ...
    try:
        ...
        try:
            ...
            logger.warning("Cache wrapper not fully implemented, returning base coordinator")
            
        except ImportError as e:
            logger.warning(f"Could not import cache module, using base coordinator: {e}")
        
        logger.info(f"Successfully created cached coordinator for workspace {workspace_id}")
        return coordinator
        
    except Exception as e:
        logger.error(f"Failed to create cached coordinator for workspace {workspace_id}: {e}")
        raise
