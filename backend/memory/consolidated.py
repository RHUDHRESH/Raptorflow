...
class MemoryFragment:
    ...
    def __post_init__(self):
        ...
        if self.importance_score < 0 or self.importance_score > 1:
            raise ValueError(f"Invalid importance_score: {self.importance_score}. Must be between 0 and 1.")
        if self.memory_tier not in ["L1", "L2", "L3"]:
            raise ValueError(f"Invalid memory_tier: {self.memory_tier}. Must be L1, L2, or L3.")
        if not self.id:
            self.id = str(uuid4())
...
class ConsolidatedMemory:
    ...
    consolidation_interval: timedelta = field(default_factory=lambda: timedelta(minutes=5))
...
class SwarmMemoryConsolidator:
    ...
    async def _gather_memory_fragments(self, agent_ids: Optional[List[str]] = None) -> List[MemoryFragment]:
    ...
    async def _gather_l1_fragments(self, agent_ids: Optional[List[str]] = None) -> List[MemoryFragment]:
    ...
    async def _gather_l2_fragments(self, agent_ids: Optional[List[str]] = None) -> List[MemoryFragment]:
    ...
    async def _gather_l3_fragments(self, agent_ids: Optional[List[str]] = None) -> List[MemoryFragment]:
    ...
    async def _gather_swarm_fragments(self, agent_ids: Optional[List[str]] = None) -> List[MemoryFragment]:
    ...
    async def _deduplicate_fragments(self, fragments: List[MemoryFragment]) -> List[MemoryFragment]:
        ...
        logger.info(f"Deduplicated {len(fragments)} fragments to {len(deduplicated)} unique fragments")
        return deduplicated

    async def _score_fragments(self, fragments: List[MemoryFragment]) -> List[MemoryFragment]:
    ...
    async def _build_cross_agent_links(self, fragments: List[MemoryFragment]) -> Dict[str, List[str]]:
        ...
        for i, fragment1 in enumerate(fragments):
            for fragment2 in fragments[i+1:]:
                if fragment1.agent_id != fragment2.agent_id:
                    similarity = await self._calculate_fragment_similarity(fragment1, fragment2)
    ...
    async def _calculate_fragment_similarity(self, f1: MemoryFragment, f2: MemoryFragment) -> float:
    ...
    async def _synthesize_swarm_knowledge(self, fragments: List[MemoryFragment]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            themes = {
                "strategy": [f for f in top_fragments if "strategy" in str(f.content).lower()],
                "performance": [f for f in top_fragments if "outcome" in str(f.content).lower() or "result" in str(f.content).lower()],
                "collaboration": [f for f in top_fragments if "team" in str(f.content).lower() or "coordination" in str(f.content).lower()]
            }
    ...
    async def _synthesize_theme(self, theme: str, fragments: List[MemoryFragment]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "confidence": sum(f.importance_score for f in fragments) / len(fragments),
    ...
    async def _cleanup_expired_cache(self):
        ...
        expired_fragments = [
            fid for fid, fragment in self._fragment_cache.items()
            ...
            for synthesis in self.consolidated_memory.synthesized_knowledge:
                ...
                await self.memory_manager.l3.remember_foundation(
                    ...
                    metadata={
                        ...
                        "consolidation_timestamp": self.consolidated_memory.last_consolidation.isoformat()
                    }
                )
                
            # Store consolidation metadata to L1
            consolidation_metadata = {
                "fragment_count": len(self.consolidated_memory.fragments),
                "agent_count": len(set(f.agent_id for f in self.consolidated_memory.fragments)),
                "synthesis_count": len(self.consolidated_memory.synthesized_knowledge),
                "last_consolidation": self.consolidated_memory.last_consolidation.isoformat()
            }
            
            await self.memory_manager.l1.store(
                f"consolidation:{self.workspace_id}",
                consolidation_metadata,
    ...
    async def search_consolidated_memory(
        ...
        scored_fragments.sort(
            key=lambda x: (x[1] * 0.7 + x[0].importance_score * 0.3),
            reverse=True
        )
        ...
        logger.info(f"Search returned {len(results)} fragments for query: {query[:50]}...")
        return results

    async def get_agent_memory_summary(self, agent_id: str) -> Dict[str, Any]:
        """Returns memory summary for a specific agent."""
        agent_fragments = [
            f for f in self.consolidated_memory.fragments 
        ...
        return {
            ...
            "average_importance": sum(f.importance_score for f in agent_fragments) / len(agent_fragments) if agent_fragments else 0,
            "most_accessed": max(agent_fragments, key=lambda f: f.access_count) if agent_fragments else None,
            "recent_activity": len([f for f in agent_fragments if (datetime.now() - f.timestamp).total_seconds() < 3600])
        }
    ...
    async def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_importance": sum(f.importance_score for f in fragments) / len(fragments) if fragments else 0,
