...
from backend.memory.consolidated import (
...
class SwarmMemoryCoordinator:
    ...
    def __init__(self, workspace_id: str):
        ...
        self.consolidator = create_swarm_memory_consolidator(workspace_id)
        self.cache = SwarmMemoryCache(self.consolidator)
        ...
        try:
            ...
            if importance > 0.8:
                ...
                await self.consolidator.memory_manager.l2.store_episode(
                    ...
                    metadata={**metadata, "agent_id": agent_id, "importance": importance}
                )
            ...
            if agent_id in self.agent_memory_usage:
                ...
                self.agent_memory_usage[agent_id]["last_activity"] = datetime.now().isoformat()
            
            # Cache for fast retrieval
            cache_key = f"fragment:{fragment.id}"
            await self.cache.set(cache_key, fragment, ttl=300)  # 5 minutes
            
            logger.debug(f"Recorded memory fragment {fragment.id} from agent {agent_id}")
            return True
    ...
    async def get_agent_context(
        ...
        try:
            ...
            agent_fragments = await self.search_swarm_memory(
                query=query or "recent activity",
            ...
            if include_cross_agent and query:
                swarm_fragments = await self.search_swarm_memory(
                    query=query,
                    limit=15
                )
                # Filter out agent's own memories
                context["relevant_swarm_memory"] = [
                    f for f in swarm_fragments 
            ...
            if include_cross_agent:
                ...
                for synthesis in self.consolidator.consolidated_memory.synthesized_knowledge:
            ...
            recent_fragments = await self.search_swarm_memory(
                query="recent activity outcome result",
                limit=10
            )
    ...
    async def consolidate_swarm_memories(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            for agent_id in set(fragment.agent_id for fragment in consolidated.fragments):
                if agent_id in self.agent_memory_usage:
    ...
    async def cleanup_agent_memory(self, agent_id: str) -> bool:
        ...
        try:
            ...
            for fragment in important_fragments:
                if fragment.memory_tier == "L1":
                    ...
                    await self.consolidator.memory_manager.l2.store_episode(
                        ...
                        embedding=fragment.embedding or await self.consolidator.embedder.aembed_query(str(fragment.content)),
            ...
            logger.info(f"Cleaned up memory for agent {agent_id}, preserved {len(important_fragments)} important fragments")
            return True
    ...
    async def get_swarm_memory_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["performance"] = {
                ...
                "memory_efficiency": len(self.consolidator.consolidated_memory.fragments) / max(len(self.active_agents), 1)
            }
...
async def hydrate_state_with_swarm_memory(state: CognitiveIntelligenceState) -> CognitiveIntelligenceState:
    """
...
async def record_agent_execution(state: CognitiveIntelligenceState, result: Dict[str, Any]) -> bool:
    """
