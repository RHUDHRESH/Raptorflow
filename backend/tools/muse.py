...
from backend.core.prompts import MusePrompts, MarketingFrameworks
...
class AssetGenTool(BaseRaptorTool):
    ...
    async def _execute(self, topic: str, format: str, framework: Optional[str] = None, context: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        chain = prompt | llm
        response = await chain.ainvoke({"topic": topic, "format": format, "context": context or "No additional context."})
