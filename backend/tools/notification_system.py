...
class NotificationSystemTool(BaseRaptorTool):
    ...
    async def _execute(
        ...
        if action == "send_notification":
            return await self._send_notification(
                notification_data, recipients, channels
            )
    ...
    async def _send_notification(
        ...
        total_sent = sum(
            ...
            for result in notification["delivery_results"].values()
        )
        total_failed = sum(
        ...
        notification["delivery_summary"] = {
            ...
            "success_rate": (
                (total_sent / (len(recipients) * len(channels))) * 100
    ...
    async def _schedule_notification(
        ...
        schedule = {
            ...
            "recipients": recipients,
            "channels": channels,
            "scheduled_time": notification_data.get(
                "scheduled_time", (datetime.now() + timedelta(hours=1)).isoformat()
            ),
            "frequency": notification_data.get("frequency", "once"),
            "created_at": datetime.now().isoformat(),
            "status": "scheduled",
            "next_run": notification_data.get(
                "scheduled_time", (datetime.now() + timedelta(hours=1)).isoformat()
            ),
        }
    ...
    async def _get_notification_preferences(
        ...
        return {"success": True, "data": preferences, "action": "get_preferences"}
    ...
    async def _track_notification_delivery(
        ...
        return {"success": True, "data": tracking_data, "action": "track_delivery"}
    ...
    async def _manage_notification_templates(
        ...
        return {"success": True, "data": templates, "action": "manage_templates"}
    ...
    async def _get_notification_analytics(
        ...
        return {"success": True, "data": analytics, "action": "notification_analytics"}

    async def _send_batch_notifications(
        self, notification_data: Dict[str, Any], recipients: List[str]
    ) -> Dict[str, Any]:
        ...
        for i, batch in enumerate(batches):
            try:
                batch_result = await self._send_notification(
                    notification_data,
                    batch,
        ...
        # Generate summary
        total_sent = sum(
            result.get("delivery_summary", {}).get("total_sent", 0)
