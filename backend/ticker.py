...
class CampaignTicker:
    ...
    async def add_task(self, workspace_id: str, task_type: str, payload: Dict, scheduled_for: datetime, move_id: Optional[str] = None):
    ...
    async def _process_due_tasks(self):
        ...
        try:
            ...
            result = self.client.table("scheduled_tasks") \
                .select("*") \
                .eq("status", "pending") \
    ...
    async def _execute_task(self, task: Dict):
        ...
        try:
            # Mark as processing
            self.client.table("scheduled_tasks").update({"status": "processing"}).eq("id", task_id).execute()
            ...
            if task_type == "move_execution":
                ...
                result = await brain.run_move(move_name, context, flow_id=task.get("move_id") or "task_run")
