...
class MigrationManager:
    ...
    async def rollback(self, target_version: Optional[str] = None) -> Dict[str, any]:
        ...
        if not target_version and applied_migrations:
            ...
            target_migration = max(applied_migrations.values(), key=lambda m: m.applied_at)
        ...
        if target_version:
            ...
            versions_to_rollback = [
                v for v in applied_migrations.keys() 
                if v >= target_version
            ]
    ...
    async def get_migration_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            'latest_applied': max(applied_migrations.keys()) if applied_migrations else None,
            'all_migrations': [
                {
                    'version': m.version,
                    'name': m.name,
                    'applied': m.version in applied_migrations,
                    'applied_at': applied_migrations[m.version].applied_at if m.version in applied_migrations else None
                }
...
async def show_migration_status():
    ...
    for migration in status['all_migrations']:
        ...
        print(f"  {migration['version']}: {migration['name']} - {status_str}{applied_at}")
