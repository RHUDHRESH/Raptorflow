...
async def talk_to_fortress():
    ...
    async def mock_invoke(prompt, **kwargs):
        ...
        model_name = "unknown"
        if "Classify" in str(prompt): model_name = "Gemini 2.0 Flash (Intake)"
        elif "architect a surgical" in str(prompt): model_name = "Gemini 3 Pro (Ultra)"
        elif "Thought Leader" in str(prompt): model_name = "Gemini 2.5 Flash (Driver)"
        elif "ruthless visual editor" in str(prompt): model_name = "Gemini 1.5 Flash (Mundane)"
        
        print(f"   [AI] {model_name} is thinking...")
        return MagicMock(content=f"SOTA Output from {model_name}", model_dump=lambda: {})
    ...
    with patch("backend.inference.ChatVertexAI") as MockChat:
        ...
        mock_inst.with_structured_output.return_value.ainvoke = AsyncMock(side_effect=mock_invoke)
        ...
        mock_inst.with_structured_output.return_value.ainvoke.return_value = Intent(
            classification="campaign", confidence=0.99, reasoning="Clear campaign intent."
        )
        from backend.graphs.spine_v3 import discovery_node
        await discovery_node({"raw_prompt": user_prompt})

        print("\n--- STRATEGY PHASE (Ultra) ---")
        from backend.agents.strategists import UVPAnalysis, UVP
        mock_inst.with_structured_output.return_value.ainvoke.return_value = UVPAnalysis(
            winning_positions=[UVP(title="Logic Fortress", description="Unbreakable code.", why_it_wins="SOTA Reasoning")]
        )
        ...
        mock_inst.with_structured_output.return_value.ainvoke.return_value = LinkedInPost(
            hook="Stop guessing.", body="Code is a liability.", cta="Build a fortress.", post_vibe="Calm"
        )
