...
class BrotliMiddleware(BaseHTTPMiddleware):
    ...
    async def _compress_with_brotli(self, response: Response) -> Response:
        ...
        try:
            ...
            logger.debug(f"Compressed response with Brotli from {len(body)} to {len(compressed_body)} bytes")
...
def add_compression_middleware(
    ...
    logger.info(f"Compression middleware added (gzip: {enable_gzip}, brotli: {enable_brotli})")
