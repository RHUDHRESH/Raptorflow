...
class CacheAwareMiddleware:
    ...
    def _setup_default_rules(self):
        ...
        self.cache_rules.append(CacheRule(
            pattern=r'^/api/v1/(users|campaigns|icp)/[^/]+$',
    ...
    def get_cache_rule(self, path: str, method: str, headers: Dict[str, str]) -> Optional[CacheRule]:
    ...
    async def __call__(self, scope, receive, send):
        ...
        await self._cache_response(
            scope, receive, send, start_time, cache_key, rule, user_id, workspace_id, session_id
        )
    ...
    async def _send_cached_response(self, cached_entry: CacheEntry, send, start_time):
        ...
        cached_headers['x-cache-age'] = str(int((datetime.now() - cached_entry.timestamp).total_seconds()))
        
        if cached_entry.etag:
            cached_headers['etag'] = cached_entry.etag
        
        if cached_entry.last_modified:
            cached_headers['last-modified'] = cached_entry.last_modified.strftime('%a, %d %b %Y %H:%M:%S GMT')
        ...
        async def wrapped_send(message):
            ...
            if message.get('type') == 'http.response.start':
                ...
                if not self.should_cache_response(
                    scope['path'], scope['method'], response_status, response_headers, content_length
                ):
