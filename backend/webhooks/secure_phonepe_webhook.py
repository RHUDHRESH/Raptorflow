...
class SecurePhonePeWebhookHandler:
    ...
    def __init__(
        self, redis_client: redis.Redis, payment_repository: PaymentRepository
    ):
        ...
        self.replay_protection_window_hours = (
            24  # Track processed webhooks for 24 hours
        )
    ...
    async def validate_and_process_webhook(
        ...
        try:
            ...
            if not basic_result.valid:
                basic_result.processing_time_ms = int(
            ...
            if not rate_limit_result.valid:
                rate_limit_result.processing_time_ms = int(
                    (datetime.now() - start_time).total_seconds() * 1000
                )
            ...
            if security_level in [
                WebhookSecurityLevel.REPLAY,
                WebhookSecurityLevel.STRICT,
            ]:
                replay_result = await self._check_replay_attack(payload)
                if not replay_result.valid:
                    replay_result.processing_time_ms = int(
                        (datetime.now() - start_time).total_seconds() * 1000
                    )
            ...
            if security_level in [
                WebhookSecurityLevel.SIGNATURE,
                WebhookSecurityLevel.STRICT,
            ]:
                signature_result = await self._validate_x_verify_signature(
                    payload, x_verify_header
                )
                if not signature_result.valid:
                    signature_result.processing_time_ms = int(
                        (datetime.now() - start_time).total_seconds() * 1000
                    )
            ...
            if not timestamp_result.valid:
                timestamp_result.processing_time_ms = int(
                    (datetime.now() - start_time).total_seconds() * 1000
                )
    ...
    async def _validate_basic_structure(
        ...
        try:
            ...
            return WebhookValidationResult(
                valid=True, security_level=WebhookSecurityLevel.BASIC
            )
    ...
    async def _check_replay_attack(
        ...
        try:
            ...
            return WebhookValidationResult(
                valid=True, security_level=WebhookSecurityLevel.REPLAY
            )
    ...
    async def _validate_x_verify_signature(
        self, payload: Dict[str, Any], x_verify_header: str
    ) -> WebhookValidationResult:
        ...
        try:
            ...
            return WebhookValidationResult(
                valid=True, security_level=WebhookSecurityLevel.SIGNATURE
            )
    ...
    async def _validate_webhook_timestamp(
        self, payload: Dict[str, Any]
    ) -> WebhookValidationResult:
        ...
        try:
            ...
            if not timestamp_str:
                ...
                return WebhookValidationResult(
                    valid=True, security_level=WebhookSecurityLevel.STRICT
                )
    ...
    async def _handle_webhook_event(self, webhook_event: WebhookEvent):
        ...
        try:
            ...
            if webhook_event.event_type == WebhookEventType.PAYMENT_SUCCESS:
                ...
                logger.info(
                    f"Unhandled webhook event type: {webhook_event.event_type.value}"
                )
        ...
        try:
            if webhook_event.transaction_id:
                ...
                await self.payment_repo.update_payment_status(
                    webhook_event.transaction_id, "COMPLETED", webhook_event.raw_data
                )
    ...
    async def _handle_payment_failed(self, webhook_event: WebhookEvent):
        ...
        try:
            if webhook_event.transaction_id:
                # Update payment status in database
                await self.payment_repo.update_payment_status(
                    webhook_event.transaction_id, "FAILED", webhook_event.raw_data
                )
    ...
    async def _handle_refund_completed(self, webhook_event: WebhookEvent):
        ...
        try:
            if webhook_event.refund_id:
                ...
                success = await self.refund_manager.update_refund_from_webhook(
                ...
                if success:
                    ...
                    logger.warning(
                        f"Failed to update refund: {webhook_event.refund_id}"
                    )
        except Exception as e:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check configuration
            config_valid = all(
                [self.webhook_secret, self.webhook_username, self.webhook_password]
            )
            ...
            return {
                ...
                "message": (
                    "Webhook handler is operational"
                    if overall_healthy
                    else "Webhook handler has issues"
                ),
                "features": {
...
def get_secure_webhook_handler(
    redis_client: redis.Redis, payment_repository: PaymentRepository
    ...
    if secure_webhook_handler is None:
        secure_webhook_handler = SecurePhonePeWebhookHandler(
            redis_client, payment_repository
        )
