...
class SwarmCompetitorAdapter:
    ...
    async def enhance_swarm_state(self, state: SwarmState) -> SwarmState:
        ...
        if competitor_profiles:
            ...
            state["competitive_landscape_summary"] = self._generate_landscape_summary(competitor_profiles)
    ...
    def _generate_landscape_summary(self, profiles: List[Any]) -> str:
        ...
        market_leaders = [p for p in profiles if p.competitor_type.value == "market_leader"]
        if market_leaders:
            summary += f"Market leaders: {', '.join([p.name for p in market_leaders[:3]])}. "
        
        return summary
    
    async def route_to_competitor_intelligence(self, state: SwarmState) -> Dict[str, Any]:
...
def register_competitor_nodes_with_orchestrator(orchestrator: SwarmOrchestrator):
    ...
    thread_id = orchestrator.thread_id if hasattr(orchestrator, 'thread_id') else "default"
    
    # Register nodes
    orchestrator.register_node("competitor_intelligence", 
                             SwarmCompetitorNodeFactory.create_competitor_intelligence_node(thread_id))
    orchestrator.register_node("competitor_monitoring", 
                             SwarmCompetitorNodeFactory.create_competitor_monitoring_node(thread_id))
    orchestrator.register_node("competitor_analysis", 
...
def enhance_swarm_routing_with_competitors(orchestrator: SwarmOrchestrator):
    ...
    async def enhanced_route(query: str) -> Any:
        ...
        competitor_keywords = ["competitor", "competition", "market", "rival", "opponent"]
