...
from .utils import generate_competitor_id, validate_competitor_data, format_competitor_summary
from .monitoring import CompetitorMonitoringService, CompetitorAnalysisService, MonitoringFrequency
...
class CompetitorIntelligenceManager:
    ...
    def __init__(self, thread_id: str, config: Optional[Dict[str, Any]] = None):
        ...
        self.config = CompetitorConfig.validate_config(config or CompetitorConfig.get_default_config())
    ...
    async def discover_competitors(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Competitor discovery completed: {discovered_count} competitors found")
    ...
    async def add_competitor_manually(self, competitor_data: Dict[str, Any]) -> Dict[str, Any]:
        """Manually add a competitor profile."""
        logger.info(f"Adding competitor manually: {competitor_data.get('name', 'Unknown')}")
    ...
    # Analysis Operations
    async def analyze_competitors(self, competitor_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        """Analyze specified competitors or all known competitors."""
        logger.info(f"Starting competitor analysis for {len(competitor_ids) if competitor_ids else 'all'} competitors")
        ...
        try:
            ...
            if not competitor_ids:
                return {"error": "No competitors available for analysis", "status": "no_competitors"}
            ...
            logger.info(f"Competitor analysis completed for {len(competitor_ids)} competitors")
            return result
    ...
    async def perform_swot_analysis(self, competitor_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    async def identify_competitive_gaps(self, competitor_ids: Optional[List[str]] = None) -> List[str]:
    ...
    async def start_monitoring(self, competitor_ids: List[str], frequency: str = "daily") -> Dict[str, Any]:
        """Start monitoring specified competitors."""
        logger.info(f"Starting monitoring for {len(competitor_ids)} competitors with {frequency} frequency")
    ...
    async def generate_intelligence_report(self, competitor_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Intelligence report generated: {report.get('competitors_analyzed', 0)} competitors")
            return report
    ...
    async def generate_competitive_benchmarks(self, competitor_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            benchmarks = await self.analysis_service.generate_competitive_benchmarks(competitor_ids)
            self._last_operation_time = datetime.now()
    ...
    async def get_competitor_profiles(self, competitor_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    async def get_competitor_insights(self, competitor_id: Optional[str] = None, days_back: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if competitor_id:
                all_insights = [i for i in all_insights if i.competitor_id == competitor_id]
    ...
    async def add_competitor_insight(self, insight_data: Dict[str, Any]) -> Dict[str, Any]:
        """Add a new competitor insight."""
        logger.info(f"Adding competitor insight: {insight_data.get('title', 'Unknown')}")
    ...
    async def update_competitor_watchlist(self, competitor_ids: List[str]) -> Dict[str, Any]:
        """Update the competitor watchlist."""
        logger.info(f"Updating competitor watchlist with {len(competitor_ids)} competitors")
    ...
    async def get_competitor_summary(self, competitor_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not profile:
                return {"error": f"Competitor {competitor_id} not found", "status": "not_found"}
            
            # Get recent insights
            recent_insights = await self.get_competitor_insights(competitor_id, days_back=7)
            
            # Get analyses
            all_analyses = await self.memory_manager.get_all_competitor_analyses()
            competitor_analyses = [a for a in all_analyses if competitor_id in a.competitor_ids]
    ...
    async def cleanup(self):
        ...
        logger.info(f"Cleaning up CompetitorIntelligenceManager for thread {self.thread_id}")
    ...
    async def get_swarm_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            landscape_summary = ""
            if profiles:
                high_threat = len([p for p in profiles if p.threat_level.value in ["high", "critical"]])
                direct_competitors = len([p for p in profiles if p.competitor_type.value == "direct"])
                landscape_summary = (
                ...
                # Add top competitors
                top_competitors = sorted(profiles, key=lambda x: x.threat_level.value, reverse=True)[:3]
                landscape_summary += f"Key competitors: {', '.join([p.name for p in top_competitors])}."
            ...
            return {
                ...
                "high_threat_count": len([p for p in profiles if p.threat_level.value in ["high", "critical"]]),
                "direct_competitor_count": len([p for p in profiles if p.competitor_type.value == "direct"])
            }
