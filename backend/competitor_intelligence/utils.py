...
from .config import CompetitorConfig, CompetitorDataSource, CompetitorAlertType
...
def validate_competitor_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "confidence_score" in data and data["confidence_score"] is not None:
        try:
            ...
            logger.warning(f"Invalid confidence_score value: {data['confidence_score']}")
    
    # List fields
    for list_field in ["target_audience", "key_features", "strengths", "weaknesses", 
                      "opportunities", "threats", "marketing_channels", "tech_stack"]:
        if list_field in data and data[list_field]:
            if isinstance(data[list_field], str):
                # Convert string to list
                validated[list_field] = [item.strip() for item in data[list_field].split(',')]
            elif isinstance(data[list_field], list):
                validated[list_field] = [str(item).strip() for item in data[list_field] if item]
            else:
    ...
    for insight in insights:
        ...
        if "time_filter" in criteria:
            ...
            if discovered_at:
                ...
                if time_filter == "recent" and discovered_at > now - timedelta(days=7):
                    relevance_score += 2
                elif time_filter == "very_recent" and discovered_at > now - timedelta(days=1):
                    relevance_score += 3
        ...
        if "impact_filter" in criteria:
            ...
            if impact_filter == "high" and insight_impact in ["high", "critical"]:
                relevance_score += 3
            elif impact_filter == "medium" and insight_impact in ["medium", "high", "critical"]:
                relevance_score += 2
