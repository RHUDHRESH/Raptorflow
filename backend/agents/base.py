...
class BaseCognitiveAgent(ABC):
    ...
    async def self_correct(self, content: str, state: CognitiveIntelligenceState) -> str:
        """
    ...
    async def __call__(self, state: CognitiveIntelligenceState) -> Dict[str, Any]:
        ...
        if self.output_schema:
            ...
            metadata = getattr(response, "response_metadata", {}) if hasattr(response, "response_metadata") else {}
        else:
