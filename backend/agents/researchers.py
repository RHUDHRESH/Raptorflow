...
class RedditInsight(BaseModel):
    ...
    pain_point: str = Field(description="The specific user problem or frustration found.")
    sentiment: str = Field(description="The emotional tone (e.g., angry, confused, hopeful).")
    evidence_quote: str = Field(description="A direct quote or paraphrase from the data.")

class RedditAnalysis(BaseModel):
    """Aggregate insights from social research."""
    insights: List[RedditInsight] = Field(description="List of extracted customer insights.")
...
class RedditSentimentAnalyst:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master social listening specialist. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        raw_data = state.get("research_bundle", {}).get("raw_social_data", "No data available")
        logger.info("Analyzing Reddit sentiment...")
        
        analysis = await self.chain.ainvoke({"data": raw_data})
        logger.info(f"Reddit analysis complete. Found {len(analysis.insights)} insights.")
...
class LinkedInAnalysis(BaseModel):
    ...
    profiles: List[LinkedInProfile] = Field(description="List of analyzed thought leaders.")
...
class LinkedInProfiler:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert executive headhunter and trend analyst. "
    ...
    async def __call__(self, state: TypedDict):
        """Node execution logic."""
        raw_data = state.get("research_bundle", {}).get("raw_linkedin_data", "No data available")
        logger.info("Profiling LinkedIn thought leaders...")
        
        analysis = await self.chain.ainvoke({"data": raw_data})
        logger.info(f"LinkedIn profiling complete. Profiled {len(analysis.profiles)} leaders.")
...
class CompetitorFeatureMapper:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a surgical product marketer. "
    ...
    async def __call__(self, state: TypedDict):
        """Node execution logic."""
        raw_data = state.get("research_bundle", {}).get("raw_competitor_data", "No data available")
...
class EvidenceBundler:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a research analyst. Consolidate the following raw search data "
...
class SourceValidator:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a factual verification specialist. "
...
class TrendExtractor:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master trend forecaster. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(f"Trend extraction complete. Identified {len(analysis.trends)} trends.")
...
class GapFinder:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master of competitive differentiation. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(f"Gap identification complete. Found {len(analysis.gaps)} opportunities.")
        
        return {"research_bundle": {"gaps": analysis.model_dump()}}

from backend.db import summarize_recursively, get_memory, save_entity
...
class CompetitorTracker:
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(f"Syncing competitor '{comp_map['brand_name']}' to permanent memory...")
...
class VisualTrendAnalyzer:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master visual creative director. "
...
class PositioningMap(BaseModel):
    ...
    x_axis: str
    y_axis: str
    competitor_coordinates: Dict[str, Dict[str, float]] # e.g., {"CompA": {"x": 0.5, "y": 0.2}}
...
class PositioningMapper:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master of strategic market mapping. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke({"data": str(comp_data)})
        logger.info(f"Market mapping complete for {len(analysis.competitor_coordinates)} entities.")
...
class SWOTAnalyzer:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master strategic consultant. "
...
class ResearchQAGuard:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a ruthless editorial research lead. "
