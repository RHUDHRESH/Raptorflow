...
class IntentClassifier:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a tactical marketing intake specialist. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Intent classified as '{intent.classification}' with {intent.confidence*100}% confidence."
        )

        return {
            "next_node": intent.classification,
            "context_brief": {"intent": intent.model_dump()},
        }
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical marketing strategist. The user's prompt is too vague. "
