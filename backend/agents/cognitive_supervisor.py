...
class SupervisorDecision(BaseModel):
    ...
    next_action: str = Field(description="The next status or node to transition to: 'planning', 'researching', 'execution', or 'complete'.")
    rationale: str = Field(description="Reasoning for the delegation decision.")
    crew_instructions: str = Field(description="Specific surgical instructions for the chosen crew.")
...
class CognitiveSupervisor:
    ...
    def __init__(self, llm: Optional[Any] = None):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ...
            ("user", "Current Status: {status}. Quality Score: {quality_score}. Decide the next strategic move.")
        ])
        self._chain = None
    ...
    def chain(self):
        if self._chain is None:
            self._chain = self.prompt | self.llm.with_structured_output(SupervisorDecision)
        return self._chain
    ...
    async def __call__(self, state: CognitiveIntelligenceState) -> Dict[str, Any]:
        ...
        logger.info(f"Supervisor Decision: {decision.next_action}. Rationale: {decision.rationale}")
        ...
        return {
            ...
            "messages": [AgentMessage(role="supervisor", content=f"DELEGATE: {target_action}. {decision.crew_instructions}")]
        }
