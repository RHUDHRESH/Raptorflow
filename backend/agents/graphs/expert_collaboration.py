...
async def researcher_node(state: ExpertState) -> ExpertState:
    ...
    try:
        ...
        if result.success:
            ...
            state["collaboration_logs"] = ["Researcher: Completed deep dive using Titan Engine."]
        else:
...
async def strategist_node(state: ExpertState) -> ExpertState:
    ...
    try:
        ...
        state["context_summary"] = f"Research Findings: {state.get('research_data', {}).get('intelligence_map', {}).get('summary', 'No summary available.')}"
        ...
        if result_state.get("output"):
            ...
            state["collaboration_logs"] = ["Strategist: Synthesized research into a strategic move plan."]
...
async def creator_node(state: ExpertState) -> ExpertState:
    ...
    try:
        ...
        strategy_summary = state.get("strategy_plan", {}).get("move_strategy", {}).get("description", "No strategy description.")
        ...
        if result_state.get("output"):
            ...
            state["collaboration_logs"] = ["Creator: Generated campaign assets aligned with strategy."]
...
async def director_node(state: ExpertState) -> ExpertState:
    ...
    try:
        ...
        if result_state.get("output"):
            ...
            state["collaboration_logs"] = [f"Director: Quality review {'PASSED' if is_passed else 'FAILED'}."]
...
def should_continue(state: ExpertState) -> Literal["strategist", "creator", "director", "end"]:
