...
class LangGraphOptionalOrchestrator:
    ...
    async def _resolve_runtime_profile(self, state: OptionalModuleState) -> OptionalModuleState:
        payload = state.get("payload") or {}
        intensity = normalize_intensity(payload.get("intensity") or settings.AI_DEFAULT_INTENSITY)
        execution_mode = normalize_execution_mode(payload.get("execution_mode") or settings.AI_EXECUTION_MODE)
        return {
            "intensity": intensity,
    ...
    async def _finalize(self, state: OptionalModuleState) -> OptionalModuleState:
        ...
        runtime_intensity = state.get("intensity") or normalize_intensity(settings.AI_DEFAULT_INTENSITY)
        module_profile = (intensity_profile(runtime_intensity).get(state["operation"]) or {})
        result.setdefault("orchestrator", "langgraph")
        result.setdefault("module", state["operation"])
        result.setdefault("execution_mode", state.get("execution_mode") or normalize_execution_mode(settings.AI_EXECUTION_MODE))
