...
class ICPDemographics(BaseModel):
    ...
    target_role: str = Field(
        description="The job title or role (e.g., Early-stage founder)."
    )
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of market segmentation. "
...
class ICPPsychographics(BaseModel):
    ...
    core_motivations: List[str] = Field(
        description="Top 3 drivers (e.g., status, speed, security)."
    )
    perceived_risks: List[str] = Field(
        description="Fear-based blockers (e.g., vendor lock-in)."
    )
    values_alignment: List[str] = Field(
        description="What they care about (e.g., transparency)."
    )
...
class ICPPsychographicProfiler:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of consumer psychology. "
                    "Based on the research evidence provided, define the "
    ...
    async def __call__(self, state: TypedDict):
        ...
        profile = await self.chain.ainvoke({"evidence": str(evidence)})
        logger.info(
            f"Psychographic profiling complete. Found {len(profile.core_motivations)} motivations."
        )
...
class PainPointMapper:
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Pain-point mapping complete. Found {len(analysis.burning_problems)} problems."
        )
...
class UVPArchitect:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master positioning architect. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"pain_points": str(pain_points), "evidence": str(evidence)}
        )
        logger.info(
            f"UVP architecture complete. Drafted {len(analysis.winning_positions)} positions."
        )
...
class BrandVoiceAligner:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical brand guardian. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"brand_kit": str(brand_kit), "uvps": str(uvps)}
        )
...
class AntiPersonaProfiler:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of strategic focus. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"evidence": str(evidence), "icp": str(icp)}
        )
        logger.info(
            f"Anti-persona profiling complete. Defined {len(analysis.anti_personas)} segments to avoid."
        )
...
class CategoryArchitect:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of category design. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"uvps": str(uvps), "evidence": str(evidence)}
        )
        logger.info(
            f"Category architecture complete. Proposed {len(analysis.proposals)} names."
        )
...
class TaglineGenerator:
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Tagline generation complete. Created {len(analysis.taglines)} hooks."
        )
...
class StrategyReplanner:
    ...
    def __init__(self, llm: any):
        ...
        self.prompt = ChatPromptTemplate.from_messages(
            [
                ...
                (
                    "user",
                    "The current UVPs are not aligned with our 'Calm, Expensive' vibe. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        avg_score = (
            sum(a["score"] for a in alignments) / len(alignments) if alignments else 0
        )
        ...
        if avg_score < 0.7:
            ...
            return {
                "next_node": "pivot",
                "context_brief": {"pivot_instruction": res.content},
            }
...
class CampaignArcDesigner:
    ...
    async def __call__(self, state: TypedDict):
        ...
        arc = await self.chain.ainvoke(
            {"uvps": str(uvps), "evidence": "\n".join(evidence)}
        )
...
class MoveSequencer:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of high-velocity marketing execution. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Move sequencing complete. Generated {len(sequence.moves)} weekly packets."
        )
...
class ChannelSelector:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of distribution strategy. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"icp": str(icp), "evidence": str(evidence)}
        )
        logger.info(
            f"Channel selection complete. Selected {len(analysis.selected_channels)} channels."
        )
...
class KPIDefiner:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical data analyst. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"channels": str(channels), "arc": str(arc)}
        )
        logger.info(
            f"KPI definition complete. Identified {len(analysis.kpis)} metrics."
        )
...
class FunnelDesigner:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of conversion rate optimization. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        analysis = await self.chain.ainvoke(
            {"arc": str(arc), "channels": str(channels)}
        )
...
class StrategicConflictResolver:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical brand continuity lead. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Strategic conflict check complete. Safe to proceed: {analysis.is_safe_to_proceed}"
        )
...
class StrategyRefreshHook:
    ...
    async def __call__(self, state: TypedDict):
        ...
        if len(evidence.get("key_findings", [])) < 2:
            logger.warning(
                "Strategic evidence too thin. Requesting research refresh..."
            )
            res = await self.llm.ainvoke(
                "The research is insufficient. Generate a specific query to find more competitive data."
            )
            return {
                "next_node": "research_refresh",
                "context_brief": {"refresh_instruction": res.content},
            }
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a world-class executive coach. "
