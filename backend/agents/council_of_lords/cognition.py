...
class CognitionLord(BaseAgent):
    ...
    async def _synthesize_knowledge(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            description_input = kwargs.get("description", "")
            learning_ids = kwargs.get("learning_ids", [])
            
            # Fetch actual learning content
            learnings_content = []
            for lid in learning_ids:
                if lid in self.learnings:
                    l = self.learnings[lid]
                    learnings_content.append(f"- [{l.type}] {l.key_insight} ({l.description})")
            
            # Use LLM to synthesize if we have learnings, otherwise use input
            description = description_input
            recommendations = kwargs.get("recommendations", [])
            
            if learnings_content:
                prompt = f"""
                You are the Cognition Lord. Synthesize the following organizational learnings into a cohesive insight.
                
                Synthesis Type: {synthesis_type.value}
                Context: {description_input}
                
                Learnings:
                {chr(10).join(learnings_content)}
                ...
                try:
                    ...
                    title = result.get("synthesis_title", title)
                    description = result.get("synthesis_description", description)
                    recommendations = result.get("key_recommendations", recommendations)
                except Exception as e:
    ...
    async def _make_decision(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if synthesis_ids:
                ...
                syntheses_text = "\n".join([f"- {s.title}: {s.description}" for s in valid_syntheses])

            # Use LLM for recommendation
            prompt = f"""
            You are the Cognition Lord. Make a strategic decision based on the following context.
            ...
            try:
                ...
                decision_confidence = result.get("confidence", 0.8)
                impact_forecast = result.get("impact_forecast", impact_forecast)
            except Exception as e:
                ...
                if valid_syntheses:
                    ...
                    if strongest.recommendations:
                        recommendation = f"Based on {strongest.title}: {strongest.recommendations[0]}"
    ...
    async def _mentor_agent(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use LLM to generate personalized mentoring
            prompt = f"""
            You are the Cognition Lord providing mentoring to an agent.
            ...
            Relevant Insights:
            {json.dumps(mentoring_points, indent=2)}
            
            Provide a concise, actionable mentoring summary (3-4 sentences) and 3 specific action items.
            Return JSON: {{ "summary": "string", "action_items": ["string"] }}
            """
            ...
            try:
                ...
                mentoring_summary = result.get("summary", mentoring_summary)
                # Append action items to points or summary
                if result.get("action_items"):
                    mentoring_summary += "\n\nAction Items:\n" + "\n".join([f"- {item}" for item in result["action_items"]])
            except Exception as e:
                logger.error(f"LLM mentoring failed: {e}")
                if mentoring_points:
                    mentoring_summary += f"\n\nKey Points:\n"
