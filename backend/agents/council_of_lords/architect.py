...
class ArchitectLord(LordAgent, AgentInterface):
    ...
    async def _design_initiative(
        ...
        **kwargs
        ...
        # Generate initiative design using LLM
        prompt = f"""
        You are the Architect Lord, responsible for strategic planning and system architecture.
        Design a strategic initiative with the following parameters:
        - Name: {initiative_name}
        - Objectives: {objectives}
        - Target Guilds: {target_guilds}
        - Timeline: {timeline_weeks} weeks
        - Success Metrics: {success_metrics}

        Context from knowledge base:
        {context.get('retrieved_knowledge', [])}

        Return a JSON object with the following structure:
        {{
            "phases": [
                {{"name": "string", "duration_weeks": int, "deliverables": ["string"]}}
            ],
            "resource_allocation": {{
                "guild_name": {{"agents": int, "budget": int, "role": "string"}}
            }},
            "risk_assessment": {{
                "high_risks": ["string"],
                "mitigation": ["string"]
        ...
        try:
            design = await vertex_ai_client.generate_json(
                prompt=prompt,
                system_prompt="You are the Architect Lord. Output valid JSON only.",
                model_type="reasoning"
            )
            design["knowledge_base_support"] = len(context.get("retrieved_knowledge", []))
        except Exception as e:
            logger.error(f"LLM generation failed: {e}. Falling back to default design.")
            # Fallback to deterministic design
            design = {
                "phases": [
                ...
                "resource_allocation": {
                    guild: {"agents": 2, "budget": 5000, "role": "Execution"}
                    for guild in target_guilds
                },
                "risk_assessment": {
                    "high_risks": ["Resource constraints", "Timeline slippage"],
