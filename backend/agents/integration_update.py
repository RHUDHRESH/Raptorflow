...
def update_agent_imports_and_tools():
    ...
    for agent_path, config in agent_updates.items():
        try:
            ...
            logger.info(
                f"Would update {agent_path} with role {config['role']} and tools {config['tools']}"
            )
    ...
    integration_status["integration_summary"] = {
        "most_used_tools": sorted(
            tool_usage_summary.items(), key=lambda x: x[1], reverse=True
        ),
...
def create_agent_tool_matrix():
    ...
    for tool in tool_matrix["tools"]:
        usage_count = sum(
            1 for agent_tools in tool_matrix["agents"].values() if tool in agent_tools
        )
        tool_usage[tool] = usage_count

    tool_matrix["tool_usage_stats"] = tool_usage
    tool_matrix["most_used_tools"] = sorted(
...
if __name__ == "__main__":
    ...
    print(
        f"Total Tool Assignments: {update_status['integration_summary']['total_tool_assignments']}"
    )
    print(
        f"Unique Tools Used: {update_status['integration_summary']['unique_tools_used']}"
    )
