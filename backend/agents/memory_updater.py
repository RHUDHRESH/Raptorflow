...
class StylePreference(BaseModel):
    key: str = Field(description="The preference key (e.g. 'emoji_usage', 'sentence_length')")
    value: str = Field(description="The learned rule")
    confidence: float = Field(description="Confidence 0-1")

class MemoryUpdaterAgent:
    def __init__(self):
        self.llm = InferenceProvider.get_model(model_tier="smart").with_structured_output(StylePreference)

    async def learn_from_edit(self, draft: str, final: str) -> Optional[StylePreference]:
