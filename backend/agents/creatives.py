...
class LinkedInArchitect:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master ghostwriter for high-stakes founders. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        context = state.get("research_bundle", {}).get("executive_brief", "General marketing")
...
class TwitterArchitect:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master of high-velocity social signals. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        thread = await self.chain.ainvoke({"brief": str(brief), "context": str(context)})
...
class ContentTransformer:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master editorial lead. "
    ...
    async def __call__(self, state: TypedDict):
        """Node execution logic."""
        brief = state.get("research_bundle", {}).get("executive_brief", "General marketing research")
        ...
        return {"variants": [post.model_dump()], "current_draft": post.introduction + "\n" + post.conclusion}
...
class ToneAdjuster:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master brand editor. Fine-tune the following copy. "
...
class VisualPrompter:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master visual director. Generate a surgical image prompt "
...
class DiagramArchitect:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a master technical illustrator. Generate a surgical SVG diagram "
...
class MultiVariantGenerator:
    ...
    def __init__(self, llm: any):
        self.llm = llm
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a surgical creative director. Generate a unique variant "
    ...
    async def __call__(self, state: TypedDict):
        ...
        variants = [{"variant_index": i, "content": res.content} for i, res in enumerate(responses)]
...
class FormattingFilter:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a ruthless visual editor. Clean the provided content. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(f"Formatting complete. Emojis detected: {lint.has_prohibited_emojis}")
