...
class DatabaseTool(RaptorflowTool):
    ...
    async def _arun(
        ...
        try:
            # Generate cache key
            cache_key = self._generate_cache_key(table, workspace_id, filters, limit, offset)
            ...
            if query_time > 1.0:  # Slow query threshold
                ...
                logger.warning(f"Slow database query detected: {cache_key} took {query_time:.2f}s")
            
            logger.debug(f"Database query executed successfully: {cache_key} in {query_time:.2f}s")
    ...
    async def _execute_new_database_query(
        self, db_service, table: str, workspace_id: str, 
    ...
    async def _execute_pooled_query(
        self, db_pool, table: str, workspace_id: str, 
        filters: Dict[str, Any], limit: int, offset: int
    ) -> Dict[str, Any]:
    ...
    def _optimize_filters(self, table: str, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        index_priority = {
            "id": 1, "workspace_id": 2, "created_at": 3, "updated_at": 4,
            "status": 5, "name": 6, "type": 7
        }
        ...
        sorted_filters = sorted(
            filters.items(),
            key=lambda x: index_priority.get(x[0], 99)
        )
    ...
    def _build_optimized_supabase_query(
        self, supabase, table: str, workspace_id: str,
        filters: Dict[str, Any], limit: int, offset: int
    ):
    ...
    def _generate_cache_key(
        self, table: str, workspace_id: str, 
        filters: Dict[str, Any], limit: int, offset: int
    ) -> str:
