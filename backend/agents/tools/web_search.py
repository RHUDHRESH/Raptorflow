...
class WebSearchTool(RaptorflowTool):
    ...
    async def _arun(self, query: str, max_results: int = 10, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not query or len(query.strip()) < 3:
                return ToolResult(
                    success=False, error="Search query too short. Minimum 3 characters."
                )
            ...
            logger.info(
                f"Native Search Machine: '{query}' returned {len(results)} results in {latency_ms}ms"
            )

            return ToolResult(success=True, data=output, latency_ms=latency_ms)
