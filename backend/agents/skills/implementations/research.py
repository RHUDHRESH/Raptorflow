...
class WebDeepDiveSkill(Skill):
    def __init__(self):
        super().__init__(
            ...
            capabilities=["Detailed topic research", "Fact gathering", "Source citation"]
        )
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await agent._call_llm(f"Research Topic: {topic}", system_prompt=system_prompt)
...
class CompetitorScoutSkill(Skill):
    def __init__(self):
        super().__init__(
            ...
            capabilities=["Competitor analysis", "USP identification", "Pricing analysis"]
        )
...
class TrendSpotterSkill(Skill):
    def __init__(self):
        super().__init__(
            ...
            capabilities=["Trend analysis", "Viral topic identification", "Market shift detection"]
        )
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await agent._call_llm(f"Analyze Trends for Industry: {industry}", system_prompt=system_prompt)
...
class KeywordDominanceSkill(Skill):
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        response = await agent._call_llm(f"Seed Keyword: {seed_keyword}", system_prompt=system_prompt)
...
class SentimentGaugeSkill(Skill):
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await agent._call_llm(f"Analyze Text: {text[:2000]}", system_prompt=system_prompt)
