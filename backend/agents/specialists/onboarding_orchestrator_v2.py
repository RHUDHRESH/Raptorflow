...
Always be systematic, thorough, and user-focused in your orchestration."""
    ...
    async def _start_session(
        ...
        try:
            ...
            onboarding_state = self.state_manager.create_session(
                session_id, workspace_id
            )

            # Start first step
            success, message = self.state_manager.start_step(
                session_id, OnboardingStep.EVIDENCE_VAULT
            )
    ...
    async def _advance_step(
        ...
        try:
            ...
            if (
                current_step_state
                and current_step_state.status == StepStatus.IN_PROGRESS
            ):
            ...
            # Advance to next step
            success, next_step, message = self.state_manager.advance_to_next_step(
                session_id
            )
            ...
            return OrchestrationResult(
                ...
                data={
                    ...
                    "estimated_duration": (
                        step_def.estimated_duration if step_def else 0
                    ),
                },
                next_actions=[
                    (
                        f"Complete {next_step.value.replace('_', ' ').title()}"
                        if not requires_ai
                        else "Wait for AI processing"
                    )
                ],
            )
    ...
    async def _process_step(
        ...
        try:
            ...
            return OrchestrationResult(
                ...
                session_id=session_id,
                step=current_step,
                data={"processing_completed": True, "agent_results": agent_result},
    ...
    async def _validate_step(
        ...
        try:
            ...
            current_step = session.current_step
            is_valid, errors = self.state_manager.validate_step_completion(
            ...
            return OrchestrationResult(
                ...
                session_id=session_id,
                step=current_step,
                data={"validation_passed": is_valid, "validation_errors": errors},
            ...
            for task in tasks:
                ...
                if agent:
                    result = await self._execute_agent_task(
                        agent, task.step, task.input_data
                    )
    ...
    async def _handle_unknown_action(
        self, session_id: str, action: OrchestratorAction, state: AgentState
    ) -> OrchestrationResult:
    ...
    def _prepare_agent_input(
        self, session_id: str, step: OnboardingStep, state: AgentState
    ) -> Dict[str, Any]:
        ...
        if step == OnboardingStep.AUTO_EXTRACTION:
            ...
            input_data["evidence"] = evidence_data.get("files", []) + evidence_data.get(
                "urls", []
            )

        elif step == OnboardingStep.CONTRADICTION_CHECK:
            # Get extracted facts
            facts_data = session.steps.get(
                OnboardingStep.AUTO_EXTRACTION, {}
            ).ai_results
            ...
            competitors_data = session.steps.get(
                OnboardingStep.COMPETITOR_ANALYSIS, {}
            ).data
            input_data["company_info"] = self._extract_company_info(
                session.steps.get(OnboardingStep.EVIDENCE_VAULT, {}).data
            )
            ...
            positioning_data = session.steps.get(
                OnboardingStep.PERCEPTUAL_MAP, {}
            ).ai_results
    ...
    async def _execute_agent_task(
        self, agent, step: OnboardingStep, input_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute task with specific AI agent"""
        try:
            if step == OnboardingStep.AUTO_EXTRACTION:
                result = await agent.extract_facts_from_evidence(
                ...
                result = await agent.analyze_reddit_market(
                ...
                result = await agent.generate_copywriting_campaign(
                    input_data.get("product_info", {})
                )
                return {"success": True, "result": result}
    ...
    def _format_orchestration_response(self, result: OrchestrationResult) -> str:
        ...
        if result.step:
            response += (
                f"Current Step: {result.step.value.replace('_', ' ').title()}\n\n"
            )
