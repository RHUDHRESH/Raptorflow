...
class ICPArchitectAgent:
    ...
    async def build_profile(self, company_context: str) -> AgentResponse:
        ...
        hypothesis_prompt = (
            f"Given this context: {company_context}, hypothesize 3 specific, "
            "non-obvious segments that would pay 5x the average."
        )
        hypotheses = await self.researcher.ainvoke(
        ...
        """
        profile = await self.llm.ainvoke(
            [SystemMessage(content=system), HumanMessage(content=hypotheses.content)]
        )
        ...
        return AgentResponse(
            ...
            inner_monologue=[
                CognitiveStep(
                    thought="Hypothesized segments, then deep-profiled the highest value one."
                )
            ],
        )
...
class CompetitorAnalystAgent:
    ...
    async def gap_analysis(
        self, company_usp: List[str], competitors: List[str]
    ) -> AgentResponse:
        ...
        return AgentResponse(
            content=res.content,
            rationale="Competitor gap analysis completed.",
...
class OfferArchitectAgent:
    ...
    async def structure_offer(self, product_details: str) -> AgentResponse:
        system = (
            "You are the Offer Architect. Increase perceived value. Decrease "
            "effort. Add urgency. Add proof."
        )
        res = await self.llm.ainvoke(
            [SystemMessage(content=system), HumanMessage(content=product_details)]
        )
        return AgentResponse(
            content=res.content,
            rationale="Offer structured for maximum perceived value.",
