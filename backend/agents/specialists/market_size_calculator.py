...
class MarketSizeCalculator(BaseAgent):
    ...
    async def calculate_market_size(self, company_info: Dict[str, Any]) -> MarketSizeResult:
        """Generation logic"""
        tam = MarketSizeData(
            tier=MarketTier.TAM, value=50e9, value_formatted="$50B", percentage_of_tam=100.0,
            description="Global Cybersecurity Market", calculation_method=CalculationMethod.TOP_DOWN,
            assumptions=["Gartner 2025"], data_sources=["Analyst Reports"], confidence_level=0.8, growth_rate=0.12
        )
        sam = MarketSizeData(
            tier=MarketTier.SAM, value=5e9, value_formatted="$5B", percentage_of_tam=10.0,
            description="Enterprise AI Threat Detection", calculation_method=CalculationMethod.TOP_DOWN,
            assumptions=["Fortune 500 focus"], data_sources=["Market research"], confidence_level=0.7
        )
        som = MarketSizeData(
            tier=MarketTier.SOM, value=100e6, value_formatted="$100M", percentage_of_tam=0.2,
            description="Year 3 obtainable target", calculation_method=CalculationMethod.BOTTOM_UP,
            assumptions=["Sales team capacity"], data_sources=["Internal benchmarks"], confidence_level=0.6
        )
        
        viz = MarketVisualization(
            concentric_circles=[], funnel_data=[], key_insights=["High growth sector"],
            comparison_benchmarks=[], visualization_type="concentric"
        )
        
        return MarketSizeResult(
            tam=tam, sam=sam, som=som, visualization=viz,
