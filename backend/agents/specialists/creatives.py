...
from models.cognitive import AgentResponse, CognitiveStep, ModelTier, MarketingBrief
...
class EmailSpecialistAgent:
    ...
    async def generate_variants(self, brief: MarketingBrief, count: int = 2) -> List[AgentResponse]:
        ...
        for i in range(count):
            ...
            monologue = [
                CognitiveStep(thought=f"Attempting variant {i+1}. Focus: {['Problem-led', 'Outcome-led', 'Curiosity-led'][i % 3]}"),
            ]
            
            # Step 2: Generation
            res = await self.generator.ainvoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content=f"Goal: {brief.goal}\nAudience: {brief.target_audience}\nUSPs: {brief.key_usp}")
            ])
            
            # Step 3: Self-Optimization (The SOTA Secret)
            optimization_prompt = f"Critique and shorten this email. Remove 20% of the words without losing impact.\n\nCONTENT: {res.content}"
            optimized = await self.optimizer.ainvoke([SystemMessage(content=optimization_prompt)])
...
class SocialSpecialistAgent:
    ...
    async def draft_post(self, topic: str, platform: str = "linkedin") -> AgentResponse:
        ...
        res = await self.llm.ainvoke([SystemMessage(content=system), HumanMessage(content=topic)])
        return AgentResponse(content=res.content, rationale="Drafted based on CEO ghostwriting patterns.", confidence_score=0.9)
