...
class LoadBalancer:
    ...
    async def initialize(self, agent_registry: AgentRegistry) -> None:
        ...
        logger.info(f"Initialized {len(self.workers)} workers for {len(available_agents)} agents")
        ...
        for worker_id, worker in self.workers.items():
            ...
            if worker.is_busy and worker.current_request:
                ...
                if time_since_activity > self.config.request_timeout:
                    ...
                    logger.warning(f"Worker {worker_id} appears stuck (busy for {time_since_activity}s)")
    ...
    async def _check_circuit_breaker(self) -> None:
        ...
        if total_failures >= self.config.circuit_breaker_threshold:
            if not self.circuit_breaker.is_open:
                ...
                self.circuit_breaker.next_attempt_time = datetime.now() + timedelta(seconds=self.config.circuit_breaker_timeout)
    ...
    async def _select_worker(self, request_data: Dict[str, Any]) -> Optional[AgentWorker]:
    ...
    def _consistent_hash_select(self, workers: List[AgentWorker], request_data: Dict[str, Any]) -> AgentWorker:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        worker_metrics = {}
        for worker_id, worker in self.workers.items():
            error_rate = worker.error_count / worker.request_count if worker.request_count > 0 else 0
            worker_metrics[worker_id] = {
                "request_count": worker.request_count,
                "error_count": worker.error_count,
                "error_rate": error_rate,
                "avg_response_time": sum(worker.response_times) / len(worker.response_times) if worker.response_times else 0,
