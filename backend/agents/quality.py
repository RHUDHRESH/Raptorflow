...
class BrandGuardian:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a ruthless editorial guardian. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        guidelines = state.get("context_brief", {}).get(
            "brand_kit", "Default RaptorFlow SOTA Rules"
        )

        logger.info("Guardian is auditing brand alignment...")
        audit = await self.chain.ainvoke(
            {"draft": draft, "guidelines": str(guidelines)}
        )
...
class HypeWordFilter:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of editorial restraint. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Hype filtering complete. Found {len(lint.banned_words_found)} banned words."
        )

        return {
            "current_draft": lint.clean_content,
            "telemetry": [
                {"banned_words": lint.banned_words_found, "hype_score": lint.hype_score}
            ],
        }
...
class StatsScorer:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical linguistic analyst. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        return {"telemetry": [stats.model_dump()]}
...
class RefinerNode:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical creative refiner. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        res = await self.llm.ainvoke(
            self.prompt.format(draft=draft, feedback=str(feedback))
        )
...
class RedTeamAgent:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of strategic risk management. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Red Teaming complete. Found {len(analysis.risks)} strategic risks."
        )

        return {"telemetry": [{"risks": analysis.model_dump()["risks"]}]}
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of marketing compliance. "
...
class QualityGate:
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Quality Gate Evaluating: Score={score}, Brand={brand_pass}, Legal={legal_pass}"
        )
...
class UIFeedbackIntegrator:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a surgical creative coordinator. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        return {"messages": [f"UI Feedback: {res.content}"], "next_node": "refine"}
...
class MemoryGovernor:
    ...
    def __init__(self, llm: any):
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a master of strategic knowledge management. "
    ...
    async def __call__(self, state: TypedDict):
        ...
        logger.info(
            f"Memory decision complete. Should remember: {decision.should_remember}"
        )
