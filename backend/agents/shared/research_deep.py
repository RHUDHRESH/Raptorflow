...
class ResearchDeepAgent:
    ...
    def __init__(self):
        self.planner = InferenceProvider.get_model(model_tier="smart").with_structured_output(ResearchPlan)
        self.synthesizer = InferenceProvider.get_model(model_tier="ultra").with_structured_output(DeepInsight)
        self.engine = ResearchEngine()
        self.search_api = SearchProvider(api_key=os.getenv("SERPER_API_KEY", ""))

    async def execute(self, task: str, context: dict = {{}}) -> DeepInsight:
        # Step 1: Query Planning
        plan = await self.planner.ainvoke([
            SystemMessage(content="You are the RaptorFlow Lead Researcher. Create a surgical plan to gather evidence for the user's task."),
        ...
        data_packet = "\n\n".join([f"SOURCE: {d['url']}\nCONTENT: {d['content'][:3000]}" for d in scraped_data])
