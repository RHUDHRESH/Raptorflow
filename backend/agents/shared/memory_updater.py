...
class StyleRule(BaseModel):
    key: str = Field(
        description="The preference key (e.g. 'emoji_usage', 'paragraph_style')"
    )
    value: str = Field(
        description=(
            "The learned rule (e.g. 'Never use emojis', "
            "'Max 2 sentences per paragraph')"
        )
    )
    confidence: float = Field(description="Confidence 0-1. Only return if > 0.8")
    evidence: str = Field(
        description="One sentence explaining what changed to trigger this rule"
    )
...
class MemoryUpdaterAgent:
    def __init__(self):
        self.llm = InferenceProvider.get_model(
            model_tier="smart"
        ).with_structured_output(StyleRule)
