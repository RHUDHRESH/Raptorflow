...
class IntentRouter:
    def __init__(self):
        self.llm = InferenceProvider.get_model(model_tier="fast").with_structured_output(Intent)

    async def execute(self, prompt: str) -> Intent:
        return await self.llm.ainvoke([
            SystemMessage(content="Route intent surgically. Classify into family and extract entities."),
...
class QualityGate:
    def __init__(self):
        self.llm = InferenceProvider.get_model(model_tier="smart").with_structured_output(QualityCheck)

    async def audit(self, content: str, brief: dict) -> QualityCheck:
        return await self.llm.ainvoke([
            SystemMessage(content=f"You are the RaptorFlow Quality Gate. Audit against brief: {brief}. Be brutal."),
...
class Telemetry:
    ...
    def log_run(thread_id: str, agent_id: str, cost: float, success: bool):
        ...
        print(f"[TELEMETRY] Thread: {thread_id} | Agent: {agent_id} | Cost: ${cost:.4f} | Success: {success}")
