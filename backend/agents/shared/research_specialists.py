...
from models.research_schemas import ResearchHypothesis, FactClaim, WebDocument, SearchResult
...
class SynthesisAgent:
    ...
    async def generate_report(self, objective: str, verified_claims: List[FactClaim]) -> str:
        system = SystemMessage(content="""
        ...
        """)
        claims_json = json.dumps([c.dict() for d in verified_claims])
        user = HumanMessage(content=f"OBJECTIVE: {objective}\n\nVERIFIED EVIDENCE:\n{claims_json}")
        res = await self.llm.ainvoke([system, user])
        return res.content
