...
class DailyReflection(BaseModel):
    summary: str = Field(description="High-level narrative of the day's agentic activity.")
    learnings: List[str] = Field(description="Surgical learnings extracted from successes and failures.")
    confidence: float = Field(description="Score indicating how well the data supports these learnings.")
...
class MemoryReflectionAgent:
    ...
    def __init__(self, model_tier: str = "smart"):
        self.llm = InferenceProvider.get_model(model_tier=model_tier).with_structured_output(DailyReflection)

    async def reflect_on_traces(self, workspace_id: str, traces: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyzes a batch of traces to generate a daily summary and learnings."""
        if not traces:
            return {"summary": "No activity detected.", "learnings": [], "confidence": 1.0}
            
        logger.info(f"MemoryReflectionAgent: Analyzing {len(traces)} traces for workspace {workspace_id}")
