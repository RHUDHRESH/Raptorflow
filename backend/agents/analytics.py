...
class AgentAnalytics:
    ...
    def __init__(self, config: AnalyticsConfig):
        ...
        self._performance_cache: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
        ...
        try:
            ...
            for agent_name, metrics in agent_metrics.items():
                if agent_name not in self.usage_patterns:
                    self.usage_patterns[agent_name] = UsagePattern(agent_name=agent_name)
                ...
                # Find peak hour
                if metrics.execution_count > pattern.hourly_usage.get(pattern.peak_hour, 0):
                    pattern.peak_hour = current_hour
                
                # Calculate average
                total_hourly = sum(pattern.hourly_usage.values())
                pattern.avg_requests_per_hour = total_hourly / 24 if total_hourly > 0 else 0
        ...
        try:
            ...
            for agent_name, metrics in agent_metrics.items():
                ...
                if len(cache) >= 10:
                    ...
                    older_values = list(cache)[-20:-10] if len(cache) >= 20 else list(cache)[:-10]
                    ...
                    if older_values:
                        ...
                        # Generate insight
                        confidence = min(len(recent_values) / 100, 1.0)  # Max confidence
                        recommendation = self._generate_performance_recommendation(trend, recent_avg, agent_name)
    ...
    def _generate_performance_recommendation(self, trend: str, current_value: float, agent_name: str) -> str:
            ...
            for agent_name, metrics in agent_metrics.items():
                if agent_name not in self.feature_usage:
                    self.feature_usage[agent_name] = FeatureUsage(
                        ...
                        avg_response_time=metrics.avg_latency_ms / 1000,  # Convert to seconds
    ...
    async def _generate_insights(self) -> None:
        ...
        try:
            ...
            # Usage insights
            total_requests = sum(pattern.total_requests for pattern in self.usage_patterns.values())
            if total_requests > 0:
                most_used_agent = max(self.usage_patterns.items(), key=lambda x: x[1].total_requests)
                insights.append({
                    "type": "usage_insight",
            ...
            high_impact_insights = [i for i in self.performance_insights if i.impact == "high"]
            ...
            critical_errors = {agent: errors for agent, errors in self.error_patterns.items() if "high_error_rate" in errors or "performance_degradation" in errors}
            if critical_errors:
                insights.append({
    ...
    async def get_usage_analytics(self, agent_name: Optional[str] = None, time_range: str = "24h") -> Dict[str, Any]:
        """Get usage analytics for agents."""
        try:
            if agent_name:
                return asdict(self.usage_patterns.get(agent_name, UsagePattern(agent_name=agent_name)))
            else:
                return {
                    "all_agents": {name: asdict(pattern) for name, pattern in self.usage_patterns.items()},
                    "total_requests": sum(pattern.total_requests for pattern in self.usage_patterns.values()),
                    "most_used": max(self.usage_patterns.items(), key=lambda x: x[1].total_requests, default=(None, UsagePattern(""))),
                }
    ...
    async def get_performance_analytics(self, agent_name: Optional[str] = None) -> Dict[str, Any]:
        """Get performance analytics for agents."""
        try:
            if agent_name:
                insights = [i for i in self.performance_insights if i.agent_name == agent_name]
                return {
                    "agent_name": agent_name,
                    "insights": [asdict(i) for i in insights[-10:]],  # Last 10 insights
                    "current_trend": insights[-1].trend if insights else "stable",
                    "performance_score": insights[-1].current_value if insights else 0.0,
                }
            else:
                return {
                    "all_insights": [asdict(i) for i in self.performance_insights[-20:]],  # Last 20 insights
                    "critical_issues": [asdict(i) for i in self.performance_insights if i.impact == "high"],
                    "trend_summary": {
                        "improving": len([i for i in self.performance_insights if i.trend == "improving"]),
                        "declining": len([i for i in self.performance_insights if i.trend == "declining"]),
                        "stable": len([i for i in self.performance_insights if i.trend == "stable"]),
                    },
                }
    ...
    async def get_feature_analytics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "feature_usage": {name: asdict(usage) for name, usage in self.feature_usage.items()},
                ...
                "satisfaction_scores": {
                    name: usage.user_satisfaction for name, usage in self.feature_usage.items()
                },
            }
...
async def get_usage_analytics(agent_name: Optional[str] = None, time_range: str = "24h") -> Dict[str, Any]:
    """Get usage analytics for agents."""
    analytics = get_agent_analytics()
