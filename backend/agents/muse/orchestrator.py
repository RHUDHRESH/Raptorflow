...
def resolve_generation_profile(
    *,
    ...
    profile = REASONING_DEPTH_PROFILES.get(
        depth, REASONING_DEPTH_PROFILES[default_depth]
    )
...
class LangGraphMuseOrchestrator:
    ...
    async def _load_workspace_context(self, state: MuseGraphState) -> MuseGraphState:
        ...
        if manifest:
            try:
                memories = (
                    bcm_memory.get_relevant_memories(
                        workspace_id,
                ...
                logger.warning(
                    "Memory fetch failed for workspace %s: %s", workspace_id, exc
                )
    ...
    async def _run_generation(self, state: MuseGraphState) -> MuseGraphState:
        ...
        try:
            ...
            logger.warning(
                "Muse generation failed; using deterministic fallback: %s", exc
            )
    ...
    def _build_deterministic_fallback(
        self, state: MuseGraphState, reason: str
    ) -> Dict[str, Any]:
    ...
    def _merge_ensemble_results(
        ...
        *,
        ...
        total_time = sum(
            float(r.get("generation_time_seconds") or 0.0) for r in all_results
        )
    ...
    async def _assemble_response(self, state: MuseGraphState) -> MuseGraphState:
        ...
        response = {
            ...
            "metadata": {
                ...
                "memory_limit": int(
                    (state.get("profile") or {}).get("memory_limit", 0)
                ),
