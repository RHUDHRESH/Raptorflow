...
class RouterOutput(BaseModel):
    asset_type: str = Field(description="The family of asset (email, social, meme, strategy)")
    confidence: float = Field(description="Confidence score 0.0 - 1.0")
    extracted_goal: str = Field(description="What the user wants to achieve")
    entities: List[str] = Field(description="Extracted @mentions like cohorts or campaigns")
    ambiguity_reasons: Optional[List[str]] = Field(description="Reasons for low confidence")

class IntentRouterAgent:
    def __init__(self):
        # Use the 'fast' tier for routing
        self.llm = InferenceProvider.get_model(model_tier="fast", temperature=0.0).with_structured_output(RouterOutput)
