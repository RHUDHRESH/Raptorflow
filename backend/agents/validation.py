...
def validate_tool_registry_integration():
    ...
    try:
        ...
        validation_results["registered_tools"] = [
            entry.name for entry in registered_entries
        ]
...
def validate_agent_tool_integration():
    ...
    try:
        ...
        from agents.tool_integration import (
            validate_agent_tool_integration as validate_integration,
        )

        # Get tool integration validation
        integration_validation = validate_integration()
        validation_results["agent_validations"] = integration_validation.get(
            "agent_validations", []
        )
        validation_results["integration_summary"] = integration_validation.get(
            "integration_summary", {}
        )
...
def run_comprehensive_validation():
    ...
    validation_report["summary"] = {
        ...
        "registry_status": end_to_end_results["components"]["registry"][
            "registry_status"
        ],
        "agent_status": end_to_end_results["components"]["agents"][
            "integration_status"
        ],
        "frontend_status": end_to_end_results["components"]["frontend"][
            "frontend_status"
        ],
