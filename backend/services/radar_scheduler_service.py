...
class RadarSchedulerService:
    ...
    async def start_scheduler(self, tenant_id: str, sources: List[RadarSource]):
        ...
        task = asyncio.create_task(
            self._scheduler_loop(tenant_id, by_frequency)
        )
        self.active_schedulers[tenant_id] = task
        
        logger.info(f"Started Radar scheduler for tenant {tenant_id} with {len(sources)} sources")
    ...
    async def _scheduler_loop(self, tenant_id: str, by_frequency: Dict[str, List[RadarSource]]):
            ...
            try:
                ...
                for frequency, sources in by_frequency.items():
                    ...
                    if current_time >= next_scans[frequency]:
                        ...
                        source_ids = [source.id for source in sources if source.is_active]
                        if source_ids:
                        ...
                        next_scans[frequency] = current_time + timedelta(seconds=interval)
    ...
    async def _execute_scan_job(self, job: ScanJob):
        ...
        try:
            ...
            if all_signals:
                ...
                logger.info(f"Processed {len(processed_signals)} signals into {len(clusters)} clusters")
            ...
            logger.info(f"Scan job {job.id} completed: {job.signals_found} signals found")
    ...
    async def get_source_health(self, sources: List[RadarSource]) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_scan": max((s.last_checked for s in sources if s.last_checked), default=None),
        }
