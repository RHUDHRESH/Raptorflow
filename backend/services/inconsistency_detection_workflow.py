...
class InconsistencyDetectionWorkflow:
    ...
    def _initialize_detection_rules(self) -> Dict[ContradictionType, List[Dict[str, Any]]]:
    ...
    async def detect_contradictions(self, facts: List[Dict[str, Any]]) -> ContradictionAnalysis:
        ...
        try:
            agent_result = await self.contradiction_detector.detect_contradictions(facts)
            agent_contradictions = self._convert_agent_contradictions(agent_result.contradictions)
        ...
        deduplicated_contradictions = self._deduplicate_contradictions(all_contradictions)
        
        # Calculate analysis metrics
        contradictions_by_type = self._calculate_type_distribution(deduplicated_contradictions)
        contradictions_by_severity = self._calculate_severity_distribution(deduplicated_contradictions)
        resolution_summary = self._calculate_resolution_summary(deduplicated_contradictions)
        consistency_score = self._calculate_consistency_score(deduplicated_contradictions, len(facts))
        
        # Generate recommendations
        recommendations = self._generate_contradiction_recommendations(deduplicated_contradictions, facts)
        ...
        return ContradictionAnalysis(
            ...
            analysis_metadata={
                ...
                "contradiction_rate": len(deduplicated_contradictions) / len(facts) if facts else 0.0
            }
        )
    
    async def _detect_with_rules(self, facts: List[Dict[str, Any]]) -> List[Contradiction]:
    ...
    def _apply_rule(self, rule: Dict[str, Any], facts: List[Dict[str, Any]]) -> List[Contradiction]:
        """Apply detection rule to facts"""
        contradictions = []
        
        if rule["name"] == "revenue_mismatch":
            contradictions.extend(self._check_numerical_contradiction(facts, "revenue", rule))
        elif rule["name"] == "employee_count_mismatch":
            contradictions.extend(self._check_numerical_contradiction(facts, "employees", rule))
            ...
            contradictions.extend(self._check_categorical_contradiction(facts, "industry", rule))
        elif rule["name"] == "opposite_statements":
            contradictions.extend(self._check_semantic_contradiction(facts, rule))
        
        return contradictions
    
    def _check_numerical_contradiction(self, facts: List[Dict[str, Any]], value_type: str, rule: Dict[str, Any]) -> List[Contradiction]:
            ...
            for j, val2 in enumerate(numerical_values[i+1:], i+1):
                ...
                if diff > tolerance:
                    contradiction = Contradiction(
                        ...
                        resolution_suggestion=self._generate_numerical_resolution_suggestion(val1, val2, value_type),
                        metadata={
                            "rule": rule["name"],
                            "difference": diff,
    ...
    def _check_year_contradiction(self, facts: List[Dict[str, Any]], rule: Dict[str, Any]) -> List[Contradiction]:
    ...
    def _check_categorical_contradiction(self, facts: List[Dict[str, Any]], category_type: str, rule: Dict[str, Any]) -> List[Contradiction]:
        ...
        for i, cat1 in enumerate(categories):
            for j, cat2 in enumerate(categories[i+1:], i+1):
                ...
                if cat1["category"] != cat2["category"] and cat1["category"] and cat2["category"]:
                    # Check if they're actually different (not just wording)
                    similarity = self._calculate_text_similarity(cat1["category"], cat2["category"])
    ...
    def _check_semantic_contradiction(self, facts: List[Dict[str, Any]], rule: Dict[str, Any]) -> List[Contradiction]:
    ...
    def _generate_numerical_resolution_suggestion(self, val1: Dict, val2: Dict, value_type: str) -> str:
...
"""
            ...
            if response["status"] == "success":
                ...
                self.logger.warning(f"AI contradiction detection failed: {response.get('error')}")
                return []
    ...
    def _parse_ai_contradictions(self, ai_response: str, facts: List[Dict[str, Any]]) -> List[Contradiction]:
                ...
                try:
                    ...
                    for fact_data in contradiction_data.get("conflicting_facts", []):
                        ...
                        matching_facts = [
                            ...
                            if fact.get("label", "").lower() == fact_data.get("label", "").lower()
                        ]
                        if matching_facts:
                    ...
                    if len(conflicting_facts) >= 2:
                        contradiction = Contradiction(
                            id=self._generate_contradiction_id(),
                            type=ContradictionType(contradiction_data.get("type", "semantic")),
                            severity=ContradictionSeverity(contradiction_data.get("severity", "medium")),
                            description=contradiction_data.get("description", "AI-detected contradiction"),
                            ...
                            resolution_suggestion=contradiction_data.get("resolution_suggestion", "Manual review required"),
                            extraction_method="ai_detection",
                            metadata={"ai_confidence": contradiction_data.get("confidence", 0.5)}
                        )
    ...
    def _convert_agent_contradictions(self, agent_contradictions: List[Any]) -> List[Contradiction]:
        ...
        for contradiction in agent_contradictions:
            try:
                converted = Contradiction(
                    ...
                    severity=ContradictionSeverity(contradiction.get("severity", "medium")),
                    description=contradiction.get("description", "Agent-detected contradiction"),
                    ...
                    resolution_suggestion=contradiction.get("resolution_suggestion", ""),
    ...
    def _are_contradictions_similar(self, c1: Contradiction, c2: Contradiction) -> bool:
        ...
        desc_similarity = self._calculate_text_similarity(c1.description.lower(), c2.description.lower())
        if desc_similarity > 0.8:
            return True
        
        return False
    
    def _calculate_type_distribution(self, contradictions: List[Contradiction]) -> Dict[str, int]:
    ...
    def _calculate_resolution_summary(self, contradictions: List[Contradiction]) -> Dict[str, int]:
    ...
    def _calculate_consistency_score(self, contradictions: List[Contradiction], total_facts: int) -> float:
    ...
    def _generate_contradiction_recommendations(self, contradictions: List[Contradiction], facts: List[Dict[str, Any]]) -> List[str]:
        """Generate recommendations for resolving contradictions"""
        recommendations = []
        
        if not contradictions:
            recommendations.append("No contradictions detected - data appears consistent")
            return recommendations
        
        # High-level recommendations
        critical_count = len([c for c in contradictions if c.severity == ContradictionSeverity.CRITICAL])
        if critical_count > 0:
            recommendations.append(f"Address {critical_count} critical contradictions immediately")
        ...
        unresolved_count = len([c for c in contradictions if c.resolution_status == ResolutionStatus.UNRESOLVED])
        if unresolved_count > 0:
            recommendations.append(f"Resolve {unresolved_count} unresolved contradictions")
        
        # Quality recommendations
        contradiction_rate = len(contradictions) / len(facts) if facts else 0
        if contradiction_rate > 0.15:
            recommendations.append("Consider improving data quality and source verification")
    ...
    async def resolve_contradiction(self, contradiction_id: str, resolution_action: str, user_context: str = "") -> bool:
        """Resolve a contradiction"""
        # This would update the contradiction in storage
        # For now, return success
        return True
    
    async def escalate_contradiction(self, contradiction_id: str, escalation_reason: str) -> bool:
