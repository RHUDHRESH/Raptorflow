...
class OnboardingStateService:
    ...
    async def update_step_state(
        ...
        try:
            ...
            # Update step
            step_state = state.steps.get(step_id, StepState(step_id=step_id, status=StepStatus.NOT_STARTED))
    ...
    async def can_execute_step(self, workspace_id: str, step_id: str) -> tuple[bool, str]:
    ...
    async def get_progress_summary(self, workspace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_steps = len([s for s in state.steps.values() if s.status == StepStatus.COMPLETED])
            failed_steps = len([s for s in state.steps.values() if s.status == StepStatus.FAILED])
            in_progress_steps = len([s for s in state.steps.values() if s.status == StepStatus.IN_PROGRESS])
            ...
            return {
                ...
                "next_step": next_step,
                "steps": {step_id: asdict(step_state) for step_id, step_state in state.steps.items()},
                "created_at": state.created_at,
                "updated_at": state.updated_at,
                "is_locked": state.lock_expires_at and state.lock_expires_at > time.time()
            }
    ...
    async def _validate_step_transition(self, state: OnboardingState, step_id: str, status: StepStatus) -> bool:
    ...
    async def _create_new_state(self, workspace_id: str) -> OnboardingState:
        ...
        for step_id in self.step_dependencies.keys():
            state.steps[step_id] = StepState(step_id=step_id, status=StepStatus.NOT_STARTED)
    ...
    async def _save_state(self, state: OnboardingState) -> bool:
        ...
        try:
            ...
            result = self.db_client.table("onboarding_states").upsert(state_data).execute()
    ...
    async def _load_from_db(self, workspace_id: str) -> Optional[OnboardingState]:
        ...
        try:
            result = self.db_client.table("onboarding_states") \
                .select("*") \
    ...
    async def _cache_state(self, state: OnboardingState) -> bool:
        ...
        try:
            ...
            await self.redis.set(cache_key, json.dumps(state_data), ex=self.state_cache_ttl)
            return True
