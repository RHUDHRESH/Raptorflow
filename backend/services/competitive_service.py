...
class WebScraper:
        ...
        try:
            ...
            async with session.get(competitor_url) as response:
                if response.status != 200:
                    logger.warning(
                        f"Failed to scrape {competitor_url}: {response.status}"
                    )
                    return {}
    ...
    async def _extract_pricing(self, soup: BeautifulSoup) -> Dict:
        ...
        pricing["free_trial"] = any(
            keyword in page_text for keyword in free_trial_keywords
        )

        # Check for contact required
        contact_keywords = ["contact us", "call for pricing", "enterprise pricing"]
        pricing["contact_required"] = any(
            keyword in page_text for keyword in contact_keywords
        )
        ...
        for selector in about_selectors:
            ...
            if element:
                ...
                year_match = re.search(
                    r"(?:founded|since|est\.?)\s*(\d{4})", text, re.IGNORECASE
                )
                if year_match:
                ...
                size_match = re.search(
                    r"(\d+(?:\s*\+\s*\d+)*)\s*(?:employees|people|team)",
                    text,
                    re.IGNORECASE,
                )
                if size_match:
    ...
    async def _extract_team_info(self, soup: BeautifulSoup) -> Dict:
        ...
        team = {"executives": [], "founders": [], "board_members": []}
            ...
            for element in elements:
                ...
                if len(text) > 10:
                    ...
                    if any(
                        ...
                        for keyword in ["ceo", "founder", "president"]
                    ):
                        team["founders"].append(text[:100])
                    elif any(
                        keyword in text.lower() for keyword in ["cto", "director", "vp"]
                    ):
    ...
    async def _extract_contact_info(self, soup: BeautifulSoup) -> Dict:
        ...
        # Extract address
        address_selectors = ["address", ".address", ".location", "div.contact-info"]
...
class MarketAnalyzer:
    ...
    async def analyze_positioning(
        self, competitor_data: List[CompetitorData], company_info: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            llm_result = await self.llm_service.generate_competitive_intelligence(
                company_info
            )
    ...
    def _basic_positioning_analysis(
        self, competitors: List[CompetitorData], company_info: Dict
    ) -> Dict:
        """Basic positioning analysis without LLM."""
        positioning = {"competitors": [], "market_gaps": [], "opportunities": []}
...
class CompetitiveService:
    ...
    async def analyze_competitors(self, company_info: Dict) -> CompetitiveAnalysis:
        ...
        try:
            ...
            # Analyze positioning
            positioning_matrix = await self.market_analyzer.analyze_positioning(
                competitor_data, company_info
            )
            ...
            recommendations = await self.generate_recommendations(
                competitor_data, positioning_matrix, market_analysis
            )
    ...
    async def generate_recommendations(
        self, competitors: List[CompetitorData], positioning: Dict, market: Dict
    ) -> List[str]:
        ...
        if len(competitors) > 0:
            ...
            if avg_features > 5:
                recommendations.append(
                    "Consider expanding feature set to match market leaders"
                )

            # Pricing recommendations
            pricing_strategies = [comp.pricing for comp in competitors if comp.pricing]
            if pricing_strategies:
                free_trial_count = sum(
                    1 for p in pricing_strategies if p.get("free_trial", False)
                )
                if free_trial_count > len(competitors) / 2:
                    recommendations.append(
                        "Consider offering free trial to increase conversion"
                    )
            ...
            if market.get("competition_level") == "high":
                ...
                recommendations.append(
                    "Consider strategic partnerships to increase market share"
                )
            ...
            if common_strengths:
                top_strengths = sorted(
                    common_strengths.items(), key=lambda x: x[1], reverse=True
                )[:3]
                recommendations.append(
                    f"Leverage strengths in: {', '.join([s[0] for s in top_strengths])}"
                )
