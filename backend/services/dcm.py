...
class DistanceContextManager:
    ...
    async def get_relevant_context(
        ...
        """
        logger.info(f"DCM: Calculating context distance for goal: '{goal_prompt[:50]}...'")
    ...
    async def prioritize_and_prune(self, context_list: List[Dict[str, Any]], max_tokens: int = 2000) -> List[Dict[str, Any]]:
        """
        Prunes context that is too 'Distant' or exceeds token budgets.
        """
        # Simple pruning by score and count for now
        sorted_context = sorted(context_list, key=lambda x: x.get("distance_score", 0), reverse=True)
