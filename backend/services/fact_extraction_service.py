...
class FactValidator:
    ...
    async def validate(self, fact: ExtractedFact, ocr_result: OCRResult) -> ValidationResult:
        """
        ...
        if fact.source_citation and fact.source_citation not in ocr_result.extracted_text:
...
class FactDeduplicator:
    ...
    async def deduplicate(self, facts: List[ExtractedFact]) -> List[ExtractedFact]:
        ...
        try:
            ...
            logger.info(f"Deduplicated {len(facts)} facts to {len(deduplicated_facts)} unique facts")
...
class FactQualityScorer:
    ...
    async def _calculate_specificity(self, statement: str) -> float:
        ...
        vague_words = ['several', 'many', 'some', 'few', 'approximately', 'about', 'around']
...
class FactExtractionService:
    ...
    async def extract_facts_from_documents(self, documents: List[DocumentMetadata]) -> FactExtractionResult:
        ...
        try:
            ...
            for document in documents:
                ...
                for fact in extracted_facts:
                    ...
                    if validation_result.is_valid:
                        ...
                        logger.warning(f"Fact validation failed: {validation_result.issues}")
            ...
            confidence_distribution = self._calculate_confidence_distribution(deduplicated_facts)
            category_distribution = self._calculate_category_distribution(deduplicated_facts)
    ...
    async def _convert_llm_result(self, llm_facts: List[Dict], document: DocumentMetadata, ocr_result: OCRResult) -> List[ExtractedFact]:
        ...
        for i, fact_data in enumerate(llm_facts):
            try:
                ...
                fact = ExtractedFact(
                    ...
                    category=FactCategory(fact_data.get('category', 'business_metrics')),
                    fact_type=fact_type,
    ...
    def _calculate_confidence_distribution(self, facts: List[ExtractedFact]) -> Dict[str, float]:
    ...
    async def _calculate_quality_metrics(self, facts: List[ExtractedFact]) -> Dict[str, float]:
