...
# PhonePe SDK imports
from phonepe.sdk.pg.common.models.request.refund_request import RefundRequest as SDKRefundRequest
from phonepe.sdk.pg.common.models.response.refund_response import RefundResponse as SDKRefundResponse
from phonepe.sdk.pg.common.models.response.refund_status_response import RefundStatusResponse as SDKRefundStatusResponse
...
from backend.core.payment_fraud_detection import fraud_detector
from backend.core.payment_compliance import compliance_manager
...
class RefundManager:
    ...
    async def validate_refund_eligibility(
        ...
        try:
            ...
            original_transaction = await self.payment_repo.get_by_merchant_order_id(merchant_order_id)
            ...
            if original_transaction.get("status") != "COMPLETED":
                return RefundEligibilityResult(
                    eligible=False,
                    reason="Original transaction not completed"
                )
            ...
            if transaction_date:
                ...
                if days_since_transaction > 30:
                    return RefundEligibilityResult(
                        ...
                        time_restrictions={"max_days": 30, "days_elapsed": days_since_transaction}
                    )
            
            # Check for existing refunds
            existing_refunds = await self.payment_repo.get_refunds_for_order(merchant_order_id)
    ...
    async def create_refund_request(
        ...
        try:
            ...
            merchant_refund_id = f"RF{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:8]}"
            ...
            refund_record = {
                ...
                "reason_code": request.reason_code.value if request.reason_code else "OTHER",
                "user_id": request.user_id,
            ...
            response = RefundResponseData(
                ...
                estimated_completion_time=datetime.now() + timedelta(hours=24),  # Estimate
                security_metadata={
                    "fraud_score": fraud_score.__dict__,
    ...
    async def process_refund_with_phonepe(
        ...
        try:
            # Get refund record
            refund_record = await self.payment_repo.get_refund_by_merchant_id(merchant_refund_id)
            ...
            if sdk_response.success:
                ...
                update_data = {
                    ...
                    "status": sdk_response.status.value if isinstance(sdk_response.status, RefundState) else sdk_response.status,
                    "updated_at": datetime.now()
                }
    ...
    async def get_refund_status(
        ...
        try:
            ...
            # Get status history
            status_history = await self.payment_repo.get_refund_status_history(merchant_refund_id)
            ...
            status_data = RefundStatusData(
                ...
                previous_statuses=[RefundState(status["status"]) for status in status_history],
    ...
    async def update_refund_from_webhook(
        self, 
        ...
        try:
            ...
            if not refund_record:
                logger.warning(f"Refund record not found for PhonePe refund ID: {refund_id}")
                return False
            ...
            logger.info(f"Refund updated from webhook: {merchant_refund_id} -> {status}")
            return True
