...
class EvidenceVaultService:
    ...
    async def add_evidence_file(
        self, file_path: str, file_content: bytes, metadata: Dict[str, Any] = None
    ) -> EvidenceItem:
    ...
    async def add_evidence_text(
        self, text: str, name: str, metadata: Dict[str, Any] = None
    ) -> EvidenceItem:
    ...
    async def _process_evidence(self, evidence: EvidenceItem):
        ...
        try:
            ...
            self.logger.info(
                f"Processed evidence {evidence.id} in {processing_time:.2f}s"
            )
    ...
    async def _extract_file_content(self, file_path: str, file_content: bytes) -> str:
        ...
        try:
            # Use OCR service
            result = await self.ocr_service.extract_text_from_bytes(
    ...
    def _calculate_evidence_quality(self, evidence: EvidenceItem) -> float:
        ...
        if any(
            keyword in evidence.content.lower()
            for keyword in ["summary", "conclusion", "overview"]
        ):
            score += 10
        if any(
            keyword in evidence.content.lower()
            for keyword in ["data", "analysis", "research"]
        ):
            score += 10
    ...
    async def generate_vault_insights(self) -> VaultInsights:
        ...
        quality_scores = [
            e.metadata.get("quality_score", 0) for e in self.evidence_items.values()
        ]
        quality_score = (
        ...
        recommendations = self._generate_recommendations(
            type_counts, status_counts, quality_score
        )
    ...
    def _generate_recommendations(
        ...
        if quality_score < 50:
            recommendations.append(
                "Improve evidence quality with more detailed documents"
            )
    ...
    async def search_evidence(self, query: str) -> List[EvidenceItem]:
        ...
        for evidence in self.evidence_items.values():
            ...
            for fact in evidence.extracted_facts:
                if (
                    query_lower in fact.get("label", "").lower()
                    or query_lower in fact.get("value", "").lower()
                ):
                    results.append(evidence)
                    break
    ...
    async def export_evidence_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "all_facts": [
                fact for e in self.evidence_items.values() for fact in e.extracted_facts
            ],
