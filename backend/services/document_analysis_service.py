...
class ContentClassifier:
    ...
    def __init__(self):
        self.model = None
        self.categories = [cat.value for cat in DocumentCategory if cat != DocumentCategory.UNKNOWN]
    ...
    def _rule_based_classification(self, text: str) -> Tuple[DocumentCategory, float]:
        ...
        # Business indicators
        business_keywords = [
            'revenue', 'profit', 'market', 'customer', 'business', 'sales',
            'growth', 'strategy', 'competition', 'industry', 'investment'
        ]
        
        # Technical indicators
        technical_keywords = [
            'algorithm', 'database', 'api', 'software', 'technology', 'system',
            'architecture', 'programming', 'development', 'infrastructure', 'platform'
        ]
        
        # Marketing indicators
        marketing_keywords = [
            'campaign', 'brand', 'advertising', 'promotion', 'marketing',
            'audience', 'conversion', 'engagement', 'reach', 'impression'
        ]
        
        # Legal indicators
        legal_keywords = [
            'contract', 'legal', 'agreement', 'liability', 'compliance',
            'regulation', 'terms', 'policy', 'law', 'court'
        ]
        
        # Financial indicators
        financial_keywords = [
            'financial', 'budget', 'cost', 'expense', 'income', 'investment',
            'roi', 'profit', 'loss', 'capital', 'finance'
        ]
        
        # Count keyword matches
        scores = {
            DocumentCategory.BUSINESS: sum(1 for kw in business_keywords if kw in text_lower),
            DocumentCategory.TECHNICAL: sum(1 for kw in technical_keywords if kw in text_lower),
            DocumentCategory.MARKETING: sum(1 for kw in marketing_keywords if kw in text_lower),
            DocumentCategory.LEGAL: sum(1 for kw in legal_keywords if kw in text_lower),
            DocumentCategory.FINANCIAL: sum(1 for kw in financial_keywords if kw in text_lower)
        }
...
class KeyPhraseExtractor:
    ...
    async def extract_key_phrases(self, text: str, num_phrases: int = 20) -> List[KeyPhrase]:
        """
    ...
    async def _extract_tfidf_phrases(self, text: str, num_phrases: int) -> List[KeyPhrase]:
    ...
    async def _score_phrases(self, phrases: List[KeyPhrase], text: str, sentences: List[str]) -> List[KeyPhrase]:
...
class DocumentAnalysisService:
    ...
    async def analyze_document(self, ocr_result: OCRResult) -> DocumentAnalysis:
        ...
        try:
            ...
            readability_score = results[4] if not isinstance(results[4], Exception) else 0.0
    ...
    def _detect_language(self, text: str) -> str:
        ...
        english_indicators = ['the', 'and', 'is', 'are', 'was', 'were', 'be', 'have', 'has']
        words = word_tokenize(text.lower())
