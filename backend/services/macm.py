...
from backend.services.dcm import get_dcm
...
class MacMHub:
    ...
    async def get_session_context(self, session_id: str, workspace_id: str) -> Dict[str, Any]:
        """
        ...
        working_context = await self.memory.retrieve_memory(f"session:{session_id}", memory_type="working")
        
        # 2. Get recent move interactions from Supabase
        recent_moves = self.client.table("moves") \
