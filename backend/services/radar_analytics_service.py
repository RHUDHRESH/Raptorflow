...
class RadarAnalyticsService:
    ...
    async def analyze_signal_trends(
        self, signals: List[Signal], window_days: int = 30
    ) -> Dict[str, Any]:
        ...
        for category in SignalCategory:
            ...
            category_trends[category.value] = {
                ...
                "freshness_distribution": self._calculate_freshness_distribution(
                    category_signals
                ),
                "growth_rate": self._calculate_growth_rate(
                    category_signals, window_days
                ),
            }
    ...
    async def analyze_competitor_patterns(
        ...
        for competitor, competitor_signals in by_competitor.items():
            analysis = {
                ...
                "strength_profile": self._calculate_strength_profile(
                    competitor_signals
                ),
    ...
    def _identify_market_leaders(
        ...
        for competitor, analysis in competitor_analysis.items():
            if (
                analysis["total_signals"] > 10
                and analysis["strength_profile"]["high_strength"] > 0.3
            ):
                leaders.append(competitor)
        return leaders[:3]  # Top 3 leaders

    def _identify_emerging_threats(
        self, competitor_analysis: Dict[str, Any]
    ) -> List[str]:
        """Identify emerging threats based on recent activity spikes."""
        threats = []
        for competitor, analysis in competitor_analysis.items():
            if analysis["activity_pattern"]["recent_spike"] and analysis[
                "threat_level"
            ] in ["medium", "high"]:
    ...
    async def generate_market_intelligence(
        self, signals: List[Signal], clusters: List[SignalCluster]
    ) -> Dict[str, Any]:
    ...
    async def identify_opportunities(
        self, signals: List[Signal], tenant_objectives: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        scored_opportunities = self._score_opportunities(
            opportunities, tenant_objectives
        )
    ...
    def _calculate_freshness_distribution(
        self, signals: List[Signal]
    ) -> Dict[str, int]:
    ...
    def _calculate_signal_velocity(self, signals: List[Signal]) -> Dict[str, float]:
        ...
        # Determine trend
        recent_signals = [
            s for s in signals if s.created_at >= datetime.utcnow() - timedelta(days=7)
        ]
    ...
    def _analyze_activity_pattern(self, signals: List[Signal]) -> Dict[str, Any]:
        ...
        for i in range(1, len(sorted_signals)):
            interval = (
                sorted_signals[i].created_at - sorted_signals[i - 1].created_at
            ).days
        ...
        if len(signals) >= 2:
            ...
            previous_signals = [
                s for s in signals if two_weeks_ago <= s.created_at < week_ago
            ]
    ...
    def _calculate_strength_profile(self, signals: List[Signal]) -> Dict[str, Any]:
        ...
        high_strength_count = len(
            [s for s in signals if s.strength == SignalStrength.HIGH]
        )
    ...
    def _assess_threat_level(self, signals: List[Signal]) -> str:
        ...
        high_strength_ratio = len(
            [s for s in signals if s.strength == SignalStrength.HIGH]
        ) / len(signals)
        recent_activity = len(
            [s for s in signals if s.freshness == SignalFreshness.FRESH]
        ) / len(signals)
        signal_diversity = len(set(s.category for s in signals)) / len(SignalCategory)

        threat_score = (
            (high_strength_ratio * 0.4)
            + (recent_activity * 0.4)
    ...
    def _analyze_clusters(self, clusters: List[SignalCluster]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cluster_size": round(
                sum(c.signal_count for c in clusters) / len(clusters), 1
            ),
            "top_themes": [
                {
                    "theme": cluster.theme,
    ...
    def _get_cluster_category_distribution(
    ...
    def _identify_messaging_opportunities(
        self, signals: List[Signal]
    ) -> List[Dict[str, Any]]:
    ...
    def _score_opportunities(
        self, opportunities: List[Dict[str, Any]], objectives: List[str]
    ) -> List[Dict[str, Any]]:
