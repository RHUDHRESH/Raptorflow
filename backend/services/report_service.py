...
class PDFGenerator:
    ...
    async def _create_table_of_contents(self, report: GeneratedReport) -> List:
        ...
        toc_style = ParagraphStyle(
            "TOCTitle", parent=self.styles["Heading2"], fontSize=18, spaceAfter=20
        )
    ...
    async def _create_executive_summary(self, report: GeneratedReport) -> List:
        ...
        if report.sections:
            ...
            content.append(
                Paragraph(first_section.content[:500] + "...", summary_style)
            )
    ...
    async def _create_section(self, section: ReportSection) -> List:
        ...
        if section.charts:
            ...
            for chart in section.charts:
                content.append(
                    Paragraph(f"• {chart.get('title', 'Chart')}", content_style)
                )
        ...
        if section.tables:
            ...
            for table in section.tables:
                content.append(
                    Paragraph(f"• {table.get('title', 'Table')}", content_style)
                )
...
class ExcelGenerator:
    ...
    async def _create_section_sheet(
        self, section: ReportSection, writer, sheet_name: str
    ):
        ...
        for i, table in enumerate(section.tables):
            if table.get("data"):
                ...
                df_table.to_excel(
                    writer, sheet_name=f"{sheet_name}_Table_{i+1}", index=False
                )
...
class HTMLGenerator:
    ...
    def _generate_key_metrics_html(self, key_metrics: Dict) -> str:
        ...
        metrics_html = (
            '<div class="key-metrics"><h3>Key Metrics</h3><div class="metrics-grid">'
        )
        ...
        for table in tables:
            if table.get("data"):
                table_html = self._create_html_table(
                    table["data"], table.get("title", "Table")
                )
    ...
    def _generate_javascript(self, report: GeneratedReport) -> str:
        ...
        function generateCharts() {
            const charts = """
            + json.dumps(report.metadata.get("charts", []))
            + """;
...
class ChartGenerator:
    ...
    async def _create_bar_chart(self, config: ChartConfig) -> Dict:
        ...
        chart_data = [{"x": x_data, "y": y_data, "type": "bar", "name": config.title}]
    ...
    async def _create_pie_chart(self, config: ChartConfig) -> Dict:
        ...
        chart_data = [{"labels": labels, "values": values, "type": "pie"}]
        ...
        return {
            ...
            "title": config.title,
            "data": chart_data,
            "config": {"layout": {"title": config.title}},
        }
...
class ReportService:
    ...
    async def generate_report(
        self, report_data: Dict, report_type: ReportType, format: ReportFormat
    ) -> GeneratedReport:
        """
    ...
    async def _generate_report_content(
        self, data: Dict, report_type: ReportType
    ) -> Dict:
        """Generate report content using LLM."""
        try:
            context = ExtractionContext(business_context=data)
    ...
    async def _generate_charts(self, data: Dict, report_type: ReportType) -> List[Dict]:
        ...
        if report_type == ReportType.MARKET_ANALYSIS:
            charts.extend(
                [
                    await self.chart_generator.generate_chart(
                        ChartConfig(
                            type=ChartType.BAR,
    ...
    async def _create_sections(
        self, content: Dict, charts: List[Dict]
    ) -> List[ReportSection]:
        """Create report sections."""
        sections = []
