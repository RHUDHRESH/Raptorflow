...
class SecurityTester:
    ...
    async def run_all_tests(self) -> SecurityReport:
        ...
        logger.info(
            f"Security testing completed in {(datetime.now() - start_time).total_seconds():.2f}s"
        )
        return report
    ...
    async def _run_test(
        self, name: str, category: str, description: str, test_func: callable
    ):
    ...
    async def _test_oauth_token_security(self) -> Dict[str, Any]:
        ...
        return {
            "status": (
                SecurityTestResult.PASS if score >= 70 else SecurityTestResult.FAIL
            ),
            "score": score,
            "details": details,
    ...
    async def _test_data_masking(self) -> Dict[str, Any]:
        ...
        try:
            ...
            details["masking_effective"] = (
                ...
                and "***@example.com" in masked_data["email"]
                and "******3210" in masked_data["phone"]
            )
    ...
    async def _generate_report(self) -> SecurityReport:
        ...
        for category_name, category_info in self.categories.items():
            category_tests = [
                t for t in self.test_results if t.category == category_name
            ]
            if category_tests:
                weighted_score = sum(
                    t.score * category_info["weight"] for t in category_tests
                ) / len(category_tests)
                category_scores[category_name] = {
                    ...
                    "passed": len(
                        [
                            ...
                            if t.result == SecurityTestResult.PASS
                        ]
                    ),
                    "failed": len(
                        [
                            t
                            for t in category_tests
                            if t.result == SecurityTestResult.FAIL
                        ]
                    ),
                    "warnings": len(
                        [
                            t
                            for t in category_tests
                            if t.result == SecurityTestResult.WARNING
                        ]
                    ),
                    "errors": len(
                        [
                            t
                            for t in category_tests
                            if t.result == SecurityTestResult.ERROR
                        ]
                    ),
                }
            else:
        ...
        weighted_score = sum(
            cat["score"] * cat["weight"] for cat in category_scores.values()
        )
        ...
        passed_tests = len(
            [t for t in self.test_results if t.result == SecurityTestResult.PASS]
        )
        failed_tests = len(
            [t for t in self.test_results if t.result == SecurityTestResult.FAIL]
        )
        warning_tests = len(
            [t for t in self.test_results if t.result == SecurityTestResult.WARNING]
        )
        error_tests = len(
            [t for t in self.test_results if t.result == SecurityTestResult.ERROR]
        )
        skipped_tests = len(
            [t for t in self.test_results if t.result == SecurityTestResult.SKIP]
        )
