...
class LinkSignalScorer:
    ...
    async def score_links(self, query: str, links: List[str], focus_areas: List[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        links_str = "\n".join([f"ID: {i} | URL: {link}" for i, link in enumerate(links)])
...
{links_str}"""
        ...
        try:
            ...
            keywords = ["pricing", "features", "docs", "case-study", "whitepaper", "product"]
...
class CognitiveSynthesizer:
    ...
    async def synthesize(self, query: str, data: List[Dict[str, Any]], mode: str) -> Dict[str, Any]:
        """
        ...
        for i, item in enumerate(data):
            text = item.get("text") or item.get("full_content") or item.get("snippet") or ""
...
class TitanOrchestrator:
    ...
    async def execute(
        ...
        if mode not in TitanMode.__members__:
            raise ValueError(f"Invalid mode: {mode}. Must be one of {list(TitanMode.__members__.keys())}")
        ...
        if mode_enum == TitanMode.LITE:
            ...
            result = await self._execute_research(query, focus_areas, max_results, use_stealth)
        elif mode_enum == TitanMode.DEEP:
            result = await self._execute_deep(query, focus_areas, max_results, use_stealth)
    ...
    async def _scrape_with_escalation(self, url: str, use_stealth: bool = True) -> Dict[str, Any]:
        """
    ...
    async def _execute_research(self, query: str, focus_areas: Optional[List[str]], max_results: int, use_stealth: bool) -> Dict[str, Any]:
        """
        RESEARCH mode: Multi-query search + semantic ranking + targeted scraping.
        """
        try:
            # 1. Multiplexed Search
            all_search_results = await self.multiplexer.execute_multiplexed(query, focus_areas, count=5)
            
            # 2. Semantic Ranking
            ranked_results = await self.rank_results(query, all_search_results, focus_areas)
            
            # 3. Scrape top results in parallel
            top_candidates = ranked_results[:5]
            scrape_tasks = [self._scrape_with_escalation(res.get("url"), use_stealth) for res in top_candidates if res.get("url")]
            ...
            intelligence_map = await self.synthesizer.synthesize(query, enriched_results, "RESEARCH")
            ...
            return {
                ...
                "mode": "RESEARCH",
                "error": str(e),
                "intelligence_map": {"summary": "Synthesis failed due to API error.", "details": str(e)},
                "results": [],
                "count": 0,
                "timestamp": datetime.now().isoformat()
            }

    async def _execute_deep(self, query: str, focus_areas: Optional[List[str]], max_results: int, use_stealth: bool) -> Dict[str, Any]:
        """
        DEEP mode: High-concurrency discovery + recursive traversal.
        """
        try:
            # 1. Broad Multiplexed Search
            all_search_results = await self.multiplexer.execute_multiplexed(query, focus_areas, count=10)
            ranked_results = await self.rank_results(query, all_search_results, focus_areas)
            ...
            traversal_results = await asyncio.gather(
                *[self._recursive_traverse(query, url, focus_areas, depth=2) for url in top_urls],
            ...
            intelligence_map = await self.synthesizer.synthesize(query, all_pages, "DEEP")
            ...
            return {
                ...
                "mode": "DEEP",
                "error": str(e),
                "intelligence_map": {"summary": "Recursive traversal failed.", "details": str(e)},
                "results": [],
                "count": 0,
                "timestamp": datetime.now().isoformat()
            }

    async def _recursive_traverse(self, query: str, start_url: str, focus_areas: List[str] = None, depth: int = 2) -> Dict[str, Any]:
        """
        ...
        while queue and len(results) < 10: # Safety limit for sync execution
            ...
            if current_depth < depth:
                ...
                if links:
                    ...
                    same_domain_links = [l for l in links if urlparse(l).netloc == domain]
                    
                    high_signal_links = await self.link_scorer.score_links(query, same_domain_links, focus_areas)
    ...
    async def rank_results(self, query: str, results: List[Dict[str, Any]], focus_areas: List[str] = None) -> List[Dict[str, Any]]:
