...
class AIExtractionEngine:
    ...
    def _initialize_fact_patterns(self) -> Dict[str, List[re.Pattern]]:
        ...
        return {
            "revenue": [
                re.compile(
                    r"\$(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion|thousand)?\s*(?:in\s*)?revenue",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"revenue\s*(?:of|was|is)?\s*\$(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion|thousand)?",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion|thousand)?\s*(?:in\s*)?revenue",
            ...
            "employees": [
                ...
                re.compile(
                    r"(?:employees|staff|team members?)\s*(?:of|:)?\s*(\d+(?:,\d{3})*)",
                    re.IGNORECASE,
                ),
            ],
            "founded": [
            ...
            "location": [
                re.compile(
                    r"(?:located|based|headquartered)\s*(?:in|at)?\s*([A-Za-z\s,]+)",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"([A-Za-z\s,]+)\s*(?:office|headquarters|location)", re.IGNORECASE
                ),
            ],
            "funding": [
                re.compile(
                    r"\$(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion)?\s*(?:in\s*)?funding",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"raised\s*\$(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion)?",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"funding\s*(?:of|round)?\s*\$(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:million|billion)?",
                    re.IGNORECASE,
                ),
            ],
            "customers": [
                re.compile(
                    r"(\d+(?:,\d{3})*)\s*(?:customers|clients|users)", re.IGNORECASE
                ),
                re.compile(
                    r"(?:customers|clients|users)\s*(?:of|:)?\s*(\d+(?:,\d{3})*)",
                    re.IGNORECASE,
                ),
            ],
            "growth": [
                re.compile(
                    r"(\d+(?:\.\d+)?)%\s*(?:growth|increase|rise)", re.IGNORECASE
                ),
    ...
    async def extract_facts_from_evidence(
        ...
        for evidence in evidence_list:
            try:
                ...
                self.logger.error(
                    f"Error extracting from evidence {evidence.get('id', 'unknown')}: {e}"
                )
        ...
        recommendations = self._generate_extraction_recommendations(
            deduplicated_facts, evidence_list
        )
        ...
        return ExtractionResult(
            ...
            extraction_metadata={
                "extraction_methods": list(
                    set(f.extraction_method for f in deduplicated_facts)
                ),
                "average_confidence": (
                    sum(f.confidence for f in deduplicated_facts)
                    / len(deduplicated_facts)
                    if deduplicated_facts
                    else 0.0
                ),
                "high_confidence_facts": len(
                    [f for f in deduplicated_facts if f.confidence > 0.8]
                ),
            },
        )

    async def _extract_from_single_evidence(
        ...
        if vertex_ai_service:
            try:
                ai_facts = await self._extract_with_ai(
                    content, evidence_id, evidence_name
                )
        ...
        try:
            orchestrator_result = (
                await self.extraction_orchestrator.extract_facts_from_evidence(
                    [evidence]
                )
            )
            orchestrator_facts = self._convert_orchestrator_facts(
                orchestrator_result.facts, evidence_id, evidence_name
            )
            facts.extend(orchestrator_facts)
        except Exception as e:
            self.logger.warning(
                f"Orchestrator extraction failed for {evidence_id}: {e}"
            )

        return facts

    def _extract_with_patterns(
        self, content: str, evidence_id: str, evidence_name: str
    ) -> List[ExtractedFact]:
    ...
    def _calculate_pattern_confidence(
        self, match: re.Match, pattern_type: str, content: str
    ) -> float:
...
"""
            ...
            if response["status"] == "success":
                return self._parse_ai_facts(
                    response["text"], evidence_id, evidence_name
                )
            else:
    ...
    def _parse_ai_facts(
        self, ai_response: str, evidence_id: str, evidence_name: str
    ) -> List[ExtractedFact]:
        ...
        try:
            ...
            for fact_data in ai_facts:
                try:
                    ...
                    fact_type = self._determine_fact_type_from_value(
                        fact_data.get("value", "")
                    )
                    ...
                    fact = ExtractedFact(
                        ...
                        confidence_level=self._get_confidence_level(
                            float(fact_data.get("confidence", 0.5))
                        ),
                        sources=[
                            {
                                "type": "evidence",
            ...
            for category, keywords in self.category_keywords.items():
                for keyword in keywords:
                    if keyword.lower() in sentence.lower():
                        ...
                        fact = self._extract_fact_from_sentence(
                            sentence, category, evidence_id, evidence_name
                        )
                        if fact:
                            facts.append(fact)
    ...
    def _extract_fact_from_sentence(
        self,
        sentence: str,
        category: FactCategory,
        evidence_id: str,
    ...
    def _convert_orchestrator_facts(
        self, orchestrator_facts: List[Any], evidence_id: str, evidence_name: str
    ) -> List[ExtractedFact]:
        ...
        for fact in facts:
            ...
            if key not in seen_facts:
                ...
                existing_fact = next(
                    ...
                    if f"{f.category.value}:{f.label.lower()}:{f.value.lower()}" == key
                )
    ...
    def _generate_extraction_recommendations(
        self, facts: List[ExtractedFact], evidence_list: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if not facts:
            recommendations.append(
                "No facts extracted - check evidence quality and content"
            )
        ...
        if missing_categories:
            recommendations.append(
                f"Consider adding evidence for missing categories: {', '.join(c.value for c in missing_categories)}"
            )
        ...
        if len(low_conf_facts) > len(facts) * 0.3:
            recommendations.append(
                "Many low-confidence facts - consider reviewing and validating"
            )

        # Evidence coverage
        if len(evidence_list) > 0 and len(facts) < len(evidence_list) * 2:
            recommendations.append(
                "Low fact extraction rate - consider adding more detailed evidence"
            )
        ...
        if FactType.NUMERICAL not in fact_types:
            recommendations.append(
                "No numerical facts extracted - consider adding financial or operational data"
            )
    ...
    async def search_facts(
        ...
        for fact in facts:
            ...
            if (
                ...
                or query_lower in fact.context.lower()
            ):
                results.append(fact)
    ...
    async def validate_facts(self, facts: List[ExtractedFact]) -> List[ExtractedFact]:
        ...
        for fact in facts:
            ...
            if (
                fact.category == FactCategory.FINANCIAL
                and fact.type == FactType.NUMERICAL
            ):
                try:
