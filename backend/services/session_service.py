...
class RedisSessionManager:
    ...
    async def cache_step_data(self, step_data: StepData):
        ...
        logger.info(f"Cached step {step_data.step_id} for session {step_data.session_id}")
    ...
    async def get_cached_step_data(self, session_id: str, step_id: int) -> Optional[StepData]:
...
class SupabaseSessionManager:
    ...
    async def create_session(self, workspace_id: str, user_id: str, total_steps: int = 25) -> OnboardingSession:
        ...
        try:
            ...
            result = client.table('onboarding_sessions').insert(asdict(session)).execute()
    ...
    async def update_session(self, session: OnboardingSession) -> OnboardingSession:
        ...
        try:
            result = client.table('onboarding_sessions').update(asdict(session)).eq('id', session.id).execute()
    ...
    async def get_session(self, session_id: str) -> Optional[OnboardingSession]:
        ...
        try:
            client = await self._get_client()
            result = client.table('onboarding_sessions').select('*').eq('id', session_id).execute()
            ...
            if result.data:
                ...
                session_dict['updated_at'] = datetime.fromisoformat(session_dict['updated_at'].replace('Z', '+00:00'))
                if session_dict.get('completed_at'):
                    session_dict['completed_at'] = datetime.fromisoformat(session_dict['completed_at'].replace('Z', '+00:00'))
    ...
    async def update_step_data(self, session_id: str, step_id: int, data: Dict, status: StepStatus = StepStatus.COMPLETED) -> StepData:
        ...
        try:
            # Upsert step data
            result = client.table('onboarding_step_data').upsert(asdict(step_data)).execute()
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to update step data"
            )
    ...
    async def get_step_data(self, session_id: str, step_id: int) -> Optional[StepData]:
        ...
        try:
            client = await self._get_client()
            result = client.table('onboarding_step_data').select('*').eq('session_id', session_id).eq('step_id', step_id).execute()
            
            if result.data:
                step_dict = result.data[0]
                step_dict['updated_at'] = datetime.fromisoformat(step_dict['updated_at'].replace('Z', '+00:00'))
                return StepData(**step_dict)
            else:
                return None
    ...
    async def create_snapshot(self, session_id: str, description: Optional[str] = None) -> SessionSnapshot:
        ...
        # Get all step data
        all_steps_result = client.table('onboarding_step_data').select('*').eq('session_id', session_id).execute()
        ...
        try:
            result = client.table('session_snapshots').insert(asdict(snapshot)).execute()
            
            if result.data:
                logger.info(f"Created snapshot {snapshot.version} for session {session_id}")
                return snapshot
            else:
    ...
    async def get_snapshots(self, session_id: str) -> List[SessionSnapshot]:
        ...
        try:
            client = await self._get_client()
            result = client.table('session_snapshots').select('*').eq('session_id', session_id).order('created_at', desc=True).execute()
            
            snapshots = []
            for snapshot_dict in result.data if result.data else []:
                snapshot_dict['created_at'] = datetime.fromisoformat(snapshot_dict['created_at'].replace('Z', '+00:00'))
...
class SessionService:
    ...
    async def update_step_data(self, session_id: str, step_id: int, data: Dict, status: StepStatus = StepStatus.COMPLETED) -> StepData:
        ...
        """
        # Update in database
        step_data = await self.supabase_manager.update_step_data(session_id, step_id, data, status)
    ...
    async def restore_from_snapshot(self, session_id: str, snapshot_version: int) -> OnboardingSession:
    ...
    async def get_user_sessions(self, user_id: str, limit: int = 10) -> List[OnboardingSession]:
        """Get all sessions for a user."""
        try:
            client = await self.supabase_manager._get_client()
            result = client.table('onboarding_sessions').select('*').eq('user_id', user_id).order('created_at', desc=True).limit(limit).execute()
    ...
    async def cleanup_expired_sessions(self):
        ...
        try:
            ...
            result = client.table('onboarding_sessions').delete().lt('updated_at', expiry_date.isoformat()).execute()
