...
class CostTracker:
    ...
    def __init__(self):
        self.pricing = {
            ...
            LLMProvider.GOOGLE_VERTEX: {
                ModelType.GEMINI_PRO: {"input": 0.0005, "output": 0.0015},  # per 1K tokens
                ModelType.GEMINI_PRO_VISION: {"input": 0.0025, "output": 0.0075}
            }
        }
    
    async def track_usage(self, usage: TokenUsage, model: ModelType, provider: LLMProvider) -> float:
        """
    ...
    async def _log_usage(self, usage: TokenUsage, model: ModelType, provider: LLMProvider, cost: float):
        """Log usage for billing and analytics."""
        # Implementation would log to database or analytics service
        logger.info(f"LLM Usage - Provider: {provider}, Model: {model}, "
...
class OpenAIProvider:
    ...
    async def generate_response(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            cost = await cost_tracker.track_usage(usage, request.model, LLMProvider.OPENAI)
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to generate response with OpenAI"
            )
...
class GoogleVertexProvider:
    ...
    async def generate_response(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            finish_reason = prediction.get('safety_ratings', [{}])[0].get('blocked', False)
            ...
            # Calculate cost
            cost_tracker = CostTracker()
            cost = await cost_tracker.track_usage(usage, request.model, LLMProvider.GOOGLE_VERTEX)
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Google Vertex AI error: {str(e)}"
            )
...
class LLMService:
    ...
    async def extract_facts(self, content: str, context: ExtractionContext) -> Dict:
        ...
        formatted_prompt = prompt.format(content=content[:8000])  # Limit for token count
    ...
    async def detect_contradictions(self, facts: List[Dict]) -> Dict:
        ...
        prompt = await self.prompt_manager.get_prompt('contradiction_detection', context)
    ...
    async def generate_competitive_intelligence(self, business_data: Dict) -> Dict:
        ...
        # Get prompt template
        prompt = await self.prompt_manager.get_prompt('competitive_intelligence', context)
    ...
    async def call_llm_with_retry(self, request: LLMRequest, max_retries: int = 3) -> LLMResponse:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise
                logger.warning(f"LLM call failed, retrying in {base_delay * (2 ** attempt)}s: {e}")
