...
class TAMCalculator:
    ...
    async def calculate_tam(self, business_data: Dict, industry: str) -> Dict:
        ...
        try:
            ...
            enhanced_tam = await self._enhance_tam_with_llm(tam_result, business_data, industry)
    ...
    async def _calculate_tam_tech(self, business_data: Dict) -> Dict:
        ...
        market_research_tam = await self._get_market_research_tam('technology', business_data)
    ...
    async def _get_market_research_tam(self, industry: str, business_data: Dict) -> float:
    ...
    async def _enhance_tam_with_llm(self, tam_result: Dict, business_data: Dict, industry: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'assumptions': tam_result.get('assumptions', []) + enhancement.get('additional_assumptions', [])
            }
...
class SAMCalculator:
    ...
    async def calculate_sam(self, tam: float, business_data: Dict, constraints: List[str]) -> Dict:
        """
        ...
        try:
            ...
            serviceability_factors = await self._calculate_serviceability_factors(business_data, constraints)
            ...
            # Validate with LLM
            enhanced_sam = await self._enhance_sam_with_llm(sam, sam_percentage, serviceability_factors, business_data)
    ...
    async def _calculate_serviceability_factors(self, business_data: Dict, constraints: List[str]) -> Dict:
        ...
        if 'resources' in constraints:
            resource_availability = business_data.get('resource_availability', 0.7)  # 70% default
            factors['resources'] = resource_availability
        
        # Regulatory constraints
        if 'regulatory' in constraints:
            regulatory_compliance = business_data.get('regulatory_compliance', 0.8)  # 80% default
            factors['regulatory'] = regulatory_compliance
        
        # Technical constraints
        if 'technical' in constraints:
            technical_feasibility = business_data.get('technical_feasibility', 0.9)  # 90% default
    ...
    async def _enhance_sam_with_llm(self, sam: float, sam_percentage: float, factors: Dict, business_data: Dict) -> Dict:
        ...
        try:
            ...
            return {'sam': sam, 'sam_percentage': sam_percentage, 'serviceability_factors': factors}
...
class SOMCalculator:
    ...
    async def calculate_som(self, sam: float, business_data: Dict, competitive_factors: Dict) -> Dict:
        """
        ...
        try:
            ...
            competition_factors = await self._calculate_competition_factors(competitive_factors)
            ...
            # Validate with LLM
            enhanced_som = await self._enhance_som_with_llm(som, som_percentage, competition_factors, business_data)
    ...
    async def _enhance_som_with_llm(self, som: float, som_percentage: float, factors: Dict, business_data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'confidence': enhancement.get('confidence', 0.5),
                'time_to_target_som': enhancement.get('time_to_target_som', '2-3 years'),
                'recommendations': enhancement.get('recommendations', [])
            }
            
        except Exception as e:
            logger.error(f"SOM enhancement failed: {e}")
            return {'som': som, 'som_percentage': som_percentage, 'competitive_factors': factors}
...
class MarketSizingService:
    ...
    async def calculate_market_sizing(self, business_data: Dict) -> MarketSizing:
        ...
        try:
            ...
            # Calculate TAM
            tam_result = await self.tam_calculator.calculate_tam(business_data, industry)
            tam = tam_result.get('tam', 0)
            
            # Calculate SAM
            constraints = business_data.get('service_constraints', ['geographic', 'resources', 'regulatory'])
            sam_result = await self.sam_calculator.calculate_sam(tam, business_data, constraints)
            sam = sam_result.get('sam', tam * 0.6)
            
            # Calculate SOM
            competitive_factors = business_data.get('competitive_factors', {})
            som_result = await self.som_calculator.calculate_som(sam, business_data, competitive_factors)
            som = som_result.get('som', sam * 0.5)
            
            # Generate market segments
            market_segments = await self._generate_market_segments(business_data, tam, sam, som)
            
            # Calculate growth projections
            growth_projections = await self._calculate_growth_projections(business_data, tam, sam, som)
            
            # Calculate penetration analysis
            penetration_analysis = await self._calculate_penetration_analysis(tam, sam, som)
            
            # Calculate overall confidence
            confidence_score = self._calculate_overall_confidence(tam_result, sam_result, som_result)
            ...
            return MarketSizing(
                ...
                assumptions=tam_result.get('assumptions', []) + sam_result.get('additional_constraints', []) + som_result.get('recommendations', [])
            )
    ...
    async def _generate_market_segments(self, business_data: Dict, tam: float, sam: float, som: float) -> List[MarketSegment]:
    ...
    async def _calculate_growth_projections(self, business_data: Dict, tam: float, sam: float, som: float) -> Dict:
        """Calculate growth projections."""
        projections = {}
        ...
        for year in range(1, 6):
            projections[f'year_{year}'] = {
                ...
                'som': som * ((1 + base_growth_rate * 0.6) ** year)   # SOM grows slowest
            }
        
        return projections
    
    async def _calculate_penetration_analysis(self, tam: float, sam: float, som: float) -> Dict:
    ...
    def _calculate_overall_confidence(self, tam_result: Dict, sam_result: Dict, som_result: Dict) -> float:
