...
class ShadowInferenceService:
    ...
    def __init__(self, prod_tier: ModelTier = ModelTier.DRIVER, shadow_tier: ModelTier = ModelTier.SMART):
    ...
    async def run_comparison(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"Shadow Inference Complete. Latency Delta: {comparison['latency_delta']:.4f}s")
        return comparison
    ...
    def log_comparison(self, comparison: Dict[str, Any]):
        ...
        logger.info(f"Comparison Logged: {comparison['production']['tier']} vs {comparison['shadow']['tier']}")
