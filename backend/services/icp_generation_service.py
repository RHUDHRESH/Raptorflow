...
class PsychographicAnalyzer:
    ...
    async def analyze(self, business_data: Dict, fact_data: List[Dict]) -> Psychographics:
        """
        ...
        personality_traits = await self._analyze_personality_traits(business_data, fact_data)
        
        # Communication style analysis
        communication_style = await self._analyze_communication_style(business_data, fact_data)
        
        # Decision factors
        decision_factors = await self._analyze_decision_factors(business_data, fact_data)
    ...
    async def _extract_values(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
    ...
    async def _analyze_interests(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        ...
        industry_interests = {
            'technology': ['software development', 'cloud computing', 'AI/ML', 'cybersecurity', 'automation'],
            'healthcare': ['wellness', 'preventive care', 'medical research', 'health tech'],
            'finance': ['investment', 'financial planning', 'risk management', 'fintech'],
            'retail': ['shopping', 'product discovery', 'deals', 'customer experience'],
            'education': ['learning', 'skill development', 'certification', 'knowledge sharing']
        }
        ...
        for fact in fact_data:
            if self.nlp:
                ...
                entities = [ent.text for ent in doc.ents if ent.label_ in ['PRODUCT', 'ORG', 'EVENT']]
    ...
    async def _analyze_lifestyle(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        ...
        if 'b2c' in business_model:
            lifestyle.extend(['consumer-oriented', 'convenience-focused', 'experience-driven'])
    ...
    async def _analyze_personality_traits(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
    ...
    async def _analyze_communication_style(self, business_data: Dict, fact_data: List[Dict]) -> str:
    ...
    async def _analyze_decision_factors(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        ...
        industry = business_data.get('industry', '').lower()
        industry_factors = {
            'healthcare': ['safety', 'efficacy', 'recommendations', 'regulatory_compliance'],
            'finance': ['roi', 'risk_assessment', 'security', 'compliance'],
            'technology': ['features', 'integration', 'scalability', 'support', 'innovation'],
            'retail': ['price', 'quality', 'convenience', 'brand_reputation', 'availability']
        }
    ...
    async def _determine_segment(self, values: List[str], interests: List[str], lifestyle: List[str]) -> PsychographicSegment:
        """Determine psychographic segment."""
        # Innovation-oriented signals
        innovation_signals = 0
        if any(value in ['innovation', 'creativity', 'forward-thinking'] for value in values):
            innovation_signals += 2
        if any(interest in ['AI/ML', 'automation', 'cloud computing'] for interest in interests):
            innovation_signals += 2
        
        # Professional signals
        professional_signals = 0
        if any(value in ['efficiency', 'productivity', 'reliability'] for value in values):
            professional_signals += 2
        if any(lifestyle in ['professional', 'career-focused'] for lifestyle in lifestyle):
            professional_signals += 2
...
class BehavioralAnalyzer:
    ...
    async def analyze(self, business_data: Dict, fact_data: List[Dict]) -> BehavioralProfile:
        """
        ...
        # Preferred channels
        preferred_channels = await self._analyze_preferred_channels(business_data, fact_data)
        
        # Engagement preferences
        engagement_preferences = await self._analyze_engagement_preferences(business_data, fact_data)
        
        # Pain points
        pain_points = await self._analyze_pain_points(fact_data)
        
        # Determine pattern
        pattern = await self._determine_behavioral_pattern(buying_patterns, research_habits)
    ...
    async def _analyze_buying_patterns(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        """Analyze buying patterns."""
        patterns = []
    ...
    async def _analyze_research_habits(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        """Analyze research habits."""
        habits = []
        ...
        industry = business_data.get('industry', '').lower()
        industry_habits = {
            'technology': ['technical_documentation', 'feature_comparison', 'trial_usage', 'peer_reviews'],
            'healthcare': ['medical_research', 'second_opinions', 'regulatory_research'],
            'finance': ['market_analysis', 'due_diligence', 'case_studies', 'expert_consultation']
        }
    ...
    async def _analyze_preferred_channels(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        ...
        business_model = business_data.get('business_model', '').lower()
        if 'b2b' in business_model:
            channels.extend(['email', 'linkedin', 'professional_networks', 'trade_shows'])
        elif 'b2c' in business_model:
            channels.extend(['social_media', 'mobile_apps', 'online_marketplaces', 'reviews'])
    ...
    async def _analyze_engagement_preferences(self, business_data: Dict, fact_data: List[Dict]) -> List[str]:
        ...
        preferences.extend(['quick_responses', 'multi_channel_support', 'proactive_communication'])
        
        # Community preferences
        preferences.extend(['community_engagement', 'user_generated_content', 'social_proof'])
    ...
    async def _analyze_pain_points(self, fact_data: List[Dict]) -> List[str]:
        ...
        common_pains = [
            'cost_management', 'efficiency_challenges', 'integration_complexity',
            'security_concerns', 'scalability_issues', 'user_experience',
            'support_quality', 'feature_limitations', 'time_constraints'
        ]
    ...
    async def _determine_behavioral_pattern(self, buying_patterns: List[str], research_habits: List[str]) -> BehavioralPattern:
        """Determine behavioral pattern."""
        # Price sensitivity analysis
        price_sensitive = any(pattern in ['price_comparison', 'deal_seeking', 'bulk_purchasing'] for pattern in buying_patterns)
        
        # Research intensity analysis
        research_intensive = any(habit in ['extensive_research', 'technical_documentation', 'feature_comparison'] for habit in research_habits)
        ...
        elif 'quick_responses' in [pref for pref in ['quick_responses', 'multi_channel_support'] for pref in ['quick_responses', 'multi_channel_support']]:
...
class ICPGenerationService:
    ...
    async def generate_icp_profiles(self, business_data: Dict) -> Dict:
        ...
        try:
            ...
            for persona in base_personas['icp_profiles']:
                ...
                fit_scores = await self._calculate_fit_scores(persona, business_analysis)
                
                # Assess market sophistication
                market_sophistication = await self._assess_market_sophistication(persona, business_data)
                ...
                enhanced_persona = ICPProfile(
                    ...
                    demographics=Demographics(
                        age_range=persona.get('demographics', {}).get('age_range', (25, 65)),
                        income_range=persona.get('demographics', {}).get('income_range', (50000, 200000)),
                        education_level=persona.get('demographics', {}).get('education_level', 'Bachelor'),
                        location=persona.get('demographics', {}).get('location', 'United States'),
                        company_size=persona.get('demographics', {}).get('company_size', 'Medium'),
                        industry=business_data.get('industry', ''),
                        job_title=persona.get('demographics', {}).get('job_title', 'Manager')
                    ),
                    psychographics=psychographics,
                    behaviors=behaviors,
                    ...
                    communication_preferences={
                        'tone': persona.get('communication', {}).get('tone', 'professional'),
                        'channels': persona.get('communication', {}).get('channels', ['email']),
                        'frequency': persona.get('communication', {}).get('frequency', 'weekly')
                    },
            ...
            return {
                ...
                'generation_metadata': {
                    ...
                    'business_context_score': business_analysis.get('confidence_score', 0.5),
    ...
    async def _calculate_fit_scores(self, persona: Dict, business_analysis: Dict) -> List[float]:
    ...
    async def _assess_market_sophistication(self, persona: Dict, business_data: Dict) -> str:
        ...
        avg_sophistication = (business_complexity + target_sophistication + product_complexity) / 3
    ...
    async def _rank_profiles(self, profiles: List[ICPProfile]) -> List[ICPProfile]:
        ...
        return sorted(profiles, key=lambda x: (x.fit_score, x.confidence_score), reverse=True)
