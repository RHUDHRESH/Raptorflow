...
class AssetService:
    ...
    @staticmethod
    def _merge_metadata(base: Optional[Dict[str, Any]], patch: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        merged: Dict[str, Any] = dict(base or {})
        if patch:
            merged.update(patch)
        return merged

    def _build_storage_path(self, workspace_id: str, original_name: str) -> Tuple[str, str]:
    ...
    async def get_asset(self, workspace_id: str, asset_id: str) -> Optional[AssetRecordOut]:
        result = (
            self.supabase.table("assets")
            .select("*")
            .eq("id", asset_id)
    ...
    async def delete_asset(self, workspace_id: str, asset_id: str) -> bool:
        ...
        self.supabase.table("assets").delete().eq("id", asset_id).eq("workspace_id", workspace_id).execute()
        return True
