...
class BusinessContextGraph:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        builder.add_node(
            "analyze_competitor_matrix", self.analyze_competitor_matrix_node
        )
        builder.add_node(
            "analyze_customer_segments", self.analyze_customer_segments_node
        )
        builder.add_node(
            "analyze_value_proposition", self.analyze_value_proposition_node
        )
        ...
        builder.add_node(
            "identify_brand_archetypes", self.identify_brand_archetypes_node
        )

        # 4. Strategy Generation Nodes
        builder.add_node(
            "formulate_growth_strategy", self.formulate_growth_strategy_node
        )
        ...
        while retries < self.max_retries:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"JSON decode error in {node_name} (attempt {retries + 1}): {e}"
                    )
                    retries += 1
                ...
                try:
                    ...
                    logger.warning(
                        f"Validation error in {node_name} (attempt {retries + 1}): {e}"
                    )
                    retries += 1
                    state["retry_count"][node_name] = retries
                    continue
                ...
                logger.error(
                    f"Unexpected error in {node_name} (attempt {retries + 1}): {e}"
                )
                retries += 1
                state["retry_count"][node_name] = retries
    ...
    def _update_metadata(
        self, state: BusinessContextWorkflowState, source: str = "vertex_ai"
    ):
        """
        ...
        state["context_data"].metadata["ai_enhanced"] = (
            True if source != "fallback" else False
        )
        state["updated_at"] = datetime.now()

    def get_fallback_context(
        self, foundation_data: Dict[str, Any]
    ) -> BusinessContextSchemasState:
        ...
        context = BusinessContextSchemasState(
            ...
            market_analysis=MarketAnalysis(
                overview="Market analysis unavailable (fallback mode)"
            ),
            competitive_landscape=CompetitiveLandscape(
                overview="Competitive intelligence unavailable (fallback mode)"
            ),
            customer_segments=CustomerSegments(
                primary="Primary segment details unavailable"
            ),
            value_proposition=ValueProposition(
                core=foundation_data.get("description", "Value proposition unavailable")
            ),
    ...
    def get_fallback_messaging(self) -> MessagingStrategy:
        ...
        return MessagingStrategy(
            ...
            core_messages=CoreMessagePillars(
                pillars=["Quality service", "Customer first"]
            ),
            framework=MessagingFramework(structure="Problem-Solution-Benefit"),
            source="fallback",
        )

    async def generate_profile_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        profile = await self._safe_generate(
            prompt, CompanyProfile, "generate_profile", state
        )
        state["context_data"].company_profile = profile
        self._update_metadata(state)
        return state

    async def analyze_market_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        analysis = await self._safe_generate(
            prompt, MarketAnalysis, "analyze_market", state
        )
        state["context_data"].market_analysis = analysis
        self._update_metadata(state)
        return state

    async def analyze_competitors_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        landscape = await self._safe_generate(
            prompt, CompetitiveLandscape, "analyze_competitors", state
        )
        state["context_data"].competitive_landscape = landscape
        self._update_metadata(state)
        return state

    async def analyze_competitor_matrix_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        """Node to generate a feature comparison matrix."""
        competitors = (
            state["context_data"].competitive_landscape.competitors
            if state["context_data"].competitive_landscape
            else []
        )
        ...
        """
        matrix = await self._safe_generate(
            prompt, CompetitorMatrix, "analyze_competitor_matrix", state
        )
        state["context_data"].competitor_matrix = matrix
        self._update_metadata(state)
        return state

    async def analyze_customer_segments_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        segments = await self._safe_generate(
            prompt, CustomerSegments, "analyze_customer_segments", state
        )
        state["context_data"].customer_segments = segments
        self._update_metadata(state)
        return state

    async def analyze_value_proposition_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        proposition = await self._safe_generate(
            prompt, ValueProposition, "analyze_value_proposition", state
        )
        state["context_data"].value_proposition = proposition
        self._update_metadata(state)
        return state

    async def analyze_business_model_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        model = await self._safe_generate(
            prompt, BusinessModel, "analyze_business_model", state
        )
        state["context_data"].business_model = model
        self._update_metadata(state)
        return state

    async def derive_initial_cohorts_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        """
        ...
        if state["icp_list"]:
            logger.info(
                "ICP list already populated, skipping initial cohort derivation."
            )
            return state
        ...
        try:
            ...
            suggested = await self._safe_generate(
                prompt, CohortList, "derive_initial_cohorts", state
            )
            ...
            state["icp_list"] = [
                {"name": "Primary Segment", "description": segments.primary}
            ]

        return state

    async def formulate_growth_strategy_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        strategy = await self._safe_generate(
            prompt, GrowthStrategy, "formulate_growth_strategy", state
        )
        state["context_data"].growth_strategy = strategy
        self._update_metadata(state)
        return state

    async def assess_risk_factors_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        risks = await self._safe_generate(
            prompt, RiskFactors, "assess_risk_factors", state
        )
        state["context_data"].risk_factors = risks
        self._update_metadata(state)
        return state

    async def generate_swot_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
    ...
    async def generate_pestel_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        pestel = await self._safe_generate(
            prompt, PESTELAnalysis, "generate_pestel", state
        )
        state["context_data"].pestel_analysis = pestel
        self._update_metadata(state)
        return state

    async def analyze_value_chain_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        chain = await self._safe_generate(
            prompt, ValueChainAnalysis, "analyze_value_chain", state
        )
        state["context_data"].value_chain_analysis = chain
        self._update_metadata(state)
        return state

    async def identify_brand_archetypes_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        """
        archetypes = await self._safe_generate(
            prompt, BrandArchetypes, "identify_brand_archetypes", state
        )
        state["context_data"].brand_archetypes = archetypes
        self._update_metadata(state)
        return state

    async def enhance_icp_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        """
        ...
        for icp in state["icp_list"][:3]:
            ...
            try:
                ...
                ricp_entity = await self._safe_generate(
                    prompt, RICP, f"generate_ricp_{icp_id}", state
                )
                ...
                """
                enhanced = await self._safe_generate(
                    legacy_prompt, EnhancedICP, f"enhance_icp_{icp_id}", state
                )
                ...
                state["context_data"].ricps.append(
                    RICP(name=icp.get("name", "Unknown Segment"))
                )
                state["context_data"].icp_enhancements[icp_id] = self.get_fallback_icp(
                    icp
                )
    ...
    async def generate_messaging_node(
        self, state: BusinessContextWorkflowState
    ) -> BusinessContextWorkflowState:
        ...
        try:
            strategy = await self._safe_generate(
                prompt, MessagingStrategy, "generate_messaging", state
            )
