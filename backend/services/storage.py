...
class EnhancedStorageService:
    ...
    def __init__(self):
        ...
        self.enable_security_scanning = os.getenv('ENABLE_FILE_SCANNING', 'true').lower() == 'true'
        ...
        logger.info(f"Enhanced storage service initialized - CDN: {self.cdn_base_url}, Security scanning: {self.enable_security_scanning}, Image processing: {self.enable_image_processing}")

    def _validate_file(self, file_content: bytes, filename: str) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            # Block script files
            if mime_type in ['application/x-sh', 'application/x-bat', 'application/x-python']:
        ...
        if self.enable_security_scanning:
            ...
            if security_result['suspicious']:
                errors.append(f"File contains suspicious content: {', '.join(security_result['flags'])}")
        ...
        try:
            ...
            for sig in self.suspicious_signatures:
                if sig in content_lower:
                    ...
                    result['flags'].append(f"Suspicious pattern: {sig.decode('utf-8', errors='ignore')}")
            ...
            malware_patterns = [
                ...
                b'SetWindowsHookEx', b'keylogger', b'backdoor'
            ]
            
            for pattern in malware_patterns:
                if pattern in content_lower:
                    result['warnings'].append(f"Potentially suspicious pattern: {pattern.decode('utf-8', errors='ignore')}")
    ...
    def _process_image_if_needed(self, content: bytes, filename: str) -> Tuple[bytes, Dict[str, Any]]:
        ...
        try:
            ...
            if img.width > max_size or img.height > max_size:
                ...
                processing_info['resize'] = f"{original_size} -> {img.width}x{img.height}"
            ...
            if img.format in ['JPEG', 'JPG']:
                ...
                img.save(output, format='JPEG', quality=85, optimize=True)  # Convert to JPEG for better compression
            ...
            processing_info['compression_ratio'] = round((1 - len(processed_content) / len(content)) * 100, 2)
            
            logger.info(f"Image processed: {filename}, compression: {processing_info['compression_ratio']}%")
    ...
    async def upload_file(
        ...
        try:
            # Enhanced validation
            is_valid, validation_message, warnings = self._validate_file(file_content, filename)
            ...
            processed_content, processing_info = self._process_image_if_needed(file_content, filename)
            ...
            upload_result = await self.cloud_storage.upload_file(
                ...
                category=FileCategory(file_category) if file_category in [c.value for c in FileCategory] else FileCategory.UPLOADS,
                ...
                custom_metadata={
                    ...
                    "security_flags": ",".join(security_result['flags']) if security_result['flags'] else "",
            ...
            metadata = FileMetadata(
                ...
                is_scanned=True,
                scan_result="clean" if not security_result['suspicious'] else "suspicious",
            ...
            logger.info(f"Enhanced file upload: {filename} -> {storage_path} ({len(processed_content)} bytes)")
