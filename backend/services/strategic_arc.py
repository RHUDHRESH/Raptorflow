...
class StrategicArcService:
    ...
    async def resolve_dependencies(self, workspace_id: str, move_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            move_res = await self.supabase.table("moves").select("dependencies").eq("id", move_id).eq("workspace_id", workspace_id).single().execute()
            ...
            for dep_id in dependencies:
                dep_res = await self.supabase.table("moves").select("status").eq("id", dep_id).single().execute()
                if dep_res.data and dep_res.data.get("status") != "completed":
                    blocked_by.append(dep_id)
            
            if blocked_by:
                return {"blocked": True, "blocked_by": blocked_by, "reason": "Dependencies not completed"}
    ...
    async def synchronize_moves(self, campaign_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            moves_res = await self.supabase.table("moves").select("*").eq("campaign_id", campaign_id).execute()
    ...
    async def align_agendas(self, arc_id: str):
        ...
        try:
            ...
            moves = self.client.table("moves").select("id, name, goal, metadata").eq("arc_id", arc_id).eq("status", "active").execute()
            ...
            await brain.run_node("conflict_resolver_node", {
                "arc_id": arc_id,
                "moves": active_moves
            })
    ...
    async def expand_arc(self, move_id: str, workspace_id: str):
        ...
        try:
            ...
            move = self.client.table("moves").select("*").eq("id", move_id).single().execute()
            ...
            await brain.run_node("strategy_node", {
