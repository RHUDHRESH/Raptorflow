...
from models.radar_models import Signal, SignalCategory, SignalFreshness, SignalStrength
...
class RadarNotificationService:
    ...
    async def process_signal_notifications(
        self, signals: List[Signal], tenant_preferences: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        ...
        high_strength_notifications = await self._check_high_strength_signals(
            signals, rules
        )
    ...
    async def _check_high_strength_signals(
        self, signals: List[Signal], rules: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _check_competitor_spikes(
        ...
        for competitor, competitor_signals in by_competitor.items():
            ...
            recent_signals = [
                s for s in competitor_signals if s.created_at >= cutoff_time
            ]
    ...
    async def _check_trend_signals(
        ...
        recent_trend_signals = [
            s
            for s in signals
            if s.category == SignalCategory.TREND and s.created_at >= cutoff_time
        ]
    ...
    def _apply_tenant_preferences(
        self, tenant_preferences: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def create_daily_digest(
        self, signals: List[Signal], tenant_id: str
    ) -> Dict[str, Any]:
        ...
        digest = {
            ...
            "top_signals": [
                ...
                for s in sorted(
                    daily_signals,
                    key=lambda x: self._strength_to_numeric(x.strength),
                    reverse=True,
                )[:5]
            ],
            "created_at": datetime.utcnow().isoformat(),
