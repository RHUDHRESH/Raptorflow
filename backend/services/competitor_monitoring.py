...
class CompetitorMonitoringService:
    ...
    async def start_monitoring(
        ...
        logger.info(
            f"Starting competitor monitoring for {len(competitor_ids)} competitors"
        )
    ...
    async def _monitoring_loop(
        self, competitor_ids: List[str], frequency: MonitoringFrequency
    ):
    ...
    async def _perform_competitor_scan(self, competitor_ids: List[str]):
        ...
        for competitor_id in competitor_ids:
            try:
                ...
                profile = await self.memory_manager.get_competitor_profile(
                    competitor_id
                )
                if not profile:
    ...
    async def _scan_competitor_changes(
    ...
    async def generate_competitor_intelligence_report(
        ...
        analyses = [
            ...
            if any(cid in a.competitor_ids for cid in competitor_ids)
        ]
        ...
        report = {
            ...
            "recent_insights": [
                i.model_dump()
                for i in sorted(insights, key=lambda x: x.discovered_at, reverse=True)[
                    :10
                ]
            ],
            "latest_analyses": [
                a.model_dump()
                for a in sorted(analyses, key=lambda x: x.analyzed_at, reverse=True)[:5]
            ],
            ...
            "recommendations": self._generate_recommendations(
                profiles, insights, analyses
            ),
        }

        logger.info(
            f"Competitor intelligence report generated for {len(profiles)} competitors"
        )
        return report
    ...
    def _calculate_insight_trends(
        self, insights: List[CompetitorInsight]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_count": len(
                [
                    i
                    for i in insights
                    if i.discovered_at > datetime.now() - timedelta(days=7)
                ]
            ),
        }
    ...
    def _generate_recommendations(
        ...
        high_threat = [
            p
            for p in profiles
            if p.threat_level == CompetitorThreatLevel.HIGH
            or p.threat_level == CompetitorThreatLevel.CRITICAL
        ]
        if high_threat:
            recommendations.append(
                f"Priority monitoring required for {len(high_threat)} high-threat competitors"
            )

        # Recent insights
        recent_insights = [
            i for i in insights if i.discovered_at > datetime.now() - timedelta(days=7)
        ]
        if len(recent_insights) > 5:
            recommendations.append(
                "High competitor activity detected - consider increasing monitoring frequency"
            )

        # Market leaders
        market_leaders = [
            p for p in profiles if p.competitor_type == CompetitorType.MARKET_LEADER
        ]
        if market_leaders:
            recommendations.append(
                f"Monitor {len(market_leaders)} market leaders for strategic positioning insights"
            )
        ...
        if profiles:
            ...
            if common_weaknesses:
                ...
                recommendations.append(
                    f"Common weakness identified: {most_common[0]} (appears in {most_common[1]} competitors)"
                )
...
class CompetitorAnalysisService:
    ...
    async def perform_swot_analysis(self, competitor_ids: List[str]) -> Dict[str, Any]:
        ...
        for category in ["strengths", "weaknesses", "opportunities", "threats"]:
            ...
            swot_data[category] = dict(
                sorted(item_counts.items(), key=lambda x: x[1], reverse=True)
            )

        logger.info("SWOT analysis completed")
        return swot_data

    async def perform_positioning_analysis(
        self, competitor_ids: List[str]
    ) -> Dict[str, Any]:
        """Analyze competitive positioning."""
        logger.info(
            f"Performing positioning analysis for {len(competitor_ids)} competitors"
        )
        ...
        for competitor_id in competitor_ids:
            ...
            positioning_data["market_segments"][segment] = (
                positioning_data["market_segments"].get(segment, 0) + 1
            )
            ...
            for audience in profile.target_audience:
                positioning_data["target_audiences"][audience] = (
                    positioning_data["target_audiences"].get(audience, 0) + 1
                )

            # Pricing model analysis
            if profile.pricing_model:
                pm = profile.pricing_model
                positioning_data["pricing_models"][pm] = (
                    positioning_data["pricing_models"].get(pm, 0) + 1
                )

            # Threat level analysis
            threat = profile.threat_level.value
            positioning_data["threat_levels"][threat] = (
                positioning_data["threat_levels"].get(threat, 0) + 1
            )
    ...
    async def identify_competitive_gaps(self, competitor_ids: List[str]) -> List[str]:
        ...
        logger.info(
            f"Identifying competitive gaps for {len(competitor_ids)} competitors"
        )
        ...
        rare_features = [
            f for f, c in feature_counts.items() if c <= len(profiles) * 0.3
        ]
        if rare_features:
        ...
        if len(all_audiences) < 5:
            gaps.append(
                "Limited target audience segmentation - opportunity for specialized focus"
            )
    ...
    async def generate_competitive_benchmarks(
        ...
        logger.info(
            f"Generating competitive benchmarks for {len(competitor_ids)} competitors"
        )
        ...
        for profile in profiles:
            coverage = (
                len([f for f in all_features if f in profile.key_features])
                / len(all_features)
                * 100
            )
            benchmarks["feature_coverage"][profile.id] = {
                ...
                "unique_features": len(
                    [
                        f
                        for f in profile.key_features
                        if f not in all_features - set(profile.key_features)
                    ]
                ),
            }
