...
from .models import (
    ...
    DocumentCharacteristics, DocumentType
)
...
class TextCoherenceCheck:
    ...
    async def validate(self, text: str, language: str = "eng") -> ValidationResult:
        ...
        try:
            ...
            words = text.split()
            valid_words = [w for w in words if self.min_word_length <= len(w) <= self.max_word_length]
            ...
            return ValidationResult(
                ...
                details=details,
                error_message=None if passed else f"Coherence score {coherence_score:.2f} below threshold {self.coherence_threshold}",
                processing_time=time.time() - start_time
            )
    ...
    def _validate_language_patterns(self, text: str, language: str) -> float:
        ...
        if language == "eng":
            ...
            english_patterns = [
                ...
                r'\bin\b', r'\bthat\b', r'\bhave\b', r'\bI\b', r'\bit\b'
            ]
...
class LayoutConsistencyValidation:
    ...
    async def validate(self, text: str, structured_data: Optional[Dict] = None) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                score=score,
                details=details,
                error_message=None if passed else f"Layout score {score:.2f} below threshold {self.consistency_threshold}",
                processing_time=time.time() - start_time
            )
    ...
    def _validate_text_layout(self, text: str) -> float:
        ...
        if indented_lines:
            ...
            indent_consistency = 1.0 - (unique_indents - 1) / len(indent_sizes) if len(indent_sizes) > 1 else 1.0
    ...
    async def validate(self, result: OCRModelResult) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                score=confidence,
                details=details,
                error_message=None if passed else f"Confidence {confidence:.2f} below threshold {self.min_confidence}",
                processing_time=time.time() - start_time
            )
...
class SemanticValidation:
    ...
    async def validate(self, text: str, document_type: DocumentType = DocumentType.PDF) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                details=details,
                error_message=None if passed else f"Semantic score {semantic_score:.2f} below threshold {self.semantic_threshold}",
                processing_time=time.time() - start_time
            )
    ...
    def _validate_invoice_content(self, text: str) -> float:
        ...
        invoice_keywords = [
            ...
            'date', 'number', 'account', 'balance', 'credit', 'debit'
        ]
    ...
    def _validate_receipt_content(self, text: str) -> float:
        ...
        receipt_keywords = [
            'receipt', 'purchase', 'total', 'cash', 'card', 'change',
            'item', 'price', 'tax', 'store', 'shop', 'transaction'
        ]
        
        text_lower = text.lower()
        keyword_matches = sum(1 for keyword in receipt_keywords if keyword in text_lower)
    ...
    def _validate_form_content(self, text: str) -> float:
        ...
        form_keywords = [
            'name', 'address', 'phone', 'email', 'signature', 'date',
            'application', 'form', 'please', 'complete', 'fill', 'information'
        ]
    ...
    def _validate_general_content(self, text: str) -> float:
        ...
        common_words = ['the', 'and', 'to', 'of', 'in', 'that', 'have', 'for', 'not', 'you']
        ...
        try:
            if len(results) < 2:
                return ValidationResult(
                    ...
                    score=1.0,
                    details={"message": "Single model result, cannot verify cross-model agreement"},
                    error_message=None,
                    processing_time=time.time() - start_time
                )
            ...
            for i in range(len(results)):
                for j in range(i + 1, len(results)):
                    similarity = self._calculate_similarity(results[i].extracted_text, results[j].extracted_text)
            ...
            return ValidationResult(
                ...
                details=details,
                error_message=None if passed else f"Cross-model agreement {avg_similarity:.2f} below threshold {self.agreement_threshold}",
                processing_time=time.time() - start_time
            )
...
class HumanReviewTrigger:
    ...
    async def validate(self, validation_results: List[ValidationResult], 
                      characteristics: DocumentCharacteristics) -> ValidationResult:
        """Determine if human review is required."""
        start_time = time.time()
        
        try:
            review_required, reasons = self._assess_review_need(validation_results, characteristics)
    ...
    def _assess_review_need(self, validation_results: List[ValidationResult], 
                          characteristics: DocumentCharacteristics) -> Tuple[bool, List[str]]:
        ...
        if critical_failures:
            reasons.extend([f"Critical validation failed: {r.check_name}" for r in critical_failures])
        
        # Check overall validation score
        failed_validations = [r for r in validation_results if not r.passed]
        failure_rate = len(failed_validations) / len(validation_results) if validation_results else 0
    ...
    def _matches_condition(self, condition: str, validation_results: List[ValidationResult],
                          characteristics: DocumentCharacteristics) -> bool:
...
class QualityAssurance:
    ...
    def _initialize_checks(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config.get("text_coherence", {}).get("enabled", True):
            checks["text_coherence"] = TextCoherenceCheck(config.get("text_coherence", {}))
        
        # Layout consistency check
        if config.get("layout_consistency", {}).get("enabled", True):
            checks["layout_consistency"] = LayoutConsistencyValidation(config.get("layout_consistency", {}))
        
        # Confidence threshold check
        if config.get("confidence_threshold", {}).get("enabled", True):
            checks["confidence_threshold"] = ConfidenceThresholdCheck(config.get("confidence_threshold", {}))
        
        # Semantic validation
        if config.get("semantic_validation", {}).get("enabled", True):
            checks["semantic_validation"] = SemanticValidation(config.get("semantic_validation", {}))
        
        # Cross-model verification
        if config.get("cross_model_verification", {}).get("enabled", True):
            checks["cross_model_verification"] = CrossModelVerification(config.get("cross_model_verification", {}))
        
        # Human review trigger
        if config.get("human_review_trigger", {}).get("enabled", True):
            checks["human_review_trigger"] = HumanReviewTrigger(config.get("human_review_trigger", {}))
        
        return checks

    async def validate_ocr_result(self, result: OCRModelResult, 
                                 characteristics: DocumentCharacteristics,
                                 additional_results: Optional[List[OCRModelResult]] = None) -> QualityAssessment:
        ...
        if "text_coherence" in self.enabled_checks:
            validation = await self.enabled_checks["text_coherence"].validate(result.extracted_text, characteristics.language)
            validation_results.append(validation)
        
        if "layout_consistency" in self.enabled_checks:
            validation = await self.enabled_checks["layout_consistency"].validate(result.extracted_text, result.structured_data)
            validation_results.append(validation)
        
        if "confidence_threshold" in self.enabled_checks:
            validation = await self.enabled_checks["confidence_threshold"].validate(result)
            validation_results.append(validation)
        
        if "semantic_validation" in self.enabled_checks:
            validation = await self.enabled_checks["semantic_validation"].validate(result.extracted_text, characteristics.document_type)
            validation_results.append(validation)
        
        if "cross_model_verification" in self.enabled_checks and additional_results:
            validation = await self.enabled_checks["cross_model_verification"].validate([result] + additional_results)
            validation_results.append(validation)
        
        # Human review trigger (last check)
        if "human_review_trigger" in self.enabled_checks:
            validation = await self.enabled_checks["human_review_trigger"].validate(validation_results, characteristics)
            validation_results.append(validation)
        ...
        review_reasons = [
            r.error_message for r in validation_results 
            if r.check_name == "human_review_trigger" and not r.passed and r.error_message
        ]
        
        # Generate recommendations
        recommendations = self._generate_recommendations(validation_results, characteristics)
    ...
    def _extract_quality_flags(self, validation_results: List[ValidationResult]) -> List[str]:
    ...
    def _generate_recommendations(self, validation_results: List[ValidationResult],
                                characteristics: DocumentCharacteristics) -> List[str]:
        ...
        for result in validation_results:
            if not result.passed:
                if result.check_name == "text_coherence":
                    recommendations.append("Consider image preprocessing to improve text clarity")
                elif result.check_name == "confidence_threshold":
                    recommendations.append("Try using a higher accuracy model for this document type")
                    ...
                    recommendations.append("Consider ensemble processing for better accuracy")
