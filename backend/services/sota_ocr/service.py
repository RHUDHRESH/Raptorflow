...
class SOTAOCRService:
    ...
    async def process_document(
        self, file_data: bytes, filename: str, options: Optional[Dict[str, Any]] = None
    ) -> OCRProcessingResponse:
        """
        ...
        try:
            ...
            try:
                ...
                characteristics = await self.orchestrator.analyze_document(
                    temp_path, file_data
                )
                ...
                if processing_strategy == "ensemble" and self.enable_ensemble:
                    result = await self._process_with_ensemble(
                        temp_path, file_data, characteristics
                    )
                else:
                    result = await self._process_with_single_model(
                        temp_path, file_data, characteristics
                    )
                ...
                if self.enable_quality_check:
                    if isinstance(result, EnsembleResult):
                        quality_assessment = (
                            await self.quality_assurance.validate_ocr_result(
                                (
                ...
                if self.enable_monitoring:
                    success = (
                        bool(result.extracted_text) and result.confidence_score > 0.1
                    )
    ...
    async def process_batch(
        self, documents: List[Dict[str, Any]], options: Optional[Dict[str, Any]] = None
    ) -> BatchProcessingResponse:
        """
            ...
            async with semaphore:
                try:
                    return await self.process_document(
                        doc["file_data"], doc["filename"], options
                    )
                except Exception as e:
        ...
        average_confidence = (
            sum(r.confidence_score for r in processed_results) / len(processed_results)
    ...
    async def analyze_document_only(
        ...
        try:
            ...
            recommendations = self.orchestrator.get_model_recommendations(
                characteristics
            )

            # Estimate processing time and cost
            processing_estimate = self._estimate_processing(
            ...
            return {
                ...
                "recommended_model": (
                    recommendations[0][0].name if recommendations else "unknown"
                ),
                "alternative_models": [
                    model.name for model, score in recommendations[1:4]
                ],
    ...
    async def _process_with_ensemble(
        self, temp_path: str, file_data: bytes, characteristics: DocumentCharacteristics
    ) -> EnsembleResult:
        """Process document using ensemble approach."""
        return await self.ensemble.process_with_ensemble(
            temp_path, file_data, characteristics
        )

    async def _process_with_single_model(
        self, temp_path: str, file_data: bytes, characteristics: DocumentCharacteristics
    ) -> OCRModelResult:
        ...
        return await self.orchestrator.process_with_model(
            optimal_model, temp_path, file_data
        )
    ...
    def _create_processing_response(
        ...
        if isinstance(result, OCRModelResult):
            ...
            return OCRProcessingResponse(
                ...
                page_count=(
                    result.model_results[0].page_count if result.model_results else 0
                ),
                detected_language=(
                    result.model_results[0].detected_language
                    if result.model_results
                    else "unknown"
                ),
                structured_data=result.structured_data,
                quality_metrics=quality_assessment,
                processing_stats=self._create_ensemble_processing_stats(
                    result, processing_time
                ),
            )

    def _create_processing_stats(
        self, result: OCRModelResult, processing_time: float
    ) -> ProcessingStats:
        ...
        return ProcessingStats(
            ...
            throughput_pages_per_hour=(
                3600 / processing_time if processing_time > 0 else 0
            ),
            error_rate=0.0 if result.extracted_text else 1.0,
        )

    def _create_ensemble_processing_stats(
        self, result: EnsembleResult, processing_time: float
    ) -> ProcessingStats:
        ...
        for model_result in result.model_results:
            model_usage[model_result.model_name] = (
                model_usage.get(model_result.model_name, 0) + 1
            )
    ...
    def _estimate_processing(
        self, characteristics: DocumentCharacteristics, recommendations: List
    ) -> Dict[str, Any]:
        """Estimate processing time and cost."""
        ...
        processing_time = (
            1.0 / best_model.throughput_pages_per_sec
            if best_model.throughput_pages_per_sec > 0
            else 5.0
        )
...
DEFAULT_CONFIG = {
    ...
    "preprocessor": {"target_dpi": 200, "enable_enhancement": True, "pipeline": "auto"},
    "quality_assurance": {
    ...
    "monitoring": {
        "metrics": {"retention_hours": 24, "collection_interval": 60},
        "alerts": {
