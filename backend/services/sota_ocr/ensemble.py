...
from .models import (
    ...
    DocumentCharacteristics, ModelCapabilities
)
...
class EnsembleVoter:
    ...
    async def vote(self, results: List[OCRModelResult]) -> Tuple[str, float, str]:
        ...
        if len(results) == 1:
            return results[0].extracted_text, results[0].confidence_score, results[0].model_name
        
        # Apply voting method
        voting_method = self.voting_methods.get(self.config.voting_method, self._weighted_voting)
        return await voting_method(results)

    async def _weighted_voting(self, results: List[OCRModelResult]) -> Tuple[str, float, str]:
        """Weighted voting based on confidence scores."""
        # Calculate weights based on confidence
        total_confidence = sum(r.confidence_score for r in results if r.confidence_score > 0)
        
        if total_confidence == 0:
            # Fallback to best model
            best_result = max(results, key=lambda r: r.confidence_score)
            return best_result.extracted_text, best_result.confidence_score, best_result.model_name
    ...
    async def _majority_voting(self, results: List[OCRModelResult]) -> Tuple[str, float, str]:
    ...
    async def _best_model_voting(self, results: List[OCRModelResult]) -> Tuple[str, float, str]:
        """Select result from best performing model."""
        best_result = max(results, key=lambda r: r.confidence_score)
        return best_result.extracted_text, best_result.confidence_score, best_result.model_name

    async def _consensus_voting(self, results: List[OCRModelResult]) -> Tuple[str, float, str]:
    ...
    def _group_similar_texts(self, results: List[OCRModelResult]) -> List[List[OCRModelResult]]:
        ...
        for i, result in enumerate(results):
            ...
            for j, other_result in enumerate(results):
                ...
                similarity = self._calculate_similarity(result.extracted_text, other_result.extracted_text)
...
class SpecialistModelManager:
    ...
    def get_specialist_models(self, characteristics: DocumentCharacteristics) -> List[str]:
...
class OCREnsemble:
    ...
    async def process_with_ensemble(self, 
                                  document_path: str, 
                                  file_data: bytes, 
                                  characteristics: DocumentCharacteristics) -> EnsembleResult:
        """
        ...
        try:
            ...
            if processing_strategy == "ensemble":
                result = await self._process_with_ensemble_models(document_path, file_data, characteristics)
                self.ensemble_stats["ensemble_used"] += 1
            elif processing_strategy == "specialist":
                result = await self._process_with_specialist_models(document_path, file_data, characteristics)
                self.ensemble_stats["specialist_used"] += 1
            else:
                result = await self._process_with_fallback_model(document_path, file_data, characteristics)
            ...
            self.ensemble_stats["processing_time"] = (
                (self.ensemble_stats["processing_time"] * (self.ensemble_stats["total_processed"] - 1) + processing_time) /
                self.ensemble_stats["total_processed"]
            )
            
            # Update average confidence
            self.ensemble_stats["average_confidence"] = (
                (self.ensemble_stats["average_confidence"] * (self.ensemble_stats["total_processed"] - 1) + result.confidence_score) /
                self.ensemble_stats["total_processed"]
            )
    ...
    def _determine_processing_strategy(self, characteristics: DocumentCharacteristics) -> str:
        """Determine optimal processing strategy based on document characteristics."""
        # Check for specialist needs
        specialist_models = self.specialist_manager.get_specialist_models(characteristics)
    ...
    async def _process_with_ensemble_models(self, 
                                           document_path: str, 
                                           file_data: bytes, 
                                           characteristics: DocumentCharacteristics) -> EnsembleResult:
        ...
        model_results = await self._process_models_parallel(ensemble_models, document_path, file_data, characteristics)
        
        # Filter successful results
        successful_results = [r for r in model_results if r.extracted_text and r.confidence_score > 0.1]
        
        if not successful_results:
            # Fallback to best model
            return await self._process_with_fallback_model(document_path, file_data, characteristics)
        
        # Vote on best result
        final_text, confidence_score, best_model = await self.voter.vote(successful_results)
    ...
    async def _process_with_specialist_models(self, 
                                            document_path: str, 
                                            file_data: bytes, 
                                            characteristics: DocumentCharacteristics) -> EnsembleResult:
        ...
        general_models = self._select_ensemble_models(characteristics)[:2]  # Top 2 general models
        all_models = list(dict.fromkeys(specialist_models + general_models))  # Remove duplicates
        
        # Process with models
        model_results = await self._process_models_parallel(all_models, document_path, file_data, characteristics)
        ...
        weighted_results = self._apply_specialist_weights(successful_results, specialist_models)
        
        # Vote on best result
        final_text, confidence_score, best_model = await self.voter.vote(weighted_results)
    ...
    async def _process_with_fallback_model(self, 
                                         document_path: str, 
                                         file_data: bytes, 
                                         characteristics: DocumentCharacteristics) -> EnsembleResult:
        ...
        model_result = await self._process_single_model(fallback_model, document_path, file_data, characteristics)
    ...
    def _select_ensemble_models(self, characteristics: DocumentCharacteristics) -> List[str]:
        ...
        for model_name in available_models:
            ...
            if model_name in ["dots_ocr", "olm_ocr_2_7b"]:  # These support many languages
    ...
    async def _process_models_parallel(self, 
                                      models: List[str], 
                                      document_path: str, 
                                      file_data: bytes, 
                                      characteristics: DocumentCharacteristics) -> List[OCRModelResult]:
        """Process document with multiple models in parallel."""
        tasks = []
        
        for model_name in models:
            task = self._process_single_model(model_name, document_path, file_data, characteristics)
            tasks.append(task)
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    # Create error result
                    model_results.append(OCRModelResult(
                        model_name=models[i],
                        extracted_text="",
                        confidence_score=0.0,
                        processing_time=0.0,
            ...
            timeout_results = []
            for model_name in models:
                timeout_results.append(OCRModelResult(
    ...
    async def _process_single_model(self, 
                                  ...
                                  document_path: str, 
                                  file_data: bytes, 
                                  characteristics: DocumentCharacteristics) -> OCRModelResult:
        ...
        try:
            ...
            # Process document
            model_response = await model.process_document(file_data, characteristics.language)
    ...
    def _apply_specialist_weights(self, results: List[OCRModelResult], specialist_models: List[str]) -> List[OCRModelResult]:
    ...
    def _calculate_consensus_score(self, results: List[OCRModelResult]) -> float:
        ...
        for i in range(len(results)):
            for j in range(i + 1, len(results)):
                similarity = self._calculate_text_similarity(results[i].extracted_text, results[j].extracted_text)
                similarities.append(similarity)
    ...
    def _merge_structured_data(self, results: List[OCRModelResult]) -> Optional[Dict[str, Any]]:
