...
from .models import (
    DocumentCharacteristics, DocumentType, DocumentComplexity,
    LanguageCategory, ProcessingVolume
)
...
class ImageQualityAssessment:
    ...
    def _calculate_contrast(self, gray_image: np.ndarray) -> float:
        ...
        rms_contrast = np.sqrt(np.sum(((np.arange(256) - mean_intensity) ** 2) * hist.flatten()) / gray_image.size)
...
class DocumentPreprocessor:
    ...
    async def process_document(self, file_data: bytes, model_name: str) -> PreprocessingResult:
            ...
            for i, image in enumerate(images):
                ...
                if self.enable_enhancement:
                    ...
                    if abs(quality_metrics.skew_angle) > 2.0:
                        processed_image, skew_angle = self.skew_corrector.correct_skew(processed_image)
                        processing_steps.append(f"Page {i+1}: Skew correction ({skew_angle:.2f}°)")
                    
                    # Noise reduction
                    if quality_metrics.noise_level > 0.3:
                        processed_image = self.noise_reducer.reduce_noise(processed_image)
                        processing_steps.append(f"Page {i+1}: Noise reduction")
                    
                    # Contrast enhancement
                    if quality_metrics.contrast_ratio < 0.3:
                        processed_image = self.contrast_enhancer.enhance_contrast(processed_image)
    ...
    async def _document_to_images(self, document_path: str, file_data: bytes) -> List[np.ndarray]:
    ...
    def _classify_document_type(self, image: np.ndarray, all_images: List[np.ndarray]) -> DocumentType:
    ...
    def _determine_complexity(self, image: np.ndarray, quality_metrics: ImageQualityMetrics) -> DocumentComplexity:
