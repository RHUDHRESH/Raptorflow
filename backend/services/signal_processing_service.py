...
class SignalProcessingService:
    ...
    async def process_signals(
        ...
        try:
            ...
            processed_signals = self._update_signal_strength(
                deduplicated_signals, clusters
            )

            logger.info(
                f"Processed {len(signals)} signals -> {len(processed_signals)} unique, {len(clusters)} clusters"
            )
    ...
    async def deduplicate_against_existing(
        self, new_signals: List[Signal], existing_signals: List[Signal]
    ) -> List[Signal]:
    ...
    async def _cluster_category_signals(
        self, signals: List[Signal], category: SignalCategory, tenant_id: str
    ) -> List[SignalCluster]:
        ...
        while unclustered:
            ...
            if len(cluster_signals) > 1:
                cluster = await self._create_cluster(
                    cluster_signals, category, tenant_id
                )
    ...
    async def get_signal_summary(
        ...
        for signal in signals:
            ...
            summary["by_freshness"][freshness] = (
                summary["by_freshness"].get(freshness, 0) + 1
            )
