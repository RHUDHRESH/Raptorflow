...
from backend.services.upstash_client import upstash_redis
...
class AuthToken(BaseModel):
    ...
    def is_expired(self) -> bool:
        return datetime.now() > self.created_at + timedelta(seconds=self.expires_in - 60)
...
class PhonePeAuthClient:
    ...
    async def get_auth_header(self) -> dict:
        ...
        return {"Authorization": f"Bearer {token}"} if self._auth_mode == "oauth" else {"X-API-KEY": token}
    ...
    async def get_token(self) -> str:
        ...
        await upstash_redis.set("phonepe_auth_token", token, ex=self._token.expires_in - 60)
        return token
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    "https://api.phonepe.com/v1/auth/ping",
                    headers=headers
                )
