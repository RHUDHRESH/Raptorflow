...
class RadarIntegrationService:
        ...
        for signal in signals:
            ...
            for objective in campaign_objectives:
                if objective in category_objectives:
                    ...
                    signal_relevance = self._calculate_signal_relevance(
                        signal, objective
                    )
    ...
    async def generate_experiment_ideas(
        self, signal: Signal, objective: MoveObjective, stage: MoveStage, channel: str
    ) -> List[str]:
    ...
    async def create_dossier(
        self, campaign_id: str, signals: List[Signal], tenant_id: str
    ) -> Dossier:
        ...
        try:
            ...
            market_narrative = await self._generate_market_narrative(
                signals, by_category
            )
    ...
    def _calculate_signal_relevance(
        self, signal: Signal, objective: MoveObjective
    ) -> float:
    ...
    def _determine_channel(self, signal: Signal) -> str:
        ...
        channel_options = self.channel_mapping.get(
            signal.category, [{"channel": "website", "priority": 0.5}]
        )

        # Sort by priority and return the highest priority channel
        sorted_channels = sorted(
            channel_options, key=lambda x: x["priority"], reverse=True
        )
        return sorted_channels[0]["channel"]

    async def _generate_dossier_summary(
        self, signals: List[Signal], by_category: Dict[SignalCategory, List[Signal]]
    ) -> List[str]:
        """Generate dossier summary."""
        summary = []
        ...
        # Overall summary
        summary.append(
            f"Analyzed {len(signals)} signals across {len(by_category)} categories"
        )

        # Category summaries
        for category, category_signals in by_category.items():
            high_strength = len(
                [s for s in category_signals if s.strength.value == "high"]
            )
            summary.append(
                f"{category.value.title()}: {len(category_signals)} signals, {high_strength} high strength"
            )
    ...
    async def _generate_hypotheses(
        ...
        for category, category_signals in by_category.items():
            if (
                len(category_signals) >= 2
            ):  # Only generate hypotheses with multiple signals
                prompt = f"""
    ...
    async def _generate_experiment_recommendations(
        ...
        strongest_signals = sorted(
            signals, key=lambda s: self._strength_to_numeric(s.strength), reverse=True
        )[:5]
    ...
    async def _extract_copy_snippets(
        self, signals: List[Signal]
    ) -> List[Dict[str, str]]:
    ...
    def _strength_to_numeric(self, strength) -> float:
        ...
        return mapping.get(
            strength.value if hasattr(strength, "value") else strength, 0.5
        )
