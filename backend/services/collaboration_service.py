...
class CollaborationService:
    ...
    async def add_comment(
        self, asset_id: str, workspace_id: str, user_id: str, text: str
    ) -> Dict[str, Any]:
    ...
    async def update_approval_status(
        self, asset_id: str, status: ApprovalStatus, user_id: str
    ) -> Dict[str, Any]:
    ...
    async def get_approval_status(self, asset_id: str) -> Dict[str, Any]:
        ...
        try:
            row = await db.fetchrow(
                "SELECT status, approved_by, updated_at FROM muse_assets WHERE id = $1",
                asset_id,
            )
            if row:
                return {
                    "status": row["status"],
                    "approved_by": (
                        str(row["approved_by"]) if row["approved_by"] else None
                    ),
                    "updated_at": (
