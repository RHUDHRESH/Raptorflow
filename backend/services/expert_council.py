...
class ExpertCouncilSwarm:
    ...
    def swarm_router(self, state: ExpertCouncilSwarmState) -> str:
        ...
        if (
            ...
            or state["discussion"].consensus_reached
        ):
            return "finalize"
    ...
    async def escalate_node(
        self, state: ExpertCouncilSwarmState
    ) -> ExpertCouncilSwarmState:
        """
        ...
        state["discussion"].contributions.append(
            CouncilContribution(
                agent_id="system",
    ...
    async def _agent_deliberate(
        self, agent_id: str, state: ExpertCouncilSwarmState
    ) -> ExpertCouncilSwarmState:
        ...
        state["discussion"].contributions.append(
            CouncilContribution(
                ...
                content=resp
                or f"CRITICAL_ERROR: Agent {agent.name} failed to generate a strategic response. Check API Key.",
                type="suggestion",
            )
        )
    ...
    def _format_history(self, discussion: CouncilDiscussion) -> str:
        return "\n".join(
            [f"[{c.agent_name}]: {c.content}" for c in discussion.contributions]
        )
...
def create_swarm_session(
    mission: str, workspace_context: Dict[str, Any]
) -> ExpertCouncilSwarmState:
    return {
        "mission": mission,
