...
class PhonePeSDKGateway:
    ...
    def _validate_configuration(self):
        ...
        if not self.merchant_id:
            raise ValueError(
                "Missing required environment variable: PHONEPE_CLIENT_ID or PHONEPE_MERCHANT_ID"
            )

        if not self.client_secret:
            raise ValueError(
                "Missing required environment variable: PHONEPE_CLIENT_SECRET or PHONEPE_SALT_KEY"
            )
        ...
        try:
            ...
            if request.idempotency_key:
                existing_response = await idempotency_manager.get_response(
                    "payment_initiation", request.idempotency_key
                )
            ...
            if request.idempotency_key:
                await idempotency_manager.store_response(
                    "payment_initiation", request.idempotency_key, response.__dict__
                )
    ...
    async def process_refund(self, request: RefundRequestData) -> RefundResponseData:
        ...
        try:
            ...
            sdk_response = await asyncio.to_thread(
                client.refund, request, headers=headers
            )

            return RefundResponseData(success=True, refund_id=sdk_response.refund_id)

        except Exception as e:
            return RefundResponseData(success=False, error=f"Refund failed: {str(e)}")
    ...
    async def check_refund_status(self, merchant_refund_id: str) -> RefundResponseData:
        ...
        try:
            ...
            sdk_response = await asyncio.to_thread(
                client.get_refund_status, merchant_refund_id
            )
            ...
            response = RefundResponseData(
                ...
                security_metadata={
                    ...
                    "payment_details_count": (
                        len(sdk_response.payment_details)
                        if sdk_response.payment_details
                        else 0
                    ),
                },
            )
    ...
    async def validate_webhook(
        ...
        try:
            ...
            await audit_logger.log_event(
                ...
                level=LogLevel.INFO,
                request_data={
                    "merchant_transaction_id": callback_response.merchant_transaction_id
                },
