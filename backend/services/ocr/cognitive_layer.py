...
class VertexAICognitiveLayer:
    ...
    async def cleanup(self, raw_ocr: OCRResponse, context: Optional[Dict[str, Any]] = None) -> OCRResponse:
        """
        ...
        try:
            ...
            if response["status"] != "success":
                logger.warning(f"Gemini cleanup failed: {response.get('error')}, returning raw OCR")
                return raw_ocr
            ...
            return OCRResponse(
                ...
                confidence=max(raw_ocr.confidence, 0.95),  # Boost confidence if Gemini successfully processes it
                pages=raw_ocr.pages,
    ...
    def _build_cleanup_prompt(self, text: str, context: Optional[Dict[str, Any]] = None) -> str:
