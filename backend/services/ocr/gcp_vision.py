...
class GCPVisionProcessor(BaseOCRProcessor):
    ...
    async def process(self, file_content: bytes, **kwargs) -> OCRResponse:
        ...
        try:
            ...
            if not annotations:
                return OCRResponse(
                    text="", confidence=0.0, metadata={"provider": self.name}
                )
            ...
            return OCRResponse(
                ...
                raw_data={
                    "text_annotations": [
                        a.description for r in [response] for a in r.text_annotations
                    ]
                },
                metadata={
                    "provider": self.name,
                    ...
                    "detected_languages": (
                        [
                            lang.language_code
                            for lang in response.full_text_annotation.pages[
                                0
