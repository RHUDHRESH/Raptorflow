...
class SemanticAnalyzer:
                                ...
                                conflicts.append(
                                    {
                                        ...
                                        "topic": topic,
                                        "confidence": min(
                                            fact1.confidence_score,
                                            fact2.confidence_score,
                                        ),
                                    }
                                )
    ...
    async def _group_facts_by_topic(
...
class LogicalAnalyzer:
    ...
    async def analyze_logical_contradictions(
        self, facts: List[ExtractedFact]
    ) -> List[Dict]:
        """
                        ...
                        contradictions.append(
                            {
                                ...
                                "conflicting_facts": contradiction.get(
                                    "conflicting_facts", []
                                ),
...
class ContradictionMerger:
    ...
    async def merge_contradictions(
        self, semantic_conflicts: List[Dict], logical_conflicts: List[Dict]
    ) -> List[Dict]:
        """
        ...
        for conflict in all_conflicts:
            ...
            fact_ids = tuple(
                sorted([conflict.get("fact1_id", ""), conflict.get("fact2_id", "")])
            )
