...
class DriftDetectionService:
    ...
    def detect_drift(self, baseline_metrics: Dict[str, List[float]], current_metrics: Dict[str, List[float]], threshold: float = 0.05) -> Dict[str, Any]:
        """
        ...
        for metric_name, baseline_data in baseline_metrics.items():
            if metric_name in current_metrics:
                ...
                if is_drifting:
                    overall_drift = True
                    logger.warning(f"DRIFT DETECTED in metric '{metric_name}': p-value {p_value:.4f}")
                    
        return {
            "is_drifting": overall_drift,
            "metrics": results
        }
