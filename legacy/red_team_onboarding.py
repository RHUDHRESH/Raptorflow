...
async def test_onboarding_full_industrial_flow():
    ...
    mock_responses = {
        "EvidenceClassifier": {
            "evidence_type": "Deck",
            "confidence": 0.95,
            "reasoning": "Standard pitch deck structure.",
        },
        "ExtractionOrchestrator": {
            "facts": [
                {"category": "identity", "label": "Company Name", "value": "GhostOps"}
            ]
        },
        "ContradictionDetector": {"contradictions": []},
        "TruthSheetGenerator": {
            "verified_facts": [{"category": "identity", "value": "GhostOps"}]
        },
        ...
        "InsightExtractor": {
            "pain_points": [
                {"category": "UX", "description": "High slippage", "sentiment": -0.9}
            ],
        ...
        "PerceptualMapGenerator": {
            "primary_axis": {"name": "Speed"},
            "secondary_axis": {"name": "Stealth"},
            "competitors": [],
            "positioning_options": [],
            "only_you_quadrant": "Top Right",
        },
        "StrategicGridGenerator": {
            "selected_position": "The Ghost",
            "rationale": "Unmatchable",
        ...
        "ConstraintEngine": {
            "focus_areas": ["HFT"],
            "sacrifices": [{"target": "Retail"}],
            "logic": "David vs Goliath",
        },
        "ICPArchitect": {
            "profiles": [
                {
                    "name": "Whale",
                    "who_they_want_to_become": "Market King",
        ...
        "MessageHierarchyArchitect": {
            "levels": {},
            "mapping": [],
            "manifesto_assembly": "Full assembly",
            "validation": {"integrity": 0.99},
        },
    ...
    with patch(
        ...
        "backend.infrastructure.storage.delete_file", new_callable=AsyncMock
    ) as mock_delete, patch(
        "backend.tools.reddit_scraper.RedditScraperTool._run", new_callable=AsyncMock
    ) as mock_reddit:
        ...
        async def industrial_side_effect(prompt, **kwargs):
            ...
            if "revenue_model" in prompt:
                return json.dumps(mock_responses["OfferArchitect"])
            if "manifesto" in prompt:
                return json.dumps(mock_responses["NeuroscienceCopywriter"])
            if "Who They Want To Become" in prompt:
                return json.dumps(mock_responses["ICPArchitect"])
        ...
        initial_input = {
            ...
            "business_context": {
                "ucid": "RF-2026-GHOST",
                "identity": {"company_name": "GhostOps"},
            },
        ...
        offer_val = (
            ctx.get("offer", {}).get("revenue_model", "MISSING")
            if ctx.get("offer")
            else "MISSING"
        )
        ...
        if (
            offer_val != "MISSING"
            and manifesto_val != "MISSING"
            and strategy_val != "MISSING"
        ):
            print(
