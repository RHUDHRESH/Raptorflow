...
CREATE POLICY IF NOT EXISTS "plans_select_authenticated" ON public.plans
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "plans_manage_service_role" ON public.plans
...
CREATE POLICY IF NOT EXISTS "admin_actions_select_admin" ON public.admin_actions
    FOR SELECT USING (
        auth.role() = 'authenticated' AND
        EXISTS (
            SELECT 1 FROM public.profiles
...
CREATE POLICY IF NOT EXISTS "admin_actions_insert_service" ON public.admin_actions
    FOR INSERT WITH CHECK (auth.role() = 'service_role');

-- Security events policies
CREATE POLICY IF NOT EXISTS "security_events_select_own" ON public.security_events
    FOR SELECT USING (
        auth.role() = 'authenticated' AND
        user_id = (select auth.uid())
    );

CREATE POLICY IF NOT EXISTS "security_events_select_admin" ON public.security_events
    FOR SELECT USING (
        auth.role() = 'authenticated' AND
        EXISTS (
            SELECT 1 FROM public.profiles
...
CREATE POLICY IF NOT EXISTS "security_events_insert_service" ON public.security_events
    FOR INSERT WITH CHECK (auth.role() = 'service_role');
...
CREATE POLICY IF NOT EXISTS "icp_disqualifiers_select_workspace" ON public.icp_disqualifiers
    FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "icp_disqualifiers_insert_workspace" ON public.icp_disqualifiers
...
CREATE POLICY IF NOT EXISTS "icp_disqualifiers_update_workspace" ON public.icp_disqualifiers
    FOR UPDATE USING (
        auth.role() = 'authenticated' AND
        workspace_id = (
            SELECT workspace_id FROM public.profiles
...
CREATE POLICY IF NOT EXISTS "icp_disqualifiers_delete_workspace" ON public.icp_disqualifiers
    FOR DELETE USING (
        auth.role() = 'authenticated' AND
        workspace_id = (
            SELECT workspace_id FROM public.profiles
...
CREATE POLICY IF NOT EXISTS "icp_firmographics_select_workspace" ON public.icp_firmographics
    FOR SELECT USING (
        auth.role() IN ('authenticated', 'service_role') AND
        profile_id IN (
...
CREATE POLICY IF NOT EXISTS "icp_firmographics_insert_workspace" ON public.icp_firmographics
    FOR INSERT WITH CHECK (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_firmographics_update_workspace" ON public.icp_firmographics
    FOR UPDATE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_firmographics_delete_workspace" ON public.icp_firmographics
    FOR DELETE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_pain_map_select_workspace" ON public.icp_pain_map
    FOR SELECT USING (
        auth.role() IN ('authenticated', 'service_role') AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_pain_map_insert_workspace" ON public.icp_pain_map
    FOR INSERT WITH CHECK (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_pain_map_update_workspace" ON public.icp_pain_map
    FOR UPDATE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_pain_map_delete_workspace" ON public.icp_pain_map
    FOR DELETE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_psycholinguistics_select_workspace" ON public.icp_psycholinguistics
    FOR SELECT USING (
        auth.role() IN ('authenticated', 'service_role') AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_psycholinguistics_insert_workspace" ON public.icp_psycholinguistics
    FOR INSERT WITH CHECK (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_psycholinguistics_update_workspace" ON public.icp_psycholinguistics
    FOR UPDATE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "icp_psycholinguistics_delete_workspace" ON public.icp_psycholinguistics
    FOR DELETE USING (
        auth.role() = 'authenticated' AND
        profile_id IN (
            SELECT id FROM public.icp_profiles
...
CREATE POLICY IF NOT EXISTS "subscriptions_select_optimized" ON public.subscriptions
    FOR SELECT USING (
        auth.role() = 'authenticated' AND (
...
CREATE POLICY IF NOT EXISTS "payment_transactions_select_optimized" ON public.payment_transactions
    FOR SELECT USING (
        auth.role() = 'authenticated' AND (
            user_id = (select auth.uid()) OR
            EXISTS (
