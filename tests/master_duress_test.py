...
from backend.agents.specialists.pricing_optimization_agent import PricingOptimizationAgent
...
from backend.services.repurposing_service import repurposing_service, PlatformType
...
async def run_master_duress():
    ...
    chaos_state = {
        "ucid": "RF-CHAOS-999",
        "business_context": {
            "identity": {"company_name": "", "industry": "None", "mission": "To make things and stuff."},
            "positioning": {"differentiator": "None"},
        },
        "step_data": {
            "auto_extraction": {"facts": []},
            "truth_sheet": {"entries": []}
        }
    }
    ...
    services = [
        ("CRM Personalize", crm_service.personalize_content("", {"name": "Test", "company": "TestCorp", "title": "Tester"})),
        ("Repurpose", repurposing_service.repurpose_content("junk text", PlatformType.TWITTER)),
        ("Voice Analysis", brand_voice_service.analyze_brand_voice(["sample 1"], "test-user")),
