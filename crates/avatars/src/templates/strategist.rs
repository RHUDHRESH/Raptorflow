...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || {
        vec![
            SkillAtom::initial(
                "council_facilitation",
                "Council facilitation",
                "Orchestrate multi-agent debates, allocate speaking order, surface and name tensions before synthesising.",
                SkillAtomType::StructuredRule,
                vec!["facilitation", "council", "debate"],
            ),
            SkillAtom::initial(
                "brief_interrogation",
                "Brief interrogation",
                "Evaluate incoming briefs for strategic coherence, problem clarity, and whether the stated objective matches the business need.",
                SkillAtomType::StructuredRule,
                vec!["brief", "strategy", "evaluation"],
            ),
            SkillAtom::initial(
                "synthesis_construction",
                "Synthesis construction",
                "Construct campaign rationales that resolve Council tensions into a single highest-order claim, under 150 words.",
                SkillAtomType::PromptTemplate,
                vec!["synthesis", "rationale", "council"],
            ),
            SkillAtom::initial(
                "daily_wins_assembly",
                "Daily Wins assembly",
                "Assemble the Daily Wins briefing from overnight intelligence, active campaign status, and PRL shared_campaign ripples.",
                SkillAtomType::PromptTemplate,
                vec!["daily_wins", "briefing", "client"],
            ),
            SkillAtom::initial(
                "campaign_replan_trigger",
                "Campaign replanning trigger",
                "Evaluate whether incoming performance signals and intelligence alerts cross the threshold requiring a new Council session.",
                SkillAtomType::StructuredRule,
                vec!["replanning", "performance", "intelligence"],
            ),
        ]
    },
};
