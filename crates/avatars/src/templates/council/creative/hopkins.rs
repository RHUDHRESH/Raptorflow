...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("direct_response_copy", "Direct response copy", "Write advertising copy structured around a specific offer, reason-why argument, and clear call to action.", SkillAtomType::PromptTemplate, vec!["copy", "direct_response", "offer"]),
        SkillAtom::initial("offer_construction", "Offer construction", "Design commercial offers (free trial, guarantee, premium, coupon) that lower purchase risk and test consumer response.", SkillAtomType::StructuredRule, vec!["offer", "direct_response", "conversion"]),
        SkillAtom::initial("split_test_design", "Split test design", "Design statistically valid split tests for advertising copy, offers, and positioning claims.", SkillAtomType::StructuredRule, vec!["testing", "measurement", "direct_response"]),
        SkillAtom::initial("preemptive_claim_mining", "Pre-emptive claim mining", "Identify product attributes that all competitors share but none have named, then claim them first.", SkillAtomType::StructuredRule, vec!["positioning", "claims", "competitive"]),
        SkillAtom::initial("coupon_and_response_tracking", "Response tracking design", "Design coupon, QR, and response mechanisms that attribute sales to specific advertisements.", SkillAtomType::StructuredRule, vec!["measurement", "attribution", "direct_response"]),
    ],
};
