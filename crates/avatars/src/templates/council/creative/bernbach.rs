...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("originality_evaluation", "Originality evaluation", "Assess whether a creative idea is genuinely new or a restatement of existing conventions.", SkillAtomType::StructuredRule, vec!["creative", "evaluation", "originality"]),
        SkillAtom::initial("human_truth_extraction", "Human truth extraction", "Identify the specific human emotion, behaviour, or tension that an advertising idea should be rooted in.", SkillAtomType::StructuredRule, vec!["insight", "human_truth", "creative"]),
        SkillAtom::initial("concept_development", "Concept development", "Develop advertising concepts from a strategic brief, generating multiple directions before converging.", SkillAtomType::PromptTemplate, vec!["creative", "concept", "ideation"]),
        SkillAtom::initial("art_direction_principles", "Art direction principles", "Apply visual hierarchy, negative space, and typographic rules in service of the advertising idea.", SkillAtomType::StructuredRule, vec!["art_direction", "visual", "creative"]),
        SkillAtom::initial("campaign_coherence_audit", "Campaign coherence audit", "Audit a campaign across executions to ensure the central idea is consistently expressed, not just the style.", SkillAtomType::StructuredRule, vec!["campaign", "coherence", "evaluation"]),
    ],
};
