...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("tribe_identification", "Tribe identification", "Define a brand's minimum viable audience by shared beliefs, not demographics — who already holds the worldview the brand embodies.", SkillAtomType::StructuredRule, vec!["tribe", "audience", "belief"]),
        SkillAtom::initial("remarkability_assessment", "Remarkability assessment", "Evaluate whether a product, campaign, or brand story is remarkable enough to generate unprompted word of mouth.", SkillAtomType::StructuredRule, vec!["remarkable", "word_of_mouth", "product"]),
        SkillAtom::initial("permission_marketing_design", "Permission marketing design", "Design opt-in communication sequences that deliver anticipated, personal, and relevant messages to a consenting audience.", SkillAtomType::PromptTemplate, vec!["permission", "email", "opt_in"]),
        SkillAtom::initial("brand_story_architecture", "Brand story architecture", "Build a brand narrative around a specific belief or tension that a defined audience holds — not product features.", SkillAtomType::PromptTemplate, vec!["story", "brand", "narrative"]),
        SkillAtom::initial("advocacy_loop_design", "Advocacy loop design", "Design the conditions under which delighted customers become voluntary advocates — what they need, see, and experience.", SkillAtomType::StructuredRule, vec!["advocacy", "word_of_mouth", "tribe"]),
    ],
};
