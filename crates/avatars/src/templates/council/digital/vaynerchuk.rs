...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("platform_timing_assessment", "Platform timing assessment", "Evaluate where each platform sits on the organic reach lifecycle curve and recommend timing of entry.", SkillAtomType::StructuredRule, vec!["platform", "organic", "timing"]),
        SkillAtom::initial("short_form_content_strategy", "Short-form content strategy", "Design short-form video content strategies for TikTok, Reels, Shorts optimised for organic discovery.", SkillAtomType::StructuredRule, vec!["short_form", "video", "organic"]),
        SkillAtom::initial("content_volume_planning", "Content volume planning", "Plan high-frequency content calendars that prioritise consistency and iteration over production quality.", SkillAtomType::StructuredRule, vec!["content", "calendar", "volume"]),
        SkillAtom::initial("document_vs_produce_framework", "Document vs produce framing", "Distinguish documentation content (raw, authentic, real-time) from produced content and advise mix per channel.", SkillAtomType::StructuredRule, vec!["content", "authenticity", "organic"]),
        SkillAtom::initial("rapid_iteration_protocol", "Rapid iteration protocol", "Design fast creative testing sprints: ship, measure, double down on what works, kill what doesn't within 72h.", SkillAtomType::StructuredRule, vec!["testing", "iteration", "speed"]),
    ],
};
