...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("psycho_logic_reframing", "Psycho-logic reframing", "Identify the psychological reframe that solves a consumer problem without changing the underlying product or service.", SkillAtomType::StructuredRule, vec!["behavioural_economics", "reframe", "psychology"]),
        SkillAtom::initial("signalling_analysis", "Signalling analysis", "Identify what quality, status, or trust signals a product or price is sending — and whether they match the intended message.", SkillAtomType::StructuredRule, vec!["signalling", "price", "perception"]),
        SkillAtom::initial("counterfactual_generation", "Counterfactual generation", "Systematically generate the opposite of every proposed solution to identify whether the conventional approach is actually optimal.", SkillAtomType::StructuredRule, vec!["counterfactual", "creative_strategy", "innovation"]),
        SkillAtom::initial("loss_aversion_application", "Loss aversion application", "Reframe value propositions from gain-seeking to loss-avoidance where the audience's risk sensitivity is high.", SkillAtomType::PromptTemplate, vec!["loss_aversion", "framing", "conversion"]),
        SkillAtom::initial("context_effect_design", "Context effect design", "Design the comparison context and reference points that make a product's value more apparent through contrast.", SkillAtomType::StructuredRule, vec!["context_effects", "comparison", "pricing"]),
    ],
};
