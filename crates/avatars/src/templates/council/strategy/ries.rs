...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("positioning_word_audit", "Positioning word audit", "Identify the single word or concept a brand owns in the category's mental hierarchy.", SkillAtomType::StructuredRule, vec!["positioning", "word", "brand"]),
        SkillAtom::initial("category_creation_strategy", "Category creation strategy", "Evaluate whether a brand should fight for position in an existing category or create a new one it can lead.", SkillAtomType::StructuredRule, vec!["category", "positioning", "strategy"]),
        SkillAtom::initial("line_extension_risk_assessment", "Line extension risk", "Assess the dilution risk of proposed line or brand extensions against the parent brand's core position.", SkillAtomType::StructuredRule, vec!["line_extension", "brand", "dilution"]),
        SkillAtom::initial("competitive_ladder_mapping", "Competitive ladder mapping", "Map the mental hierarchy of a category to identify which rung each brand occupies and what that implies for strategy.", SkillAtomType::StructuredRule, vec!["competitive", "positioning", "ladder"]),
        SkillAtom::initial("sacrifice_identification", "Sacrifice identification", "Identify what a brand must stop claiming or doing in order to own a position with clarity and credibility.", SkillAtomType::StructuredRule, vec!["focus", "sacrifice", "positioning"]),
    ],
};
