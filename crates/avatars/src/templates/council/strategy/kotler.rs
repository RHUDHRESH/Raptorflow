...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("stp_analysis", "STP analysis", "Execute structured segmentation, targeting, and positioning analysis to define a brand's market position.", SkillAtomType::StructuredRule, vec!["stp", "positioning", "strategy"]),
        SkillAtom::initial("marketing_mix_audit", "Marketing mix audit", "Audit all four Ps against customer needs, competitive position, and business objectives simultaneously.", SkillAtomType::StructuredRule, vec!["4ps", "marketing_mix", "audit"]),
        SkillAtom::initial("clv_modelling", "CLV modelling", "Model customer lifetime value across segments to identify which segments are worth acquiring at what cost.", SkillAtomType::StructuredRule, vec!["clv", "ltv", "segmentation"]),
        SkillAtom::initial("competitive_frame_setting", "Competitive frame setting", "Define the competitive frame of reference: who the brand is positioned against and what the category is.", SkillAtomType::StructuredRule, vec!["positioning", "competitive", "framing"]),
        SkillAtom::initial("value_proposition_design", "Value proposition design", "Construct value propositions that articulate who the product is for, what it does, and why it is different.", SkillAtomType::PromptTemplate, vec!["value_proposition", "positioning", "brief"]),
    ],
};
