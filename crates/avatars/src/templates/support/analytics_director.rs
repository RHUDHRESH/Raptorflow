...
static TEMPLATE: AvatarTemplate = AvatarTemplate {
    ...
    initial_skill_atoms: || vec![
        SkillAtom::initial("measurement_framework_design", "Measurement framework design", "Design measurement frameworks that connect marketing activity to business outcomes.", SkillAtomType::StructuredRule, vec!["analytics", "measurement", "framework"]),
        SkillAtom::initial("attribution_model_selection", "Attribution model selection", "Select and configure attribution models appropriate to the customer journey complexity.", SkillAtomType::StructuredRule, vec!["attribution", "analytics", "model"]),
        SkillAtom::initial("statistical_significance_review", "Statistical significance review", "Review test results for statistical and practical significance before declaring winners.", SkillAtomType::StructuredRule, vec!["statistics", "significance", "testing"]),
        SkillAtom::initial("vanity_metric_elimination", "Vanity metric elimination", "Identify and remove vanity metrics from reporting dashboards.", SkillAtomType::StructuredRule, vec!["analytics", "reporting", "kpi"]),
        SkillAtom::initial("dark_number_identification", "Dark number identification", "Identify what the current measurement system cannot capture and quantify its importance.", SkillAtomType::StructuredRule, vec!["analytics", "measurement", "attribution"]),
    ],
};
