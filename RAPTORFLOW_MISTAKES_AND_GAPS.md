...
**Problem:** The entire ripple memory retrieval system is **never wired into sessions**. The `RippleHarvester` exists (in `crates/harness/src/ripples.rs`) with 200+ lines of extraction logic, the DB has `ripples` table with salience, tags, emotion vectors — but when a session starts, the working memory is **always an empty vector**. Avatars have no memory of past learnings.

**Downstream Impact:** Every avatar session starts with a blank memory. No past learnings influence current decisions. The whole "ripple metabolism" concept is broken.

**Fix:**
1. Replace `load_working_memory` to actually query ripples from DB by org_id + agent_id with salience ordering
2. Use `avatar_memory_edges` table to find ripples linked to this avatar
3. Respect token_budget to limit number of ripples loaded

---

### 1.2 NO WEB SEARCH / INTERNET ACCESS FOR AVATARS

**Files:** Entire codebase (searched all crates, hooks, routes)

**Problem:** The marketing vision explicitly requires avatars to:
- Research competitors
- Find sources for claims
- Search the web for proof, statistics, case studies
- Do competitive intelligence

**Zero web search capability exists.** Not in any Rust crate, not in any frontend hook, not in any backend service. The `allowed_tools` in capability definitions only include `["bedrock.fast"]` — LLM inference only.

**Fix:**
1. Add `web-search` crate or use existing search API (SerpAPI, Tavily, etc.)
2. Add `allowed_tools: ["web_search"]` to capability definitions
3. Create `WebSearchEngine` service that takes a query and returns structured results
4. Wire into Cortex context assembly as a context source
...
**Files:**
- `crates/harness/src/council_orchestrator.rs`
- `crates/harness/src/avatar_soul.rs` — `should_challenge()`, `derive_instinct_frame()`

**Problem:** The council orchestrator's entire debate logic — challenge routing, instinct derivation, position synthesis — is **deterministic rules-based code**, not LLM calls. The only AI calls are:
- `start_council_session` (in `council.rs` routes) — generates initial positions
- `synthesize_council_session` — generates synthesis

The council **orchestrator** (`council_orchestrator.rs`) is completely deterministic. Challenges are decided by hardcoded functions like:
```rust
fn should_challenge(pack, event) -> ChallengeDecision { ... }
fn compute_challenge_routing(avatar_key) -> Vec<&str> { ... }
fn synthesize_council_result(...) -> Value { ... }
```

**Fix:**
1. Add AI-powered challenge evaluation (pass the position + event to Bedrock for critique)
2. Make instinct frame derivation LLM-assisted (context + avatar personality → instinct)
3. Keep deterministic layer as fallback/safety net
...
1. Wire ripple working memory to actual DB queries
2. Add web search capability for avatars  
