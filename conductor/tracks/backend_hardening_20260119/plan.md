...
- [~] Task: Conductor - User Manual Verification 'Phase 1: Supabase & PostgreSQL Hardening' (Protocol in workflow.md)
...
- [~] Task: Redis Pub/Sub for Real-time State Sync.
    - [ ] Create a message bus for agent progress updates.
    - [ ] Connect frontend listeners to Redis channels.
- [x] Task: Session Migration to Redis.
    - [x] Benchmarking current session latency.
    - [x] Migrate transient UI state to Redis KV pairs.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Redis (Upstash) Caching & Sync' (Protocol in workflow.md)
...
- [ ] Task: Migration Versioning System.
    - [ ] Set up a structured migration folder (`/migrations`).
    - [ ] Create a CLI utility to apply and track migrations via Supabase.
- [ ] Task: Conductor - User Manual Verification 'Phase 3: Task Reliability & Worker Hardening' (Protocol in workflow.md)

## Phase 4: Benchmarking & Final Audit
Final verification of performance gains and security.

- [ ] Task: Performance Benchmarking.
    - [ ] Run load tests simulating concurrent AI agent activity.
    - [ ] Verify latency improvements (>50% faster for cached paths).
- [ ] Task: Security Red-Team Audit.
    - [ ] Attempt unauthorized access to verify RLS effectiveness.
    - [ ] Verify SQL transaction atomicity under failure conditions.
- [ ] Task: Conductor - User Manual Verification 'Phase 4: Benchmarking & Final Audit' (Protocol in workflow.md)
