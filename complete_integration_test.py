...
class RaptorflowIntegrationTester:
    ...
    def test_endpoint(self, name, method, path, data=None, expected_status=200, show_response=False):
        ...
        try:
            ...
            self.log(f"{status_icon} {name} - {response.status_code} ({response.elapsed.total_seconds():.2f}s)")
    ...
    def test_core_systems(self):
        ...
        self.test_endpoint("Moves - Create", "POST", "/api/v1/moves/", {
        ...
        self.test_endpoint("Campaigns - Create", "POST", "/api/v1/campaigns/", {
        ...
        self.test_endpoint("Daily Wins - Generate", "POST", "/api/v1/daily_wins/generate", {
        ...
        # Blackbox System
        self.test_endpoint("Blackbox - List Strategies", "GET", "/api/v1/blackbox/strategies")
        self.test_endpoint("Blackbox - Generate Strategy", "POST", "/api/v1/blackbox/generate-strategy", {
        ...
        self.test_endpoint("Muse - Generate Content", "POST", "/api/v1/muse/generate", {
            "prompt": "Create content about system integration",
            "platform": "blog",
            "workspace_id": self.workspace_id,
            "user_id": self.user_id
        }, show_response=True)
        self.test_endpoint("Muse - Chat", "POST", "/api/v1/muse/chat", {
    ...
    def test_bcm_system(self):
        ...
        self.test_endpoint("BCM - Record Interaction", "POST", "/api/v1/bcm/record-interaction", {
            "agent": "integration_tester",
            "action": "complete_system_test",
            "context": {
                "test_timestamp": datetime.now().isoformat(),
                "systems_tested": ["moves", "campaigns", "daily_wins", "blackbox", "muse"]
            }
        }, show_response=True)
    ...
    def test_agents_system(self):
        ...
        self.test_endpoint("Agents - Execute Muse", "POST", "/api/v1/agents/muse/execute", {
        ...
        self.test_endpoint("Agents - Execute Blackbox", "POST", "/api/v1/agents/blackbox/execute", {
    ...
    def test_analytics_system(self):
        ...
        self.test_endpoint("Analytics - Dashboard", "GET", "/api/v1/analytics/dashboard")
    ...
    def test_cross_system_integration(self):
        ...
        self.test_endpoint("Integration - Move from Strategy", "POST", "/api/v1/blackbox/test-strategy/create-move")
        
        # Test completing a daily win
        self.test_endpoint("Integration - Complete Daily Win", "POST", "/api/v1/daily_wins/test-win/complete")
        
        # Test saving a Muse asset
        self.test_endpoint("Integration - Save Muse Asset", "POST", "/api/v1/muse/assets", {
    ...
    def run_comprehensive_test(self):
        ...
        self.log("\n🏥 System Health:")
        systems = {
            'Core Systems': ['Moves - List', 'Moves - Create', 'Campaigns - List', 'Campaigns - Create', 
                           'Daily Wins - List', 'Daily Wins - Generate', 'Blackbox - List Strategies', 
                           'Blackbox - Generate Strategy', 'Muse - List Assets', 'Muse - Generate Content'],
            'BCM System': ['BCM - Get Context', 'BCM - Get Evolution', 'BCM - Record Interaction'],
            'Tools & Services': ['Tools - Available', 'Services - Status'],
            'Agent System': ['Agents - Available', 'Agents - Execute Muse', 'Agents - Execute Blackbox'],
            'Analytics': ['Analytics - Dashboard'],
            'Cross-Integration': ['Integration - Move from Strategy', 'Integration - Complete Daily Win', 'Integration - Save Muse Asset']
        }
        
        for system_name, test_names in systems.items():
            system_success = sum(1 for name in test_names if self.test_results.get(name, {}).get('success'))
            system_total = len(test_names)
            status = "✅ HEALTHY" if system_success == system_total else "⚠️  DEGRADED" if system_success > 0 else "❌ FAILED"
        ...
        if successful_tests == total_tests:
            ...
            self.log("\n📋 What's Working:")
            self.log("   ✅ All core systems (Moves, Campaigns, Daily Wins, Blackbox, Muse)")
