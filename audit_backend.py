        ...
        for i, line in enumerate(lines):
            ...
            for category, pattern in PATTERNS.items():
                if re.search(pattern, content):
                    ...
                    if category == "PRINT_STMT" and "logger" in content:
                        continue  # accidental match? no, print( is specific.
...
def main():
    ...
    for root, dirs, files in os.walk(SEARCH_DIR):
        ...
        for file in files:
            if file in IGNORE_FILES:
                continue
            if not file.endswith((".py", ".sql", ".md", ".txt")):
                continue  # Focus on code/config
    ...
    print(
        f"Scan complete. Scanned {file_count} files. Found {len(all_findings)} issues."
    )
    print("Breakdown:")
