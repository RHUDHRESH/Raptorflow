      ...
      <GsapBridge stagger={true} className="flex-1 flex overflow-hidden">
        ...
        <aside className="gsap-reveal w-64 border-r border-[var(--border)] bg-[var(--card)] flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            <div className="text-[10px] font-mono uppercase tracking-widest text-[var(--muted-foreground)] mb-4">
              Feed Categories
            </div>
            {CATEGORIES.map((feed) => (
              <button
        ...
        <main className="flex-1 border-r border-[var(--border)] flex flex-col h-full bg-[var(--background)]">
          <div className="p-6 border-b border-[var(--border)] flex items-center justify-between">
            ...
            <div className="flex gap-1 bg-[var(--card)] border border-[var(--border)] p-1">
              <button className="px-4 py-1.5 text-[9px] font-mono uppercase tracking-widest transition-colors bg-[var(--foreground)] text-[var(--background)]">
                feed
              </button>
            </div>
          </div>
                ...
                {isSignalsLoading ? (
                  ...
                  signals?.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="gsap-reveal border border-[var(--border)] bg-[var(--card)] p-5 hover:border-[var(--primary)] transition-colors relative group cursor-pointer block"
                      >
                        <div
                          className={`absolute top-0 bottom-0 left-0 w-1 ${item.severity === "high" || item.severity === "critical" ? "bg-[var(--destructive)]" : item.severity === "medium" ? "bg-[#c7772d]" : "bg-[#5f8768]"}`}
                        />
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              <ExclamationTriangleIcon
                                className={`h-4 w-4 ${item.severity === "high" || item.severity === "critical" ? "text-[var(--destructive)] opacity-100" : "opacity-0"}`}
                              />
                            </div>
          ...
          <div className="p-6 space-y-8">
            {isOverviewLoading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-end border-b border-[var(--border)] pb-2">
                  <span className="text-sm text-[var(--muted-foreground)] text-[var(--destructive)]">
                    High priority
                  </span>
                  <span className="font-mono text-xl text-[var(--destructive)]">
                    {stats.highPriority}
                  </span>
                </div>
              </div>
            ...
            <div className="pt-4">
              ...
              <div className="grid grid-cols-4 gap-1">
                {Array.from({ length: 16 }).map((_, i) => (
                  <div
                    key={i}
                    className={`h-8 border border-[var(--border)] transition-opacity hover:opacity-80 ${i === 3 || i === 7 ? "bg-[var(--destructive)]/80" : i === 2 || i === 9 ? "bg-[var(--accent)]/50" : "bg-[var(--background)]"}`}
                  />
                ))}
              </div>
            </div>
