...
import { ArrowLeftIcon, BellIcon, LightningBoltIcon, ChatBubbleIcon, EyeOpenIcon, ArchiveIcon } from "@radix-ui/react-icons";
...
const NUDGE_TYPES: Record<string, { label: string; description: string; severity: string; action: string }> = {
  "major-competitor-shift": { 
    label: "Strategic Directive", 
    description: "A major competitor has shifted their positioning or launched a new campaign. This requires an immediate Council review of our Move 3 creative.", 
    severity: "critical", 
    action: "Review Council Rationale" 
  },
  "budget-underutilization": { 
    label: "Media Alert", 
    description: "Campaign budget is underutilized by more than 20% this week. Performance Node suggests scaling LinkedIn spend.", 
    severity: "warning", 
    action: "Update Budget Allocation" 
  },
  "opportunity-detected": { 
    label: "Growth Signal", 
    description: "New ICP segment identified based on recent conversion data. High salience match for our 'Identity' narrative.", 
    severity: "info", 
...
export default function NudgeDetailPage({
  ...
  const typeKey = Object.keys(NUDGE_TYPES).find(k => nudgeId.includes(k)) || "opportunity-detected";
  ...
  const relatedEvents = eventLog.filter(
    (e) => (e.type || e.eventType) === "intel_alert_received" || (e.type || e.eventType) === "task_missed_notification"
  ).slice(0, 5);
    ...
    <div className="flex flex-col gap-8 py-2">
      ...
      <header className="flex items-end justify-between border-b-2 border-[var(--foreground)] pb-6">
        ...
        <div className="flex flex-col items-end gap-2 text-right">
          <Badge className={
            nudge.severity === "critical" ? "bg-red-500/10 text-red-500 border-red-500/20" :
            nudge.severity === "warning" ? "bg-amber-500/10 text-amber-500 border-amber-500/20" :
        ...
        <aside className="space-y-6 sticky top-6">
          <div className="border border-[var(--border)] bg-[var(--card)] divide-y divide-[var(--border)]">
             <div className="px-5 py-4">
                <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: "var(--muted-foreground)" }}>Management</p>
             </div>
             <button className="w-full flex items-center justify-between px-5 py-4 hover:bg-[var(--foreground)] hover:text-[var(--background)] transition-all group">
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>View in Context</span>
