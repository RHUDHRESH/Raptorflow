      ...
      <div className="w-full max-w-[600px] space-y-10">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-base text-[#6B655E]">Add their URLs. Your AI team will start monitoring them immediately.</p>
          </div>
        ...
        {suggestions.length > 0 && slots.every(s => s.url) === false && (
          <div className="space-y-3 animate-in fade-in duration-700">
            <p className="text-[10px] uppercase font-bold tracking-widest text-[#6B655E]">Suggested from your website scan:</p>
          ...
          {slots.slice(0, visibleSlots).map((slot, index) => (
            <div key={index} className="space-y-1.5 group animate-in slide-in-from-top-2 duration-300">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-mono text-[#9A948C] w-5 tracking-tighter">0{index + 1}</span>
                
                <div className={cn(
                  "flex-1 relative border transition-all duration-300",
                  slot.error ? "border-red-500/50" : slot.isValidated ? "border-green-500/30" : "border-[#D5CBC0] focus-within:border-[#f59e0b]"
                )}>
                  <input
