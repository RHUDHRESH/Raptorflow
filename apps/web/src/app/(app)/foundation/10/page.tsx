...
function AutoWidthInput({
  ...
  return (
    <div className="inline-block relative">
      <span ref={spanRef} className="invisible absolute whitespace-pre text-xl font-medium px-1">
        {value || placeholder}
      </span>
      <input
...
export default function FoundationStep10() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await fetch(`${getApiBaseUrl()}/api/v1/foundation/section/positioning`, {
        method: "PATCH",
        headers: {
        ...
        {mode === "builder" ? (
          <div className="space-y-12">
            <div className="text-xl leading-[2.2] text-[#9A948C]">
              ...
              <AutoWidthInput
                value={slots.icp}
                onChange={(v) => setSlots({ ...slots, icp: v })}
                placeholder="[audience]"
                minWidth={160}
              />
              ,{" "}
              <AutoWidthInput
                value={slots.businessName}
                onChange={(v) => setSlots({ ...slots, businessName: v })}
              ...
              <AutoWidthInput
                ...
                onChange={(v) => setSlots({ ...slots, category: v })}
                placeholder="[category]"
              />{" "}
              that{" "}
              <AutoWidthInput
                value={slots.differentiator}
                onChange={(v) => setSlots({ ...slots, differentiator: v })}
                placeholder="[uniquity]"
                minWidth={220}
              />{" "}
              because{" "}
              <AutoWidthInput
                value={slots.proof}
                onChange={(v) => setSlots({ ...slots, proof: v })}
                placeholder="[proof]"
                minWidth={180}
              />
              .
            ...
            <div className="bg-[#FBF8F2] p-5 rounded-xl border border-[#E5DED4] space-y-3">
              <label className="text-[10px] uppercase font-bold tracking-widest text-[#6B655E]">
          ...
          {!showSuggestions ? (
            ...
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {[
                {
                  ...
                  category: "financial guard",
                  differentiator: "automates GST filing in 3 minutes",
                  proof: "we integrate directly with both Tally and bank feeds",
                },
                {
                  icp: "High-Growth Shopify Stores",
                  businessName: slots.businessName,
                  category: "performance partner",
                  differentiator: "scales your ad spend without increasing CAC",
                  proof: "our AI updates creative every 24 hours based on real sales data",
                },
                {
                  icp: "Operations Managers",
                  businessName: slots.businessName,
                  category: "visibility tool",
                  differentiator: "predicts stock-outs 14 days in advance",
                  proof: "we process over 1M purchase data points from your vertical baseline",
                },
              ].map((s, i) => (
                <div
                  key={i}
                  className="p-4 bg-[#262626] border border-[#D5CBC0] rounded-xl flex items-start justify-between gap-4 group"
                >
                  <p className="text-sm text-[#6B655E] leading-relaxed italic">
                    &quot;For {s.icp}, {s.businessName || "RaptorFlow"} is the {s.category} that{" "}
                    {s.differentiator} because {s.proof}.&quot;
                  </p>
                  <button
              ...
              <button
                ...
                className="text-xs text-[#9A948C] hover:text-[#6B655E]"
              >
                Hide suggestions
              </button>
            </div>
