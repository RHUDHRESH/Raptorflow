...
export default function FoundationStep11() {
  ...
  useEffect(() => {
    ...
    let nextText =
      "We help businesses like yours grow faster — without the guesswork. Here's what that looks like in practice.";

    if (f < 30 && s < 30 && b < 30) {
      nextText =
        "Hey! So honestly, we've been heads-down building something for people like you, and the results? Kind of wild.";
    } else if (f < 40 && s < 40 && b > 70) {
      nextText =
        "We're not here to be your average agency. We're here to 3x your pipeline. Let's go.";
    } else if (f > 70 && s > 70 && b < 30) {
      nextText =
        "We partner with organisations seeking measurable, sustained growth through disciplined marketing strategy.";
    } else if (f > 70 && s > 70 && b > 70) {
      nextText =
        "We deliver results. Across 200+ engagements, our clients average 47% improvement in qualified lead generation within 90 days.";
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await fetch(`${getApiBaseUrl()}/api/v1/foundation/section/brand_personality`, {
        method: "PATCH",
        headers: {
      ...
      <div className="w-full max-w-[600px] space-y-12">
          ...
          <div className="bg-[#FBF8F2] border border-[#E5DED4] rounded-xl p-6 min-h-[140px] shadow-2xl transition-all duration-300">
            ...
            <div className="flex flex-wrap gap-2 mt-6">
              {traits.map((t) => (
                <span
                  key={t}
                  className="px-3 py-1 bg-[#E5DED4] text-[#9A948C] border border-[#D5CBC0] rounded-full text-[10px] font-bold uppercase tracking-wider"
                >
                  {t}
                </span>
              ))}
        ...
        <div className="space-y-3 pt-4">
          <div className="space-y-1">
            <label className="text-sm font-medium text-[#9A948C] tracking-tight">
              One thing your brand would never say:
            </label>
            <p className="text-xs text-[#6B655E] tracking-tight">
              (Optional, but powerful — this trains your AI team on brand guardrails)
            </p>
          </div>
          <input
