...
export default function FoundationStep21() {
  ...
  const Slider = ({ labelL, labelR, val, setVal }: any) => (
    <div className="flex items-center gap-4 w-full">
      <span className="w-24 text-right text-[10px] font-bold uppercase tracking-widest text-[#6B655E]">{labelL}</span>
      <input 
        type="range" min="0" max="100" value={val} 
        onChange={(e) => setVal(parseInt(e.target.value))}
        className="flex-1 h-1 bg-[#E5DED4] rounded-full appearance-none accent-[#f59e0b] cursor-pointer"
      />
      <span className="w-24 text-left text-[10px] font-bold uppercase tracking-widest text-[#6B655E]">{labelR}</span>
    </div>
  );
  ...
  const summary = useMemo(() => {
    ...
    return (
      <p className="text-lg text-[#2A2622] font-light leading-relaxed">
        ...
        You serve <span className="text-[#2A2622] font-medium">{icp.name}</span> — {icp.pain}. 
        The problem you solve is {problem}. 
        Your positioning: {positioning}. 
        Your goal for the next 90 days is <span className="text-[#2A2622] font-medium">{goal}</span>. 
        The competitors I&apos;ll be watching are {comps}. 
        And {bizName} should always sound <span className="text-[#2A2622] font-medium">{voice}</span>.&quot;
      </p>
    );
  }, [sectionData]);
      ...
      <div className="fixed inset-0 bg-[#FBF8F2] z-50 flex flex-col items-center justify-center p-6 transition-all duration-1000">
        {!isComplete && !error && (
          <div className="w-full max-w-lg space-y-12 flex flex-col items-center animate-in fade-in duration-1000">
            <h2 className="text-3xl font-bold text-[#2A2622] tracking-widest uppercase">RaptorFlow</h2>
        ...
        {part >= 1 && (
          <div className={cn("space-y-8 animate-in fade-in slide-in-from-top-4 duration-700", part > 1 && "opacity-20 pointer-events-none grayscale scale-[0.98]")}>
            <div className="space-y-2 text-center">
              <h1 className="text-3xl font-bold text-[#2A2622]">Your Campaign Strategist is about to come to life.</h1>
        ...
        {part >= 2 && (
          <div className={cn("space-y-8 animate-in fade-in slide-in-from-top-4 duration-700", part > 2 && "opacity-20 pointer-events-none grayscale scale-[0.98]")}>
            <div className="space-y-1 text-center">
              ...
              <p className="text-sm text-[#6B655E] uppercase tracking-widest font-bold">Shape how they communicate with you</p>
            </div>
            ...
            <div className="space-y-2 bg-[#FBF8F2] p-8 rounded-2xl border border-[#E5DED4]">
              ...
              <div className="mt-8 flex items-center gap-3 bg-black/40 border border-[#E5DED4] rounded-xl p-3">
                ...
                <p className="text-xs text-[#6B655E]">
                  Based on your answers, we suggest: <span className="text-[#2A2622] font-bold">[Direct, Challenger, Data-driven]</span> — adjust freely.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-1000">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#6B655E]">{strategistName} speaking:</span>
            ...
            <div className="flex flex-col items-center gap-6 pt-4">
              ...
              <p className="text-xs text-[#6B655E] uppercase tracking-tighter">By launching, you authorise your AI team to begin monitoring and strategy work.</p>
            </div>
          </div>
        )}
