...
export default function FoundationStep13() {
  ...
  const togglePriority = (name: string) => {
    ...
    setChannels((prev) =>
      prev.map((c) => (c.name === name ? { ...c, isPriority: !c.isPriority } : c)),
    );
    setError(null);
  };
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await fetch(`${getApiBaseUrl()}/api/v1/foundation/section/content_channels`, {
        method: "PATCH",
        headers: {
      ...
      <div className="w-full max-w-[640px] space-y-12">
        ...
        <div className="space-y-2 text-center">
          ...
          <p className="text-base text-[#6B655E]">
            Select every channel that matters to your business.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {channels.map((c) => (
            ...
            >
                  ...
                  >
                    <Star
                      className={cn(
                        ...
                        c.isPriority ? "fill-[#f59e0b] text-[#f59e0b]" : "",
                      )}
                    />
                  </button>
                )}
              ...
              >
                <label className="text-[10px] uppercase font-bold tracking-widest text-[#f59e0b]/70">
                  You are currently:
                </label>
                <div className="relative">
                  ...
                  >
                    {[
                      ...
                      "Consistent and performing",
                    ].map((opt) => (
                      <option key={opt} value={opt} className="bg-[#F5F0E8]">
                        {opt}
                      </option>
                    ))}
                  </select>
        ...
        {selectedCount > 0 && (
          <div className="space-y-6 pt-6 border-t border-[#E5DED4] animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="space-y-1 text-center">
              <h2 className="text-base font-medium text-[#2A2622]">
                Which 2 channels should we prioritise first?
              </h2>
