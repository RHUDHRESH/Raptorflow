...
export default function FoundationStep9() {
  ...
  const MODELS = [
    {
      ...
      desc: "Customers pay once per product",
      icon: ShoppingCart,
    },
    {
      id: "recurring",
      label: "Recurring subscription",
      desc: "Monthly or annual fees",
      icon: RefreshCw,
    },
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await fetch(`${getApiBaseUrl()}/api/v1/foundation/section/pricing_model`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="flex flex-col items-center px-6 pt-20 pb-24 min-h-screen bg-[#FBF8F2]">
      <div className="w-full max-w-[560px] space-y-10">
        {/* HEADER */}
        <h1 className="text-3xl font-bold text-[#2A2622]">How does your business make money?</h1>
          ...
          {MODELS.map((model) => {
            ...
            return (
              ...
              >
                ...
                <model.icon
                  className={cn("w-5 h-5 mb-3", isSelected ? "text-[#f59e0b]" : "text-[#6B655E]")}
                />
                <div className="space-y-0.5">
                  <p className="text-sm font-semibold text-[#2A2622] leading-tight">
                    {model.label}
                  </p>
          ...
          <div className="space-y-4 pt-4 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-[#9A948C] tracking-tight">
                What does your core offering cost?
              </label>
              {!priceRange.varies && (
                <span className="text-[10px] text-[#6B655E] uppercase tracking-widest font-bold">
                  In INR (₹)
                </span>
              )}
            </div>
            ...
            {priceRange.varies ? (
              <div className="p-4 bg-[#262626] border border-[#E5DED4] rounded-lg">
                <p className="text-sm text-[#f59e0b] font-medium italic">
                  Pricing varies per customer or project
                </p>
              </div>
            ) : (
              <div className="flex items-center gap-4">
                <div className="relative flex-1">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B655E] text-sm">
                    ₹
                  </span>
                  <input
                    type="number"
            ...
            <label className="flex items-center gap-2 cursor-pointer group w-fit">
              ...
              />
              <span className="text-sm text-[#6B655E] group-hover:text-[#2A2622] transition-colors">
                Varies widely
              </span>
            </label>
          </div>
        ...
        <div className="space-y-2 pt-4">
          <label className="text-sm text-[#9A948C] font-medium">
          ...
          <p className="text-xs text-[#6B655E]">
            What does a typical customer spend with you over their entire relationship?
          </p>
        </div>
