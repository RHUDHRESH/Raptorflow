    ...
    <div className="flex flex-col items-center px-6 pt-20 pb-24 min-h-screen bg-[#1a1a1a]">
      <div className="w-full max-w-[560px] space-y-10">
          ...
          {MODELS.map((model) => {
            ...
            return (
              ...
              >
                ...
                <model.icon className={cn("w-5 h-5 mb-3", isSelected ? "text-[#f59e0b]" : "text-zinc-500")} />
                <div className="space-y-0.5">
                  <p className="text-sm font-semibold text-white leading-tight">{model.label}</p>
                  <p className="text-[11px] text-zinc-400 leading-tight">{model.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        {selectedModels.length > 0 && (
          <div className="space-y-4 pt-4 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-zinc-300 tracking-tight">What does your core offering cost?</label>
              {!priceRange.varies && (
                <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">In INR (₹)</span>
              )}
            </div>

            {priceRange.varies ? (
              <div className="p-4 bg-[#262626] border border-zinc-800 rounded-lg">
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="number"
                    className="w-full bg-[#262626] border border-zinc-700 rounded-lg pl-8 pr-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#f59e0b] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    placeholder="From 0"
            ...
            <label className="flex items-center gap-2 cursor-pointer group w-fit">
              ...
              />
              <span className="text-sm text-zinc-400 group-hover:text-white transition-colors">Varies widely</span>
            </label>
          </div>
        )}
        ...
        <div className="space-y-2 pt-4">
          <label className="text-sm text-zinc-300 font-medium">Average customer lifetime value (optional)</label>
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 text-sm">₹</span>
            <input
              type="number"
              className="w-full bg-[#262626] border border-zinc-700 rounded-lg pl-8 pr-4 py-3 text-white focus:outline-none focus:border-[#f59e0b]"
              placeholder="0"
              value={ltv}
              onChange={(e) => setLtv(e.target.value)}
            />
          </div>
          <p className="text-xs text-zinc-500">What does a typical customer spend with you over their entire relationship?</p>
        </div>
