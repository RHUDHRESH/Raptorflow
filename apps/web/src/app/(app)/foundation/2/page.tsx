...
export default function FoundationStep2() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const response = await fetch(`${getApiBaseUrl()}/api/v1/foundation/scan/status`, {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // Save to Backend
      await fetch(`${getApiBaseUrl()}/api/v1/foundation/section/scan_results`, {
        method: "PATCH",
        headers: {
        ...
        {status === "loading" ? (
          ...
          <div className="space-y-6">
                  ...
                  className={cn(
                    ...
                    formData.confidence === "High"
                      ...
                      : formData.confidence === "Medium"
                        ? "bg-[#FBE9DE] border-[#D97757] text-[#D97757]"
            ...
            <div className="space-y-4">
              ...
              >
                {showMultiProduct
                  ? "Hide additional areas ↑"
                  : "This business sells multiple things →"}
              </button>

              {showMultiProduct && (
                <div className="p-6 bg-[#262626] border border-[#D5CBC0] rounded-2xl space-y-2">
                  <label className="text-xs font-bold text-[#6B655E] uppercase tracking-wider">
                    Tell us about your other product or service areas
                  </label>
                  <textarea
                    rows={2}
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, multiProductDetails: e.target.value })
                    }
                  />
                </div>
              )}
