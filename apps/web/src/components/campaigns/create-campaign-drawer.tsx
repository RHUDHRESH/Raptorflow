...
export function CreateCampaignDrawer({ open, onClose }: DrawerProps) {
  ...
  return (
    <>
              ...
              <div className="h-full flex flex-col justify-center animate-in fade-in duration-700">
                ...
                {briefStatus === "failed" && (
                  <div className="text-center space-y-4">
                    ...
                    <Button variant="ghost" className="text-[#6B655E]" onClick={() => setStep(1)}>
                      Try again
                    </Button>
                  </div>
                )}
      ...
      {/* Persistence Toast */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-[100] bg-[#D97757] text-black px-6 py-3 rounded-xl font-bold border border-white/20 shadow-2xl animate-in fade-in slide-in-from-right-4 duration-300">
          {toastMessage}
        </div>
      )}
    </>
