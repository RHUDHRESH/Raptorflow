...
- **Why it matters:** DeepWiki provides AI-grounded documentation browsing + Q&A for GitHub repositories. It can be treated as an MCP resource when agents need quick, structured context about upstream libraries, specs, or vendor repos.
- **Setup (SSE):** `serverUrl: https://mcp.deepwiki.com/sse` (recommended; supported by Claude and most MCP clients).
- **Setup (Streamable HTTP):** `serverUrl: https://mcp.deepwiki.com/mcp` (newer protocol; also supports the legacy `/sse`).
- **Available tools:** `read_wiki_structure`, `read_wiki_contents`, `ask_question`.
