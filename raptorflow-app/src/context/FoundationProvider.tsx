...
const FoundationContext = createContext<FoundationContextValue | undefined>(
  undefined
);
const FOUNDATION_METADATA_PATH =
  process.env.NEXT_PUBLIC_FOUNDATION_METADATA_PATH || '/foundation_test.json';
const FOUNDATION_METADATA_FALLBACK = '/foundation_text.json';
...
export function FoundationProvider({ children }: { children: ReactNode }) {
  ...
  const loadFoundation = useCallback(async () => {
    ...
    try {
      const loadFrom = async (path: string) => {
        ...
        return (await response.json()) as FoundationData;
      };

      try {
        const data = await loadFrom(FOUNDATION_METADATA_PATH);
        setFoundation(data);
      } catch (primaryError) {
        if (FOUNDATION_METADATA_PATH !== FOUNDATION_METADATA_FALLBACK) {
          const data = await loadFrom(FOUNDATION_METADATA_FALLBACK);
          setFoundation(data);
        } else {
          throw primaryError;
        }
      ...
      setError(
        err instanceof Error ? err.message : 'Unknown error loading foundation'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const getCampaigns = useCallback((): SimpleCampaign[] => {
    ...
    foundation.positioning?.components?.value_themes?.forEach(
      (theme, index) => {
        campaigns.push({
...
export function useFoundationMentions() {
  const { getCohorts, getCompetitors, getCampaigns, isLoading } =
    useFoundation();
  return {
    cohorts: getCohorts(),
