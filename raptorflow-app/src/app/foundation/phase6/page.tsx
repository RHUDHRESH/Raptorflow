...
export default function Phase6Page() {
  ...
  const { data, isLoading, isSaving, updateData, saveProgress } =
    useOnboarding();
    ...
    try {
      const response = await fetch('/api/v1/synthesis/soundbites', {
        ...
        body: JSON.stringify({
          ...
          proof_points:
            (data.proof as any)?.evidence?.map((e: any) => e.content) || [],
        }),
      });
  ...
  const handleUpdate = (id: string, text: string) => {
    updateData((prev) => ({
      ...
      phase6: {
        ...prev.phase6!,
        soundbites: prev.phase6!.soundbites.map((sb) =>
          sb.id === id ? { ...sb, text } : sb
        ),
      },
    }));
  ...
  const handleContinue = async () => {
    const lockedCount =
      data.phase6?.soundbites?.filter((sb) => sb.isLocked).length || 0;
    if (lockedCount < 7) {
      toast.error('Validate all 7 soundbites.', {
        description: 'Each resonance vector must be locked to proceed.',
      });
      return;
    }
