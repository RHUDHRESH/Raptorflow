...
function generateCandidateSegments(phase4: Phase4Data): CandidateSegment[] {
    ...
    const alternatives = [
        ...phase4.competitiveAlternatives.statusQuo.map(a => ({ name: a.name, type: 'statusQuo' })),
        ...phase4.competitiveAlternatives.direct.map(a => ({ name: a.name, type: 'direct' })),
        ...phase4.competitiveAlternatives.indirect.map(a => ({ name: a.name, type: 'indirect' })),
    ];

    // Generate segment candidates from phase4 segments
    phase4.targetSegments.filter(s => !s.isExcluded).forEach(segment => {
        const dominantValue = phase4.differentiatedValue.find(v => v.isDominant)?.value || '';
    ...
    candidates.forEach(c => {
        c.fitScore.total = Math.round(
            (c.fitScore.valueFit + c.fitScore.switchability + c.fitScore.reachability +
                c.fitScore.dealSize + c.fitScore.confidence) / 5 * 20
        );
    });
 ...
 */
function convertToICPs(candidates: CandidateSegment[], phase4: Phase4Data): ICP[] {
    return candidates.filter(c => c.kept).slice(0, 5).map((candidate, index) => {
...
function buildForces(phase4: Phase4Data): ICPForcesOfProgress {
    ...
    return {
        ...
        pull: [
            { text: phase4.differentiatedValue.find(v => v.isDominant)?.value || 'Systematic approach', confidence: 'inferred' }
        ],
        anxiety: [
...
function buildBuyingGroup(): BuyingGroup {
    const roles: BuyingRole[] = [
        { id: uuidv4(), role: 'Economic Buyer (Founder/CEO)', isActive: true, influence: 'high' },
        { id: uuidv4(), role: 'Champion (Head of Marketing)', isActive: true, influence: 'high' },
        { id: uuidv4(), role: 'Primary User (Marketing Manager)', isActive: true, influence: 'medium' },
        { id: uuidv4(), role: 'Technical Evaluator', isActive: false, influence: 'low' },
        { id: uuidv4(), role: 'Finance/Procurement', isActive: false, influence: 'low' },
        { id: uuidv4(), role: 'Blocker (Status Quo Priest)', isActive: true, influence: 'medium' }
    ];

    const buyingJobs: BuyingJob[] = [
        { job: 'problem-id', primaryRoleId: roles[0].id, supportRoleIds: [roles[1].id] },
        { job: 'solution-explore', primaryRoleId: roles[1].id, supportRoleIds: [roles[2].id] },
        { job: 'requirements', primaryRoleId: roles[1].id, supportRoleIds: [roles[0].id] },
        { job: 'supplier-select', primaryRoleId: roles[0].id, supportRoleIds: [roles[1].id] }
    ];
...
function buildHabitat(): ICPHabitat {
    return {
        ...
        searchTermsProblem: ['how to do marketing as a founder', 'marketing for startups', 'marketing strategy template'],
        searchTermsSolution: ['marketing automation', 'positioning tool', 'marketing OS'],
        contentTypes: ['Case studies', 'Templates', 'Benchmarks', 'How-to guides'],
        triggerMoments: ['After funding', 'Before launch', 'Revenue plateau', 'Hiring marketing'],
...
function buildPersona(role: BuyingRole, icp: ICP): ICPPersona {
    ...
    const template = Object.entries(personaTemplates).find(([key]) =>
    ...
    return {
        id: uuidv4(),
        role: role.role,
        goals: template.goals || [{ text: 'Achieve their objectives', confidence: 'assumed' }],
        kpis: template.kpis || [],
