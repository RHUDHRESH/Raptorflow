...
export function extractCompactObjects(
  ...
  return {
    ...
    bestCustomerSeeds: (customerInsights?.bestCustomers || []).map(
      (desc, i) => ({
        description: desc,
    ...
    buyingSystem: {
      buyerRole:
        cohorts.buyerRole || cohorts.buyerRoleChips?.join(', ') || 'Founder',
...
export async function deriveFromFoundation(
  foundation: FoundationData
): Promise<DerivedData> {
        ...
        platforms: [
          {
            ...
            vibe: 'Professional thought leadership',
          },
          {
            name: 'Twitter/X',
            timing: 'Throughout day',
            vibe: 'Industry hot takes',
          },
        ],
  ...
  return {
    ...
    ladder: [
      ...
      },
      {
        ...
        description: positioning.ownedPosition || 'Unique approach',
        score: 85,
        isYou: true,
      },
      {
    ...
    oneThing:
      positioning.ownedPosition || 'The only marketing OS that thinks with you',
    defensibility: 'medium',
  };
}

function generateMockCompetitive(
  objects: CompactObjects,
  foundation: FoundationData
): DerivedCompetitive {
  return {
...
function generateMockSoundbites(
  objects: CompactObjects,
  foundation: FoundationData
): DerivedSoundbites {
  ...
  return {
    oneLiner:
      messaging.promisePillar ||
      `${business.name} helps you ${positioning.psychologicalOutcome || 'win'}.`,
    soundbites: [
      {
        type: 'problem',
...
function generateMockMarket(
  ...
  return {
    tam: {
      value: 5000000000,
      confidence: 'low',
      method: 'Global market for category',
    },
