...
import { BlackboxState, Experiment, LearningArtifact } from './blackbox-types';
...
export async function createExperimentDB(
  experiment: Experiment
): Promise<void> {
  const tenantId = await getTenantId();
  const dbExp = {
    ...mapFrontendExperimentToDB(experiment),
    tenant_id: tenantId,
  };
  const { error } = await supabase.from('experiments').insert(dbExp);
  ...
  return [
    {
      ...
      content:
        'Recent analysis shows that customers who engage with onboarding materials are 3x more likely to stay...',
    ...
    },
    {
      ...
      content:
        'Based on market analysis, tiered pricing with a free trial increases conversion rates by 25%...',
    ...
    },
    {
      ...
      content:
        'Features introduced with interactive tutorials see 40% higher adoption rates...',
...
export async function triggerLearningCycle(moveId: string) {
  try {
    const response = await fetch(
      `${API_URL}/v1/blackbox/learning/cycle/${moveId}`,
      {
        method: 'POST',
      }
    );
...
export async function runSpecialistAgent(
  ...
  try {
    const response = await fetch(
      `${API_URL}/v1/blackbox/specialist/run/${agentId}/${moveId}`,
      {
        method: 'POST',
