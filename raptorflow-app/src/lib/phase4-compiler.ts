...
function buildCategoryOptions(phase3: Phase3Data): MarketCategory[] {
  ...
  const primaryClaim = phase3.claims.find(
    (c) => c.id === phase3.primaryClaimId
  );
...
function buildAlternatives(phase3: Phase3Data): CompetitiveAlternatives {
  ...
  const statusQuo: CompetitiveAlternative[] = phase3.jtbd.forces.habit
    .slice(0, 3)
    .map((h, i) => ({
      id: uuidv4(),
...
function translateCapabilitiesToValue(phase3: Phase3Data): {
  ...
  const capabilities: DifferentiatedCapability[] = phase3.differentiators.map(
    (d) => ({
      id: uuidv4(),
...
function generatePositioningOutputs(
  ...
  const dominantValue =
    values.find((v) => v.isDominant)?.value || 'marketing clarity and control';
  ...
  const objections: ObjectionKillshot[] = [
    {
      ...
      response:
        'This replaces 5 tools, not adds another. Net time saved: 10+ hours/week.',
      evidence: '',
    },
    {
      id: uuidv4(),
      ...
      response:
        'Agencies cost $5K+/mo with no knowledge transfer. This builds internal capability.',
      evidence: '',
    },
    {
      id: uuidv4(),
      ...
      response:
        'Notion has no strategy engine. You still have to figure out WHAT to do.',
      evidence: '',
    },
  ];
...
function generateVisuals(
  ...
  const perceptualMap: PerceptualMap = {
    xAxis: {
      ...
      rationale:
        'Status quo lacks direction; direct competitors focus on execution only',
  ...
  const ladder: CategoryLadder = {
    ...
    whyDifferentLadder:
      'We created a new category: Founder Marketing System. Not competing on "marketing automation" ladder.',
  };
...
export async function compilePhase4(phase3: Phase3Data): Promise<Phase4Data> {
  ...
  const marketCategory =
    categoryOptions.find((c) => c.risk === 'ok') || categoryOptions[0];
  ...
  const { statement, pitch, weAreWeAreNot, objections } =
    generatePositioningOutputs(
      marketCategory,
      targetSegments,
      values,
      competitiveAlternatives
