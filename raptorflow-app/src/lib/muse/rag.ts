...
async function loadFoundationFallback(): Promise<string> {
  for (const candidate of fallbackCandidates) {
    try {
      ...
      console.warn(`Failed to load ${candidate} fallback:`, error);
    }
  }
  return '';
}
...
export async function searchMuseAssets(
  ...
  if (!backendUrl) {
    console.warn(
      'Muse RAG backend not configured. Falling back to local metadata.'
    );
  ...
  if (!auth) {
    console.warn('Muse RAG auth missing. Falling back to local metadata.');
    return loadFoundationFallback();
  }
