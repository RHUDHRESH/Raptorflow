...
export function selectSkill(context: MuseContext, systemSkills: Skill[]): Skill {
    // 1. Filter by intent
    const eligibleSkills = systemSkills.filter(s => s.intent === context.intent);
    ...
    const scored = eligibleSkills.map(skill => {
        ...
        const painMatch = context.icp.pains.primary.some(p => skill.compatiblePains.includes(p));
        if (painMatch) score += 0.15;
        else if (skill.compatiblePains.length > 0) score -= 0.20; // Penalize if skill targets specific pains but ICP doesn't have them

        // Tone Match (+0.10 / -0.10)
        const toneMatch = context.icp.language.tone.some(t => skill.compatibleTone.includes(t));
        if (toneMatch) score += 0.10;
        else if (skill.compatibleTone.length > 0) score -= 0.10;

        // Urgency Match (+0.10 / -0.15)
        if (context.icp.targeting.urgency && skill.compatibleUrgency.includes(context.icp.targeting.urgency)) {
            score += 0.10;
        } else if (skill.compatibleUrgency.includes('now') && context.icp.targeting.urgency === 'someday') {
            score -= 0.15; // Mismatch
        }

        // CTA Alignment (+0.05 / -0.10)
        const ctaMatch = context.icp.language.ctaStyle.some(c => skill.compatibleCta.includes(c));
        ...
        const conflict = allDisqualifiers.some(d => skill.incompatibleTraits.includes(d));
    ...
    scored.forEach(s => console.log(`[${s.skill.name}]: ${s.score.toFixed(2)}`));
