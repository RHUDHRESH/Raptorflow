...
export const saveAssetTool = new DynamicStructuredTool({
  ...
  description:
    'Saves a generated marketing asset to the Muse Library. Use this once an asset (email, post, etc.) is finalized.',
  schema: z.object({
    title: z.string().describe('Clear title for the asset'),
    content: z.string().describe('The full text content of the asset'),
    type: z
      .string()
      .describe("The type of asset (e.g., 'social-post', 'sales-email')"),
    metadata: z
      .record(z.any())
      .optional()
      .describe('Additional context like campaign ID or cohort ID'),
  }),
  func: async ({ title, content, type, metadata }) => {
    ...
    if (!auth) {
      return 'Authentication required to save assets.';
    }
