...
function derivePrimaryContext(phase1: Phase1Discovery): PrimaryContext {
    ...
    const audience = phase1.buyerUser?.userRoles?.length > 0
...
function extractJTBDForces(phase1: Phase1Discovery): JTBDForces {
    ...
    phase1.triggers?.triggers?.forEach(t => {
        if (t.type) {
            ...
            push.push(triggerLabels[t.type] || t.freeText || 'Frustration with current approach');
        }
    });
    ...
    // Switch triggers
    const switchTriggers = phase1.triggers?.triggers?.map(t => t.type || t.freeText || '') || [];
...
function buildVPC(phase1: Phase1Discovery, jtbd: JTBDForces): VPCData {
    ...
    const productsServices = [
        phase1.offer?.primaryType || 'Core solution',
    ];
 ...
 */
function deriveCompetitiveFactors(phase1: Phase1Discovery, vpc: VPCData): CanvasFactor[] {
    ...
    const painBasedFactors = vpc.customerProfile.pains.slice(0, 3).map(pain => ({
 ...
 */
function buildStrategyCanvas(factors: CanvasFactor[], phase1: Phase1Discovery): StrategyCanvas {
...
function generateClaims(
    ...
    const primaryJob = jtbd.jobs.find(j => j.isPrimary)?.statement || 'achieve their goals';
    ...
    claims.push({
        ...
        uniqueness: phase1.offer?.unfairAdvantage?.whyCantCopy || 'First to market with this blend',
        proof: [],
        proofStrength: 'C',
        riskFlags: [],
        isSpecific: true,
 ...
 */
function buildProofStack(claims: Claim[], phase1: Phase1Discovery): ProofStackEntry[] {
 ...
 */
function generateDifferentiators(phase1: Phase1Discovery, errc: ERRCGrid): Differentiator[] {
...
export async function derivePhase3(phase1: Phase1Discovery): Promise<Phase3Data> {
 ...
 */
export async function derivePhase3FromQuestionnaire(data: any): Promise<Phase3Data> {
    ...
    const phase1Mock = {
        ...
        origin: {
            narrative: data.messaging?.problemSolved || 'Building the solution they needed.',
        },
        triggers: {
            triggers: [
                { type: 'growth-plateau', freeText: data.target?.painPoints?.[0] || 'Growth has stalled' },
            ],
        },
        currentSystem: {
