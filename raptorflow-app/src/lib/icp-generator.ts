 ...
 */
export function generateICPsFromFoundation(
  data: FoundationData
): GeneratedIcp[] {
...
function selectTemplates(
  ...
  if (customerTypes.includes('b2b')) {
    if (
      ...
      buyerRole.toLowerCase().includes('marketing') ||
...
function extractPains(data: FoundationData): string[] {
  ...
  if (data.positioning?.psychologicalOutcome) {
    pains.push(
      `Lack of ${data.positioning.psychologicalOutcome.toLowerCase()}`
    );
  }
