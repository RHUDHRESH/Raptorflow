...
export function generateExperiments(
    ...
    return selectedLevers.map((lever, index) => {
        ...
        const defaults = CHANNEL_DEFAULTS[context.channel] || CHANNEL_DEFAULTS.other;
...
function selectConstraint(risk: RiskLevel): ConstraintTemplate {
    ...
    if (risk === 'spicy') {
        potential = [...potential, ...CONSTRAINTS.filter(c => c.risk_level === 'safe')];
    } else if (risk === 'unreasonable') {
        potential = [...potential, ...CONSTRAINTS.filter(c => c.risk_level === 'spicy')];
    }
...
function generateSkillStack(channel: ChannelType): SkillRef[] {
    ...
    if (channel === 'email') {
        return [
            { skill_id: 'hook_curiosity', version: '1.0', owner: 'system', role: 'hook' },
            { skill_id: 'structure_pas', version: '1.0', owner: 'system', role: 'structure' },
            { skill_id: 'tone_founder', version: '1.0', owner: 'system', role: 'tone' },
            { skill_id: 'cta_soft', version: '1.0', owner: 'system', role: 'cta' },
            { skill_id: 'polish_check', version: '1.0', owner: 'system', role: 'edit_polish' },
        ];
    }
    // Minimal for others
    return [
...
function generateAssetPlan(channel: ChannelType, constraint: ConstraintTemplate): AssetPlan {
