...
> = {
  ...
  'problem-agitate': {
    ...
    template:
      '{problem}. Every {time}, this means {agitation}. If this continues, {stakes}.',
  },
  outcome: {
  ...
  'objection-kill': {
    ...
    template:
      "Worried about {objection}? Here's what actually happens: {response}.",
  },
  proof: {
    awarenessStage: 'product',
...
function buildSB7Spine(phase5: Phase5Data): SB7Spine {
  ...
  if (!primaryICP) {
    return {
      ...
      problemPhilosophical:
        "You shouldn't need an agency to market your own business",
      guide: 'RaptorFlow',
      guideVibe: 'operator',
      plan: [
        'Complete Foundation',
        'Get your Messaging Kit',
        'Execute with Moves',
      ],
  ...
  return {
    ...
    problemExternal:
      primaryICP.forces.push[0]?.text || 'Current tools are fragmented',
...
function buildAwarenessLadder(phase5: Phase5Data): AwarenessStageContent[] {
  ...
  > = {
    unaware: {
      ...
      sampleLine:
        "Most founders spend 10+ hours/week on marketing that doesn't move the needle.",
    },
    problem: {
    ...
    product: {
      ...
      sampleLine:
        'Unlike agencies or point tools, RaptorFlow gives you strategy + execution in one system.',
    },
    most: {
      ...
      sampleLine:
        'Start your Foundation now. 20 minutes. Positioning locked by end of day.',
    },
  };
...
function generateSignal7Soundbites(
  phase5: Phase5Data,
  ...
  types.forEach((type, index) => {
    ...
    switch (type) {
      ...
      case 'proof':
        text =
          "Founders report saving 10+ hours/week. First campaign live in 3 days, not 3 months. If it doesn't work, we'll refund your first month.";
        proofNeeded = 'Testimonial, metric, guarantee';
        break;

      case 'cta':
        text =
          "Start your Foundation session now. It takes 20 minutes. You'll have your positioning locked by end of day.";
...
function scoreRigor(
  ...
  const hasConcreteNoun =
    /founder|marketer|ceo|tool|spreadsheet|agency|hubspot/.test(lowerText);
  let specificity = Math.min(
    5,
    Math.max(1, 4 - fluffCount + (hasNumbers ? 1 : 0) + (hasTimeframe ? 1 : 0))
  );
  ...
  const proof =
    hasMetric && hasGuarantee ? 5 : hasMetric ? 4 : hasTestimonial ? 3 : 2;

  // Gate C: Differentiation
  const hasContrast = /unlike|instead of|not like|different from/.test(
    lowerText
  );
  const hasSpecificAlt =
    /hubspot|agency|spreadsheet|notion|mailchimp|hootsuite/.test(lowerText);
  const differentiation =
    hasContrast && hasSpecificAlt
      ? 5
      : hasContrast
        ? 4
        : hasSpecificAlt
  ...
  const earlyStageTypes: Signal7Type[] = [
    'status-quo-enemy',
    'problem-agitate',
  ];
  const hasProductDetails =
    /feature|dashboard|integration|api|click|button/.test(lowerText);
  ...
  const cognitiveLoad =
    wordCount < 15 ? 5 : wordCount < 25 ? 4 : wordCount < 40 ? 3 : 2;
  ...
  const total = Math.round(
    ((specificity + proof + differentiation + awarenessFit + cognitiveLoad) /
      25) *
      100
  );
      ...
      riskReversal:
        sb.type === 'proof'
          ? {
              ...
              details: 'Money-back guarantee if not satisfied',
            }
          : undefined,
...
function buildAdAngles(soundbites: Signal7Soundbite[]): AdAngle[] {
  const angleTypes: Array<{
    angle: AdAngle['angle'];
    soundbiteType: Signal7Type;
  }> = [
      ...
      variants: [
        {
          ...
          content: {
            openingHook:
              outcomeSb?.text.split('.')[0] + '.' ||
              'Most founders waste 10+ hours/week on marketing.',
            body: `${mechanismSb?.text || ''}\n\n${proofSb?.text || ''}`,
            cta: 'Comment "SYSTEM" and I\'ll send you the framework.',
          },
          soundbiteIds: [outcomeSb?.id, mechanismSb?.id, proofSb?.id].filter(
            Boolean
          ) as string[],
          awarenessStage: 'solution',
        },
        {
          ...
          content: {
            openingHook:
              blueprint.controllingIdea || 'Marketing. Finally under control.',
...
function buildQAReport(soundbites: Signal7Soundbite[]): QAReport {
  const allText = soundbites
    .map((s) => s.text)
    .join(' ')
    .toLowerCase();

  // Fluff detection
  const fluffyWords = FLUFFY_WORDS.filter((word) => allText.includes(word)).map(
    (word) => ({
      word,
      location:
        soundbites.find((s) => s.text.toLowerCase().includes(word))?.type ||
        'unknown',
    })
  );
  ...
  return {
    sevenSecondTestPass:
      soundbites.filter((s) => s.scores.cognitiveLoad >= 4).length >= 4,
...
function buildMessageSpine(phase5: Phase5Data): MessagingBlueprint {
  const primaryICP =
    phase5.icps.find((i) => i.id === phase5.interICPGraph.primaryWedgeICP) ||
    phase5.icps[0];
...
export async function forgePhase6(
  phase5: Phase5Data,
  toneRisk: ToneRisk = 'balanced'
): Promise<Phase6Data> {
  // Step 1: VoC Mining
...
export function regenerateSoundbite(
  soundbite: Soundbite,
  phase5: Phase5Data
): Soundbite {
...
export function regenerateSignal7Soundbite(
  ...
  if (punchLevel > 3) {
    ...
    newText = soundbite.text.replace(/\d+/g, (match) =>
      String(parseInt(match) * 1.2)
    );
  }
