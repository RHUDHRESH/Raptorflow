...
async function initiatePayment(
  ...
  const response = await fetch(
    `${apiUrl}/v1/payments/initiate?${params.toString()}`,
    {
      method: 'POST',
    }
  );
  ...
  it('initiates PhonePe payment via the new payments endpoint', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining(
        '/v1/payments/initiate?user_id=user-123&amount=100&transaction_id=tx-001&redirect_url=http%3A%2F%2Flocalhost%3A3000'
      ),
