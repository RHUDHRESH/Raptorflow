...
export function FoundationTimeline({ events, className }: FoundationTimelineProps) {
    ...
    const getIcon = (type: TimelineEvent['type']) => {
        switch (type) {
            case 'voice': return <Sparkles className="h-3 w-3" />;
            case 'positioning': return <Target className="h-3 w-3" />;
            case 'checkpoint': return <CheckCircle2 className="h-3 w-3" />;
            ...
            <div className="relative space-y-8">
                ...
                {sortedEvents.length === 0 && (
                    <div className="py-12 text-center">
                        <p className="text-xs text-muted-foreground italic font-sans">No history recorded yet.</p>
                    </div>
                )}
            </div>
        </div>
