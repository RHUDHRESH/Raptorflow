...
export function FoundationResults() {
  ...
  const coreMessage =
    phase6?.blueprint?.controllingIdea || phase6?.blueprint?.coreMessage || '';
  const primaryICP = phase5?.icps?.[0];
  const primaryClaim =
    phase3?.claims?.find((c) => c.id === phase3?.primaryClaimId)?.promise ||
    phase3?.claims?.[0]?.promise ||
    '';
      ...
      <div className="bg-[#2D3538] text-white py-20 px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-white/60 text-lg max-w-xl mx-auto">
            Everything you need to communicate with clarity and close with
            confidence.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-8 py-16 space-y-8">
        ...
        <div className="result-card bg-white border border-[#E5E6E3] rounded-3xl p-10 shadow-sm relative group overflow-hidden">
          ...
          <p className="font-serif text-[28px] text-[#2D3538] leading-[1.4] mb-6">
            ...
            {positioningStatement ||
              'Complete Phase 4 to generate your positioning statement.'}
            "
          </p>
          {positioningStatement && (
            <button
              onClick={() =>
                copyToClipboard(positioningStatement, 'Positioning statement')
              }
        ...
        {primaryICP && (
          <div className="result-card bg-white border border-[#E5E6E3] rounded-2xl overflow-hidden">
            <button
              onClick={() =>
                setExpandedSection(expandedSection === 'icp' ? null : 'icp')
              }
                ...
                <div className="bg-[#FAFAF8] rounded-xl p-5">
                  ...
                  <ul className="space-y-2">
                    {(primaryICP.triggers || []).slice(0, 3).map((t, i) => (
                      <li
                        key={i}
                        className="text-sm text-[#2D3538] flex items-start gap-2"
                      >
        ...
        {phase3?.differentiators && phase3.differentiators.length > 0 && (
          <div className="result-card bg-white border border-[#E5E6E3] rounded-2xl overflow-hidden">
            <button
              onClick={() =>
                setExpandedSection(expandedSection === 'diff' ? null : 'diff')
              }
              className="w-full flex items-center justify-between p-6 hover:bg-[#FAFAF8] transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <span className="font-medium text-[#2D3538]">
                    {phase3.differentiators.length} unique capabilities
                  </span>
                </div>
              </div>
            ...
            {expandedSection === 'diff' && (
              <div className="p-6 pt-0 space-y-3">
                {phase3.differentiators.map((diff, i) => (
                  <div
                    key={diff.id || i}
                    className="bg-[#FAFAF8] rounded-xl p-5 group relative"
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div
                        className={`w-2 h-2 rounded-full ${diff.status === 'proven' ? 'bg-[#2D3538]' : 'bg-[#C0C1BE]'}`}
                      />
                      <span className="text-[10px] font-mono uppercase text-[#9D9F9F]">
                        {diff.status}
                      </span>
                    </div>
                    <p className="font-medium text-[#2D3538]">
                      {diff.capability}
                    </p>
                    {diff.mechanism && (
                    ...
                    <button
                      onClick={() =>
                        copyToClipboard(
                          `${diff.capability}: ${diff.mechanism}`,
                          'Differentiator'
                        )
                      }
        ...
        {phase6?.soundbites && phase6.soundbites.length > 0 && (
          <div className="result-card bg-white border border-[#E5E6E3] rounded-2xl overflow-hidden">
            <button
              onClick={() =>
                setExpandedSection(expandedSection === 'sound' ? null : 'sound')
              }
              className="w-full flex items-center justify-between p-6 hover:bg-[#FAFAF8] transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <span className="font-medium text-[#2D3538]">
                    {phase6.soundbites.length} ready-to-use messages
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 pt-0 space-y-3">
                {phase6.soundbites.slice(0, 5).map((sb: any, i) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        copyToClipboard(sb.copy || sb.text || '', 'Soundbite')
                      }
                      className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 hover:bg-white rounded-lg"
                    >
                      <Copy className="w-3.5 h-3.5 text-[#9D9F9F]" />
