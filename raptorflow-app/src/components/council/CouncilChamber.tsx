  ...
  useEffect(() => {
    if (isSimulating) {
      const interval = setInterval(() => {
        const randomExpert =
          EXPERTS[Math.floor(Math.random() * EXPERTS.length)].id;
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <header className="text-center space-y-2">
        <h1 className="text-4xl font-serif text-ink dark:text-primary-text">
          The Expert Council
        </h1>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
              {EXPERTS.map((expert) => (
                <motion.div
                  ...
                  className={cn(
                    ...
                    activeExpert === expert.id
                      ? 'ring-2 ring-accent border-accent bg-surface'
                      : 'hover:border-borders hover:bg-surface',
                    pulsingExpert === expert.id &&
                      'bg-accent/5 shadow-[0_0_15px_rgba(215,201,174,0.3)]'
                  )}
                ...
                >
                  ...
                  <span className="text-[10px] text-secondary-text text-center mt-1">
                    {expert.role}
                  </span>
                </motion.div>
              ))}
              ...
              <CardTitle className="text-lg font-medium flex items-center gap-2">
                {activeExpert ? (
                  <>
                    ...
                    <span className="text-xs font-normal text-secondary-text tracking-wider uppercase">
                      Thought
                    </span>
                  </>
                ) : (
                ...
                {activeExpert ? (
                  ...
                  >
                    <p className="text-primary-text leading-relaxed font-serif text-lg">
                      "Our strategy must leverage the current market delta. I
                      propose a surgical strike on the top 3 competitor keywords
                      while maintaining our core brand philosophy."
                    </p>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-12">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-secondary-text max-w-[200px]">
                        Click on a council member to inspect their contribution
          ...
          {consensusMetrics && (
            <Card className="bg-surface border-borders/50 shadow-sm overflow-hidden">
              <div
                ...
                style={{ width: `${consensusMetrics.alignment * 100}%` }}
              />
              <CardContent className="p-4 flex justify-between items-center">
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-mono font-bold text-accent">
                    {(consensusMetrics.alignment * 100).toFixed(0)}%
                  </p>
                </div>
                <div className="text-right space-y-1">
                  <p className="text-[10px] text-secondary-text uppercase tracking-widest text-right">
                    Strategic Confidence
                  </p>
                  <p className="text-sm font-medium text-primary-text">
                    {(consensusMetrics.confidence * 100).toFixed(0)}%
                  </p>
                </div>
              </CardContent>
            </Card>
