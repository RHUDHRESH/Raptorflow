...
export function ConfidenceHeatmap({ scores }: ConfidenceHeatmapProps) {
    ...
    const displayScores = scores || EXPERTS.reduce((acc, expert) => {
    ...
    return (
        <div className="p-6 rounded-2xl bg-surface border border-borders/50 shadow-sm">
            <header className="flex justify-between items-center mb-6">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary-text">Council Alignment Heatmap</h4>
                        ...
                        <div key={expert.id} className="group relative flex flex-col items-center">
                            <motion.div
                                ...
                                style={{
                                    ...
                                    boxShadow: score > 0.9 ? '0 0 10px rgba(215, 201, 174, 0.4)' : 'none'
                                }}
                            >
                                <expert.icon className={cn("size-3.5", score > 0.6 ? "text-ink" : "text-secondary-text opacity-40")} />
                            </motion.div>
                            ...
                            <div className="absolute bottom-full mb-2 hidden group-hover:block z-50 pointer-events-none">
                                <div className="bg-ink text-white text-[9px] px-2 py-1 rounded whitespace-nowrap shadow-xl">
                                    ...
                                    <p className="opacity-70">Alignment: {(score * 100).toFixed(0)}%</p>
                                </div>
                            </div>
                        </div>
                    )
