    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-xs text-secondary-text mt-1">
          Upload strategy docs, brand guides, or industry reports to train the
          Council.
        </p>
      </header>
      ...
      <div
        ...
        className={cn(
          ...
          isDragging
            ...
            : 'border-borders/50 bg-surface/30 hover:border-borders hover:bg-surface/50'
        )}
      >
        <div className="p-4 rounded-full bg-canvas border border-borders/30 mb-4 shadow-sm">
          <Upload
            className={cn(
              ...
              isDragging ? 'text-accent' : 'text-muted-fill'
            )}
          />
        </div>
        <div className="space-y-1">
          <p className="text-sm font-medium text-ink">
            Drag & Drop knowledge artifacts
          </p>
      ...
      <AnimatePresence>
        {files.length > 0 && (
          <motion.div
            ...
            className="space-y-3"
          >
            <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-fill">
              Active Training
            </h4>
            {files.map((file) => (
              <motion.div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1 bg-borders/20 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{
                        width: `${file.status === 'done' ? 100 : file.progress || 50}%`,
                      }}
                    />
                  </div>
                </div>
      ...
      {files.some((f) => f.status === 'done') && (
        <div className="p-4 rounded-2xl bg-accent/5 border border-accent/20 flex gap-3 items-center">
          ...
          <p className="text-[11px] text-primary-text font-medium italic">
            The Librarian is extracting heuristics from your documents to update
            Expert DNA.
          </p>
        </div>
      )}
