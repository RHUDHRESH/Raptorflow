  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border-2 overflow-hidden ${
        isPrimary ? 'border-[#2D3538]' : 'border-[#E5E6E3]'
      }`}
    >
      {/* Header */}
      <div
      ...
      <div className="p-5 bg-[#FAFAF8] border-t border-[#E5E6E3]">
        ...
        <ul className="space-y-2">
          {struggles.slice(0, 3).map((struggle, i) => (
            <li
              key={i}
              className="flex items-start gap-2 text-sm text-[#5B5F61]"
            >
          ...
          {struggles.length === 0 && (
            <li className="text-sm text-[#9D9F9F] italic">
              No struggles defined
            </li>
          )}
        </ul>
      </div>
  ...
  const forceCategories = [
    {
      key: 'push' as const,
      label: 'Push',
      desc: 'What pushes them away from status quo',
      icon: ArrowRight,
      color: '#E57373',
    },
    {
      key: 'pull' as const,
      label: 'Pull',
      desc: 'What pulls them toward new solution',
      icon: Lightbulb,
      color: '#81C784',
    },
    {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-[#5B5F61]">
              Push/Pull/Anxiety/Habit analysis
            </p>
          </div>
        </div>
        {isExpanded ? (
        ...
        >
          <div className="grid grid-cols-2 gap-4 p-5">
            {forceCategories.map((cat) => (
              <div key={cat.key} className="bg-[#FAFAF8] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-[#9D9F9F] block">
                      {cat.desc}
                    </span>
                  </div>
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl">
        ...
        <p className="text-sm text-[#5B5F61]">
          <strong className="text-[#2D3538]">Jobs-To-Be-Done</strong> captures
          the real "why they buy" — not demographics. Each ICP has functional,
          emotional, and social jobs. Mark one as primary.
        </p>
      </div>
