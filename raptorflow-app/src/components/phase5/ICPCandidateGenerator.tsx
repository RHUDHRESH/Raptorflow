...
import {
    Target, TrendingUp, Users, AlertTriangle, Check,
...
function CandidateCard({
    ...
    const scoreColor = fitScore.total >= 80 ? 'text-green-600' :
        fitScore.total >= 60 ? 'text-yellow-600' : 'text-red-600';
    ...
    return (
        <motion.div
            ...
            className={`bg-white border-2 rounded-2xl overflow-hidden transition-all ${isPrimary ? 'border-[#2D3538] shadow-lg' :
                    isSecondary ? 'border-[#9D9F9F]' : 'border-[#E5E6E3]'
                }`}
        >
            {/* Header */}
            <div className="p-6 border-b border-[#E5E6E3]">
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isPrimary ? 'bg-[#2D3538]' : 'bg-[#F3F4EE]'
                            }`}>
                            <Users className={`w-5 h-5 ${isPrimary ? 'text-white' : 'text-[#2D3538]'}`} />
                        </div>
                        <div>
                    ...
                    <div className="text-right">
                        ...
                        <span className="text-[10px] font-mono text-[#9D9F9F] block">Fit Score</span>
                    </div>
                </div>

                <h3 className="font-serif text-xl text-[#2D3538] mb-2">{candidate.label}</h3>
                <p className="text-sm text-[#5B5F61]">
                    Displacing: <span className="font-medium text-[#2D3538]">{candidate.displacedAlternative}</span>
                </p>
            </div>
                    ...
                    >
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="text-sm text-[#5B5F61]">
                                    {candidate.reason || 'Auto-matched based on pain/trigger alignment'}
                                </p>
                            </div>
                            <div>
                                <span className="text-[10px] font-mono uppercase text-[#9D9F9F] block mb-2">
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 p-4 bg-[#F3F4EE] rounded-xl">
                ...
                <p className="text-sm text-[#5B5F61]">
                    <span className="font-medium text-[#2D3538]">{candidates.length} candidate segments</span> generated.
                    Select a Primary ICP and optionally add Secondary ICPs.
                </p>
            </div>
                        ...
                        {primaryId && (
                            <div className="bg-white/10 rounded-xl px-4 py-2 flex items-center gap-2">
                                ...
                                <span className="text-sm">
                                    Primary: {candidates.find(c => c.id === primaryId)?.label.split(' (')[0]}
                                </span>
                            </div>
                        )}
                        {secondaryIds.map(id => (
                            <div key={id} className="bg-white/5 rounded-xl px-4 py-2 flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-white/60" />
                                <span className="text-sm text-white/80">
                                    Secondary: {candidates.find(c => c.id === id)?.label.split(' (')[0]}
                                </span>
                            </div>
                        ))}
                    </div>
