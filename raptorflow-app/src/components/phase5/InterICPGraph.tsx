  ...
  return (
    <motion.div
      ...
      className={`bg-white border-2 rounded-2xl p-5 ${
        isWedge ? 'border-[#2D3538] shadow-lg' : 'border-[#E5E6E3]'
      }`}
    >
      <div className="flex items-center gap-4">
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center ${
            isICP ? 'bg-[#2D3538]' : 'bg-[#9D9F9F]'
          }`}
        >
          {isICP ? (
...
function EdgeItem({
  ...
  const edgeConfig =
    EDGE_TYPES.find((t) => t.type === edge.type) || EDGE_TYPES[0];
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          <div
            ...
            className={`w-1 h-3 rounded-sm ${
              i <= edge.weight ? 'bg-[#2D3538]' : 'bg-[#E5E6E3]'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl">
        ...
        <div>
          <p className="text-sm text-[#5B5F61]">
            <strong className="text-[#2D3538]">ICP Relationship Graph</strong> —
            How your ICPs connect. Set your primary wedge and define the
            targeting sequence.
          </p>
        </div>
      </div>
