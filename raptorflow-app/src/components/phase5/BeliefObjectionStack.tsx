...
import {
    Lightbulb, AlertTriangle, Shield, FileText,
    Video, Award, Clock, GripVertical, Plus, X, Check
} from 'lucide-react';
...
const PROOF_TYPE_CONFIG: Record<string, { icon: React.ElementType; label: string; color: string }> = {
...
const DEFAULT_OBJECTIONS: Omit<ObjectionItem, 'id'>[] = [
    { objection: 'Is this worth my time?', rootCause: 'Previous tools abandoned', proofTypeRequired: 'case-study', isDealKiller: false, rank: 1 },
    { objection: 'Will my team actually use it?', rootCause: 'Change management concern', proofTypeRequired: 'demo', isDealKiller: false, rank: 2 },
    { objection: 'How long until we see results?', rootCause: 'ROI timeline anxiety', proofTypeRequired: 'guarantee', isDealKiller: false, rank: 3 },
    { objection: 'Is our data secure?', rootCause: 'Compliance / privacy concern', proofTypeRequired: 'testimonial', isDealKiller: true, rank: 4 }
];
...
function BeliefCard({
    ...
    return (
        <motion.div
            ...
            className={`bg-white border rounded-xl p-4 ${belief.isRequired ? 'border-[#2D3538]' : 'border-[#E5E6E3]'
                }`}
        >
            <div className="flex items-start gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${belief.isRequired ? 'bg-[#2D3538]' : 'bg-[#F3F4EE]'
                    }`}>
                    <Lightbulb className={`w-4 h-4 ${belief.isRequired ? 'text-white' : 'text-[#9D9F9F]'}`} />
                </div>
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                        <button
                            ...
                            className={`text-[10px] font-mono uppercase px-2 py-1 rounded ${belief.isRequired
...
function ObjectionCard({
    ...
    return (
        ...
        >
            <motion.div
                layout
                className={`bg-white border-2 rounded-xl overflow-hidden ${objection.isDealKiller ? 'border-red-500' : 'border-[#E5E6E3]'
                    }`}
            >
                    ...
                    <div className="flex-1 p-4">
                        <div className="flex items-start justify-between mb-2">
                            ...
                            <button
                                ...
                                className={`text-[10px] px-2 py-1 rounded transition-colors ${objection.isDealKiller
                        ...
                        <h4 className="font-medium text-[#2D3538] mb-1">{objection.objection}</h4>
                        <p className="text-xs text-[#5B5F61] mb-3">
                            <span className="text-[#9D9F9F]">Root cause:</span> {objection.rootCause}
                        </p>
...
export function BeliefObjectionStack({
    ...
    const displayObjections = objections.length > 0 ? objections :
    ...
    const dealKillerCount = displayObjections.filter(o => o.isDealKiller).length;
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[#5B5F61]">What they must believe to buy</p>
                        </div>
                    </div>
                    <Button
                        variant="outline"
                    ...
                    >
                        <div className="flex gap-2">
                            <input
                                ...
                                autoFocus
                            />
                            <Button onClick={handleAddBelief} className="bg-[#2D3538] rounded-xl">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[#5B5F61]">
                                Drag to rank by importance • {dealKillerCount} deal killer{dealKillerCount !== 1 ? 's' : ''}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-5">
                ...
                <div className="flex flex-wrap gap-3">
                    {Object.entries(PROOF_TYPE_CONFIG).map(([key, config]) => {
                        const count = displayObjections.filter(o => o.proofTypeRequired === key).length;
