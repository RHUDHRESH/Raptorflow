...
import {
    CheckCircle2, AlertTriangle, FileText, Video,
    Briefcase, Users, Shield, Wallet, Scale, HelpCircle
} from 'lucide-react';
import { BuyingJobTypeExtended, BuyingRole, BuyingJobsCoverageCell } from '@/lib/foundation';
...
const BUYING_JOBS: Array<{ id: BuyingJobTypeExtended; label: string; description: string }> = [
    { id: 'problem-id', label: 'Problem ID', description: 'Recognize they have a problem worth solving' },
    { id: 'solution-explore', label: 'Solution Explore', description: 'Research what solutions exist' },
    { id: 'requirements', label: 'Requirements', description: 'Define what they need in a solution' },
    { id: 'supplier-select', label: 'Supplier Select', description: 'Choose which vendor to work with' },
    { id: 'validation', label: 'Validation', description: 'Confirm the solution works as expected' },
    { id: 'consensus', label: 'Consensus', description: 'Get buy-in from all stakeholders' },
];
...
function getAssetSuggestion(job: BuyingJobTypeExtended, roleType: string): string {
...
function CoverageCell({
    ...
    const asset = cell?.assetSuggested || getAssetSuggestion(job, role.role.split(' ')[0]);

    return (
        <td className="border border-[#E5E6E3] p-0">
            <div
                className={`p-3 h-full min-h-[120px] cursor-pointer hover:bg-[#F3F4EE] transition-colors ${hasGap ? 'bg-red-50' : ''
                    }`}
            ...
            >
                {isEditing ? (
                    ...
                    <div className="space-y-2">
                        {/* Belief */}
                        <div>
                            <span className="text-[8px] font-mono uppercase text-[#9D9F9F]">Belief</span>
                        ...
                        {/* Asset */}
                        <div>
                            <span className="text-[8px] font-mono uppercase text-[#9D9F9F]">Asset</span>
            ...
            <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl">
                ...
                <div>
                    <p className="text-sm text-[#5B5F61]">
                        <strong className="text-[#2D3538]">Buying Jobs Coverage</strong> — Map what each role
                        must believe at each stage of the buying process, and what asset resolves it.
                    </p>
                        ...
                        <thead>
                            <tr className="bg-[#FAFAF8]">
                                ...
                                {activeRoles.map(role => {
                                    const RoleIcon = ROLE_ICONS[role.role.split(' (')[0]] || ROLE_ICONS.default;
                                    return (
                                        <th key={role.id} className="border border-[#E5E6E3] p-3 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                <RoleIcon className="w-4 h-4 text-[#9D9F9F]" />
                                                <span className="text-[10px] font-medium text-[#2D3538]">
                                                    {role.role.split(' (')[0]}
                                                </span>
                                                <span className={`text-[8px] font-mono uppercase px-1.5 py-0.5 rounded ${role.influence === 'high'
            ...
            {gaps.length > 0 && (
                ...
                >
                    ...
                    <p className="text-sm text-red-700">
                        These gaps will be addressed in Phase 6 (Soundbite Forge) where we'll create
                        the missing assets and messaging.
                    </p>
                </motion.div>
            )}
        </div>
