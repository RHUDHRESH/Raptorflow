...
import {
    Users, Target, Zap, Shield, AlertCircle,
    Globe, Building2, Briefcase, DollarSign, Wrench,
...
function calculateDataQuality(foundation: FoundationData | null): DataQualityMeter {
    if (!foundation) {
        return { firmographic: 0, pain: 0, trigger: 0, proof: 0, constraints: 0, overall: 0 };
    }
    ...
    if (foundation.phase1Discovery?.identity?.company) firmographic += 30;
    if (foundation.phase1Discovery?.value?.priceRangeMin) firmographic += 20;
    if (foundation.phase1Discovery?.buyerUser?.buyerRoles?.length) firmographic += 20;
    ...
    // Phase 1/3: Triggers
    if (foundation.phase1Discovery?.triggers?.triggers?.length) trigger += 50;
    ...
    // Phase 1: Constraints
    if (foundation.phase1Discovery?.proofGuardrails?.forbiddenClaims?.length) constraints += 50;
    if (foundation.phase1Discovery?.proofGuardrails?.wordsToAvoid?.length) constraints += 50;

    const overall = Math.round((firmographic + pain + trigger + proof + constraints) / 5);
...
function extractEvidenceEntities(foundation: FoundationData | null): EvidenceEntity[] {
    ...
    foundation.phase1Discovery?.currentSystem?.workflowSteps?.forEach((step, i) => {
        if (step.tool) {
            entities.push({
                id: `tool-${i}`,
                type: 'tool',
    ...
    // Price from Phase 1
    if (foundation.phase1Discovery?.value?.priceRangeMin) {
        entities.push({
            id: 'price-range',
            type: 'price',
            value: `$${foundation.phase1Discovery.value.priceRangeMin} - $${foundation.phase1Discovery.value.priceRangeMax || 'N/A'}`,
...
export function ICPEngineLanding({
    ...
    const [dataQuality, setDataQuality] = useState<DataQualityMeter>(
        { firmographic: 0, pain: 0, trigger: 0, proof: 0, constraints: 0, overall: 0 }
    );
    ...
    const qualityBars = [
        { label: 'Firmographic', value: dataQuality.firmographic, color: '#2D3538' },
        ...
        <div className="space-y-8">
                ...
                <div className="flex items-start gap-6">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-white/70 text-lg leading-relaxed mb-6">
                            We will generate <span className="text-white font-medium">3 ICP candidates</span> +
                            <span className="text-white font-medium"> 1 negative ICP</span> based on everything
                            collected so far. No more questioning — just confirm and adjust.
                        </p>
                    ...
                    <div className="flex gap-2">
                        {(['b2b', 'b2c', 'hybrid'] as const).map(type => (
                            <button
                                ...
                                className={`flex-1 px-4 py-3 rounded-xl text-sm font-medium transition-all ${businessType === type
                    ...
                    <div className="flex gap-2">
                        {(['india', 'global', 'mixed'] as const).map(geo => (
                            <button
                                ...
                                className={`flex-1 px-4 py-3 rounded-xl text-sm font-medium transition-all capitalize ${geoFocus === geo
                                        ? 'bg-[#2D3538] text-white'
                                        : 'bg-[#F3F4EE] text-[#5B5F61] hover:bg-[#E5E6E3]'
                                    }`}
                            >
                                {geo === 'india' ? '🇮🇳 India' : geo === 'global' ? '🌍 Global' : '🔀 Mixed'}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            >
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full ${dataQuality.overall >= 70 ? 'bg-green-500' :
                                dataQuality.overall >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                            }`} />
            ...
            >
                ...
                {evidenceEntities.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3 text-[#9D9F9F]">
                        ...
                        <span className="text-sm">No evidence entities extracted yet. Complete earlier phases for richer ICPs.</span>
                    </div>
                )}
            </motion.div>
            ...
            >
                ...
                <p className="text-sm text-[#5B5F61] leading-relaxed">
                    <strong className="text-[#2D3538]">Why this matters:</strong> B2B purchases involve 6–10 stakeholders,
                    and buyers spend only ~17% of the journey talking to suppliers. If you don't map the buying group
                    and their "buying jobs," you're building fantasy marketing.
                </p>
            </motion.div>
        </div>
    );
