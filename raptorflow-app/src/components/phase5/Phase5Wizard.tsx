...
import {
    Check, ArrowRight, Users, AlertCircle, Sparkles
} from 'lucide-react';
...
export function Phase5Wizard() {
    ...
    // Session state
    const [businessType, setBusinessType] = useState<'b2b' | 'b2c' | 'hybrid'>('b2b');
    const [geoFocus, setGeoFocus] = useState<'india' | 'global' | 'mixed'>('global');
    ...
    const [primaryJobType, setPrimaryJobType] = useState<'functional' | 'emotional' | 'social'>('functional');
    ...
    const handleSelectPrimary = (id: string) => {
        ...
        if (idx >= 0) {
            ...
            const icpIdx = phase5.icps.findIndex(icp => icp.name.includes(phase5.candidateSegments[idx].label.split(' (')[0]));
    ...
    const handleUpdateCandidate = (id: string, updates: Partial<CandidateSegment>) => {
        setPhase5({
            ...phase5,
        ...
        >
            ...
            {currentScreen === 'jtbd' && currentICP && (
                <JTBDStruggleMoments
                    ...
                    onUpdateJTBD={(updates) => handleUpdateICP({ jtbd: { ...currentICP.jtbd, ...updates } })}
                    onUpdateForces={(updates) => handleUpdateICP({ forces: { ...currentICP.forces, ...updates } })}
                    onUpdateStruggles={() => { }}
            ...
            {currentScreen === 'buying-group' && currentICP && (
                <BuyingGroupMap
                    ...
                    onReorderRoles={(roles) => handleUpdateICP({
            ...
            {currentScreen === 'buying-jobs' && currentICP && (
                <BuyingJobsCoverage
                    ...
                    onUpdateCell={(cell) => {
                        ...
                        const existing = cells.findIndex(c =>
                            c.buyingJob === cell.buyingJob && c.roleId === cell.roleId
                        );
                        if (existing >= 0) {
                            cells[existing] = cell;
                        } else {
                        ...
                        setPhase5({
                            ...phase5,
                            buyingJobsCoverage: { cells, gaps: phase5.buyingJobsCoverage?.gaps || [] }
                        });
                    }}
                    onFlagGap={(gap) => {
            ...
            {currentScreen === 'triggers' && (
                <TriggerStack
                    ...
                    onAddTrigger={(trigger) => {
                        setPhase5({
                            ...phase5,
                            triggerStack: [...(phase5.triggerStack || []), { ...trigger, id: uuidv4() }]
                        });
                    }}
                ...
                <ChannelHabitatMap
                    ...
                    onTogglePrimary={(id) => {
                        setPhase5({
                            ...phase5,
                            channelHabitats: (phase5.channelHabitats || []).map(c =>
                                c.id === id ? { ...c, isPrimary: !c.isPrimary, isSecondary: false } : c
                            )
                        });
                    }}
                    ...
                    primaryCount={(phase5.channelHabitats || []).filter(c => c.isPrimary).length}
                    secondaryCount={(phase5.channelHabitats || []).filter(c => c.isSecondary).length}
                />
            )}
            ...
            {currentScreen === 'beliefs' && (
                <BeliefObjectionStack
                    ...
                    onAddObjection={(objection) => {
                        setPhase5({
                            ...phase5,
                            objectionStack: [...(phase5.objectionStack || []), { ...objection, id: uuidv4() }]
                        });
                    }}
                    onMarkDealKiller={(id) => {
                        setPhase5({
            ...
            {currentScreen === 'negative' && (
                <NegativeICPBuilder
                    negativeICP={phase5.negativeICP || { industries: [], disqualifiers: [], behaviorSignals: [] }}
                    onUpdate={(updates) => {
                        setPhase5({
                            ...phase5,
                            negativeICP: { ...(phase5.negativeICP || { industries: [], disqualifiers: [], behaviorSignals: [] }), ...updates }
                        });
                    }}
                    onAddDisqualifier={(disq) => {
                        const current = phase5.negativeICP || { industries: [], disqualifiers: [], behaviorSignals: [] };
                        setPhase5({
                            ...phase5,
                            negativeICP: {
                                ...current,
                                disqualifiers: [...current.disqualifiers, { ...disq, id: uuidv4() }]
                            }
                        });
                    }}
            ...
            {currentScreen === 'graph' && (
                <InterICPGraph
                    ...
                    onUpdateGraph={(graph) => setPhase5({ ...phase5, interICPGraph: graph })}
                    onSetTargetSequence={(sequence) => {
                        setPhase5({
                            ...phase5,
                            interICPGraph: { ...phase5.interICPGraph, targetSequence: sequence }
                        });
                    }}
                />
            )}
            ...
            {currentScreen === 'complete' && (
                <div className="text-center space-y-10 py-8">
                    ...
                    <div className="max-w-md mx-auto space-y-4">
                        ...
                        <p className="text-[#5B5F61]">
                            {phase5.icps?.length || 0} ICPs defined with buying groups, triggers,
                            channels, beliefs, and objections.
                        </p>

                        {/* Summary Cards */}
                        <div className="grid grid-cols-2 gap-3 text-left mt-8">
                            <div className="bg-[#F3F4EE] rounded-xl p-4">
                                <span className="text-[10px] font-mono uppercase text-[#9D9F9F]">Primary</span>
                                <p className="text-sm font-medium text-[#2D3538] mt-1">
                                    {phase5.icps?.[0]?.name || 'None'}
                                </p>
                            </div>
                            <div className="bg-[#F3F4EE] rounded-xl p-4">
                                <span className="text-[10px] font-mono uppercase text-[#9D9F9F]">Triggers</span>
                                <p className="text-sm font-medium text-[#2D3538] mt-1">
                                    {confirmedTriggerIds.length} confirmed
                                </p>
                            </div>
                            <div className="bg-[#F3F4EE] rounded-xl p-4">
                                <span className="text-[10px] font-mono uppercase text-[#9D9F9F]">Channels</span>
                                <p className="text-sm font-medium text-[#2D3538] mt-1">
                                    {(phase5.channelHabitats || []).filter(c => c.isPrimary).length} primary
                                </p>
                            </div>
                            <div className="bg-[#F3F4EE] rounded-xl p-4">
                                <span className="text-[10px] font-mono uppercase text-[#9D9F9F]">Disqualifiers</span>
                                <p className="text-sm font-medium text-[#2D3538] mt-1">
