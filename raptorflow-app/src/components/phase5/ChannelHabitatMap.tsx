      ...
      {[...Array(segments)].map((_, i) => (
        <div
          ...
          className={`w-4 h-1.5 rounded-full transition-colors ${
            i < filled ? 'bg-[#2D3538]' : 'bg-[#E5E6E3]'
          }`}
        />
      ))}
      <span className="text-[10px] font-mono text-[#9D9F9F] ml-1">
        {score}%
      </span>
    </div>
  );
      ...
      <div className="p-4 flex gap-2">
        <Button
          ...
          className={`flex-1 rounded-xl ${
            channel.isPrimary ? 'bg-[#2D3538] hover:bg-[#1A1D1E]' : ''
          }`}
        >
          {channel.isPrimary ? (
            <>
        ...
        <Button
          ...
          disabled={
            channel.isPrimary || (!channel.isSecondary && !canAddSecondary)
          }
          variant={channel.isSecondary ? 'default' : 'outline'}
          size="sm"
          className={`flex-1 rounded-xl ${
            channel.isSecondary ? 'bg-[#9D9F9F] hover:bg-[#7D7F7F]' : ''
          }`}
        >
          {channel.isSecondary ? (
            <>
...
export function ChannelHabitatMap({
  ...
  const displayChannels: ChannelHabitatItem[] =
    channels.length > 0
      ? channels
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl">
        ...
        <p className="text-sm text-[#5B5F61]">
          <strong className="text-[#2D3538]">Channel Habitat Map</strong> —
          Where your ICP spends time and what content works. Pick{' '}
          <strong>3 Primary</strong> and <strong>2 Secondary</strong> channels.
        </p>
      </div>
      ...
      <div className="flex gap-4">
        <div
          className={`flex-1 p-4 rounded-2xl ${
            primaryCount === 3 ? 'bg-[#2D3538]' : 'bg-[#F3F4EE]'
          }`}
        >
          <span
            className={`text-[10px] font-mono uppercase ${
              primaryCount === 3 ? 'text-white/60' : 'text-[#9D9F9F]'
            }`}
          >
            Primary Channels
          </span>
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`text-2xl font-mono ${
                primaryCount === 3 ? 'text-white' : 'text-[#2D3538]'
              }`}
            >
              {primaryCount}/3
            </span>
        ...
        <div
          className={`flex-1 p-4 rounded-2xl ${
            secondaryCount === 2 ? 'bg-[#9D9F9F]' : 'bg-[#F3F4EE]'
          }`}
        >
          <span
            className={`text-[10px] font-mono uppercase ${
              secondaryCount === 2 ? 'text-white/60' : 'text-[#9D9F9F]'
            }`}
          >
            Secondary Channels
          </span>
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`text-2xl font-mono ${
                secondaryCount === 2 ? 'text-white' : 'text-[#2D3538]'
              }`}
            >
              {secondaryCount}/2
            </span>
      ...
      <div className="grid grid-cols-2 gap-4">
        {displayChannels.map((channel, index) => (
          <ChannelCard
            ...
            channel={channel}
            config={
              CHANNEL_CONFIG[channel.platform] ||
              CHANNEL_CONFIG['Email/Newsletter']
            }
