...
import {
    Ban, Building2, DollarSign, UserX, Wrench,
    AlertTriangle, Plus, X, Check, TrendingDown
} from 'lucide-react';
...
interface NegativeICPBuilderProps {
    ...
    onAddDisqualifier: (disqualifier: Omit<NegativeICPDisqualifier, 'id'>) => void;
    onRemoveDisqualifier: (id: string) => void;
    onUpdateDisqualifier: (id: string, updates: Partial<NegativeICPDisqualifier>) => void;
}
...
const DISQUALIFIER_TYPES: Array<{
    ...
    examples: string[];
}> = [
        {
            ...
            icon: Building2,
            color: '#E57373',
            examples: ['Government', 'Heavy manufacturing', 'Highly regulated healthcare']
        },
        {
            type: 'budget',
            label: 'Budget',
        ...
        },
        {
            ...
            icon: UserX,
            color: '#9D9F9F',
            examples: ['DIY forever mindset', 'Tool hoarders', 'No decision-maker access']
        },
        {
            type: 'structure',
            label: 'Structure',
            icon: Wrench,
            color: '#64B5F6',
            examples: ['No marketing person', 'Enterprise bureaucracy', 'Committee paralysis']
        }
    ];

const DEFAULT_DISQUALIFIERS: Omit<NegativeICPDisqualifier, 'id'>[] = [
    { type: 'budget', description: 'Budget below $500/month', churnRiskScore: 85 },
    { type: 'behavior', description: 'DIY forever mindset - prefers building over buying', churnRiskScore: 90 },
    { type: 'structure', description: 'No champion or internal advocate', churnRiskScore: 75 },
    { type: 'industry', description: 'Heavily regulated industries (banking, healthcare)', churnRiskScore: 60 }
];
...
function DisqualifierCard({
    ...
    const churnColor = disqualifier.churnRiskScore >= 80
        ? 'text-red-600'
                ...
                <div className="flex-1 p-4">
                    <div className="flex items-start justify-between">
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-[#2D3538] mt-2">{disqualifier.description}</p>
                        </div>
                        <button
                            onClick={onRemove}
...
export function NegativeICPBuilder({
    ...
    const [newDisqualifier, setNewDisqualifier] = useState<Omit<NegativeICPDisqualifier, 'id'>>({
        type: 'behavior',
        description: '',
        churnRiskScore: 70
    });
    ...
    const displayDisqualifiers = negativeICP.disqualifiers?.length > 0
        ? negativeICP.disqualifiers
        : DEFAULT_DISQUALIFIERS.map((d, i) => ({ ...d, id: `default-${i}` }));

    const displaySignals = negativeICP.behaviorSignals?.length > 0
    ...
    const handleAddDisqualifier = () => {
        if (newDisqualifier.description.trim()) {
            ...
            setNewDisqualifier({ type: 'behavior', description: '', churnRiskScore: 70 });
            setShowAddForm(false);
        }
    };

    const handleAddIndustry = () => {
        if (newIndustry.trim() && !negativeICP.industries?.includes(newIndustry.trim())) {
            onUpdate({ industries: [...(negativeICP.industries || []), newIndustry.trim()] });
            setNewIndustry('');
        }
    };

    const handleRemoveIndustry = (industry: string) => {
        onUpdate({ industries: negativeICP.industries?.filter(i => i !== industry) || [] });
    };

    const handleAddSignal = () => {
        if (newSignal.trim() && !negativeICP.behaviorSignals?.includes(newSignal.trim())) {
            onUpdate({ behaviorSignals: [...(negativeICP.behaviorSignals || displaySignals), newSignal.trim()] });
            setNewSignal('');
        }
    };
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
                ...
                <div>
                    <p className="text-sm text-red-800">
                        <strong>Negative ICP</strong> — These are accounts to avoid. They churn, take too long,
                        or have fundamentally misaligned expectations.
                    </p>
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-medium text-[#2D3538]">Minimum Budget Threshold</h3>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <input
                        ...
                        onChange={(e) => onUpdate({ minBudgetThreshold: parseInt(e.target.value) || 0 })}
                ...
                <p className="text-xs text-[#9D9F9F] mt-2">
                    Accounts below this threshold typically churn or require disproportionate support.
                </p>
            </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[#5B5F61]">Red flags that signal poor fit</p>
                        </div>
                    </div>
                    <Button
                        variant="outline"
                ...
                {showAddForm && (
                    ...
                    >
                        <div className="flex gap-4">
                            <div className="flex-1">
                                <label className="text-xs text-[#5B5F61] mb-2 block">Type</label>
                                <select
                                    value={newDisqualifier.type}
                                    onChange={(e) => setNewDisqualifier({
                                        ...newDisqualifier,
                            ...
                            <div className="w-32">
                                <label className="text-xs text-[#5B5F61] mb-2 block">Churn Risk %</label>
                                <input
                                    type="number"
                                    min={0}
                                    max={100}
                        ...
                        <div>
                            <label className="text-xs text-[#5B5F61] mb-2 block">Description</label>
                            <input
                                type="text"
                                value={newDisqualifier.description}
                                onChange={(e) => setNewDisqualifier({ ...newDisqualifier, description: e.target.value })}
                ...
                <div className="space-y-3">
                    {displayDisqualifiers.map(disq => {
                        const typeConfig = DISQUALIFIER_TYPES.find(t => t.type === disq.type) || DISQUALIFIER_TYPES[2];
                        return (
                            <DisqualifierCard
                                key={disq.id}
                                disqualifier={disq}
                                typeConfig={typeConfig}
                                onUpdate={(updates) => onUpdateDisqualifier(disq.id, updates)}
                                onRemove={() => onRemoveDisqualifier(disq.id)}
                            />
                        );
                    })}
                </div>
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-medium text-[#2D3538]">Red Flag Behavior Signals</h3>
                </div>
