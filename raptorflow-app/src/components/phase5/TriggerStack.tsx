  ...
  return (
    <motion.div
      ...
      className={`bg-white border-2 rounded-2xl overflow-hidden ${
        isConfirmed ? 'border-[#2D3538]' : 'border-[#E5E6E3]'
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-5 border-b border-[#E5E6E3]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            <span className="text-[9px] font-mono uppercase text-[#9D9F9F] block">
              Strength
            </span>
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  ...
                  className={`w-2 h-4 rounded-sm ${
                    i <= trigger.strength
                      ? STRENGTH_COLORS[
                          trigger.strength as keyof typeof STRENGTH_COLORS
                        ]
                      : 'bg-[#E5E6E3]'
                  }`}
                />
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl flex-1 mr-4">
          ...
          <p className="text-sm text-[#5B5F61]">
            <strong className="text-[#2D3538]">Trigger Stack</strong> — Events
            that create urgency and how RaptorFlow will detect them. Confirm
            your <strong>top 3</strong> most relevant triggers.
          </p>
        </div>
        <Button
          variant="outline"
          ...
          {[1, 2, 3].map((i) => (
            <div
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                confirmedIds.length >= i
                  ? 'bg-white text-[#2D3538]'
      ...
      {showAddForm && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-[#5B5F61] mb-2 block">
                Trigger Name
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTrigger({
                    ...newTrigger,
                    strength: parseInt(e.target.value) || 3,
                  })
                }
                className="w-full border border-[#E5E6E3] rounded-xl px-4 py-2 focus:outline-none focus:border-[#2D3538]"
              />
