  ...
  return (
    <Reorder.Item value={role}>
      <motion.div
        ...
        className={`bg-white border-2 rounded-2xl overflow-hidden cursor-grab active:cursor-grabbing ${
          isActive
            ? 'border-[#2D3538]'
            : 'border-dashed border-[#E5E6E3] opacity-60'
        }`}
      >
        {/* Header */}
        <div
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              <Icon
                className={`w-5 h-5 ${isActive ? 'text-white' : 'text-[#9D9F9F]'}`}
              />
            </div>
          </div>
          ...
          <div className="flex-1">
            <h3
              className={`font-medium ${isActive ? 'text-[#2D3538]' : 'text-[#9D9F9F]'}`}
            >
              {role.role}
            </h3>
          </div>
            ...
            <div>
              ...
              <ul className="space-y-1">
                {config.concerns.map((c, i) => (
                  <li
                    key={i}
                    className="text-xs text-[#5B5F61] flex items-start gap-1"
                  >
            ...
            <div>
              ...
              <ul className="space-y-1">
                {config.fears.map((f, i) => (
                  <li
                    key={i}
                    className="text-xs text-[#E57373] flex items-start gap-1"
                  >
            ...
            <div>
              ...
              <ul className="space-y-1">
                {config.proofNeeds.map((p, i) => (
                  <li
                    key={i}
                    className="text-xs text-[#2D3538] flex items-start gap-1"
                  >
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 bg-[#F3F4EE] rounded-xl">
        ...
        <div>
          <p className="text-sm text-[#5B5F61]">
            <strong className="text-[#2D3538]">Buying Group Map</strong> — B2B
            purchases involve 6–10 stakeholders. Drag to reorder by influence.
            Toggle roles on/off.
          </p>
      ...
      >
        {roles.map((role, index) => {
          ...
          const config =
            Object.entries(ROLE_CONFIG).find(
              ([key]) =>
          ...
          return (
            <RoleCard
              ...
              onUpdateInfluence={(influence) =>
                onUpdateRole(role.id, { influence })
              }
              index={index}
            />
          );
