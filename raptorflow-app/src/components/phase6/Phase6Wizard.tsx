...
const SCREEN_CONTENT: Record<Phase6Screen, { title: string; subtitle: string }> = {
    ...
    awareness: {
        ...
        subtitle: 'Same product, different mental states. Tune your message per stage.'
    },
    soundbites: {
...
export function Phase6Wizard() {
        ...
        const loadData = async () => {
            try {
                ...
                if (data.phase5) {
                    ...
                    if (data.phase6 && data.phase6.signal7Soundbites && data.phase6.signal7Soundbites.length > 0) {
    ...
    const updatePhase6 = useCallback((updates: Partial<Phase6Data>) => {
    ...
    const handleToneChange = useCallback((tone: ToneRisk) => {
        updatePhase6({ toneRisk: tone });
    }, [updatePhase6]);

    const handleVoCUpdate = useCallback((phrases: VoCPhrase[]) => {
        updatePhase6({ vocPhrases: phrases });
    }, [updatePhase6]);

    const handleSB7Update = useCallback((spine: SB7Spine) => {
        updatePhase6({ sb7Spine: spine });
    }, [updatePhase6]);

    const handleLadderUpdate = useCallback((ladder: AwarenessStageContent[]) => {
        updatePhase6({ awarenessLadder: ladder });
    }, [updatePhase6]);

    const handleSoundbiteRegenerate = useCallback((id: string, punchLevel: number) => {
        if (!phase5) return;
        const soundbite = phase6.signal7Soundbites?.find(s => s.id === id);
        if (!soundbite) return;

        const regenerated = regenerateSignal7Soundbite(soundbite, phase5, punchLevel);
        const updated = phase6.signal7Soundbites?.map(s => s.id === id ? regenerated : s) || [];
        updatePhase6({ signal7Soundbites: updated });
        toast.success('Soundbite regenerated');
    }, [phase5, phase6.signal7Soundbites, updatePhase6]);

    const handleSoundbiteUpdate = useCallback((soundbites: Signal7Soundbite[]) => {
        updatePhase6({ signal7Soundbites: soundbites });
    }, [updatePhase6]);

    const handleProofUpdate = useCallback((slots: ProofSlot[]) => {
        updatePhase6({ proofSlots: slots });
    }, [updatePhase6]);

    const handleVoiceUpdate = useCallback((spec: VoiceSpec) => {
        updatePhase6({ voiceSpec: spec });
    }, [updatePhase6]);

    const handleHeroUpdate = useCallback((hero: WebsiteHeroPack) => {
        updatePhase6({ websiteHero: hero });
    }, [updatePhase6]);

    const handleAssetsUpdate = useCallback((assets: ChannelAsset[]) => {
        updatePhase6({ channelAssets: assets });
    }, [updatePhase6]);

    const handleRequestRewrite = useCallback((soundbiteId: string) => {
    ...
    const handleExportJSON = useCallback(() => {
        const blob = new Blob([JSON.stringify(phase6, null, 2)], { type: 'application/json' });
            ...
            <div className="min-h-screen flex items-center justify-center bg-[#F3F4EE]">
                <div className="text-center space-y-8 max-w-md px-8">
                    ...
                    <p className="text-[#5B5F61]">
                        Your ICP data is needed to generate messaging. Complete Phase 5 to continue.
                    </p>
                    <Button
    ...
    return (
        <PhaseScreen
            ...
            continueText={currentIndex === STEPS.length - 2 ? 'Review Kit' : 'Continue'}
            onContinue={goNext}
        >
            {/* Screen 6.0: Landing */}
            ...
            {currentScreen === 'voc' && (
                <VoCExtractor
                    phase6={phase6}
                    onUpdatePhrases={handleVoCUpdate}
                />
            )}
