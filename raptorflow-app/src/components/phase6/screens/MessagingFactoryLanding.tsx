...
export function MessagingFactoryLanding({ phase5, phase6, onToneChange, onConfirm }: Props) {
    const primaryICP = phase5.icps[0];
    const toneRisk = phase6.toneRisk || 'balanced';

    // Calculate confidence scores
    const vocScore = (phase6.vocPhrases?.length || 0) >= 5 ? 100 : ((phase6.vocPhrases?.length || 0) / 5) * 100;
    const proofScore = phase6.proofSlots?.filter(p => p.status === 'green').length || 0;
    const diffScore = phase6.qaReport?.differentiationScore || 60;

    const overallConfidence = Math.round((vocScore + (proofScore / 7 * 100) + diffScore) / 3);
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-white border border-[#E5E6E3] rounded-xl">
                        <Check className="w-4 h-4 text-[#2D3538]" />
                        <span className="text-sm text-[#2D3538]">
                            Position: {phase6.blueprint?.controllingIdea?.slice(0, 50) || 'Marketing OS'}...
                        </span>
                    </div>
                    <div className="flex items-center gap-2 px-4 py-2 bg-white border border-[#E5E6E3] rounded-xl">
                        <Check className="w-4 h-4 text-[#2D3538]" />
                        <span className="text-sm text-[#2D3538]">
                            {phase5.icps.length} ICP{phase5.icps.length !== 1 ? 's' : ''} configured
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-6">
                ...
                <div className="flex gap-2">
                    {(['conservative', 'balanced', 'aggressive'] as const).map((tone) => (
                        <button
                            ...
                            className={`flex-1 py-3 px-4 rounded-xl text-sm font-medium transition-all ${toneRisk === tone
                ...
                <p className="text-xs text-[#9D9F9F] mt-3">
                    {toneRisk === 'conservative' && 'Play it safe. Focus on proof and specificity.'}
                    {toneRisk === 'balanced' && 'Mix of confidence and caution. Recommended.'}
                    {toneRisk === 'aggressive' && 'Bold claims. Make sure you have the proof.'}
                </p>
            </div>
        </div>
    );
