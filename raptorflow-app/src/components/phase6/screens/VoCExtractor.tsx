    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-6">
        ...
        <p className="text-sm text-[#5B5F61]">
          Stop sounding like a founder. Start sounding like your buyer. Lock at
          least 5 phrases to anchor your messaging in reality.
        </p>
      </div>
        ...
        <div className="flex-1 h-2 bg-[#E5E6E3] rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${
              lockedPhrases.length >= 5 ? 'bg-[#2D3538]' : 'bg-[#9D9F9F]'
            }`}
            style={{
              width: `${Math.min(100, (lockedPhrases.length / 5) * 100)}%`,
            }}
          />
        </div>
        <span
          className={`text-sm font-mono ${
            lockedPhrases.length >= 5 ? 'text-[#2D3538]' : 'text-[#9D9F9F]'
          }`}
        >
        ...
        <div className="bg-[#2D3538] rounded-2xl p-6">
          ...
          <ul className="space-y-2">
            {lockedPhrases.map((p) => (
              ...
              >
                <Lock className="w-3 h-3 text-white/50" />"{p.text}"
              </li>
            ))}
          </ul>
        </div>
