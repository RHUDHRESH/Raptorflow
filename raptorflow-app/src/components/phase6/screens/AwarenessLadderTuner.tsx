...
export function AwarenessLadderTuner({ ladder, onUpdate }: Props) {
  ...
  const updateField = (
    ...
    field: keyof AwarenessStageContent,
    value: string
  ) => {
    const updated = ladder.map((s) =>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-6">
        ...
        <p className="text-sm text-[#5B5F61]">
          Same product, different mental states. Pick 2 stages to focus on
          first. Your messaging should adapt to where they are in their journey.
        </p>
        <div className="mt-3 flex items-center gap-2">
          <span
            className={`text-xs font-mono ${focusedCount >= 2 ? 'text-[#2D3538]' : 'text-[#9D9F9F]'}`}
          >
      ...
      <div className="space-y-3">
        {ladder.map((stageContent, index) => (
          <div
            ...
            className={`bg-white border-l-4 ${STAGE_COLORS[stageContent.stage]} border border-[#E5E6E3] rounded-xl overflow-hidden transition-all ${
              stageContent.isFocused ? 'ring-2 ring-[#2D3538]/20' : ''
            }`}
          >
            {/* Header */}
            <button
            ...
            {/* Content */}
            <div
              className={`grid grid-cols-3 gap-4 px-4 pb-4 ${
                stageContent.isFocused ? '' : 'opacity-60'
              }`}
            >
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField(stageContent.stage, 'whatToSay', e.target.value)
                  }
                  className="w-full px-3 py-2 bg-[#FAFAF8] border border-[#E5E6E3] rounded-lg text-sm text-[#2D3538] focus:outline-none focus:ring-1 focus:ring-[#2D3538]/20"
                />
              </div>
