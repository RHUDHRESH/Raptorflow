...
export function FinalMessagingKit({
  phase6,
  onLock,
  onExportJSON,
  onExportPDF,
...
${phase6.objectionKillers?.map((o) => `Q: ${o.objection}\nA: ${o.rebuttal}`).join('\n\n') || 'Not set'}
  ...
  const proofGaps =
    phase6.proofSlots?.filter((s) => s.status === 'red').length || 0;
  const isReady =
    proofGaps === 0 && (phase6.qaReport?.sevenSecondTestPass ?? true);

  return (
    <div className="space-y-6">
      {/* Summary Header */}
      <div
        className={`rounded-3xl p-8 ${isReady ? 'bg-[#2D3538]' : 'bg-amber-500'}`}
      >
        ...
        <div className="flex gap-6 text-white/80 text-sm">
          ...
          <span>
            ✓ {phase6.objectionKillers?.length || 3} Objection Killers
          </span>
      ...
      <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
        ...
        <p className="font-serif text-xl text-[#2D3538]">
          "
          {phase6.blueprint?.controllingIdea ||
        ...
        <div className="space-y-3">
          {phase6.signal7Soundbites?.slice(0, 7).map((sb, i) => (
            <div key={sb.id} className="flex items-start gap-3">
              ...
              {phase6.proofSlots?.find((p) => p.soundbiteId === sb.id)
                ?.status === 'red' && (
      ...
      {proofGaps > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
          ...
          <p className="text-sm text-amber-700">
            {proofGaps} soundbite{proofGaps !== 1 ? 's' : ''} marked as
            "unproven". Consider adding evidence before locking.
          </p>
        </div>
      )}
      ...
      <div className="flex gap-3">
        <button
          onClick={onLock}
          className={`flex-1 flex items-center justify-center gap-3 py-5 rounded-2xl font-medium transition-all ${
            isReady
