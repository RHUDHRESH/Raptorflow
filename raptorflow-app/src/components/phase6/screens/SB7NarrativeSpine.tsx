  ...
  [
    {
      value: 'expert',
      label: 'Expert',
      description: 'Authority-driven. "I have done this 1000 times."',
    },
    {
      value: 'ally',
      label: 'Ally',
      description: 'Partnership-focused. "We are in this together."',
    },
    {
      value: 'operator',
      label: 'Operator',
      description: 'Get-it-done. "Here is the system. Use it."',
    },
    {
      value: 'contrarian',
      label: 'Contrarian',
      description: 'Challenge the status quo. "Everyone else is wrong."',
    },
  ];
...
export function SB7NarrativeSpine({ sb7Spine, onUpdate }: Props) {
  ...
  const updateField = (
    field: keyof SB7Spine,
    value: string | string[] | GuideVibe | NarrativeVariant
  ) => {
      ...
      <div className="flex gap-2 p-1 bg-[#F3F4EE] rounded-xl w-fit">
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg transition-all ${
            sb7Spine.narrativeVariant === 'escape-chaos'
        ...
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg transition-all ${
            sb7Spine.narrativeVariant === 'become-operator'
              ? 'bg-white text-[#2D3538] shadow-sm'
              : 'text-[#5B5F61]'
          }`}
        >
...
function SB7Field({
  ...
  return (
    <div
      className={`bg-white border border-[#E5E6E3] rounded-xl p-4 group ${span2 ? 'col-span-2' : ''}`}
    >
