...
import { Eye, AlertTriangle, Check, RefreshCw, Sparkles, X } from 'lucide-react';
...
export function ClarityDiffTests({ qaReport, soundbites, onRequestRewrite, onAcceptFix }: Props) {
    if (!qaReport) {
        return (
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="font-medium text-[#2D3538]">Clarity & Differentiation Tests</h3>
                </div>
                <p className="text-sm text-[#5B5F61]">
                    Auto QA to avoid shipping mush. Fix issues before locking your messaging.
                </p>
            </div>

            {/* 7-Second Test */}
            <div className={`rounded-2xl p-6 ${qaReport.sevenSecondTestPass
                    ? 'bg-green-50 border border-green-200'
                    : 'bg-amber-50 border border-amber-200'
                }`}>
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${qaReport.sevenSecondTestPass ? 'bg-green-100' : 'bg-amber-100'
                            }`}>
                        ...
                        <div>
                            ...
                            <p className={`text-sm ${qaReport.sevenSecondTestPass ? 'text-green-700' : 'text-amber-700'}`}>
                                {qaReport.sevenSecondTestPass
                ...
                <div className="bg-white/50 rounded-xl p-4 border border-white">
                    ...
                    <div className="text-center py-4">
                        <p className="font-serif text-xl text-[#2D3538]">
                            {soundbites.find(s => s.type === 'outcome')?.text.split('.')[0] || 'Your headline here'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-[#2D3538]">Differentiation Check</span>
                    </div>
                    <div className={`text-2xl font-medium ${qaReport.differentiationScore >= 70 ? 'text-green-600' :
                            qaReport.differentiationScore >= 50 ? 'text-amber-600' : 'text-red-600'
                        }`}>
                ...
                <div className="h-3 bg-[#F3F4EE] rounded-full overflow-hidden mb-4">
                    <div
                        className={`h-full rounded-full transition-all ${qaReport.differentiationScore >= 70 ? 'bg-green-500' :
                                qaReport.differentiationScore >= 50 ? 'bg-amber-500' : 'bg-red-500'
                            }`}
                ...
                {qaReport.competitorSimilarity.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-[#E5E6E3]">
                        ...
                        {qaReport.competitorSimilarity.map((sim, i) => (
                            <div key={i} className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-2 rounded-lg mb-2">
                                <AlertTriangle className="w-4 h-4" />
                                <span>Soundbite similar to {sim.competitor} ({sim.similarity}%)</span>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`text-sm ${qaReport.fluffyWords.length === 0 ? 'text-green-600' : 'text-amber-600'}`}>
                        {qaReport.fluffyWords.length === 0 ? 'Clean!' : `${qaReport.fluffyWords.length} issues`}
                    </span>
                </div>
                ...
                {qaReport.fluffyWords.length > 0 ? (
                    ...
                    <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg p-4">
                        <Check className="w-5 h-5 text-green-600" />
                        <span className="text-sm text-green-700">No fluffy words detected!</span>
                    </div>
                )}
            </div>
