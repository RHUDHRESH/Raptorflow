...
import { ProofSlot, Signal7Soundbite, RiskReversalType } from '@/lib/foundation';
import { Shield, BarChart3, Users, Cog, AlertTriangle, Check, ChevronDown } from 'lucide-react';
...
const RISK_REVERSAL_OPTIONS: { type: RiskReversalType; label: string; description: string }[] = [
    { type: 'pilot', label: 'Pilot Program', description: 'Try before you commit' },
    { type: 'guarantee', label: 'Guarantee', description: 'Money-back if not satisfied' },
    { type: 'cancel-anytime', label: 'Cancel Anytime', description: 'No lock-in contracts' },
    { type: 'pay-on-results', label: 'Pay on Results', description: 'Performance-based pricing' }
];
...
export function ProofBinding({ proofSlots, soundbites, onUpdate }: Props) {
    ...
    const calculateStatus = (slot: ProofSlot): ProofSlot['status'] => {
        const hasAny = slot.metricProof || slot.socialProof || slot.mechanismProof || slot.riskReversal;
        const hasMultiple = [slot.metricProof, slot.socialProof, slot.mechanismProof].filter(Boolean).length >= 2;
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[#FAFAF8] border border-[#E5E6E3] rounded-2xl p-6">
                ...
                <p className="text-sm text-[#5B5F61] mb-4">
                    No proof = weak message. Each soundbite needs evidence or it will sound like BS.
                </p>
                ...
                <div className="flex gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-[#5B5F61]">{yellowCount} partial</span>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            {proofSlots.map(slot => {
                ...
                return (
                    ...
                    >
                        {/* Header */}
                        <button
                            onClick={() => setExpandedId(isExpanded ? null : slot.soundbiteId)}
                            className="w-full flex items-center justify-between p-4 hover:bg-[#FAFAF8] transition-colors"
                        >
                            <div className="flex items-center gap-3">
                                <div className={`w-3 h-3 rounded-full ${status === 'green' ? 'bg-green-500' :
                                        status === 'yellow' ? 'bg-amber-500' : 'bg-red-500'
                                    }`} />
                                <span className="text-sm text-[#2D3538] text-left">
                                    {soundbite.type.replace(/-/g, ' ')}
                                </span>
                                <span className={`px-2 py-0.5 rounded-full text-[10px] ${statusBadge.bg} ${statusBadge.text}`}>
                                    {statusBadge.label}
                                </span>
                            </div>
                            <ChevronDown className={`w-4 h-4 text-[#9D9F9F] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                        </button>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-mono uppercase text-[#9D9F9F]">Metric</span>
                                            {slot.metricProof && <Check className="w-3 h-3 text-green-500" />}
                                        </div>
                                        <input
                                            type="text"
                                            value={slot.metricProof || ''}
                                            onChange={e => updateSlot(slot.soundbiteId, { metricProof: e.target.value })}
                                    ...
                                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-mono uppercase text-[#9D9F9F]">Social</span>
                                            {slot.socialProof && <Check className="w-3 h-3 text-green-500" />}
                                        </div>
                                        <input
                                            type="text"
                                            value={slot.socialProof || ''}
                                            onChange={e => updateSlot(slot.soundbiteId, { socialProof: e.target.value })}
                                    ...
                                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-mono uppercase text-[#9D9F9F]">Mechanism</span>
                                            {slot.mechanismProof && <Check className="w-3 h-3 text-green-500" />}
                                        </div>
                                        <input
                                            type="text"
                                            value={slot.mechanismProof || ''}
                                            onChange={e => updateSlot(slot.soundbiteId, { mechanismProof: e.target.value })}
                                    ...
                                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-mono uppercase text-[#9D9F9F]">Risk Reversal</span>
                                            {slot.riskReversal && <Check className="w-3 h-3 text-green-500" />}
                                        </div>
                                        <select
                                            ...
                                            onChange={e => {
                                                ...
                                                if (type) {
                                                    ...
                                                    updateSlot(slot.soundbiteId, { riskReversal: undefined });
                                                }
                                            }}
                                            className="w-full px-3 py-2 bg-[#FAFAF8] border border-[#E5E6E3] rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#2D3538]/20"
                                        >
