...
export function AssetPackaging({
  ...
  const getChannelAsset = (channel: ChannelType) =>
    channelAssets.find((a) => a.channel === channel);
        ...
        <div className="space-y-4">
          {getChannelAsset('deck')?.variants.map((variant, i) => (
            ...
            >
              ...
              <p className="text-white/70">{variant.content.subhead}</p>
              <button
                onClick={() =>
                  copyToClipboard(
