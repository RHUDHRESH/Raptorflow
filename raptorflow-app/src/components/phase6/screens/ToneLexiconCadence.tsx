...
const TONE_OPTIONS: { value: ToneDial; label: string; description: string }[] =
  [
    ...
    },
    {
      value: 'confident',
      label: 'Confident',
      description: 'Assured but not arrogant',
    },
    {
      value: 'bold',
      label: 'Bold',
      description: 'Strong claims, direct language',
    },
    {
      value: 'provocative',
      label: 'Provocative',
      description: 'Challenge assumptions, contrarian',
    },
  ];
...
const CADENCE_OPTIONS: {
...
}[] = [
  {
    value: 'short',
    label: 'Short',
    example: 'Stop guessing. Start executing.',
  },
  {
    value: 'medium',
    label: 'Medium',
    example: 'You deserve a marketing system that actually works.',
  },
  {
    value: 'punchy',
    label: 'Punchy',
    example: 'Marketing. Finally under control. No more chaos. Just clarity.',
  },
];
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {voiceSpec.alwaysUse.map((word, i) => (
              ...
              >
                {word}
                <button
                  onClick={() => removeWord('alwaysUse', i)}
        ...
        <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                const input = document.querySelector(
                  'input[placeholder="Add banned word..."]'
                ) as HTMLInputElement;
                if (input) {
        ...
        <div className="grid grid-cols-3 gap-3">
          {CADENCE_OPTIONS.map((cadence) => (
            <button
              key={cadence.value}
              onClick={() =>
                onUpdate({ ...voiceSpec, cadenceStyle: cadence.value })
              }
              className={`text-left p-4 rounded-xl border transition-all ${
                voiceSpec.cadenceStyle === cadence.value
