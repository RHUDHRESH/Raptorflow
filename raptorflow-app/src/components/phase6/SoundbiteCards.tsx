...
const TYPE_LABELS: Partial<
  Record<SoundbiteType, { label: string; color: string }>
> = {
  'problem-reveal': {
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span
                className={cn(
                  'text-xs px-2 py-1 rounded-full font-medium border border-current opacity-80',
                  color
                )}
              >
                {label}
            ...
            <div className="flex items-center gap-1">
              <span
                className={cn(
                  'text-lg font-bold',
                  soundbite.scores.total >= 80
                    ? 'text-green-600'
                    : soundbite.scores.total >= 60
                      ? 'text-amber-600'
                      : 'text-red-600'
                )}
              >
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-6 p-3 bg-muted/20 rounded-lg">
            <ScoreGate
              label="Specific"
              value={soundbite.scores.specificity}
              passing={soundbite.scores.specificity >= 3}
            />
            <ScoreGate
              label="Proof"
              value={soundbite.scores.proof}
              passing={soundbite.scores.proof >= 3}
            />
            <ScoreGate
              label="Diff"
              value={soundbite.scores.differentiation}
              passing={soundbite.scores.differentiation >= 3}
            />
            <ScoreGate
              label="Fit"
              value={soundbite.scores.awarenessFit}
              passing={soundbite.scores.awarenessFit >= 3}
            />
            <ScoreGate
              label="Load"
              value={soundbite.scores.cognitiveLoad}
              passing={soundbite.scores.cognitiveLoad >= 3}
            />
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              size="sm"
              variant={soundbite.isLocked ? 'default' : 'secondary'}
...
export function SoundbiteCards({
  soundbites,
  onChange,
  onRegenerate,
  onContinue,
  ...
  const handleToggleLock = (id: string) => {
    onChange(
      soundbites.map((sb) =>
        sb.id === id ? { ...sb, isLocked: !sb.isLocked } : sb
      )
    );
  };
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground max-w-lg mx-auto">
          Each soundbite is gated by 5 rigor checks. Edit, regenerate, then
          lock.
        </p>
      </div>
        ...
        >
          <CarouselContent>
            {soundbites.map((sb, idx) => {
              const typeInfo = TYPE_LABELS[sb.type] || {
                label: sb.type,
                ...
                >
                  <ContextMenu>
                    <ContextMenuTrigger>
                      ...
                      >
                        ...
                        <Textarea
                          value={sb.text}
                          onChange={(e) =>
                            handleTextChange(sb.id, e.target.value)
                          }
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-6 p-3 bg-muted/20 rounded-lg">
                          <ScoreGate
                            label="Specific"
                            value={sb.scores.specificity}
                            passing={sb.scores.specificity >= 3}
                          />
                          <ScoreGate
                            label="Proof"
                            value={sb.scores.proof}
                            passing={sb.scores.proof >= 3}
                          />
                          <ScoreGate
                            label="Diff"
                            value={sb.scores.differentiation}
                            passing={sb.scores.differentiation >= 3}
                          />
                          <ScoreGate
                            label="Fit"
                            value={sb.scores.awarenessFit}
                            passing={sb.scores.awarenessFit >= 3}
                          />
                          <ScoreGate
                            label="Load"
                            value={sb.scores.cognitiveLoad}
                            passing={sb.scores.cognitiveLoad >= 3}
                          />
                        </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            size="sm"
                            variant={sb.isLocked ? 'default' : 'secondary'}
                            onClick={() => handleToggleLock(sb.id)}
                            className="flex-1"
                          >
                            {sb.isLocked ? (
                              <Unlock className="h-3 w-3 mr-2" />
                    ...
                    <ContextMenuContent>
                      <ContextMenuItem
                        onClick={() => {
                          navigator.clipboard.writeText(sb.text);
                          toast.success('Soundbite copied to clipboard');
                        }}
                      >
                        <Copy className="mr-2 h-4 w-4" /> Copy Text
