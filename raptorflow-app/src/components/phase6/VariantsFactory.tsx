...
export function VariantsFactory({ soundbites, variants, onContinue }: VariantsFactoryProps) {
    const [selectedIdx, setSelectedIdx] = useState(0);

    const selectedSb = soundbites[selectedIdx];
    const selectedVariant = variants.find(v => v.soundbiteId === selectedSb?.id);
        ...
        <div className="space-y-8">
            ...
            {selectedVariant && (
                <div className="max-w-3xl mx-auto space-y-6">
                    {/* Landing Hero */}
                    <div>
                        <h4 className="text-sm font-medium text-muted-foreground mb-2">Landing Hero</h4>
                        <CopyableText text={selectedVariant.landingHero} label="Landing Hero" />
                    </div>

                    {/* Subhead */}
                    <div>
                        <h4 className="text-sm font-medium text-muted-foreground mb-2">Subhead</h4>
                    ...
                    {/* Email Subjects */}
                    <VariantSection title="Email Subject Lines" items={selectedVariant.emailSubjects} />

                    {/* Sales Opener */}
                    <div>
                        <h4 className="text-sm font-medium text-muted-foreground mb-2">Sales Opener</h4>
                        <CopyableText text={selectedVariant.salesOpener} label="Sales Opener" />
                    </div>

                    {/* LinkedIn Opener */}
                    <div>
                        <h4 className="text-sm font-medium text-muted-foreground mb-2">LinkedIn Opener</h4>
                        <CopyableText text={selectedVariant.linkedInOpener} label="LinkedIn Opener" />
                    </div>
                </div>
            )}
