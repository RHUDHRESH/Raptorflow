...
const TYPE_CONFIG: Partial<
  Record<
    SoundbiteType,
    { label: string; icon: React.ReactNode; strategy: string }
  >
> = {
  'problem-reveal': {
...
export function SoundbiteStudio({
  soundbites,
  onGenerate,
  onUpdate,
  onLock,
  onSelectVariation,
  isGenerating,
}: SoundbiteStudioProps) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [viewingVariationsId, setViewingVariationsId] = useState<string | null>(
    null
  );
      ...
      <div className="flex justify-between items-center bg-[#2D3538] text-white p-10 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-8 relative z-10">
          <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center relative">
            <Sparkles
              className={cn('w-8 h-8', isGenerating && 'animate-pulse')}
            />
            {isGenerating && (
              <motion.div
          ...
          <div>
            <h3 className={`${playfair.className} text-3xl mb-1`}>
              Precision Synthesis Studio
            </h3>
            <p className="text-white/40 text-xs font-mono uppercase tracking-widest">
              Forging your final 7 resonance vectors
            </p>
          </div>
        </div>
        <button
          ...
          <div className="py-32 text-center border-2 border-dashed border-[#C0C1BE] rounded-3xl opacity-30 flex flex-col items-center gap-6">
            ...
            <div className="space-y-2">
              ...
              <span className="text-[10px] font-mono uppercase tracking-widest">
                Required: Phase 03 - 05 Completion
              </span>
            </div>
          </div>
        )}
              ...
              <div className="flex items-stretch min-h-[160px]">
                ...
                <div
                  className={cn(
                    ...
                    sb.isLocked
                      ? 'border-[#C0C1BE] bg-white/50'
                      : 'border-[#C0C1BE] bg-[#F3F4EE]/30'
                  )}
                >
                  <div
                    className={cn(
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        <span className="text-[11px] font-mono uppercase tracking-[0.3em] opacity-40 font-bold">
                          {config.label}
                        </span>
                        {sb.isLocked && (
                          <span className="text-[9px] font-mono uppercase bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                            Validated
                          </span>
                        )}
                      </div>
                      <p className="text-[12px] text-[#5B5F61] italic">
                        {config.strategy}
                      </p>
                    </div>
                    <div className="flex gap-3">
                      {!sb.isLocked && (
                        <>
                          <button
                            onClick={() =>
                              setViewingVariationsId(
                                showVariations ? null : sb.id
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              setEditingId(isEditing ? null : sb.id)
                            }
                            className="p-2.5 rounded-xl hover:bg-[#F3F4EE] opacity-20 group-hover:opacity-100 transition-all"
                            title="Manual Refine"
                          >
