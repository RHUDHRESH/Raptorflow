  ...
  useEffect(() => {
    if (experiment && open) {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const [telemetryData, outcomesData, learningsData] =
            await Promise.all([
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 gap-0 bg-white dark:bg-zinc-950 border-0 shadow-xl rounded-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <Tabs defaultValue="overview" className="flex-1 flex flex-col min-h-0">
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Card className="p-4 rounded-xl border-stone-200 bg-white shadow-sm flex flex-col items-center justify-center text-center">
                      ...
                      <div className="text-2xl font-bold text-stone-800">
                        +{Math.round(roiData.avgRoi * 100)}%
                      </div>
                      <p className="text-[9px] text-stone-500 font-sans">
                        Based on similar {experiment.goal} tests
                      </p>
                    </Card>
                    <Card className="p-4 rounded-xl border-stone-200 bg-white shadow-sm flex flex-col items-center justify-center text-center">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-1">
                      ...
                      <div className="text-2xl font-bold text-stone-800">
                        {roiData.avgMomentum.toFixed(1)}
                      </div>
                      <Badge
                        variant="outline"
                        className="mt-1 text-[8px] bg-stone-100 text-stone-600 border-stone-200"
                      >
                        HIGH CONFIDENCE
                      </Badge>
                    </Card>
                ...
                {[
                  {
                    icon: Icons.Clock,
                    label: 'Effort',
                    value: experiment.effort,
                  },
                  {
                    icon: Icons.BarChart2,
                    label: 'Signal',
                    value: experiment.time_to_signal,
                  },
                  ...
                  return (
                    <div
                      key={item.label}
                      className="text-center p-3 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800"
                    >
                      <AppIcon
                        icon={item.icon}
                        className="w-4 h-4 mx-auto text-zinc-400 mb-1"
                      />
                      <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 capitalize font-sans">
                        {item.value}
                      </p>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 px-1">
                    <AppIcon
                      ...
                      className="text-stone-800"
                    />
                    <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 font-sans">
                      Surgical Learning
                    </h3>
                  </div>
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <span
                              className={cn(
                                ...
                                learning.learning_type === 'strategic'
                                  ? 'bg-purple-500/10 text-purple-500'
                                  : learning.learning_type === 'tactical'
              ...
              {outcomes.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 px-1 font-sans">
                    Business Outcomes
                  </h3>
                  <div className="grid grid-cols-1 gap-2">
                    {outcomes.map((outcome) => (
                      <div
                        key={outcome.id}
                        className="flex items-center justify-between p-4 bg-zinc-900 text-white rounded-xl"
                      >
                        <div>
                          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">
                            Experiment ID: {outcome.experiment_id}
                          </p>
                          <p className="text-xs font-medium text-zinc-300">
                            Confidence:{' '}
                            {(outcome.confidence_score * 100).toFixed(0)}%
                          </p>
                        </div>
          ...
          <TabsContent
            value="audit"
            className="flex-1 overflow-y-auto m-0 p-6 bg-zinc-50/10"
          >
        ...
        <div className="p-5 border-t border-zinc-100 dark:border-zinc-900 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/50 shrink-0">
          <Button
            variant="ghost"
            size="sm"
            className="rounded-lg text-zinc-500 font-sans hover:bg-zinc-100"
          >
          ...
          <div className="flex items-center gap-3">
            {isDraft && (
              ...
              >
                <AppIcon icon={Icons.Rocket} size={14} className="mr-2" />{' '}
                Launch Experiment
              </Button>
            )}
            ...
            {isLaunched && (
              ...
              >
                <AppIcon icon={Icons.CheckCircle2} size={14} className="mr-2" />{' '}
                Manual Check-in
              </Button>
            )}
          </div>
