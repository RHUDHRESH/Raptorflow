...
export function TodayPanel({
  ...
  return (
    <div className="bg-white border border-[#E5E6E3] rounded-2xl overflow-hidden">
      ...
      {/* Today's Queue */}
      <div className="px-6 py-5 border-b border-[#E5E6E3]">
        <h4 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#9D9F9F] mb-4">
          Queue
        </h4>
        {todayQueue.length > 0 ? (
            ...
            {todayQueue.map((item, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="block text-[14px] text-[#2D3538] truncate">
                    {item.action.label}
                  </span>
                  <span className="block text-[11px] text-[#9D9F9F] truncate">
                    {item.moveName}
                  </span>
                </div>
                <span className="text-[11px] font-mono text-[#C0C1BE]">
                  {item.action.estimatedMinutes}m
                </span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-[13px] text-[#9D9F9F] italic">
            No actions queued for today
          </p>
        )}
      </div>
      ...
      <div className="px-6 py-5 border-b border-[#E5E6E3]">
        ...
        <div className="flex items-center justify-between mb-4">
          <span className="text-[14px] text-[#2D3538]">
            Did you execute today?
          </span>
        ...
        <div className="flex items-center gap-4 mb-3">
          ...
          />
          <span className="text-[12px] font-mono text-[#2D3538] w-8 text-right">
            {quality}
          </span>
        </div>
        ...
        <button
          ...
          className={`w-full h-10 rounded-xl text-[14px] font-medium transition-all ${
            executed === null
