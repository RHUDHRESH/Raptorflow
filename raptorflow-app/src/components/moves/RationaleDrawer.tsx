...
export function RationaleDrawer({
  ...
  const mockRationale = {
    decree:
      "The Council of 12 has reached 94% alignment on this move. The primary driver is a detected shift in competitor search volume (Δ +12%) matched with our brand's unique ability to serve high-intent retention signals.",
    rationale:
      "This strategic move leverages our competitive advantage in the current market conditions. The data indicates a strong alignment between our capabilities and market demand, creating an optimal opportunity for growth.",
    expertThoughts: [
      {
        agent: 'direct_response',
        thought:
          'Conversion path is optimized for this specific keyword group. Expected CTR: 4.2%.',
      },
      {
        agent: 'brand_philosopher',
        thought:
          'Maintaining editorial restraint here will actually increase perceived value. Avoid emojis in the snippet.',
      },
      {
        agent: 'data_quant',
        thought:
          'Competitor B has dropped their bid floor on these terms by 15% over the last 48 hours.',
      },
    ],
    consensus: 0.94,
    ...
    rejected: [
      {
        path: 'Alternative A: Aggressive bidding strategy',
        reason: 'Rejected due to budget constraints'
      },
      {
        path: 'Alternative B: Conservative approach',
        reason: 'Rejected due to market opportunity window'
      }
    ],
  };
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="sm:max-w-xl overflow-y-auto bg-canvas border-l border-borders/50">
        <SheetHeader className="space-y-4 pb-6 border-b border-borders/30">
          ...
          <div>
            <SheetTitle className="text-2xl font-serif text-ink italic">
              {move.name}
            </SheetTitle>
          ...
          <section className="grid grid-cols-1 gap-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl border border-borders/30 bg-surface/50">
                <p className="text-[10px] text-secondary-text uppercase tracking-widest mb-1">
                  Alignment
                </p>
                <div className="flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-accent" />
                  <span className="text-2xl font-mono font-bold text-ink">
                    {(mockRationale.consensus * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
              <div className="p-4 rounded-xl border border-borders/30 bg-surface/50">
                ...
                >
                  {mockRationale.expertThoughts.map((thought, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex gap-4 p-4 rounded-xl hover:bg-surface/80 transition-colors"
                      >
                        <Avatar className="h-8 w-8 shrink-0">
                          <AvatarFallback className="bg-muted-fill/10">
                            <expert.icon
                              className={cn('h-4 w-4', expert.color)}
                            />
                          </AvatarFallback>
                        </Avatar>
                        <div className="space-y-1">
                          <p className="text-[11px] font-bold text-primary-text uppercase tracking-wider">
                            {expert.name}
                          </p>
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <p className="text-sm font-serif italic text-primary-text">
                    This move parallels the 1994 Nike "Play" [Exploit]. By
                    pivoting from product features to identity-based messaging,
                    they achieved a 40% increase in brand recall within 6
                    months.
                  </p>
                  <p className="text-[10px] text-accent font-bold uppercase tracking-widest mt-3">
                    RaptorFlow Precedent ID: RF-94-NK
                  </p>
                </div>
              </div>
            </div>
