...
import { Clock, ChevronRight, MoreHorizontal, AlertCircle, CheckCircle2, Circle } from 'lucide-react';
 ...
 */
function getProgress(move: Move): { completed: number; total: number; percent: number } {
...
function calculateRAG(move: Move): { status: RAGStatus; reason: string } {
    ...
    if (progress.percent < expectedProgress - 30) {
        return { status: 'red', reason: `Behind pace — ${progress.percent}% done by day ${dayNumber}` };
    }
    ...
    if (confidence <= 5) {
        return { status: 'amber', reason: `Moderate confidence (${confidence}/10)` };
    }
 ...
 */
function getNextTask(move: Move): { label: string; estimatedMinutes: number } | null {
    // If nextAction is set, use it
    if (move.nextAction) {
        return { label: move.nextAction.label, estimatedMinutes: move.nextAction.estimatedMinutes };
    }
...
export function MoveCardEnhanced({ move, onClick, onLogProgress, compact = false }: MoveCardEnhancedProps) {
    const dayNumber = getDayNumber(move);
    const progress = getProgress(move);
            ...
            <div className={styles.moveCardHeader}>
                <div>
                    ...
                    <div className={styles.moveChips}>
                        ...
                        {move.campaignName && (
                            <span className={cn(styles.chip, styles.objective)} style={{ opacity: 0.6 }}>
                                {move.campaignName}
                            </span>
                        )}
                    </div>
