...
import { Move, MoveStatus, RAGStatus, ChannelType, MoveGoal, MoveDuration } from '@/lib/campaigns-types';
...
const RAG_OPTIONS: { value: RAGStatus | 'all'; label: string; color?: string }[] = [
    { value: 'all', label: 'RAG' },
...
export function CommandStrip({
    ...
    const updateFilter = <K extends keyof MoveFilters>(key: K, value: MoveFilters[K]) => {
        ...
        <div className={styles.commandStrip}>
            ...
            <div className={styles.filterChips}>
                ...
                <select
                    ...
                    onChange={(e) => updateFilter('status', e.target.value as MoveStatus | 'all')}
                    className={cn(
                        styles.filterChip,
                ...
                <select
                    value={filters.goal}
                    onChange={(e) => updateFilter('goal', e.target.value as MoveGoal | 'all')}
                    className={cn(
                        styles.filterChip,
                ...
                <select
                    ...
                    onChange={(e) => updateFilter('channel', e.target.value as ChannelType | 'all')}
                    className={cn(
                        styles.filterChip,
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        updateFilter('duration', val === 'all' ? 'all' : parseInt(val) as MoveDuration);
                    }}
                    className={cn(
                        styles.filterChip,
                    ...
                    {RAG_OPTIONS.map(opt => (
                        <button
                            ...
                            style={{
                                ...
                                color: filters.rag === opt.value && opt.color ? '#fff' : undefined,
                                background: filters.rag === opt.value && opt.color ? opt.color : undefined,
                                borderColor: filters.rag === opt.value && opt.color ? opt.color : undefined
                            }}
                        >
                            {opt.label}
                        </button>
            ...
            <div className={styles.viewToggle}>
                <button
                    ...
                    className={cn(styles.viewOption, viewMode === 'list' && styles.active)}
                    title="List view"
                >
                ...
                <button
                    ...
                    className={cn(styles.viewOption, viewMode === 'board' && styles.active)}
                    title="Board view"
                >
...
export function applyFilters(moves: Move[], filters: MoveFilters) {
    return moves.filter(move => {
        ...
        // Channel filter
        if (filters.channel !== 'all' && move.channel !== filters.channel) return false;

        // Duration filter
        if (filters.duration !== 'all' && move.duration !== filters.duration) return false;
