...
import { Watchlist, WatchlistType, AlertType, ScanFrequency, Competitor } from './types';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { ArrowRight, ArrowLeft, X, Check, Plus, Trash2, Users, TrendingUp, Sparkles } from 'lucide-react';
import gsap from 'gsap';

interface WatchlistWizardProps {
    onComplete: (watchlist: Omit<Watchlist, 'id' | 'createdAt' | 'updatedAt'>) => void;
    onCancel: () => void;
    existingCompetitors?: string[]; // From Foundation
}

type WizardStep = 'name' | 'type' | 'competitors' | 'signals' | 'schedule' | 'review';

const STEPS: WizardStep[] = ['name', 'type', 'competitors', 'signals', 'schedule', 'review'];

const STEP_INFO: Record<WizardStep, { title: string; subtitle: string }> = {
    name: { title: 'Name Your Watchlist', subtitle: 'Give it a memorable name you\'ll recognize' },
    type: { title: 'What Are You Tracking?', subtitle: 'This helps us optimize the scanning strategy' },
    competitors: { title: 'Add Competitors', subtitle: 'Who should we keep an eye on?' },
    signals: { title: 'Signal Types', subtitle: 'What changes matter most to you?' },
    schedule: { title: 'Scan Frequency', subtitle: 'How often should we check for updates?' },
    review: { title: 'Review & Launch', subtitle: 'Everything looks good. Ready to start monitoring?' },
};

const WATCHLIST_TYPES: { value: WatchlistType; label: string; description: string; icon: React.ReactNode }[] = [
    { value: 'competitors', label: 'Competitors', description: 'Track direct and indirect competitors', icon: <Users className="w-5 h-5" /> },
    { value: 'trends', label: 'Industry Trends', description: 'Monitor keywords, topics, and shifts', icon: <TrendingUp className="w-5 h-5" /> },
    { value: 'custom', label: 'Custom', description: 'Build your own monitoring criteria', icon: <Sparkles className="w-5 h-5" /> },
];
...
const FREQUENCIES: { value: ScanFrequency; label: string; description: string }[] = [
    { value: 'hourly', label: 'Hourly', description: 'For fast-moving markets (higher token cost)' },
    { value: 'daily', label: 'Daily', description: 'Recommended for most use cases' },
    { value: 'weekly', label: 'Weekly', description: 'For slower-moving industries' },
];

export function WatchlistWizard({ onComplete, onCancel, existingCompetitors = [] }: WatchlistWizardProps) {
    ...
    const canProceed = (): boolean => {
        switch (currentStep) {
            ...
            case 'competitors': return type !== 'competitors' || competitors.length > 0;
            case 'signals': return signals.length > 0;
        ...
        <div className="fixed inset-0 z-50 flex bg-background">
                        ...
                        return (
                            ...
                            >
                                <div className={cn(
                                    ...
                                    isActive ? "w-2.5 h-2.5 bg-white shadow-[0_0_12px_rgba(255,255,255,0.5)]" : "bg-white"
                                )} />
            ...
            <div className="flex-1 bg-[#F8F9F7] flex flex-col relative overflow-hidden">
                ...
                <div className="flex-1 flex items-center justify-center p-16">
                    <div ref={contentRef} className="w-full max-w-[600px]" onKeyDown={handleKeyDown}>
                        {/* Step: Name */}
                        {currentStep === 'name' && (
                                ...
                                {WATCHLIST_TYPES.map((t) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <div className={cn(
                                                "text-sm",
                                                type === t.value ? "text-white/60" : "text-muted-foreground"
                                            )}>
                                                {t.description}
                                            </div>
                                        </div>
                        ...
                        {currentStep === 'competitors' && (
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    />
                                    <Button onClick={addCompetitor} className="h-12 px-6 rounded-xl">
                                ...
                                {FREQUENCIES.map((freq) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <div className={cn(
                                                "text-sm",
                                                frequency === freq.value ? "text-white/60" : "text-muted-foreground"
                                            )}>
                                                {freq.description}
                                            </div>
                                        </div>
