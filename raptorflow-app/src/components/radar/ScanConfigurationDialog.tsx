...
export function ScanConfigurationDialog({
    ...
    const [selectedCohorts, setSelectedCohorts] = useState<string[]>(['founders']);
    const [selectedFormats, setSelectedFormats] = useState<string[]>(['linkedin']);
    const [selectedSources, setSelectedSources] = useState<string[]>(['linkedin', 'x']);
            ...
            <DialogContent className="sm:max-w-[550px] p-0 gap-0 overflow-hidden bg-card">
                ...
                <div className="p-6 space-y-8 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="space-y-3">
                        <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Scanning Mode</Label>
                        <Tabs value={mode} onValueChange={(v) => setMode(v as RadarMode)} className="w-full">
                            <TabsList className="grid w-full grid-cols-2 h-14 bg-secondary/50 p-1 rounded-xl">
                                <TabsTrigger value="recon" className="gap-2 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-md transition-all">
                                    <Zap className="h-5 w-5" />
                                    <div className="text-left">
                                        <div className="text-sm font-semibold">Recon</div>
                                        <div className="text-[10px] text-muted-foreground leading-tight">Fast Signal Pulse</div>
                                    </div>
                                </TabsTrigger>
                                <TabsTrigger value="dossier" className="gap-2 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-md transition-all">
                                    <FileText className="h-5 w-5" />
                                    <div className="text-left">
                                        <div className="text-sm font-semibold">Dossier</div>
                                        <div className="text-[10px] text-muted-foreground leading-tight">Deep Synthesis</div>
                                    </div>
                                </TabsTrigger>
                            </TabsList>
                        </Tabs>
                    ...
                    <p className="text-xs text-center text-muted-foreground">
                        Ready to scan for <span className="text-foreground font-medium">all active cohorts</span> using standard parameters.
                    </p>

                    {/* Advanced Section */}
                    <Accordion type="single" collapsible className="w-full border rounded-xl overflow-hidden border-border/60 shadow-sm">
                        <AccordionItem value="advanced" className="border-none">
                            <AccordionTrigger className="px-5 py-4 hover:bg-secondary/20 transition-all hover:no-underline">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className="text-xs text-muted-foreground">Tweak sources, cohorts & window</div>
                                    </div>
                                </div>
                            </AccordionTrigger>
                            <AccordionContent className="p-5 pt-2 space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Lookback</Label>
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Keywords</Label>
                                        <Input
                                ...
                                <div className="space-y-4">
                                    <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Target Cohorts</Label>
                                ...
                                {/* Sources */}
                                <div className="space-y-4">
                                    <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Intelligence Sources</Label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {SOURCES.map(source => (
                                            <div
                                                key={source.id}
                                ...
                                {/* Content Formats */}
                                <div className="space-y-4">
                                    <Label className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Output Formats</Label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {FORMATS.map(format => (
                                            <div
                                                key={format.id}
