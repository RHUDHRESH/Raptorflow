        ...
        <div ref={containerRef} className={`relative ${className}`}>
            ...
            <div className="fixed left-8 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col gap-3">
                {steps.map((_, index) => (
                    <div
                        key={index}
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${index === activeStep
    ...
    const textContent = (
        <div className="space-y-32 lg:space-y-48 py-24">
            {items.map((item, index) => (
                <div
                    key={item.id}
                    className={`split-text-item transition-opacity duration-500 ${index === activeIndex ? 'opacity-100' : 'opacity-40'
                        }`}
                >
