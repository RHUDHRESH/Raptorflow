...
export function TransformationCanvas({ isTransformed }: TransformationCanvasProps) {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            const foregroundColor = style.getPropertyValue('--foreground').trim() || '45 45 45';
...
export function ChaosToClarity() {
    ...
    return (
        <div className="w-full">
            {/* Canvas Container */}
            <div className="relative rounded-2xl overflow-hidden bg-muted/30 border border-border" style={{ height: '300px' }}>
                <TransformationCanvas isTransformed={isTransformed} />

                {/* Labels */}
                <div className="absolute inset-0 flex items-center justify-between px-8 pointer-events-none">
                    <div className={`text-center transition-opacity duration-500 ${isTransformed ? 'opacity-30' : 'opacity-100'}`}>
                        <div className="text-2xl font-display font-medium">Chaos</div>
                        <div className="text-sm text-muted-foreground">5+ tools, random tactics</div>
                    </div>
                    <div className={`text-center transition-opacity duration-500 ${isTransformed ? 'opacity-100' : 'opacity-30'}`}>
                        <div className="text-2xl font-display font-medium">Clarity</div>
                        <div className="text-sm text-muted-foreground">1 system, compounding results</div>
                    </div>
                </div>
            </div>
