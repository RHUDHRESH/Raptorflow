...
export function ActiveThreads({ threads, className }: ActiveThreadsProps) {
  const getStatusIcon = (status: AgentThread['status']) => {
    switch (status) {
      case 'thinking':
        return <Brain className="h-3 w-3 animate-pulse text-blue-500" />;
      case 'executing':
        return (
          <Cpu className="h-3 w-3 animate-spin [animation-duration:3000ms] text-accent" />
        );
      case 'validating':
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="h-1 w-full bg-muted rounded-full overflow-hidden">
                  <motion.div
                    ...
                    initial={{ width: '0%' }}
                    animate={{
                      width: thread.status === 'executing' ? '65%' : '100%',
                    }}
                    transition={{
                      duration: 2,
                      repeat: thread.status === 'executing' ? Infinity : 0,
                    }}
                  />
                </div>
              </div>
