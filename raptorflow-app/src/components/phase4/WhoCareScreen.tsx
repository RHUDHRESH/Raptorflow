...
export function WhoCareScreen({
  ...
  const getFrictionLabel = (friction: string) => {
    switch (friction) {
      case 'low':
        return { label: 'Fast close', color: 'text-green-600' };
      case 'medium':
        return { label: 'Some friction', color: 'text-amber-600' };
      case 'high':
        return { label: 'Long cycle', color: 'text-red-600' };
      default:
        return { label: '', color: '' };
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
        <p className="text-sm text-[#5B5F61]">
          <strong>Dunford Step:</strong> Identify who really cares about the
          value. Not everyone is your customer — focus on the segments that care
          most.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div
          className={`rounded-xl p-4 border-2 ${primarySegment ? 'bg-[#2D3538] border-[#2D3538]' : 'bg-white border-dashed border-[#C0C1BE]'}`}
        >
          <div className="flex items-center gap-2 mb-2">
            <Star
              className={`w-4 h-4 ${primarySegment ? 'text-white' : 'text-[#9D9F9F]'}`}
            />
            <span
              className={`text-[10px] font-mono uppercase tracking-wider ${primarySegment ? 'text-white/70' : 'text-[#9D9F9F]'}`}
            >
              Primary
            </span>
          </div>
          <p
            className={`text-sm font-medium ${primarySegment ? 'text-white' : 'text-[#9D9F9F]'}`}
          >
        ...
        <div
          className={`rounded-xl p-4 border ${secondarySegments.length > 0 ? 'bg-white border-[#2D3538]' : 'bg-white border-dashed border-[#C0C1BE]'}`}
        >
          <div className="flex items-center gap-2 mb-2">
        ...
        <div
          className={`rounded-xl p-4 border ${avoidSegments.length > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-dashed border-[#C0C1BE]'}`}
        >
          <div className="flex items-center gap-2 mb-2">
            <XCircle
              className={`w-4 h-4 ${avoidSegments.length > 0 ? 'text-red-500' : 'text-[#9D9F9F]'}`}
            />
            <span
              className={`text-[10px] font-mono uppercase tracking-wider ${avoidSegments.length > 0 ? 'text-red-500' : 'text-[#9D9F9F]'}`}
            >
              Avoid
            </span>
          </div>
          <p
            className={`text-sm ${avoidSegments.length > 0 ? 'text-red-700' : 'text-[#9D9F9F]'}`}
          >
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h4 className="font-serif text-lg text-[#2D3538]">
                    {segment.name}
                  </h4>
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <span
                      className={`text-xs flex items-center gap-1 ${frictionInfo.color}`}
                    >
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-1">
                  {segment.valueAlignment.map((item, i) => (
                    <li
                      key={i}
                      className="text-xs text-[#5B5F61] flex items-start gap-2"
                    >
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {segment.triggerEvents.map((trigger, i) => (
                    <span
                      key={i}
                      className="text-[10px] bg-[#F3F4EE] text-[#5B5F61] px-2 py-1 rounded"
                    >
                      {trigger}
                    </span>
                  ))}
      ...
      {primarySegment && (
        <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
          ...
          <p className="text-sm text-[#5B5F61]">
            <strong>{primarySegment.name}</strong> becomes your "For whom" in
            positioning statements. Their trigger events (
            {primarySegment.triggerEvents.slice(0, 2).join(', ')}) will shape
            campaign timing. Phase 5 will deep-dive into this ICP.
          </p>
        </div>
      )}
