...
export function PositioningStatementBuilder({
  ...
  const primarySegment = phase4.whoCareSegments?.find(
    (s) => s.rank === 'primary'
  );
  const statusQuo = phase4.competitiveAlternatives?.statusQuo?.[0]?.name;
  const primaryValue =
    phase4.valueClaims?.find((c) => c.isSelected)?.claimText ||
    phase4.valueClaims?.find((c) => c.isSelected)?.claim ||
    phase4.differentiatedValue?.[0]?.value;
  ...
  const activeVariant =
    variants.find((v) => v.style === activeStyle) || variants[0];
  const canonicalVariant = variants.find((v) => v.isCanonical);
  const proofCoverage = activeVariant.proofCoverage ?? 0;
  ...
  const handleLockAsCanonical = (variantId: string) => {
    ...
    onUpdate({
      ...
      oneThing:
        activeVariant.components?.ownedWord || activeVariant.components?.because,
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
        <p className="text-sm text-[#5B5F61]">
          ...
          — it's the foundation beneath all messaging. This statement won't
          appear on your website; it guides everything that does.
        </p>
      </div>
      ...
      <div className="flex gap-2 p-1 bg-[#F3F4EE] rounded-xl">
        {STATEMENT_STYLES.map(({ style, label, description }) => (
          <button
            ...
            className={`flex-1 px-4 py-3 rounded-lg transition-all text-left ${
              activeStyle === style
          ...
          >
            ...
            <span className="block text-xs text-[#9D9F9F] mt-0.5">
              {description}
            </span>
          </button>
        ))}
      ...
      >
        ...
        {isEditing ? (
          <textarea
            ...
            onChange={(e) =>
              handleUpdateVariant(activeVariant.id, e.target.value)
            }
            onBlur={() => setIsEditing(false)}
            autoFocus
            className={`w-full font-serif text-[24px] leading-[1.5] bg-transparent border-none focus:outline-none resize-none ${
              activeVariant.isCanonical ? 'text-white' : 'text-[#2D3538]'
            }`}
            rows={4}
          />
        ) : (
          <p
            ...
            className={`font-serif text-[24px] leading-[1.5] cursor-text ${
              activeVariant.isCanonical ? 'text-white' : 'text-[#2D3538]'
            }`}
          >
            "{activeVariant.text}"
        ...
        <div className="mt-6 flex items-center gap-4">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span
                className={`text-xs ${activeVariant.isCanonical ? 'text-white/60' : 'text-[#9D9F9F]'}`}
              >
                Proof Coverage
              </span>
              <span
                className={`text-xs font-mono ${activeVariant.isCanonical ? 'text-white' : 'text-[#2D3538]'}`}
              >
                {proofCoverage}%
              </span>
            </div>
            <div
              className={`h-1.5 rounded-full overflow-hidden ${activeVariant.isCanonical ? 'bg-white/20' : 'bg-[#E5E6E3]'}`}
            >
              <div
      ...
      {activeStyle === 'dunford' && (
        <div className="bg-[#F3F4EE] rounded-xl p-5 border border-[#E5E6E3]">
          ...
          <p className="text-sm text-[#5B5F61] font-mono">
            For <span className="text-[#2D3538]">[WHO CARES segment]</span>,{' '}
            <br />
            who struggle with{' '}
            <span className="text-[#2D3538]">[PROBLEM / STATUS QUO]</span>,{' '}
            <br />
            <span className="text-[#2D3538]">[PRODUCT]</span> is a{' '}
            <span className="text-[#2D3538]">[CATEGORY]</span>, <br />
            that delivers{' '}
            <span className="text-[#2D3538]">[PRIMARY VALUE]</span>, <br />
            unlike <span className="text-[#2D3538]">[PRIMARY ALTERNATIVE]</span>
            , <br />
            because{' '}
            <span className="text-[#2D3538]">[UNIQUE MECHANISM + PROOF]</span>.
          </p>
        </div>
      )}
