...
import { Phase4Data, PerceptualMapPoint, PerceptualMapAxis } from '@/lib/foundation';
...
const AXIS_PRESETS: { x: Partial<PerceptualMapAxis>; y: Partial<PerceptualMapAxis>; label: string }[] = [
    {
...
export function PerceptualMapScreen({
    ...
    const [pendingMove, setPendingMove] = useState<{ id: string; x: number; y: number } | null>(null);

    // Get or create default map data
    const perceptualMap = phase4.visuals?.perceptualMap || {
        xAxis: { label: 'Generic', rationale: '', minLabel: 'Generic', maxLabel: 'Specialized' },
        yAxis: { label: 'Complex', rationale: '', minLabel: 'Complex', maxLabel: 'Simple' },
        points: []
    };
    ...
    const getDefaultPoints = (): PerceptualMapPoint[] => {
        ...
        const competitors = [
            ...(phase4.competitiveAlternatives?.direct || []).slice(0, 3).map(c => ({
                id: uuidv4(),
                name: c.name,
        ...
        return competitors.length > 1 ? competitors : [
    ...
    const handleAxisChange = (axis: 'x' | 'y', field: 'label' | 'minLabel' | 'maxLabel', value: string) => {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
                <p className="text-sm text-[#5B5F61]">
                    <strong>Perceptual Map:</strong> Shows how customers perceive you vs competitors
                    on two key dimensions. Axes should reflect real decision criteria.
                </p>
            </div>
                ...
                <div className="bg-white border border-[#E5E6E3] rounded-xl p-4">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            onChange={(e) => handleAxisChange('x', 'minLabel', e.target.value)}
                        ...
                        <input
                            ...
                            onChange={(e) => handleAxisChange('x', 'maxLabel', e.target.value)}
                ...
                <div className="bg-white border border-[#E5E6E3] rounded-xl p-4">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            onChange={(e) => handleAxisChange('y', 'minLabel', e.target.value)}
                        ...
                        <input
                            ...
                            onChange={(e) => handleAxisChange('y', 'maxLabel', e.target.value)}
                ...
                >
                    ...
                    {points.map((point) => (
                        <div
                            ...
                            className={`absolute transform -translate-x-1/2 -translate-y-1/2 cursor-move transition-all ${draggingPoint === point.id ? 'scale-110 z-10' : ''
                                }`}
                            style={{
                        ...
                        >
                            <div className={`rounded-full flex items-center justify-center ${point.isYou
                            ...
                            <div className={`absolute left-1/2 -translate-x-1/2 mt-1 text-[10px] whitespace-nowrap ${point.isYou ? 'text-[#2D3538] font-medium' : 'text-[#5B5F61]'
                                }`}>
                                {point.name}
                            </div>
                        </div>
