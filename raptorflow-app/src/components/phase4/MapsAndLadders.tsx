...
function CanvasView({ canvas }: { canvas: StrategyCanvas }) {
  ...
  if (n === 0)
    return (
      <p className="text-center text-muted-foreground">No factors defined</p>
    );
          ...
          <polyline
            points={factors
              .map((f, i) => {
                ...
                const y =
                  height -
                  ((canvas.curves.youTarget[i] - 1) / 4) * (height - 40);
                return `${x},${y}`;
              })
              .join(' ')}
            fill="none"
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
      {columns.map(({ key, label, color }) => (
        <div
          key={key}
          className={cn('rounded-xl p-4', color.split(' ').slice(1).join(' '))}
        >
          <h4 className={cn('font-semibold text-sm mb-3', color.split(' ')[0])}>
            {label}
          </h4>
...
export function MapsAndLadders({
  visuals,
  onChange,
  onContinue,
}: MapsAndLaddersProps) {
    ...
    <div className="space-y-8">
      ...
      <div className="min-h-[400px]">
        {activeTab === 'perceptual' && (
          <PerceptualMapView map={visuals.perceptualMap} />
        )}
        {activeTab === 'ladder' && <LadderView ladder={visuals.ladder} />}
        {activeTab === 'canvas' && (
          <CanvasView canvas={visuals.strategyCanvas} />
        )}
