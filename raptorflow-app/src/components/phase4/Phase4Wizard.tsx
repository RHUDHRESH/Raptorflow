...
export function Phase4Wizard() {
  ...
  const savePhase4 = useCallback(
    async (data: Phase4Data) => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F3F4EE]">
        <div className="text-center space-y-6">
          ...
          <p className="font-serif text-2xl text-[#2D3538]">
            {isProcessing
              ? 'Compiling positioning from Phase 3...'
              : 'Loading...'}
          </p>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* Screen: Lock & Export */}
      {currentScreen === 'lock' && (
        <LockExportScreen phase4={phase4} onLock={handleLock} onBack={goBack} />
      )}
    </PhaseScreen>
  );
}
