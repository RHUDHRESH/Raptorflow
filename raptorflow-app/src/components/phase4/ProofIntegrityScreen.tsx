...
const TIER_INFO: Record<ProofTier, { label: string; color: string; description: string }> = {
    1: { label: 'Tier 1', color: 'bg-green-100 text-green-700 border-green-200', description: 'Quantified case study' },
    2: { label: 'Tier 2', color: 'bg-blue-100 text-blue-700 border-blue-200', description: 'Benchmark/third-party' },
    3: { label: 'Tier 3', color: 'bg-amber-100 text-amber-700 border-amber-200', description: 'Testimonial' },
    4: { label: 'Tier 4', color: 'bg-red-100 text-red-700 border-red-200', description: 'Internal assertion' },
};
...
export function ProofIntegrityScreen({
    ...
    const buildProofItems = (): ProofIntegrityItem[] => {
        ...
        phase4.valueClaims?.forEach(claim => {
            items.push({
                ...
                evidenceAttached: claim.evidenceIds.length > 0,
                tier: claim.evidenceIds.length > 0 ? 2 : 4,
                tierLabel: claim.evidenceIds.length > 0 ? 'Has proof' : 'Needs proof',
                needsFix: claim.evidenceIds.length === 0 && claim.isSelected,
                isHypothesis: claim.evidenceIds.length === 0
            });
        });
        ...
        return items.length > 0 ? items : [
            { id: uuidv4(), claimId: 'sample-1', claimText: 'Save 10+ hours per week on marketing operations', evidenceAttached: true, tier: 1 as ProofTier, tierLabel: 'Case study available', needsFix: false, isHypothesis: false },
            { id: uuidv4(), claimId: 'sample-2', claimText: 'Unified positioning for all marketing activities', evidenceAttached: false, tier: 4 as ProofTier, tierLabel: 'No proof yet', needsFix: true, isHypothesis: true },
            { id: uuidv4(), claimId: 'sample-3', claimText: 'AI-powered insights that competitors lack', evidenceAttached: false, tier: 4 as ProofTier, tierLabel: 'No proof yet', needsFix: true, isHypothesis: true },
        ];
    };

    const proofItems = buildProofItems();
    const needsFixItems = proofItems.filter(p => p.needsFix);
    const verifiedItems = proofItems.filter(p => !p.needsFix && !p.isHypothesis);
    ...
    const integrityScore = proofItems.length > 0
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
                <p className="text-sm text-[#5B5F61]">
                    <strong>Anti-BS Firewall:</strong> Every claim must attach proof or be marked as hypothesis.
                    This is where you stop sounding like every other SaaS. No claim without evidence.
                </p>
            </div>
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h3 className="font-serif text-xl text-[#2D3538]">Claim Integrity Score</h3>
                        <p className="text-sm text-[#5B5F61] mt-1">
                            {verifiedItems.length} verified • {needsFixItems.length} need attention
                        </p>
                    </div>
                    <div className={`text-4xl font-serif ${integrityScore > 70 ? 'text-green-600' :
                            integrityScore > 40 ? 'text-amber-600' :
                                'text-red-600'
                        }`}>
                        {integrityScore}%
                    </div>
                ...
                <div className="h-2 bg-[#E5E6E3] rounded-full overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all ${integrityScore > 70 ? 'bg-green-500' :
                                integrityScore > 40 ? 'bg-amber-500' :
                                    'bg-red-500'
                            }`}
                            ...
                            >
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        ...
                                        <span className={`text-[10px] font-mono uppercase px-2 py-1 rounded border ${TIER_INFO[item.tier].color}`}>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <Button
                                            size="sm"
                                            variant="ghost"
                                            onClick={() => handleMarkHypothesis(item.id)}
                                ...
                                <div className="mt-4 pt-4 border-t border-amber-200">
                                    ...
                                    <div className="flex gap-2">
                                        {([1, 2, 3, 4] as ProofTier[]).map((tier) => (
                                            <button
                                                key={tier}
                                                onClick={() => handleTierChange(item.id, tier)}
                                                className={`px-3 py-1.5 rounded-lg text-[10px] font-mono transition-colors ${item.tier === tier
                    ...
                    <div className="space-y-2">
                        {verifiedItems.map((item) => (
                            ...
                            >
                                ...
                                <p className="text-sm text-[#2D3538] flex-1">{item.claimText}</p>
                                <span className={`text-[10px] font-mono uppercase px-2 py-1 rounded border ${TIER_INFO[item.tier].color}`}>
            ...
            {proofItems.filter(p => p.isHypothesis && !p.needsFix).length > 0 && (
                <div>
                    ...
                    <div className="space-y-2">
                        {proofItems.filter(p => p.isHypothesis && !p.needsFix).map((item) => (
                            <div
                                key={item.id}
                                className="bg-[#F3F4EE] border border-dashed border-[#C0C1BE] rounded-xl p-4 flex items-center gap-4"
                            >
                                <p className="text-sm text-[#5B5F61] flex-1 italic">{item.claimText}</p>
