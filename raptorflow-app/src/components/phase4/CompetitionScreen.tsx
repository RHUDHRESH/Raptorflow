...
import { Phase4Data, CompetitiveAlternative, CompetitorType } from '@/lib/foundation';
...
export function CompetitionScreen({
    ...
    const getDefaultAlternatives = (type: TabType): CompetitiveAlternative[] => {
        if (type === 'statusQuo') {
            return [
                { id: uuidv4(), name: 'Spreadsheets & Docs', whatUsedFor: 'Tracking campaigns, content calendars', whatBreaks: 'No automation, manual errors', whyTolerated: 'Free, familiar', evidence: [], isConfirmed: false },
                { id: uuidv4(), name: 'WhatsApp / Slack chaos', whatUsedFor: 'Team coordination', whatBreaks: 'No audit trail, context lost', whyTolerated: 'Everyone\'s there', evidence: [], isConfirmed: false },
                { id: uuidv4(), name: 'Founder does it all', whatUsedFor: 'Everything marketing', whatBreaks: 'Burnout, bottleneck', whyTolerated: 'No budget for tools', evidence: [], isConfirmed: false },
            ];
        } else if (type === 'indirect') {
            return [
                { id: uuidv4(), name: 'Marketing Agency', whatUsedFor: 'Strategy + execution', whatBreaks: 'Expensive, slow, misaligned', whyTolerated: 'Expertise outsourced', evidence: [], isConfirmed: false },
                { id: uuidv4(), name: 'Freelancer / Contractor', whatUsedFor: 'Specific tasks', whatBreaks: 'Inconsistent, needs management', whyTolerated: 'Flexible, cheaper', evidence: [], isConfirmed: false },
                { id: uuidv4(), name: 'Full-time Marketing Hire', whatUsedFor: 'In-house capability', whatBreaks: 'Expensive, single point of failure', whyTolerated: 'Dedicated attention', evidence: [], isConfirmed: false },
            ];
        } else {
            return [
                { id: uuidv4(), name: 'HubSpot', whatUsedFor: 'All-in-one marketing', whatBreaks: 'Complex, expensive at scale', whyTolerated: 'Market leader', evidence: [], isConfirmed: false },
                { id: uuidv4(), name: 'Mailchimp', whatUsedFor: 'Email marketing', whatBreaks: 'Limited beyond email', whyTolerated: 'Easy to start', evidence: [], isConfirmed: false },
            ];
        }
    };

    const alternatives = getAlternatives(activeTab).length > 0
    ...
    const handleUpdateAlternative = (id: string, field: keyof CompetitiveAlternative, value: string) => {
        const updated = alternatives.map(a =>
    ...
    const confidence = totalCount > 0 ? Math.round((confirmedCount / totalCount) * 100) : 0;

    return (
        <div className="space-y-6">
            {/* Tabs */}
            <div className="flex gap-2 p-1 bg-[#F3F4EE] rounded-xl">
                {(Object.entries(TAB_CONFIG) as [TabType, typeof TAB_CONFIG[TabType]][]).map(([key, config]) => (
                    <button
                        key={key}
                        onClick={() => setActiveTab(key)}
                        className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg transition-all ${activeTab === key
                    ...
                    >
                        ...
                        <span className={`text-xs px-1.5 py-0.5 rounded ${activeTab === key ? 'bg-[#2D3538] text-white' : 'bg-[#E5E6E3] text-[#5B5F61]'
                            }`}>
                            {getAlternatives(key).length || getDefaultAlternatives(key).length}
                        </span>
                    </button>
                ))}
            </div>
            ...
            <div className="bg-[#F3F4EE] rounded-xl p-4 border border-[#E5E6E3]">
                <p className="text-sm text-[#5B5F61]">
                    <strong>{TAB_CONFIG[activeTab].label}:</strong> {TAB_CONFIG[activeTab].description}
                </p>
            </div>
            ...
            <div className="space-y-4">
                {alternatives.map((alt) => (
                    <div
                        key={alt.id}
                        className={`bg-white border rounded-2xl p-5 transition-all ${alt.isConfirmed ? 'border-[#2D3538]' : 'border-[#E5E6E3]'
                            }`}
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <button
                                ...
                                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-1 ${alt.isConfirmed
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    <input
                                        type="text"
                                        value={alt.name}
                                        onChange={(e) => handleUpdateAlternative(alt.id, 'name', e.target.value)}
                                ...
                                <div className="grid grid-cols-3 gap-4 text-sm">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleUpdateAlternative(alt.id, 'whatUsedFor', e.target.value)}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleUpdateAlternative(alt.id, 'whatBreaks', e.target.value)}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleUpdateAlternative(alt.id, 'whyTolerated', e.target.value)}
            ...
            {activeTab === 'direct' && alternatives.length > 0 && (
                <div className="bg-[#F3F4EE] rounded-2xl p-6 border border-[#E5E6E3]">
                    <h4 className="font-medium text-[#2D3538] mb-4">Collision Likelihood</h4>
                    <div className="space-y-3">
                        {alternatives.slice(0, 3).map((alt) => {
                            const score = Math.floor(Math.random() * 40) + 60; // Mock score 60-100%
                            return (
                                <div key={alt.id} className="flex items-center gap-4">
                                    <span className="text-sm text-[#2D3538] w-32 truncate">{alt.name}</span>
                                    <div className="flex-1 h-2 bg-[#E5E6E3] rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full ${score > 80 ? 'bg-red-500' : score > 60 ? 'bg-amber-500' : 'bg-green-500'
                                                }`}
                                            style={{ width: `${score}%` }}
                                        />
                                    </div>
                                    <span className={`text-xs font-mono ${score > 80 ? 'text-red-600' : score > 60 ? 'text-amber-600' : 'text-green-600'
                                        }`}>
                                        {score}%
                                    </span>
                                </div>
