...
import { Phase4Data, UniqueAttribute, AttributeType, Copyability } from '@/lib/foundation';
...
const ATTRIBUTE_TYPES: { value: AttributeType; label: string; icon: React.ReactNode }[] = [
    { value: 'feature', label: 'Feature', icon: <Settings className="w-3 h-3" /> },
    { value: 'process', label: 'Process', icon: <Layers className="w-3 h-3" /> },
    { value: 'data-advantage', label: 'Data', icon: <Shield className="w-3 h-3" /> },
    { value: 'workflow', label: 'Workflow', icon: <Lightbulb className="w-3 h-3" /> },
    { value: 'distribution', label: 'Distribution', icon: <Lightbulb className="w-3 h-3" /> },
];

const COPYABILITY_OPTIONS: { value: Copyability; label: string; color: string }[] = [
    { value: 'hard', label: 'Hard to copy', color: 'bg-green-100 text-green-700' },
...
export function UniqueAttributesScreen({
    ...
    const displayAttributes = attributes.length > 0 ? attributes : [
        { id: uuidv4(), attribute: 'Integrated AI-powered positioning engine', type: 'feature' as AttributeType, copyability: 'hard' as Copyability, requiresProof: true, source: 'derived' as const, isConfidential: false, evidenceIds: [] },
        { id: uuidv4(), attribute: 'Evidence-first claim validation', type: 'process' as AttributeType, copyability: 'medium' as Copyability, requiresProof: true, source: 'derived' as const, isConfidential: false, evidenceIds: [] },
        { id: uuidv4(), attribute: 'Dunford positioning framework built-in', type: 'workflow' as AttributeType, copyability: 'medium' as Copyability, requiresProof: false, source: 'derived' as const, isConfidential: false, evidenceIds: [] },
    ];
    ...
    const handleAddAttribute = () => {
        ...
        onUpdate({ uniqueAttributes: [...displayAttributes, newAttr] });
        setNewAttributeText('');
    };

    const handleRemoveAttribute = (id: string) => {
        onUpdate({ uniqueAttributes: displayAttributes.filter(a => a.id !== id) });
    };
    ...
    const handleUpdateAttribute = <K extends keyof UniqueAttribute>(
        ...
        onUpdate({ uniqueAttributes: updated });
    };
    ...
    const proofRequired = displayAttributes.filter(a => a.requiresProof && a.evidenceIds.length === 0);
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
                <p className="text-sm text-[#5B5F61]">
                    ...
                    without judging value yet. We're cataloging capabilities, not making claims.
                </p>
            </div>
            ...
            <div className="flex items-center justify-between">
                <span className="text-sm text-[#5B5F61]">
                    {displayAttributes.length} attributes • {proofRequired.length} need proof
                </span>
                <button
            ...
            <div className="bg-white border border-[#E5E6E3] rounded-2xl overflow-hidden">
                ...
                <div className="divide-y divide-[#E5E6E3]">
                    {filteredAttributes.map((attr) => (
                        <div
                            key={attr.id}
                            className={`grid grid-cols-12 gap-4 px-5 py-4 items-center ${attr.isConfidential ? 'bg-amber-50/50' : ''
                                }`}
                        >
                            {/* Attribute Text */}
                            <div className="col-span-5 flex items-start gap-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleUpdateAttribute(attr.id, 'attribute', e.target.value)}
                            ...
                            <div className="col-span-2">
                                <select
                                    value={attr.type}
                                    onChange={(e) => handleUpdateAttribute(attr.id, 'type', e.target.value as AttributeType)}
                            ...
                            <div className="col-span-2">
                                <select
                                    ...
                                    onChange={(e) => handleUpdateAttribute(attr.id, 'copyability', e.target.value as Copyability)}
                                    className={`w-full text-xs px-2 py-1.5 rounded-lg border-none focus:outline-none cursor-pointer ${COPYABILITY_OPTIONS.find(c => c.value === attr.copyability)?.color || 'bg-[#F3F4EE]'
                                        }`}
                                >
                            ...
                            <div className="col-span-1 flex justify-center">
                                <button
                                    onClick={() => handleUpdateAttribute(attr.id, 'requiresProof', !attr.requiresProof)}
                                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${attr.requiresProof
                                            ? attr.evidenceIds.length > 0
                                                ? 'bg-green-500 border-green-500 text-white'
                                                : 'bg-amber-500 border-amber-500 text-white'
                                            : 'border-[#C0C1BE]'
                                        }`}
                                    title={attr.requiresProof ? 'Proof required' : 'No proof needed'}
                                >
                                    {attr.requiresProof && (attr.evidenceIds.length > 0 ? <Check className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />)}
                                </button>
                            </div>

                            {/* Source */}
                            <div className="col-span-1">
                                <span className={`text-[10px] uppercase font-mono ${attr.source === 'derived' ? 'text-[#5B5F61]' : 'text-blue-600'
                                    }`}>
                            ...
                            <div className="col-span-1 flex items-center gap-1 justify-end">
                                <button
                                    ...
                                    title={attr.isConfidential ? 'Make visible' : 'Mark confidential'}
                                >
                                    {attr.isConfidential ? (
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                        <p className="text-sm text-amber-800 font-medium">
                            {proofRequired.length} attribute{proofRequired.length > 1 ? 's' : ''} need proof
                        </p>
                        <p className="text-xs text-amber-700 mt-1">
                            These will be marked as "hypothesis" in your positioning pack until evidence is attached.
                        </p>
                    </div>
                </div>
            )}
