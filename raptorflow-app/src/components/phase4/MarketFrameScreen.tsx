...
const PRICING_POSTURES: {
...
}[] = [
  {
    value: 'premium',
    label: 'Premium',
    description: 'Top 20% pricing, luxury positioning',
  },
  {
    value: 'mid',
    label: 'Mid-Market',
    description: 'Competitive pricing, value for money',
  },
  {
    value: 'volume',
    label: 'Volume',
    description: 'Lower pricing, high volume play',
  },
];
...
export function MarketFrameScreen({
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    const selectedCategory = updatedCandidates?.find(
      (c) => c.id === categoryId
    );
    ...
    onUpdate({
      ...
      marketCategory: selectedCategory
        ? {
            ...
            altLabels: [],
            whyThisContext: [selectedCategory.description],
            risk: selectedCategory.risk === 'low' ? 'ok' : selectedCategory.risk === 'medium' ? 'too-crowded' : 'too-broad',
          }
  ...
  const handleAddAntiCategory = () => {
    ...
    onUpdate({
      antiCategories: [
        ...(phase4.antiCategories || []),
        antiCategoryInput.trim(),
      ],
    });
    ...
    : [
        {
          ...
          definition:
            'A unified platform that orchestrates all marketing activities from a single source of truth.',
          buyerExpectations: [
            'Strategic planning',
        ...
        },
        {
          ...
          definition:
            'Software that automates repetitive marketing tasks and workflows.',
          buyerExpectations: [
            'Email automation',
            'Lead scoring',
            'Campaign management',
        ...
        },
        {
          ...
          definition:
            'AI-powered system that provides strategic insights for go-to-market execution.',
          buyerExpectations: [
            'Market intelligence',
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-[#2D3538]">
              <strong>Why this matters:</strong> Category choice determines who
              you're compared against, what buyers expect, and which budget
              you're competing for.
            </p>
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-[#5B5F61]">
              <span>
                • "When a buyer explains you to their boss, what do they call
                you?"
              </span>
      ...
      {/* Category Candidates */}
      <div>
        <h3 className="font-serif text-xl text-[#2D3538] mb-4">
          What are you?
        </h3>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span
                        className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded ${riskInfo.color}`}
                      >
                        {riskInfo.label}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-[#5B5F61] mb-4">
                      {(category as any).definition || (category as any).description}
                    </p>

                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div>
                        <span className="text-[#9D9F9F] uppercase tracking-wider">
                          Buyers expect:
                        </span>
                        <ul className="mt-1 space-y-0.5">
                          {(category as any).buyerExpectations
                            .slice(0, 3)
                            .map((exp: string, i: number) => (
                              <li key={i} className="text-[#5B5F61]">
                                • {exp}
                              </li>
                            ))}
                        </ul>
                      ...
                      <div>
                        ...
                        <p className="text-[#2D3538] mt-1">
                          Will search for{' '}
                          {(category as any).comparisonSet?.length || 0}{' '}
                          competitors
                        </p>
                      </div>
                    </div>

                    {(category as any).risks?.length > 0 && (
                      <div className="mt-4 p-3 rounded-lg bg-amber-50 border border-amber-200">
                        <p className="text-xs font-medium text-amber-800 mb-1">
                          Risks:
                        </p>
                        <ul className="space-y-0.5">
                          {(category as any).risks.map((risk: string, i: number) => (
          ...
          {PRICING_POSTURES.map((posture) => (
            <button
              ...
              className={`p-4 rounded-xl border-2 text-left transition-all ${
                phase4.pricingPosture === posture.value
        ...
        <div className="bg-[#F3F4EE] rounded-2xl p-6 border border-[#E5E6E3]">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-[#2D3538] mt-1">
                Will search for{' '}
                {categoryCandidates.find((c) => c.id === selectedCategoryId)
                  ?.comparisonSet?.length || 0}{' '}
                competitors
              </p>
            </div>
            <div>
              <span className="text-[#9D9F9F] text-xs">
                Perceptual map axes
              </span>
              <p className="text-[#2D3538] mt-1">
            ...
            <div>
              ...
              <p className="text-[#2D3538] mt-1">
                {phase4.pricingPosture === 'premium'
                  ? 'Luxury, exclusive'
                  : phase4.pricingPosture === 'mid'
