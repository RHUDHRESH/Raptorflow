...
const VALUE_TYPES: {
...
}[] = [
  {
    ...
    description: 'Time, money, or risk',
  },
  {
    type: 'emotional',
    label: 'Emotional',
    icon: <Heart className="w-4 h-4" />,
    description: 'Control, confidence, relief',
  },
  {
    type: 'social',
    label: 'Social',
    icon: <Users className="w-4 h-4" />,
    description: 'Status, competence signal',
  },
];
...
export function ValueTranslationScreen({
  ...
  const generateClaimsForAttribute = (attr: UniqueAttribute) => {
    const claims: ValueClaim[] = [
      {
        id: uuidv4(),
        claim: `Save hours every week with ${attr.attribute.toLowerCase()}`,
      ...
      {
        id: uuidv4(),
        claim: `Finally feel in control with ${attr.attribute.toLowerCase()}`,
      ...
      {
        id: uuidv4(),
        claim: `Look like a strategic leader with ${attr.attribute.toLowerCase()}`,
  ...
  const handleSaveClaims = () => {
    ...
    const existingOtherClaims = valueClaims.filter(
      (c) => c.attributeId !== selectedAttributeId
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
        <p className="text-sm text-[#5B5F61]">
          ...
          they care what it does for them. Translate each attribute into
          customer value.
        </p>
      </div>
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-2 space-y-3">
          <h3 className="font-serif text-lg text-[#2D3538] mb-4">
            Your Attributes
          </h3>
                ...
                <div className="flex items-start gap-3">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                      claims.length > 0 ? 'bg-green-100' : 'bg-[#F3F4EE]'
                    }`}
                  >
                    {claims.length > 0 ? (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-[#5B5F61] mt-1">
                      {claims.length > 0
                        ? `${claims.length} value claims`
                        : 'No claims yet'}
                    </p>
                  </div>
                  <ArrowRight
                    className={`w-4 h-4 ${isSelected ? 'text-[#2D3538]' : 'text-[#9D9F9F]'}`}
                  />
                </div>
              </button>
            );
        ...
        <div className="col-span-3">
          {selectedAttributeId ? (
            <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
              <h3 className="font-serif text-lg text-[#2D3538] mb-2">
                Value Claims
              </h3>
              ...
              <div className="space-y-6">
                {VALUE_TYPES.map((vt) => {
                  const typeClaims = generatedClaims.filter(
                    (c) => c.valueType === vt.type
                  );
                  ...
                  return (
                    <div key={vt.type}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs text-[#9D9F9F]">
                          {vt.description}
                        </span>
                      </div>
                        ...
                        {typeClaims.map((claim) => (
                          ...
                          >
                            <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex gap-1">
                                      {PROOF_HOOKS.map((hook) => (
                                        <button
                                          key={hook.type}
                                          onClick={() =>
                                            handleProofHookChange(
                                              claim.id,
                                              hook.type
                                            )
                                          }
                                          className={`text-[10px] px-2 py-1 rounded transition-colors ${
                                            claim.proofHook === hook.type
                              ...
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded ${getRiskColor(claim.riskLevel || 'medium')}`}
                              >
                                {claim.riskLevel}
                              </span>
                            </div>
              ...
              <div className="mt-6 flex justify-end">
                <Button
                  ...
                  disabled={
                    generatedClaims.filter((c) => c.isSelected).length === 0
                  }
      ...
      <div className="bg-[#F3F4EE] rounded-2xl p-5 border border-[#E5E6E3]">
        <div className="flex items-center justify-between">
          <span className="text-sm text-[#5B5F61]">
            {valueClaims.length} value claims across{' '}
            {new Set(valueClaims.map((c) => c.attributeId)).size} attributes
          </span>
          {valueClaims.filter((c) => !c.proofHook).length > 0 && (
            <span className="text-xs text-amber-600 flex items-center gap-1">
              <AlertCircle className="w-3 h-3" />
              {valueClaims.filter((c) => !c.proofHook).length} claims need proof
              hooks
            </span>
          )}
        </div>
