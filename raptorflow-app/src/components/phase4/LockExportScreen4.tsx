...
export function LockExportScreen({
  ...
  const getCompletionStatus = () => {
    ...
    if ((phase4.competitiveAlternatives?.direct?.length || 0) > 0) {
      completed.push(
        `${phase4.competitiveAlternatives?.direct?.length} direct competitors mapped`
      );
    }

    if ((phase4.uniqueAttributes?.length || 0) > 0) {
      completed.push(
        `${phase4.uniqueAttributes?.length} unique attributes defined`
      );
    } else {
    ...
    if ((phase4.valueClaims?.filter((c) => c.isSelected)?.length || 0) > 0) {
      completed.push(
        `${phase4.valueClaims?.filter((c) => c.isSelected)?.length} value claims selected`
      );
    }

    if (phase4.whoCareSegments?.find((s) => s.rank === 'primary')) {
      completed.push(
        `Primary segment: ${phase4.whoCareSegments.find((s) => s.rank === 'primary')?.name}`
      );
    } else {
  ...
  const confidenceScore =
    ...
    Math.round(
      (completed.length / (completed.length + uncertain.length)) * 100
    );
  ...
  const handleExport = async (type: 'pdf' | 'json') => {
    ...
    if (type === 'json') {
      const blob = new Blob([JSON.stringify(phase4, null, 2)], {
    ...
    <div className="space-y-8">
      ...
      <div className="text-center max-w-xl mx-auto">
        ...
        <p className="text-[#5B5F61]">
          This will become your marketing OS source of truth. All agents will
          use this positioning pack for content and campaigns.
        </p>
      </div>
        ...
        <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
          ...
          <ul className="space-y-2">
            {completed.map((item, i) => (
              <li
                key={i}
                className="text-sm text-[#5B5F61] flex items-start gap-2"
              >
      ...
      <div className="max-w-xl mx-auto">
        <h3 className="font-serif text-lg text-[#2D3538] mb-4 text-center">
          Export Options
        </h3>
        <div className="grid grid-cols-3 gap-4">
          ...
          >
            ...
            <span className="text-sm text-[#9D9F9F] block">
              Embeddings (auto)
            </span>
          </button>
        </div>
