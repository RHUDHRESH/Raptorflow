...
export function PositioningStudioHome({
  ...
  const [activeEvidencePanel, setActiveEvidencePanel] = useState<string | null>(
    null
  );
  ...
  const getSectionStatuses = (): SectionStatus[] => {
    const statuses: SectionStatus[] = [
      ...
      },
      {
        ...
        label: 'Alternatives',
        status:
          (phase4.competitiveAlternatives?.statusQuo?.length || 0) > 0
            ? 'complete'
            : 'missing',
      },
      {
        ...
        status:
          (phase4.uniqueAttributes?.length || 0) > 0 ? 'complete' : 'missing',
      },
      {
        id: 'value',
      ...
      },
      {
        ...
        label: 'Who Cares',
        status:
          (phase4.whoCareSegments?.filter((s) => s.rank !== 'unranked')
            ?.length || 0) > 0
            ? 'complete'
            : 'missing',
      },
      ...
      },
      {
        ...
        label: 'Visual Maps',
        status:
          (phase4.visuals?.perceptualMap?.points?.length || 0) > 0
            ? 'complete'
            : 'missing',
      },
      {
        ...
        label: 'Proof Stack',
        status:
          (phase4.proofIntegrity?.filter((p) => !p.needsFix)?.length || 0) > 0
            ? 'complete'
            : phase4.proofIntegrity?.some((p) => p.needsFix)
              ? 'needs-review'
              : 'missing',
      },
    ];
  ...
  const confidenceScore =
    phase4.confidenceScore ||
    Math.round((completedCount / sectionStatuses.length) * 100);

  // Build positioning statement preview
  const positioningPreview =
    phase4.positioningStatement ||
      ...
      <div className="flex">
          ...
          <div className="space-y-2">
            {sectionStatuses.map((section) => (
              ...
              >
                ...
                />
                <span className="text-sm text-[#2D3538] flex-1">
                  {section.label}
                </span>
        ...
        <div className="flex-1 p-10 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <p className="text-[#5B5F61] mb-6">
              Draft positioning derived from your inputs
            </p>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-white border border-[#E5E6E3] rounded-2xl p-5">
                ...
                <p className="mt-2 text-sm text-[#2D3538]">
                  {phase4.competitiveAlternatives?.statusQuo?.[0]?.name ||
                    'Manual processes & spreadsheets'}
                </p>
                <p className="mt-1 text-xs text-[#5B5F61]">
                  {phase4.competitiveAlternatives?.statusQuo?.length || 0}{' '}
                  alternatives
                </p>
              </div>
              ...
              <div className="bg-white border border-[#E5E6E3] rounded-2xl p-5">
                ...
                <p className="mt-2 text-sm text-[#2D3538]">
                  {phase4.competitiveAlternatives?.indirect?.[0]?.name ||
                    'Agencies & freelancers'}
                </p>
                <p className="mt-1 text-xs text-[#5B5F61]">
                  {phase4.competitiveAlternatives?.indirect?.length || 0}{' '}
                  alternatives
                </p>
              </div>
              ...
              <div className="bg-white border border-[#E5E6E3] rounded-2xl p-5">
                ...
                <p className="mt-2 text-sm text-[#2D3538]">
                  {phase4.competitiveAlternatives?.direct?.[0]?.name ||
                    'Competing tools'}
                </p>
                <p className="mt-1 text-xs text-[#5B5F61]">
                  {phase4.competitiveAlternatives?.direct?.length || 0}{' '}
                  competitors
                </p>
              </div>
            </div>
              ...
              {(phase4.uniqueAttributes?.slice(0, 5) || []).map((attr, i) => (
                ...
                >
                  <div className="w-8 h-8 bg-[#F3F4EE] rounded-full flex items-center justify-center">
                    <span className="font-serif text-sm text-[#2D3538]">
                      {i + 1}
                    </span>
                  </div>
                  <div className="flex-1">
                    <p className="text-sm text-[#2D3538] font-medium">
                      {attr.attribute}
                    </p>
                    <p className="text-xs text-[#5B5F61]">
                      {attr.type} • {attr.copyability} to copy
                    </p>
                  </div>
                ...
                <div className="w-full aspect-square bg-[#F3F4EE] rounded-xl mb-3 flex items-center justify-center">
                  <div className="space-y-1">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className={`h-2 rounded bg-[#C0C1BE] ${i === 1 ? 'w-8' : i === 2 ? 'w-6' : 'w-4'}`}
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="w-full aspect-square bg-[#F3F4EE] rounded-xl mb-3 flex items-center justify-center">
                  <div className="flex items-end gap-1 h-10">
                    {[3, 5, 2, 4, 3].map((h, i) => (
                      <div
                        key={i}
                        className="w-2 bg-[#C0C1BE] rounded-t"
                        style={{ height: `${h * 6}px` }}
                      />
                    ))}
                  </div>
        ...
        <div className="w-80 border-l border-[#C0C1BE] bg-white p-6 min-h-screen">
          ...
          {activeEvidencePanel ? (
            <div className="space-y-4">
              ...
              {phase4.evidenceGraph
                ?.filter((e) => e.claimId === activeEvidencePanel)
                .map((evidence) => (
                  <div
                    key={evidence.id}
              ...
              {(!phase4.evidenceGraph ||
                phase4.evidenceGraph.filter(
                  (e) => e.claimId === activeEvidencePanel
                ).length === 0) && (
                <p className="text-sm text-[#5B5F61]">
