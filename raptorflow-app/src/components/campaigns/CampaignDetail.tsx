...
export function CampaignDetail({
  ...
  const nextMove = moves.find(
    (m) => m.status === 'queued' || m.status === 'draft'
  );
  ...
  const handleStartNextMove = async () => {
    ...
    const updatedMoves: Move[] = moves.map((m) =>
      m.id === nextMove.id
        ? { ...m, status: 'active' as MoveStatus }
        : m.status === 'active'
          ? { ...m, status: 'queued' as MoveStatus }
          : m
    );
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10">
            ...
            {activeMove ? (
              <div className="bg-white dark:bg-zinc-900 rounded-2xl p-8 border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-zinc-500 mt-1">
                      {CHANNEL_LABELS[activeMove.channel]} •{' '}
                ...
                <div className="grid grid-cols-3 gap-8 pt-6 border-t border-zinc-100 dark:border-zinc-800">
                  <div>
                    <div className="text-xs text-zinc-400 uppercase font-medium mb-1">
                      Micro-Goal
                    </div>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-zinc-900 dark:text-zinc-100">
                      {
                        activeMove.checklist?.filter((i: any) => i.completed)
                          .length
                      }{' '}
                      / {activeMove.checklist?.length} done
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-zinc-400 uppercase font-medium mb-1">
              ...
              <div className="bg-amber-50 dark:bg-amber-900/10 rounded-2xl p-8 border border-amber-200 dark:border-amber-800/30 text-center">
                ...
                <p className="text-amber-700 dark:text-amber-400 mb-6 max-w-sm mx-auto">
                  Momentum is zero. Start the next move to keep the campaign
                  alive.
                </p>
                {nextMove ? (
                  <Button
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              {isLoadingGantt && (
                <Clock className="w-3 h-3 text-zinc-400 animate-spin" />
              )}
            </div>
          ...
          {campaign.strategyArc &&
            (campaign.strategyArc as any).monthly_plans && (
              <section className="space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400">
                  Strategic Arc Decomposition
                </h3>
                <div className="grid grid-cols-3 gap-4">
                  {(campaign.strategyArc as any).monthly_plans.map(
                    (plan: any) => (
                      <div
                        key={plan.month_number}
                        className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 space-y-3 flex flex-col justify-between group"
                      >
                        <div>
                          <div className="text-[10px] font-bold text-accent uppercase mb-1">
                            Month {plan.month_number}
                          </div>
                          <h4 className="font-semibold text-zinc-900 dark:text-zinc-100 leading-snug mb-2">
                            {plan.theme}
                          </h4>
          ...
          <section>
            <CampaignAuditorView
              alignments={auditData as any}
              overallScore={campaign.qualityScore}
            />
          </section>
              ...
              {moves.map((move, i) => {
                ...
                return (
                  <div key={move.id} className="relative pl-12 group">
                    <div
                      className={`absolute left-[11px] top-3 w-4 h-4 rounded-full border-2 transition-colors z-10 bg-white dark:bg-zinc-900 ${
                        isActive
