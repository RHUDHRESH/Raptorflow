...
const EXCLUDED_TYPES = [
    "Enterprise", "Agencies", "Price shoppers", "Early hobbyists", "Non-English markets", "Consultants"
];
...
export default function StepExclusions({ data, onChange }: StepExclusionsProps) {

    const toggleList = (current: string[], item: string, field: keyof IcpDisqualifiers) => {
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {EXCLUDED_TYPES.map((type) => (
                            <button
                                key={type}
                                onClick={() => toggleList(data.excludedCompanyTypes, type, 'excludedCompanyTypes')}
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {EXCLUDED_BEHAVIORS.map((behavior) => (
                            <button
                                key={behavior}
                                onClick={() => toggleList(data.excludedBehaviors, behavior, 'excludedBehaviors')}
                                className={`px-4 py-2 rounded-lg border transition-all text-sm font-medium
