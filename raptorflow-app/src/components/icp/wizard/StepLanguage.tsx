...
export default function StepLanguage({ data, onChange }: StepLanguageProps) {
  const toggleList = (
    current: string[],
    item: string,
    field: keyof IcpPsycholinguistics
  ) => {
        ...
        <div className="flex flex-wrap gap-3">
          {MINDSET_TRAITS.map((trait) => (
            <button
              key={trait}
              onClick={() =>
                toggleList(data.mindsetTraits, trait, 'mindsetTraits')
              }
        ...
        <div className="grid grid-cols-2 gap-4">
          {PROOF_PREFERENCES.map((pref) => (
            <button
              key={pref.value}
              onClick={() =>
                toggleList(data.proofPreference, pref.value, 'proofPreference')
              }
        ...
        <div className="flex flex-wrap gap-2">
          {TONE_PREFERENCES.map((tone) => (
            <button
              key={tone}
              onClick={() =>
                toggleList(data.tonePreference, tone, 'tonePreference')
              }
