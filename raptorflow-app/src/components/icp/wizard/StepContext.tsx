    ...
    <div className="space-y-12">
      <div className="text-center space-y-4">
        ...
        <p className="text-[#5B5F61] text-lg max-w-md mx-auto">
          We strip away the noise. What are the hard constraints of your
          business model?
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {BUSINESS_TYPES.map((type) => (
              <button
                key={type.value}
                onClick={() =>
                  toggleSelection(data.companyType, type.value, 'companyType')
                }
          ...
          <div className="grid grid-cols-3 gap-4">
            {SALES_MOTIONS.map((motion) => (
              <button
                key={motion.value}
                onClick={() =>
                  toggleSelection(data.salesMotion, motion.value, 'salesMotion')
                }
                className={`flex flex-col items-start p-5 rounded-2xl border transition-all text-left
                  ${
