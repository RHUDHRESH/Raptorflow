...
const TextWithDefinitions = ({ text, definitions, onShowDef }: { text: string, definitions?: Record<string, string>, onShowDef: (term: string, def: string) => void }) => {
    if (!definitions || Object.keys(definitions).length === 0) return <>{text}</>;

    const parts = text.split(new RegExp(`(${Object.keys(definitions).join('|')})`, 'gi'));
    return (
        <>
            {parts.map((part, i) => {
                const def = Object.entries(definitions).find(([k]) => k.toLowerCase() === part.toLowerCase());
                if (def) {
                    return (
                        <span
                            key={i}
                            onClick={(e) => { e.stopPropagation(); onShowDef(def[0], def[1]); }}
...
export default function WizardQuestion({
    ...
    const [mobileHelp, setMobileHelp] = useState<{ title: string, body: string } | null>(null);
        ...
        <div className="space-y-12 relative">
            <div className="text-center space-y-4 animate-in fade-in zoom-in-95 duration-500 flex flex-col items-center">
                        ...
                        {whyWeAsk && (
                            <div className="group relative">
                                <HelpCircle
                                    ...
                                    onClick={() => setMobileHelp({ title: "Why we ask", body: whyWeAsk })}
                                />
                                {/* Desktop Tooltip */}
                ...
                {example && showExample && (
                    ...
                    >
                        <span className="absolute -top-3 left-4 bg-[#F3F4EE] px-2 text-[10px] uppercase font-bold tracking-widest text-[#9D9F9F]">Example</span>
                        "{example}"
                    </motion.div>
                )}
            ...
            {children ? (
                ...
                >
                    {options?.map((option, index) => {
                        ...
                        const isDisabled = multiSelect && maxSelections ? (!isSelected && selectedValues.length >= maxSelections) : false;
                        ...
                        return (
                            ...
                            >
                                ...
                                {option.description && (
                                    <div className={`text-sm ${isSelected ? 'text-[#5B5F61]' : 'text-[#9D9F9F]'}`}>
                                        {option.description}
                                    </div>
                                )}
                    ...
                    <>
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="font-serif text-xl text-[#2D3538] font-bold">{mobileHelp.title}</h3>
                                <button onClick={() => setMobileHelp(null)}><X className="w-6 h-6 text-[#9D9F9F]" /></button>
                            </div>
                            <p className="text-[#5B5F61] leading-relaxed">{mobileHelp.body}</p>
                        </motion.div>
                    </>
                )}
            </AnimatePresence>
