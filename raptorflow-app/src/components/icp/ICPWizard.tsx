...
const COMMON_PROBLEMS = [
    ...
    { value: "Founder doing marketing themselves", label: "Founder doing marketing" },
    ...
    { value: "Competitors winning on price", label: "Competitors winning on price" },
];
...
const DEFINITIONS: Record<string, string> = {
    ...
    "Churn": "The percentage of customers who stop using your product over a given period.",
...
const EXAMPLES: Record<string, string> = {
    'business_model': "We are a B2B SaaS company selling primarily to mid-market tech companies.",
    'sales_motion': "We use a self-serve PLG model for smaller teams, but a sales-assisted motion for enterprise deals.",
    'primary_pains': "They are drowning in manual data entry and losing 10+ hours a week on spreadsheets.",
    'decision_maker': "The VP of Marketing usually signs the check, but the Content Manager is the daily user.",
    'budget': "They are comfortable spending $500/mo but need executive approval for anything over $2k.",
    ...
    'excluded_types': "We do not sell to early-stage non-funded startups or agencies."
};
...
const EXCLUDED_BEHAVIORS = [
    { value: "Requires heavy customization", label: "Requires heavy customization" },
...
export default function ICPWizard() {
    ...
    const updateFirmo = (key: string, val: any) =>
        updateWithHistory(prev => ({ ...prev, firmographics: { ...prev.firmographics!, [key]: val } }));

    const updatePain = (key: string, val: any) =>
        updateWithHistory(prev => ({ ...prev, painMap: { ...prev.painMap!, [key]: val } }));

    const updatePsycho = (key: string, val: any) =>
        updateWithHistory(prev => ({ ...prev, psycholinguistics: { ...prev.psycholinguistics!, [key]: val } }));

    const updateDisqual = (key: string, val: any) =>
        updateWithHistory(prev => ({ ...prev, disqualifiers: { ...prev.disqualifiers!, [key]: val } }));
    ...
    const toggleSelection = (currentList: string[], item: string, setter: (val: any) => void) => {
    ...
    useEffect(() => {
        ...
        switch (step.id) {
            ...
            default:
                ...
                if (step.id === 'tone') valid = (formData.psycholinguistics?.tonePreference?.length || 0) > 0;
            ...
            switch (step.id) {
                case 'business_model':
                    return (
                        <WizardQuestion
                            ...
                            onSelect={(val) => setSingleSelection(val, (d) => updateFirmo('companyType', d))}
                            autoAdvance
                ...
                case 'sales_motion':
                    return (
                        <WizardQuestion
                            ...
                            onSelect={(val) => setSingleSelection(val, (d) => updateFirmo('salesMotion', d))}
                            autoAdvance
                            onAutoAdvance={handleNext}
                            cols={3}
                ...
                case 'primary_pains':
                    return (
                        <WizardQuestion
                            ...
                            multiSelect
                            maxSelections={2}
                            onSelect={(val) => toggleSelection(formData.painMap?.primaryPains!, val, (d) => updatePain('primaryPains', d))}
                            cols={2}
                ...
                case 'triggers':
                    return (
                        <WizardQuestion
                            ...
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.painMap?.triggerEvents!, val, (d) => updatePain('triggerEvents', d))}
                            cols={2}
                ...
                case 'budget':
                    return (
                        ...
                        >
                            <BudgetSlider
                                value={formData.firmographics?.budgetComfort?.[0] || BUDGET_RANGES[0].label}
                                ranges={BUDGET_RANGES.map(r => r.label)}
                                onChange={(val) => setSingleSelection(val, (d) => updateFirmo('budgetComfort', d))}
                            />
                        </WizardQuestion>
                    );
                ...
                case 'urgency':
                    return (
                        <WizardQuestion
                            ...
                            selectedValues={formData.painMap?.urgencyLevel ? [formData.painMap.urgencyLevel] : []}
                ...
                case 'decision_maker':
                    return (
                        <WizardQuestion
                            ...
                            onSelect={(val) => setSingleSelection(val, (d) => updateFirmo('decisionMaker', d))}
                            autoAdvance
                            onAutoAdvance={handleNext}
                            cols={2}
                ...
                case 'mindset':
                    return (
                        <WizardQuestion
                            ...
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.psycholinguistics?.mindsetTraits!, val, (d) => updatePsycho('mindsetTraits', d))}
                            cols={3}
                ...
                case 'proof':
                    return (
                        <WizardQuestion
                            ...
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.psycholinguistics?.proofPreference!, val, (d) => updatePsycho('proofPreference', d))}
                            cols={2}
                ...
                case 'tone':
                    return (
                        <WizardQuestion
                            ...
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.psycholinguistics?.tonePreference!, val, (d) => updatePsycho('tonePreference', d))}
                            cols={3}
                ...
                case 'excluded_types':
                    return (
                        <WizardQuestion
                            title={<span className="flex items-center justify-center gap-2">Who to <span className="text-red-700 font-serif italic">Ignore</span>.</span>}
                            subtitle="Disqualifiers. Who wastes your time? (Company Types)"
                            options={EXCLUDED_TYPES}
                            selectedValues={formData.disqualifiers?.excludedCompanyTypes || []}
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.disqualifiers?.excludedCompanyTypes!, val, (d) => updateDisqual('excludedCompanyTypes', d))}
                            cols={3}
                ...
                case 'excluded_behaviors':
                    return (
                        <WizardQuestion
                            title={<span className="flex items-center justify-center gap-2">Red Flags <Ban className="w-8 h-8 text-red-500" /></span>}
                            ...
                            multiSelect
                            onSelect={(val) => toggleSelection(formData.disqualifiers?.excludedBehaviors!, val, (d) => updateDisqual('excludedBehaviors', d))}
                            cols={2}
                ...
                <div className="text-center space-y-6 max-w-md px-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-[#9D9F9F] text-sm font-mono pt-2">
                            Analyzing firmographics &bull; Mapping pain points &bull; Locking profile
                        </p>
                    </div>
                </div>
            </div>
    ...
    const getStepHint = () => {
        ...
        if (stepId === 'sales_motion' && firmographics?.companyType.includes('saas')) {
            return "Because you selected SaaS, this choice defines your Go-to-Market velocity.";
        }
        if (stepId === 'primary_pains' && firmographics?.salesMotion.includes('sales-assisted')) {
            return "For Sales-Assisted deals, pains must be urgent enough to justify a meeting.";
        }
        if (stepId === 'decision_maker' && firmographics?.companyType.includes('agency')) {
