    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F3F4EE] space-y-4">
        ...
        <p className="text-[#5B5F61]">
          You haven't defined your Ideal Customer Profile yet.
        </p>
        <button
    ...
    <div className="min-h-screen bg-[#F3F4EE] pb-32">
      ...
      <header className="px-8 py-12 border-b border-[#C0C1BE]/30 bg-[#F3F4EE]">
        <div className="max-w-5xl mx-auto flex items-end justify-between">
          <div>
            <h1 className="font-serif text-5xl text-[#2D3538] mb-4">
              Targeting
            </h1>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className="text-sm font-semibold uppercase tracking-wider text-[#9D9F9F]">
                Confidence
              </div>
              <div className="font-serif text-3xl text-[#2D3538]">
                {(icp.confidenceScore * 100).toFixed(0)}%
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-[#C0C1BE]/30">
              <h3 className="text-xs uppercase font-bold tracking-widest text-[#9D9F9F] mb-4">
                Context
              </h3>
              <ul className="space-y-3">
                {icp.firmographics.companyType.map((t: string) => (
                  <li
                    key={t}
                    className="flex items-center gap-2 text-[#2D3538]"
                  >
            ...
            <div className="bg-white p-6 rounded-2xl border border-[#C0C1BE]/30">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="font-medium text-[#2D3538]">
                    {icp.firmographics.budgetComfort.join(', ') ||
                      'Not specified'}
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-medium text-[#2D3538]">
                    {icp.firmographics.decisionMaker.join(', ') || 'Unknown'}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="md:col-span-8">
            <div className="bg-white p-6 rounded-2xl border border-[#C0C1BE]/30">
              <div className="flex justify-between items-start mb-6">
                <h3 className="text-xs uppercase font-bold tracking-widest text-[#9D9F9F]">
                  Core Pains
                </h3>
                <span
                  className={`px-2 py-1 rounded text-xs font-medium uppercase
                                ${
                                  icp.painMap.urgencyLevel === 'now'
                                    ? 'bg-red-50 text-red-700'
                                    : icp.painMap.urgencyLevel === 'soon'
              ...
              <div className="space-y-4">
                {icp.painMap.primaryPains.map((pain: string, idx: number) => (
                  <div key={pain} className="flex items-start gap-4">
                    ...
                    <span className="text-lg text-[#2D3538] pt-0.5">
                      {pain}
                    </span>
                  </div>
                ))}
              ...
              {icp.painMap.triggerEvents.length > 0 && (
                <div className="mt-8 pt-6 border-t border-dashed border-[#C0C1BE]">
                  <h4 className="text-xs uppercase font-bold tracking-widest text-[#9D9F9F] mb-3">
                    Trigger Events
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {icp.painMap.triggerEvents.map((t: string) => (
                      <span
                        key={t}
                        className="px-3 py-1 bg-[#F3F4EE] text-[#5B5F61] rounded-full text-sm"
                      >
                        {t}
                      </span>
                    ))}
            ...
            <div className="bg-white p-6 rounded-2xl border border-[#C0C1BE]/30 flex flex-col justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {icp.psycholinguistics.tonePreference.map((t: string) => (
                  <span
                    key={t}
                    className="px-3 py-1 bg-[#E9ECE6] text-[#2D3538] rounded-lg text-sm font-medium"
                  >
                    {t}
                  </span>
                ))}
                {icp.psycholinguistics.mindsetTraits.map((t: string) => (
                  <span
                    key={t}
                    className="px-3 py-1 border border-[#C0C1BE] text-[#5B5F61] rounded-lg text-sm"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="bg-red-50/30 p-6 rounded-2xl border border-red-100">
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={exc}
                    className="px-3 py-1 bg-white text-red-900 border border-red-100 rounded text-sm"
                  >
                    {exc}
                  </span>
                ))}
