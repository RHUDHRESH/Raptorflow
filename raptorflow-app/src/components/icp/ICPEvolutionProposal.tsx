...
export default function ICPEvolutionProposal({
  ...
  const newConfidence = Math.min(
    0.99,
    currentIcp.confidenceScore + impact.confidenceDelta
  );
      ...
      >
        ...
        <div className="bg-[#2D3538] text-white p-8">
          <h2 className="font-serif text-2xl mb-2">
            Proposed Targeting Refinement
          </h2>
          <p className="text-white/70 text-sm">
            Based on your input:{' '}
            <span className="italic">"{impact.reason}"</span>
          </p>
        </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex items-center gap-8 p-4 bg-[#F3F4EE] rounded-xl border border-[#C0C1BE]/30">
            <div>
              <div className="text-xs uppercase font-bold text-[#9D9F9F]">
                Confidence Impact
              </div>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xl font-serif text-[#5B5F61]">
                  {(currentIcp.confidenceScore * 100).toFixed(0)}%
                </span>
                <ArrowRight className="w-4 h-4 text-[#9D9F9F]" />
                <span className="text-xl font-serif text-emerald-600 font-bold">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase text-[#9D9F9F]">
              Suggested Changes
            </h3>
            ...
            {proposedChanges.psycholinguistics?.tonePreference && (
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="p-3 rounded-lg border border-emerald-100 bg-emerald-50 text-emerald-900 font-medium">
                  <div className="text-xs font-bold mb-1 text-emerald-700">
                    PROPOSED
                  </div>
