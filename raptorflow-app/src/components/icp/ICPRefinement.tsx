...
export function ICPRefinement({ icp, onSave, onClose }: ICPRefinementProps) {
  ...
  const [primaryPains, setPrimaryPains] = useState(
    icp.painMap?.primaryPains?.join('\n') || ''
  );
  const [tonePreference, setTonePreference] = useState(
    icp.psycholinguistics?.tonePreference || []
  );
  const [wordsToUse, setWordsToUse] = useState(
    icp.psycholinguistics?.wordsToUse?.join(', ') || ''
  );
  const [wordsToAvoid, setWordsToAvoid] = useState(
    icp.psycholinguistics?.wordsToAvoid?.join(', ') || ''
  );
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Adjust the auto-generated profile
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              One per line. Max 2 recommended.
            </p>
          </div>
