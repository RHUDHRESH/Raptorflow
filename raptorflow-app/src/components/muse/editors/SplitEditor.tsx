...
export function SplitEditor({
  ...
  const [selectedVersion, setSelectedVersion] = useState<'A' | 'B' | null>(
    null
  );
    ...
    <div className={cn('flex flex-col h-full bg-background', className)}>
      ...
      {comparison && (
        ...
        >
          ...
          <div className="flex items-center gap-4 text-sm">
            <span>
              A: <strong>{comparison.scoreA}</strong>/100
            </span>
            <span>
              B: <strong>{comparison.scoreB}</strong>/100
            </span>
          </div>
        </div>
      )}
      ...
      <div className="px-6 py-3 border-t border-border/40 bg-muted/10">
        <p className="text-xs text-muted-foreground text-center">
          Write two versions, click Compare to see which performs better, then
          Pick your winner.
        </p>
      </div>
    </div>
