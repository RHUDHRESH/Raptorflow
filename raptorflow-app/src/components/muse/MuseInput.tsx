...
function detectAssetType(prompt: string): AssetType | undefined {
  ...
  if (lower.includes('sales email') || lower.includes('cold email'))
    return 'sales-email';
  if (lower.includes('nurture')) return 'nurture-email';
  if (lower.includes('email')) return 'email';
  if (lower.includes('tagline')) return 'tagline';
  if (lower.includes('one-liner') || lower.includes('elevator pitch'))
    return 'one-liner';
  if (lower.includes('brand story') || lower.includes('story'))
    return 'brand-story';
  if (lower.includes('video script') || lower.includes('script'))
    return 'video-script';
  if (
    ...
    return 'social-post';
  if (lower.includes('product description') || lower.includes('description'))
    return 'product-description';
  if (lower.includes('product name') || lower.includes('name idea'))
    return 'product-name';
  if (lower.includes('pdf') || lower.includes('lead magnet'))
    return 'lead-gen-pdf';
  if (lower.includes('talking point') || lower.includes('sales call'))
...
export function MuseInput({ onSubmit, disabled, className }: MuseInputProps) {
  ...
  const detectedConfig = detectedType
    ? getAssetConfig(detectedType)
    : undefined;
