...
function parseIntent(message: string): {
  ...
  const candidates = [...ASSET_TYPES].sort(
  ...
  for (const asset of candidates) {
    if (
      lower.includes(asset.type.replace('-', ' ')) ||
  ...
  if (
    lower.match(/make|create|build|generate|write|design/i) &&
...
function needsMoreContext(message: string): boolean {
  const trimmed = message.trim();
  if (!trimmed) return true;
  const wordCount = trimmed.split(/\s+/).length;
  if (wordCount < 6) return true;
  if (!/(about|for|target|audience|product|offer|topic|industry)/i.test(trimmed)) {
    return true;
  }
  return false;
}
...
export function MuseChat({
  ...
  const { playKeySound, playClickSound, playCompletionSound } =
    useTypingExperience();
  ...
  const handleMuseResponse = useCallback(
    async (userMessage: string, threadId?: string) => {
      ...
      try {
        ...
        addMessage(
          'muse',
          "Muse couldn't generate that. Add one detail and retry."
        );
      } finally {
        setIsTyping(false);
      }
  ...
  const handleOptionSelect = useCallback(
    (messageId: string, option: ConversationOption) => {
      setMessages((prev) =>
        prev.map((m) =>
      ...
      if (isAssetOption(option.value)) {
        ...
        addMessage(
          'muse',
          `Nice. What should the ${assetLabel.toLowerCase()} be about?`
        );
        return;
      }
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (currentContext.assetType && !currentContext.prompt) {
      ...
      onAssetCreate(
        ...
        Object.keys(contextForAsset).length ? contextForAsset : undefined
      );
      return;
    }
    ...
    if (intent.understood && intent.assetType) {
      if (needsMoreContext(userMessage)) {
        setCurrentContext((prev) => ({
          ...prev,
          prompt: undefined,
          assetType: intent.assetType,
        }));
        const assetLabel = getAssetConfig(intent.assetType)?.label || intent.assetType;
        addMessage(
          'muse',
          `Got it. What should the ${assetLabel.toLowerCase()} be about? Share the product, audience, and tone.`
        );
        return;
      }
    ...
    if (intent.needsClarification === 'asset_type') {
      ...
      addMessage(
        'muse',
        'Got it. What kind of asset should I create?',
        assetOptions
      );
      return;
    }
      ...
      >
        <div ref={inputWrapperRef} className="max-w-2xl mx-auto relative">
                  ...
                  {commandSuggestions.map((suggestion, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-medium text-[#2D3538] truncate">
                          {suggestion.label}
                        </p>
                        {suggestion.description && (
                          <p className="text-[11px] text-[#9D9F9F] truncate">
                            {suggestion.description}
                          </p>
                        )}
                      </div>
          ...
          <p className="text-center text-[11px] text-[#9D9F9F] mt-3">
            <kbd className="px-1.5 py-0.5 rounded-md bg-[#F8F9F7] border border-[#E5E6E3] font-mono text-[10px] text-[#5B5F61]">
              /
            </kbd>{' '}
            asset types •{' '}
            <kbd className="px-1.5 py-0.5 rounded-md bg-[#F8F9F7] border border-[#E5E6E3] font-mono text-[10px] text-[#5B5F61]">
  ...
  const smartPrompts = [
    {
      ...
      prompt:
        'Write a cold sales email that gets replies. Focus on value, not features.',
      icon: 'mail',
    },
    {
      id: 'linkedin',
      ...
      prompt:
        'Create a LinkedIn post announcing our latest update. Make it authentic, not corporate.',
      icon: 'briefcase',
    },
    {
      id: 'tagline',
      label: 'New Tagline Ideas',
      prompt:
        'Generate 5 tagline options for our brand. Short, memorable, aligned with positioning.',
      icon: 'message-circle',
    },
    {
      id: 'product-name',
      label: 'Product Name Ideas',
      prompt: 'Brainstorm product name ideas that are memorable and brandable.',
      icon: 'lightbulb',
    },
    {
      id: 'hook',
      label: 'Attention-Grabbing Hook',
      prompt: 'Write an opening hook for a landing page that stops the scroll.',
      icon: 'anchor',
    },
    {
      id: 'meme',
      label: 'Industry Meme',
      prompt:
        'Create a meme that pokes fun at a common pain point our audience faces.',
      icon: 'smile',
    },
  ];
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-8 py-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-3xl">
        {smartPrompts.map((sp, index) => (
          ...
          >
            ...
            <span className="text-[14px] font-medium text-[#2D3538] tracking-tight">
              {sp.label}
            </span>
          </motion.button>
        ))}
      ...
      <p className="text-[11px] text-[#9D9F9F] mt-10">
        <kbd className="px-1.5 py-0.5 rounded-md bg-white border border-[#E5E6E3] font-mono text-[10px] text-[#5B5F61]">
          /
        </kbd>{' '}
        asset types •{' '}
        <kbd className="px-1.5 py-0.5 rounded-md bg-white border border-[#E5E6E3] font-mono text-[10px] text-[#5B5F61]">
