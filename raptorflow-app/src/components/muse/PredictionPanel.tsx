...
import { TrendingUp, Eye, MousePointer, AlertTriangle, CheckCircle, Sparkles } from 'lucide-react';
...
function generatePredictions(content: string): PredictionData {
    ...
    const hasCTA = /\b(click|sign up|learn more|get started|try|discover|join)\b/i.test(content);
    ...
    if (!hasQuestion) suggestions.push('Try adding a question to increase open rates');
    if (!hasCTA) suggestions.push('Add a clear call-to-action');
    if (wordCount < 50) suggestions.push('Consider adding more detail');
    if (wordCount > 500) suggestions.push('Content may be too long - consider shortening');
...
export function PredictionPanel({ content, assetType = 'email', className }: PredictionPanelProps) {
    const predictions = generatePredictions(content);

    if (predictions.confidence === 0) {
        return (
            <div className={cn('p-4 rounded-xl border border-border/60 bg-card', className)}>
                <div className="flex items-center gap-3 text-muted-foreground">
                    <Sparkles className="h-5 w-5" />
                    <p className="text-sm">Add more content to see performance predictions</p>
                </div>
            </div>
        );
    }
        ...
        <div className={cn('space-y-4', className)}>
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                <div className="p-3 rounded-lg bg-muted/30 text-center">
                    ...
                    <p className={cn('text-xl font-bold', getScoreColor(predictions.openRate, 50))}>
                ...
                <div className="p-3 rounded-lg bg-muted/30 text-center">
                    ...
                    <p className={cn('text-xl font-bold', getScoreColor(predictions.clickRate, 8))}>
                ...
                <div className="p-3 rounded-lg bg-muted/30 text-center">
                    ...
                    <p className={cn('text-xl font-bold', getScoreColor(predictions.engagementScore, 80))}>
            ...
            <p className="text-[10px] text-muted-foreground/60 text-center">
                Predictions are estimates based on content analysis. Actual results may vary.
            </p>
        </div>
    );
}
...
export function PredictionBadge({ content, className }: { content: string; className?: string }) {
    const predictions = generatePredictions(content);
