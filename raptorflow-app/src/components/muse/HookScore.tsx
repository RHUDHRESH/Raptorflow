...
function calculateHookScore(text: string): {
    ...
    const hasPowerWord = /\b(free|new|proven|secret|instant|exclusive|limited|guaranteed|discover|unlock|transform|master)\b/i.test(text);
...
export function HookScore({ text, className }: HookScoreProps) {
    const { score, tips, strengths } = useMemo(() => calculateHookScore(text), [text]);
...
export function HookScoreBadge({ text, className }: HookScoreProps) {
    ...
    return (
        <div className={cn(
            ...
            score >= 40 && score < 70 && 'border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400',
            score < 40 && 'border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-400',
            className
        )}>
