        ...
        <div ref={containerRef} className={styles.flowContainer}>
            ...
            <div ref={leftPanelRef} className={styles.leftPanel}>
                <div className="w-full h-full flex flex-col">
                    ...
                    <div className="mt-12 py-6 border-t border-white/10">
                        ...
                        <p ref={oneLinerRef} className="font-serif text-xl text-white leading-snug italic">
                            "{soundbites.oneLiner}"
                        </p>
                    </div>
                    ...
                    <div className="mt-auto pt-8 border-t border-white/10">
                        <span className="text-xs font-mono uppercase tracking-wider text-white/30">
                            {soundbites.soundbites.length} soundbites ready
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className={styles.rightPanel}>
                <div className="flex-1 flex flex-col max-w-[680px] mx-auto w-full px-16 py-16 overflow-y-auto">
                    <div>
                        <h3 className="text-xs font-mono uppercase tracking-[0.15em] text-[#9D9F9F] mb-8">
                            Tactical Soundbites
                        </h3>
                                ...
                                <div key={i} className="soundbite-card group p-6 border border-[#E5E6E3] rounded-xl hover:border-[#C0C1BE] transition-colors">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                            <span className="text-xs font-mono uppercase tracking-wider text-[#9D9F9F] border border-[#E5E6E3] px-2 py-1 rounded">
                                                {bite.type}
                                            </span>
                                            <span className="text-xs text-[#9D9F9F]">→ {bite.useCase}</span>
                                        </div>
                                        <button
                                            onClick={() => handleCopy(bite.text, i)}
                                            className="opacity-0 group-hover:opacity-100 transition-opacity p-2 rounded-lg hover:bg-[#F3F4EE]"
                                        >
                                            {copiedIndex === i ? (
                                    ...
                                    <p className="font-serif text-lg text-[#2D3538] leading-relaxed">
                                        "{bite.text}"
                                    </p>
                                </div>
                            ))}
