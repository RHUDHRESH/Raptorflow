                ...
                <div className="flex-1 flex flex-col max-w-[680px] mx-auto w-full px-16 py-16 overflow-y-auto">
                    <div>
                        ...
                        <div ref={indirectRef} className="mb-10">
                            <h3 className="text-xs font-mono uppercase tracking-[0.15em] text-[#9D9F9F] mb-6">Indirect Competitors</h3>
                            <div className="space-y-4">
                                {competitive.indirect.map((comp, i) => (
                                    <div key={i} className="competitor-card p-5 border border-[#E5E6E3] rounded-xl">
                                        <div className="flex items-start justify-between">
                                            <div><p className="font-medium text-[#2D3538]">{comp.name}</p><p className="text-sm text-[#9D9F9F] mt-1">{comp.mechanism}</p></div>
                                            <span className="text-xs font-mono text-[#9D9F9F]">{comp.priceRange}</span>
                                        </div>
                                        <div className="mt-4 pt-4 border-t border-[#E5E6E3]"><span className="text-xs text-[#9D9F9F]">Your Edge: </span><span className="text-sm text-[#2D3538]">{comp.yourEdge}</span></div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        {competitive.direct.length > 0 && (
                            <div ref={directRef} className="mb-10">
                                <h3 className="text-xs font-mono uppercase tracking-[0.15em] text-[#9D9F9F] mb-6">Direct Competitors</h3>
                                <div className="space-y-4">
                                    {competitive.direct.map((comp, i) => (
                                        <div key={i} className="competitor-card p-5 border border-[#E5E6E3] rounded-xl">
                                            <div className="flex items-start justify-between">
                                                <div><p className="font-medium text-[#2D3538]">{comp.name}</p><p className="text-sm text-[#9D9F9F] mt-1">{comp.positioning}</p></div>
