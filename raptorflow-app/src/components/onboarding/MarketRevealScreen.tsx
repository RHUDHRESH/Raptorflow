...
function useCounter(target: number, duration: number = 2, trigger: boolean = true) {
    ...
    useEffect(() => {
        if (!trigger) return;
        const obj = { val: 0 };
        gsap.to(obj, { val: target, duration, ease: 'power2.out', onUpdate: () => setValue(Math.floor(obj.val)) });
...
export function MarketRevealScreen({ market, onComplete }: MarketRevealScreenProps) {
    ...
    useEffect(() => {
        ...
        const tl = gsap.timeline({ onStart: () => setAnimationStarted(true) });
        ...
        tl.to(leftPanelRef.current, { x: 0, opacity: 1, duration: 0.7, ease: 'power3.out' })
            .to(somHighlightRef.current, { y: 0, opacity: 1, duration: 0.8, ease: 'power3.out' }, '-=0.3')
            .to(metricsRef.current, { y: 0, opacity: 1, duration: 0.7, ease: 'power2.out' }, '-=0.4');

        if (metricsRef.current) {
            const cards = metricsRef.current.querySelectorAll('.metric-card');
            gsap.set(cards, { y: 30, opacity: 0, scale: 0.95 });
            tl.to(cards, { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.12, ease: 'power2.out' }, '-=0.5');
        }

        tl.to(pathRef.current, { y: 0, opacity: 1, duration: 0.6, ease: 'power2.out' }, '-=0.2')
            .to(actionRef.current, { y: 0, opacity: 1, duration: 0.4, ease: 'power2.out' }, '-=0.2');
            ...
            <div ref={leftPanelRef} className={styles.leftPanel}>
                <div className="w-full h-full flex flex-col">
                    ...
                    <div className={styles.sectionInfo}>
                        ...
                        <h2 className="font-serif text-4xl text-white tracking-tight mb-3">Market Size</h2>
                        <p className="text-sm text-white/50 leading-relaxed">Your addressable opportunity.</p>
                    </div>
                    <div ref={somHighlightRef} className="mt-12 py-6 border-t border-white/10">
                        <span className="text-xs font-mono uppercase tracking-[0.15em] text-white/40 mb-4 block">Your 3-Year Target</span>
                        <p className="font-serif text-5xl text-white">{formatCurrency(somValue)}</p>
                        <p className="text-sm text-white/50 mt-2">{market.som.timeline}</p>
                    </div>
                    <div className="mt-auto pt-8 border-t border-white/10">
                        <span className="text-xs font-mono uppercase tracking-wider text-white/30">Foundation Complete</span>
                    </div>
                </div>
            </div>
                ...
                <div className="flex-1 flex flex-col max-w-[680px] mx-auto w-full px-16 py-16">
                    <div>
                        ...
                        <div ref={pathRef} className="mb-12 p-8 border border-[#E5E6E3] rounded-2xl">
                            <h3 className="text-xs font-mono uppercase tracking-[0.15em] text-[#9D9F9F] mb-6">Path to SOM</h3>
                            <div className="grid grid-cols-2 gap-8">
                                <div><span className="text-sm text-[#9D9F9F]">Customers Needed</span><p className="font-serif text-2xl text-[#2D3538]">{customersValue.toLocaleString()}</p></div>
                                <div><span className="text-sm text-[#9D9F9F]">Leads per Month</span><p className="font-serif text-2xl text-[#2D3538]">{leadsValue.toLocaleString()}</p></div>
                                <div><span className="text-sm text-[#9D9F9F]">Win Rate</span><p className="font-serif text-2xl text-[#2D3538]">{(market.pathToSom.winRate * 100).toFixed(0)}%</p></div>
                                <div><span className="text-sm text-[#9D9F9F]">Top Channels</span><p className="text-[#2D3538]">{market.pathToSom.channelMix.slice(0, 2).map(ch => ch.channel).join(', ')}</p></div>
                            </div>
                        </div>
                        <div ref={actionRef} className="flex items-center justify-end pt-8 border-t border-[#E5E6E3]">
                            <Button onClick={onComplete} className="bg-[#2D3538] hover:bg-[#1A1D1E] text-white px-10 py-6 rounded-xl font-medium transition-all hover:scale-[1.02]">
