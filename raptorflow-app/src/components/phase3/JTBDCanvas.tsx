...
interface JTBDCanvasProps {
    ...
    emotional: string;
    social: string;
    onChange: (field: 'functional' | 'emotional' | 'social', value: string) => void;
}
...
function JobInput({ id, label, value, placeholder, icon, description, onChange }: JobInputProps) {
    return (
...
export function JTBDCanvas({ functional, emotional, social, onChange }: JTBDCanvasProps) {
    return (
        <div className="space-y-12">
            ...
            <div className="bg-[#2D3538] text-white p-8 rounded-[14px] flex gap-6 items-start">
                ...
                <div className="space-y-2">
                    <h4 className="font-medium text-lg italic opacity-90">The Progress Formula</h4>
                    <p className="text-sm text-white/60 leading-relaxed max-w-2xl">
                        "When I [Situation], I want to [Functional Job], so that I can [Emotional/Social Outcome]."
                        <br />
                        <span className="block mt-4 text-white/40 font-mono text-[10px] uppercase tracking-widest">
                            Pro-tip: Specificity is the difference between commodity and luxury.
                        </span>
                    </p>
                </div>
            </div>
