...
function ForceQuadrant({ title, color, items, onRemove, onAdd }: ForceQuadrantProps) {
    ...
    const colorClasses: Record<string, { bg: string; border: string; text: string }> = {
        push: { bg: 'bg-red-50 dark:bg-red-950/30', border: 'border-red-200 dark:border-red-900', text: 'text-red-700 dark:text-red-400' },
        pull: { bg: 'bg-green-50 dark:bg-green-950/30', border: 'border-green-200 dark:border-green-900', text: 'text-green-700 dark:text-green-400' },
        anxiety: { bg: 'bg-amber-50 dark:bg-amber-950/30', border: 'border-amber-200 dark:border-amber-900', text: 'text-amber-700 dark:text-amber-400' },
        habit: { bg: 'bg-blue-50 dark:bg-blue-950/30', border: 'border-blue-200 dark:border-blue-900', text: 'text-blue-700 dark:text-blue-400' },
    };
    ...
    return (
        <div className={cn("rounded-xl p-5 border-2", colors.bg, colors.border)}>
            <h3 className={cn("font-semibold text-sm uppercase tracking-wider mb-3", colors.text)}>
                {title}
            </h3>
            <div className="space-y-2">
                ...
                {items.length === 0 && !isAdding && (
                    <p className="text-sm text-muted-foreground italic">No items detected</p>
                )}
...
function JobCard({ job, onSetPrimary, onRemove }: { job: JTBDJob; onSetPrimary: () => void; onRemove: () => void }) {
    const typeColors: Record<JobType, string> = {
        functional: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
    ...
    return (
        <div className={cn(
            ...
            job.isPrimary ? "border-primary bg-primary/5" : "border-border hover:border-primary/30"
        )}>
            <div className="flex items-start gap-3">
                <button
                    ...
                    className={cn(
                        ...
                        job.isPrimary ? "text-primary" : "text-muted-foreground hover:text-primary"
                    )}
                >
                    <Star className={cn("h-4 w-4", job.isPrimary && "fill-current")} />
                </button>
                <div className="flex-1">
                    <span className={cn("text-xs px-2 py-0.5 rounded-full", typeColors[job.type])}>
                        {job.type}
                    </span>
...
export function JTBDForcesScreen({ jtbd, onChange, onContinue }: JTBDForcesScreenProps) {
        ...
        <div className="space-y-8">
            ...
            <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground max-w-lg mx-auto">
                    These are the forces that drive customers to switch. Edit what doesn't feel right.
                </p>
            </div>
