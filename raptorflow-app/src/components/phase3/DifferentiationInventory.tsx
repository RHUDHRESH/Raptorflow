...
function StatusBadge({ status }: { status: DifferentiatorStatus }) {
  ...
  return (
    <span
      className={cn('text-xs px-2 py-1 rounded-full font-medium', bg, text)}
    >
      {label}
    </span>
  );
            ...
            {showProofInput ? (
              <div className="flex gap-2 mt-2">
                <Input
                  ...
                  autoFocus
                />
                <Button size="sm" onClick={addProof}>
                  Add
                </Button>
                <Button
                  size="sm"
...
export function DifferentiationInventory({
  differentiators,
  onChange,
  onContinue,
}: DifferentiationInventoryProps) {
  const provenCount = differentiators.filter(
    (d) => d.status === 'proven'
  ).length;
  const totalCount = differentiators.filter(
    (d) => d.status !== 'blocked'
  ).length;
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground max-w-lg mx-auto">
          Claims without proof are marketing fiction. Attach evidence to
          graduate each differentiator.
        </p>
      </div>
