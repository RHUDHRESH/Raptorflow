...
const RISK_POLICIES = [
  {
    id: 'pilot',
    label: 'Pilot Program',
    description: 'Try before committing fully',
  },
  {
    id: 'milestone-billing',
    label: 'Milestone Billing',
    description: 'Pay as milestones are hit',
  },
  {
    id: 'guarantee',
    label: 'Money-Back Guarantee',
    description: 'Full or partial refund policy',
  },
  {
    id: 'none',
    label: 'No Special Policy',
    description: 'Standard terms, no special offer',
  },
];
...
export function OfferPackaging({
  ...
  const handleRemovePrerequisite = (index: number) => {
    onUpdate({
      ...profile,
      successPrerequisites: profile.successPrerequisites.filter(
        (_, i) => i !== index
      ),
    });
  };
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-center gap-3 bg-[#2D3538]/5 rounded-xl px-5 py-4">
        ...
        <p className="text-[14px] text-[#5B5F61]">
          This is not a full pricing strategy — just enough to ensure your
          positioning doesn't promise the wrong thing.
        </p>
      </div>

      {/* Delivery Mode */}
      <div>
        <h3 className="font-medium text-[#2D3538] text-[18px] mb-2">
          Offer Shape
        </h3>
          ...
          {DELIVERY_MODES.map((mode) => {
            ...
            return (
              ...
              >
                ...
                <Icon
                  className={`w-6 h-6 mb-3 ${isSelected ? 'text-[#2D3538]' : 'text-[#9D9F9F]'}`}
                />
      ...
      <div className="bg-[#FAFAF8] rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="font-medium text-[#2D3538] text-[18px]">
              Time to First Value
            </h3>
          ...
          {RISK_POLICIES.map((policy) => {
            ...
            return (
              <button
                key={policy.id}
                onClick={() =>
                  handleRiskPolicyChange(
                    policy.id as OfferProfile['riskPolicy']
                  )
                }
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-medium text-[#2D3538] text-[14px]">
                      {policy.label}
                    </h4>
      ...
      <div>
        ...
        <div className="space-y-2 mb-4">
          {profile.successPrerequisites.map((prereq, index) => (
            ...
            >
              ...
              <span className="flex-1 text-[14px] text-[#2D3538]">
                {prereq}
              </span>
              <button
        ...
        {profile.successPrerequisites.length < 3 && (
          <p className="text-[12px] text-[#F59E0B] mt-2">
            Add at least {3 - profile.successPrerequisites.length} more
            prerequisite(s)
          </p>
        )}
      </div>
