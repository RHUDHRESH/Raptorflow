  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <span
          className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-mono uppercase tracking-[0.1em] ${
            typeBadgeColors[job.type]
          }`}
        >
          {job.type}
        </span>
      ...
      <p className="text-[15px] text-[#2D3538] leading-relaxed">
        {job.statement}
      </p>
    </button>
  );
...
export function CustomerProgress({
  ...
  const handleForceChange = (
    force: keyof typeof forceWeights,
    value: number
  ) => {
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[14px] text-[#5B5F61] mb-6">
                What job is your customer trying to get done? Select one or add
                your own.
              </p>
            </div>
            {!isAddingJob && (
            ...
            <div className="bg-white border-2 border-[#2D3538] rounded-2xl p-5 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {(['functional', 'emotional', 'social'] as const).map(
                    (type) => (
                      <button
                        key={type}
          ...
          {jtbd.jobs.length === 0 && !isAddingJob && (
            <div className="bg-[#FAFAF8] border border-dashed border-[#C0C1BE] rounded-2xl p-8 text-center">
              <p className="text-[#5B5F61] text-[14px] mb-3">
                No jobs defined yet
              </p>
              <p className="text-[12px] text-[#9D9F9F]">
                Click "Add Job" above to define what your customer is trying to
                achieve.
              </p>
            </div>
          )}
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-[14px] text-[#5B5F61] mb-6">
              How strongly does each force affect your customer's decision to
              switch?
            </p>
          </div>
          ...
          <div className="bg-[#FAFAF8] rounded-xl p-4">
            <p className="text-[12px] text-[#5B5F61]">
              {forceWeights.push + forceWeights.pull >
              forceWeights.anxiety + forceWeights.habit
