...
import { Check, Lock, Download, ArrowRight, FileText, BarChart3, Grid3X3, Lightbulb, Target, Package } from 'lucide-react';
        ...
        <div className="bg-white border border-[#E5E6E3] rounded-2xl p-6">
            ...
            <ul className="space-y-2">
                {items.slice(0, 3).map((item, index) => (
                    <li key={index} className="text-[13px] text-[#5B5F61] flex items-start gap-2">
...
export function Phase3Review({ phase3, onLock, onBack }: Phase3ReviewProps) {
    ...
    const primaryJob = phase3.jtbd?.jobs?.find((j) => j.isPrimary)?.statement || 'Not defined';
    const topPains = phase3.vpc?.customerProfile?.pains?.slice(0, 3).map((p) => p.text) || [];
    const topGains = phase3.vpc?.customerProfile?.gains?.slice(0, 3).map((g) => g.text) || [];
    ...
    const canvasFactors = phase3.strategyCanvas?.factors?.map((f) => f.name) || [];
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <SummaryCard
                    icon={BarChart3}
                    title="Proof Stack"
                    status={hasProof ? 'complete' : proofItems.length === 0 ? 'warning' : 'complete'}
                    items={proofItems.length > 0 ? proofItems : ['No proof added (claims will be unproven)']}
                />
            ...
            <div className="bg-[#FAFAF8] rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                        <span className="text-[10px] font-mono uppercase text-[#9D9F9F] block mb-2">UVP</span>
                        <p className="text-[14px] text-[#2D3538]">
                            {phase3.uvpDrafts?.find((d) => d.isPrimary)?.text || 'Not selected'}
                        </p>
                    </div>
                    <div className="bg-white rounded-xl p-4 border border-[#E5E6E3]">
                        <span className="text-[10px] font-mono uppercase text-[#9D9F9F] block mb-2">USP</span>
                        <p className="text-[14px] text-[#2D3538]">
                            {phase3.uspDrafts?.find((d) => d.isPrimary)?.text || 'Not selected'}
                        </p>
                    </div>
                </div>
                ...
                {phase3.mechanismLine && (
                    <div className="mt-4 bg-white rounded-xl p-4 border border-[#E5E6E3]">
                        <span className="text-[10px] font-mono uppercase text-[#9D9F9F] block mb-2">Mechanism</span>
                        <p className="text-[14px] text-[#2D3538]">We do it by: {phase3.mechanismLine}</p>
                    </div>
                )}
            </div>
            ...
            {!isComplete && (
                <div className="flex items-center gap-3 bg-[#F59E0B]/10 rounded-xl px-5 py-4">
                    ...
                    <p className="text-[14px] text-[#5B5F61]">
                        Some sections are incomplete. You can still lock, but results may be limited.
                    </p>
                </div>
            )}
