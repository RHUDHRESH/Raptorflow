    ...
    <div className="group bg-white border border-[#E5E6E3] rounded-xl p-5 hover:shadow-sm transition-all">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-medium text-[#2D3538] text-[15px] mb-1">
            {artifact.title}
          </h4>
          {artifact.description && (
            <p className="text-[13px] text-[#5B5F61] mb-3">
              {artifact.description}
            </p>
          )}
        ...
        <div className="space-y-3 mb-4">
          ...
          <div className="flex gap-2">
            <Button
              onClick={handleAdd}
              size="sm"
              className="bg-[#2D3538] text-white"
            >
              Add Proof
            </Button>
            <Button
...
export function ProofStackScreen({
  ...
  const handleUpdateStrength = (
    ...
    onUpdate(proofArtifacts.map((a) => (a.id === id ? { ...a, strength } : a)));
  };

  // Calculate stats
  const totalProof = proofArtifacts.filter(
    (a) => a.category !== 'not-yet'
  ).length;
  const strongProof = proofArtifacts.filter(
    (a) => a.strength === 'strong'
  ).length;
  const notYetCount = proofArtifacts.filter(
    (a) => a.category === 'not-yet'
  ).length;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-[#E5E6E3] rounded-xl p-5 text-center">
          <span className="text-[32px] font-serif text-[#2D3538]">
            {totalProof}
          </span>
          <p className="text-[12px] text-[#5B5F61] mt-1">Total Proof Items</p>
        </div>
        <div className="bg-white border border-[#E5E6E3] rounded-xl p-5 text-center">
          <span className="text-[32px] font-serif text-[#22C55E]">
            {strongProof}
          </span>
          <p className="text-[12px] text-[#5B5F61] mt-1">Strong Proof</p>
        </div>
        <div className="bg-white border border-[#E5E6E3] rounded-xl p-5 text-center">
          <span className="text-[32px] font-serif text-[#F59E0B]">
            {notYetCount}
          </span>
      ...
      <div className="flex items-center gap-3 bg-[#2D3538]/5 rounded-xl px-5 py-4">
        ...
        <p className="text-[14px] text-[#5B5F61]">
          Claims without proof will be flagged as{' '}
          <span className="font-medium text-[#F59E0B]">"Unproven"</span>. Mark
          items as "Not Yet" to generate proof collection requests.
        </p>
      </div>
