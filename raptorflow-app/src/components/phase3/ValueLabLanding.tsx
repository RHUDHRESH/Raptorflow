...
const VALUE_LAB_STEPS: ValueLabStep[] = [
  {
    id: 'intake',
    label: 'Review Inputs',
    description: 'Check what we extracted',
  },
  {
    id: 'jtbd',
    label: 'Customer Goal',
  ...
  },
  {
    id: 'vpc-customer',
    label: 'Their World',
    description: 'Jobs, pains, and desires',
  },
  {
    id: 'vpc-solution',
    label: 'Your Solution',
    description: 'How you help them win',
  },
  {
    id: 'proof',
    label: 'Proof Library',
    description: 'Evidence for your claims',
  },
  {
    id: 'canvas',
    label: 'Competitive Edge',
    description: 'Where you stand out',
  },
  {
    id: 'errc',
    label: 'Strategic Moves',
    description: 'What to change to win',
  },
  { id: 'claims', label: 'Your Claims', description: 'Value propositions' },
  {
    id: 'offer',
    label: 'How You Deliver',
    description: 'Delivery model & timeline',
  },
  {
    id: 'review',
    label: 'Lock & Continue',
    description: 'Confirm and move on',
  },
];
        ...
        <div className="flex-1 space-y-1 overflow-y-auto">
          {VALUE_LAB_STEPS.map((step, index) => {
            ...
            return (
              <div
                key={step.id}
                className={`flex items-start gap-4 py-3 px-4 rounded-xl transition-all ${
                  isCompleted ? 'bg-white/5' : ''
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-[11px] font-mono ${
                    isCompleted
                ...
                <div className="flex-1 min-w-0">
                  <span
                    className={`text-sm block ${isCompleted ? 'text-white' : 'text-white/50'}`}
                  >
                    {step.label}
                  </span>
      ...
      <main className="flex-1 flex items-center justify-center p-16">
        <div className="max-w-[560px] text-center">
          ...
          <p className="text-[18px] text-[#5B5F61] leading-relaxed mb-12 max-w-[480px] mx-auto">
            Let's turn everything you've told us into a{' '}
            <span className="font-medium text-[#2D3538]">
              clear, provable value proposition
            </span>
            . You'll define what makes you different and why customers should
            choose you.
          </p>
                ...
                <div className="space-y-6">
                  {evidenceSnippets.length > 0 ? (
                    evidenceSnippets.map((snippet, index) => (
                      <div
                        key={index}
                        className="border-l-2 border-[#D7C9AE] pl-4"
                      >
