...
function ConfidenceMeter({ confidence }: { confidence: SourceConfidence }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono uppercase tracking-[0.1em] ${config.bg} ${config.text}`}
    >
      {config.label}
    </span>
  );
            ...
            <div key={item.id} className="group">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-shrink-0 w-[140px]">
                  <span className="text-[12px] text-[#5B5F61] block mb-1">
                    {item.label}
                  </span>
                    ...
                    {item.sourceQuotes.length > 0 && (
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent
                            side="right"
                            className="max-w-[300px] bg-[#2D3538] text-white p-4 rounded-xl"
                          >
