...
import {
  StrategyCanvas as StrategyCanvasType,
  CanvasFactor,
} from '@/lib/foundation';
...
export function StrategyCanvasScreen({
  canvas,
  onChange,
  onContinue,
}: StrategyCanvasProps) {
  ...
  const generatePath = (values: number[], factors: CanvasFactor[]) => {
    const visibleValues = factors
      .map((f, i) => (f.mattersToUs ? values[i] : null))
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground max-w-lg mx-auto">
          Your value curve shows where you compete high vs. low. Create
          differentiation by diverging from the pack.
        </p>
      </div>
        ...
        {[
          {
            key: 'statusQuo',
            label: 'Status Quo',
            color: CURVE_COLORS.statusQuo,
          },
          {
            key: 'categoryLeader',
            label: 'Leader',
            color: CURVE_COLORS.categoryLeader,
          },
          ...(showCurrent
            ? [
                {
