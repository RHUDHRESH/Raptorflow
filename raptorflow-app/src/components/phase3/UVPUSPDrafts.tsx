  ...
  return (
    ...
    >
      ...
      {isEditing ? (
        <div onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex gap-2 mt-3">
            <Button
              size="sm"
              onClick={handleSave}
              className="bg-white text-[#2D3538]"
            >
              Save
            </Button>
            <Button
        ...
        <>
          <p
            className={`text-[15px] leading-relaxed mb-4 ${isPrimary ? 'text-white' : 'text-[#2D3538]'}`}
          >
            {draft.text}
          </p>
          ...
          <div className="flex items-center gap-4 mb-4">
            <span
              className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-mono uppercase ${
                draft.proofAttached
                  ? isPrimary
                    ? 'bg-white/20 text-white'
                    : 'bg-[#22C55E]/10 text-[#22C55E]'
                  : isPrimary
                    ? 'bg-[#EF4444]/30 text-white'
...
function USPCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <span
          className={`text-[10px] font-mono uppercase tracking-[0.1em] ${isPrimary ? 'text-white/50' : 'text-[#9D9F9F]'}`}
        >
          Reeves Gates:
        </span>
        ...
        <span
          className={`text-[11px] font-medium ${passedGates === 3 ? 'text-[#22C55E]' : 'text-[#F59E0B]'}`}
        >
          {passedGates}/3
        </span>
      </div>
...
export function UVPUSPDrafts({
  ...
  const handleSelectUVP = (id: string) => {
    onUpdateUVP(uvpDrafts.map((d) => ({ ...d, isPrimary: d.id === id })));
  };

  const handleEditUVP = (id: string, text: string) => {
    onUpdateUVP(uvpDrafts.map((d) => (d.id === id ? { ...d, text } : d)));
  };

  const handleSelectUSP = (id: string) => {
    onUpdateUSP(uspDrafts.map((d) => ({ ...d, isPrimary: d.id === id })));
  };
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-center justify-center">
        ...
        >
          ...
          {vocabularyLocked
            ? 'Vocabulary Locked'
            : 'Lock Vocabulary (ban fluffy words)'}
        </Button>
      </div>
