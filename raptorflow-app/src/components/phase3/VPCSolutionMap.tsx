              ...
              <div className="absolute top-full left-0 mt-2 w-[300px] bg-white border border-[#E5E6E3] rounded-xl shadow-lg z-10 max-h-[200px] overflow-y-auto">
                {availablePains.map((p) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-[13px] hover:bg-[#F3F4EE] transition-colors ${
                      p.id === reliever.painId ? 'bg-[#F3F4EE]' : ''
                    }`}
                  >
      ...
      <div className="bg-[#FAFAF8] rounded-2xl p-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {vpc.valueMap.productsServices.length === 0 && (
            <span className="text-[14px] text-[#9D9F9F]">
          ...
          <div className="space-y-3">
            {vpc.valueMap.painRelievers.map((reliever) => (
              <RelieverCard
                key={reliever.id}
                reliever={reliever}
                pain={vpc.customerProfile.pains.find(
                  (p) => p.id === reliever.painId
                )}
                onRemove={() => handleRemoveReliever(reliever.id)}
                onLinkPain={(painId) =>
                  handleLinkRelieverPain(reliever.id, painId)
                }
          ...
          <div className="space-y-3">
            {vpc.valueMap.gainCreators.map((creator) => (
              <CreatorCard
                key={creator.id}
                creator={creator}
                gain={vpc.customerProfile.gains.find(
                  (g) => g.id === creator.gainId
                )}
                onRemove={() => handleRemoveCreator(creator.id)}
                onLinkGain={(gainId) =>
                  handleLinkCreatorGain(creator.id, gainId)
                }
      ...
      {(vpc.valueMap.painRelievers.some((r) => !r.painId) ||
        ...
        <div className="flex items-center gap-3 bg-[#F59E0B]/10 rounded-xl px-5 py-4">
          ...
          <p className="text-[14px] text-[#5B5F61]">
            Some items are not linked — features without customer purpose may
            not resonate.
          </p>
        </div>
      )}
