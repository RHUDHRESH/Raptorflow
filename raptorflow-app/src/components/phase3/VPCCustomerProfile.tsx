...
function DraggableItem({
  ...
  return (
    <div className="group flex items-center gap-3 bg-white border border-[#E5E6E3] rounded-xl px-4 py-4 hover:shadow-sm transition-all">
      ...
      {source && (
        <span
          className={`px-2 py-0.5 rounded-full text-[10px] font-mono uppercase tracking-[0.05em] flex-shrink-0 ${sourceColors[source]}`}
        >
          {source}
        </span>
      )}
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3 bg-[#2D3538]/5 rounded-xl px-5 py-4">
        ...
        <p className="text-[14px] text-[#5B5F61]">
          Drag to rank your{' '}
          <span className="font-medium text-[#2D3538]">top 3</span> in each
          category. Items at the top are most important.
        </p>
      </div>
